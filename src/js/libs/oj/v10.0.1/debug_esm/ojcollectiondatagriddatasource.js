 ...
 * @memberof CollectionDataGridDataSource
 */
CollectionDataGridDataSource.prototype._fetchCellsError = function (error$1) {
  // log the error
  error(error$1);

  // invoke any header error callbacks
  if (this.pendingHeaderCallback != null) {
    this._processPendingHeaderErrorCallbacks('column', error$1);
    this._processPendingHeaderErrorCallbacks('row', error$1);
  }

  // invoke any cell error callbacks
  if (this.pendingCellCallback != null) {
    this._processPendingCellErrorCallbacks(error$1);
  }
};
