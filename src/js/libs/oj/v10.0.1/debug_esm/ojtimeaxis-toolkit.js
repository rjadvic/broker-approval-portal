...
Obj.createSubclass(DvtTimeAxisFormatter, Obj);
...
DvtTimeAxisRenderer._renderAxisBlock = function(timeAxis, axisStart, axisSize, separatorStyle)
{
  ...
  var separatorStyle = new CSSStyle(separatorStyle);
  timeAxis._separatorStroke = new Stroke(separatorStyle.getStyle(CSSStyle.COLOR));
...
DvtTimeAxisRenderer._renderAxisTicksLabels = function(timeAxis, startPos, endPos, container, length, axisEnd, tickStart, labelStart)
{
  ...
  var block = new Container(context, 'block_' + startPos + '_' + endPos);
...
{
  var fit = TextUtils.fitText(label, maxLength, maxHeight, container);
  ...
  if (truncateStart && label.isTruncated() && fit)
  {
    ...
    if (textString !== untruncatedTextString)
    {
      ...
      var truncatedStartString = OutputText.ELLIPSIS + untruncatedTextString.substring(indexStart, indexEnd);
      label.setTextString(truncatedStartString);
    }
  }

  label.setX(x);
  TextUtils.centerTextVertically(label, y); // align text vertically
...
{
  var line = new Line(context, x1, y1, x2, y2, id);
...
TimeAxis.prototype._prepareScaleDatesLabels = function(scale, originalScale, startTime, endTime)
{
  ...
  {
    currentTime = dates[j];
    var label = new OutputText(this.getCtx(), labelTexts[j], 0, 0, 's_label' + currentTime);
