...
import { isMonthPriorToYear, getFirstDayOfWeek, getDayNames, getMonthNames } from 'ojs/ojlocaledata';
...
function _isLargeScreen() {
  return getDeviceRenderMode() !== 'phone';
}
...
function _getNativePickerDate(converter, isoString) {
  ...
  var valueParams;
  try {
    valueParams = IntlConverterUtils._dateTime(isoString,
      ['date', 'fullYear', 'month', 'hours', 'minutes', 'seconds'], true);
  } catch (e) {
    info('The value of the InputDateTime element should be an ISOString, please use a valid ISOString');
    date.setHours(0);
    date.setMinutes(0);
    date.setSeconds(0);
...
var yearDisplay = new IntlDateTimeConverter({ year: 'numeric' });

function formatYear(year, month) {
  return yearDisplay.format(
    IntlConverterUtils.dateToLocalIso(new Date(year, month, 1)));
}
...
oj.__registerWidget('oj.ojInputDate', $.oj.inputBase, {
  ...
  _GetDefaultConverter: function () {
    return new IntlDateTimeConverter({ day: '2-digit', month: '2-digit', year: '2-digit' });
  },
  ...
  */
  ...
  _datepickerShowing: function () {
    return this._isInLine ||
      (this._popUpDpDiv &&
       isComponentInitialized(this._popUpDpDiv, 'ojPopup') &&
  ...
  _setupResizePopupBind: function () {
    this._resizePopupBind = function () {
      if (isComponentInitialized(this._popUpDpDiv, 'ojPopup')) {
  ...
  _getCurrentDate: function () {
    let date = new Date();
    let converter = this._GetConverter();
    ...
    if (converterOptions && converterOptions.timeZone && converterOptions.isoStrFormat !== 'local') {
      date = IntlConverterUtils.isoToLocalDate(converter.parse(date.toISOString()));
    }
    return date;
  },
  ...
  _getDefaultIsoDate: function () {
    return IntlConverterUtils.dateToLocalIso(this._getTodayDate());
  },
  ...
  _getMinMaxDateIso: function (minOrMax) {
    ...
    if (minMaxDateIso) {
      ...
      minMaxDateIso = IntlConverterUtils._minMaxIsoString(minMaxDateIso,
        dateIso);
    }
  ...
  _generateMonthYearHeader: function (drawMonth, drawYear) {
    ...
    var positionOfMonthToYear = isMonthPriorToYear() ? 'before' : 'after';
  ...
  */
  ...
  _createNativeDatePickerConverter: function (inputConverter) {
    if (this._nativePickerConverter === null) {
      ...
      self._nativePickerConverter = new IntlDateTimeConverter(options);
    }
  },
  ...
  _getNativeDatePickerConverter: function () {
    if (this._nativePickerConverter === null) {
      ...
      this._nativePickerConverter = new IntlDateTimeConverter(options);
    }
  ...
  _ShowNativeDatePicker: function (pickerOptions) {
    ...
    function onError(error$1) {
      ...
      if (error$1.startsWith('cancel')) {
        self._onClose(self._ON_CLOSE_REASON_CANCELLED);
      } else {
        error('Error: native date or time picker failed: ' + error$1);
      }
    }
  ...
  _validateDatetime: function (isoString, actionParam, doParseValue) {
    ...
    try {
      ...
      info('The value of the InputDate element should be an ISOString, please use a valid ISOString');
      valueParams = IntlConverterUtils._dateTime(this._getDefaultIsoDate(),
                    actionParam, doParseValue);
    }
    return valueParams;
  },
...
setDefaultOptions(
  {
    ojInputDate:
    {
      firstDayOfWeek: createDynamicPropertyGetter(
      function () {
        return getFirstDayOfWeek();
      }),

      dayWide: createDynamicPropertyGetter(
      function () {
        return getDayNames('wide');
      }),

      dayNarrow: createDynamicPropertyGetter(
      function () {
        return getDayNames('narrow');
      }),

      monthWide: createDynamicPropertyGetter(
      function () {
        return getMonthNames('wide');
      }),

      monthAbbreviated: createDynamicPropertyGetter(
      function () {
        return getMonthNames('abbreviated');
      }),

      datePicker: createDynamicPropertyGetter(
      function () {
        return (parseJSONFromFontFamily('oj-inputdatetime-option-defaults') || {})
          .datePicker;
      }),

      renderMode: createDynamicPropertyGetter(
      function () {
        return (parseJSONFromFontFamily('oj-inputdatetime-option-defaults') || {})
          .renderMode;
      }),

      keyboardEdit: createDynamicPropertyGetter(
      function () {
        return (parseJSONFromFontFamily('oj-inputdatetime-option-defaults') || {})
          .keyboardEdit;
      })
    ...
    Object.defineProperty(self, 'isoMin', {
      ...
      set: function (iso) {
        ...
        if (!isNaN(newMin)) {
          ...
          error('Invalid ISO min time: %s', iso);
        }
      }
    });
    ...
    Object.defineProperty(self, 'isoMax', {
      ...
      set: function (iso) {
        ...
        if (!isNaN(newMax)) {
          ...
          error('Invalid ISO max time: %s', iso);
        }
      }
    });
    ...
    Object.defineProperty(self, 'isoValue', {
      ...
      set: function (iso) {
        ...
        if (!isNaN(newValue)) {
          ...
          error('Invalid ISO value time: %s', iso);
        }
      }
    });
  ...
  function defineEvents() {
    var hammerOptions = {
      recognizers: [
        ...
        [Tap],
        [Swipe, { direction: DIRECTION_VERTICAL }]
      ]
    };
    ...
    for (i = 0; i < codes.length; i++) {
      switch (codes[i]) {
        ...
        case 'a':
          ...
          break;
        default:
          error('Unknown wheelOrder code: %s', codes[i]);
          break;
      }
    }
   ...
   */
    ...
    // ojInputDateTime
      if (this.options.datePickerComp === null && !this._IsCustomElement()) {
        EditableValueUtils.initializeOptionsFromDom(this._GET_INIT_OPTIONS_PROPS_FOR_WIDGET,
   ...
   */
    ...
    _createWheelPicker: function () {
      ...
      // var animation = _isLargeScreen ? {"open": null, "close": null} : {"close": null};
        ...
        if (pickerAttrs) {
          EditableValueUtils.setPickerAttributes(this._popUpWheelPicker.ojPopup('widget'),
                                                    pickerAttrs);
        }
      }
    },
    ...
    _timepickerShowing: function () {
      ...
      if (this._isIndependentInput()) {
        ...
        return (picker && isComponentInitialized(picker, 'ojPopup') &&
      ...
      return (widget._isShowingDatePickerSwitcher() &&
              picker && isComponentInitialized(picker, 'ojPopup') &&
              picker.ojPopup('isOpen')) ||
        this._nativePickerShowing;
    },
   ...
   */
    ...
    _setupResizePopupBind: function () {
      this._resizePopupBind = function () {
        ...
        if (this._isIndependentInput() && isComponentInitialized(this._popUpWheelPicker, 'ojPopup')) {
...
.bind(this);
    ...
    _cleanUpTimeResources: function () {
      ...
      if (this._isIndependentInput() && this._popUpWheelPicker && isComponentInitialized(this._popUpWheelPicker, 'ojPopup')) {
    ...
    _showNativeTimePicker: function () {
      ...
      if (minDate) {
        ...
        var minDateProcessed = _getNativePickerDate(
          converter, IntlConverterUtils._minMaxIsoString(minDate,
      ...
      if (maxDate) {
        ...
        var maxDateProcessed = _getNativePickerDate(
          converter, IntlConverterUtils._minMaxIsoString(maxDate,
                                                            this._getIsoDateValue(converter)));
      ...
      function onTimePicked(cbDate) {
        ...
        if (cbDate) {
          ...
          var isoString =
              IntlConverterUtils._dateTime(self._getIsoDateValue(cbConverter),
                {
      ...
      function onError(error$1) {
        ...
        if (error$1.startsWith('cancel')) {
          ...
          error('Error: native time picker failed: ' + error$1);
        }
      }
   ...
   */
    ...
    _getIsoDateLimit: function (converter, optionName, valueDate) {
      ...
      dateIso = dateIso ?
        IntlConverterUtils._minMaxIsoString(dateIso, this._getValue()) :
        dateIso;
      ...
      var date = dateIso ? IntlConverterUtils._clearTime(dateIso) : null;
      if (valueDate && date &&
    ...
    _getIsoDateValue: function (converter) {
      ...
      if (!processDateIso) {
        ...
        processDateIso = IntlConverterUtils.dateToLocalIso(processDateIso);
      }
   ...
   */
    _getAmPmStrings: function () {
      var converter = new IntlDateTimeConverter({ pattern: 'a' });
   ...
   */
    _createWheelPickerDom: function (keepHeaderFooter) {
      ...
      if (options.isoStrFormat === 'zulu') {
        ...
        converter = new IntlDateTimeConverter(options);
      ...
      var valueDate;
      if (!value) {
        value = IntlConverterUtils.dateToLocalIso(date);
      }
      try {
        value = converter.parse(value); // Convert to proper timezone
        valueDate = converterUtils._clearTime(value);
      } catch (e) {
        info('The value of the InputTime element should be an ISOString, please use a valid ISOString');
        value = IntlConverterUtils.dateToLocalIso(date);
        valueDate = converterUtils._clearTime(value);
      }
...
setDefaultOptions(
  {
    ojInputTime:
    {
      renderMode: createDynamicPropertyGetter(
      function () {
        return (parseJSONFromFontFamily('oj-inputdatetime-option-defaults') || {}).renderMode;
      }),
      keyboardEdit: createDynamicPropertyGetter(
      function () {
        return (parseJSONFromFontFamily('oj-inputdatetime-option-defaults') || {}).keyboardEdit;
      }),

      timePicker: createDynamicPropertyGetter(
      function () {
        return (parseJSONFromFontFamily('oj-inputdatetime-option-defaults') || {}).timePicker;
      })
    }
  }
);
        ...
        switchDone: function (evt) {
          ...
          if ((evt.type === 'keydown' &&
            ...
            if (self._switcherTimeValue) {
              newVal = IntlConverterUtils._copyTimeOver(
  ...
  getValueForInputTime: function () {
    ...
    if (this._isDateTimeSwitcher()) {
      ...
      if (this._switcherTimeValue && value) {
        value = IntlConverterUtils._copyTimeOver(this._switcherTimeValue,
          value);
      }
    }
