 ...
 * @ignore
 */
import { Obj, Shadow, CSSStyle, JsonUtils, Rectangle, ArrayUtils, CustomAnimation, Animator, ColorUtils, AnimFadeIn, AnimFadeOut, PatternFill, SolidFill, Agent, SimpleObjPeer, DataAnimationHandler, BaseComponentDefaults, EventManager, EventFactory, KeyboardEvent, OutputText, DisplayableUtils, Rect, Button, TextUtils, Container, KeyboardFocusEffect, AfComponentFactory, MouseEvent, KeyboardHandler, Math as Math$1, Point, Path, Stroke, PathUtils, Circle, ToolkitUtils, IconButton, Displayable, MultilineText, Automation, BaseComponent, AfContext, BlackBoxAnimationHandler, CategoryRolloverHandler, SelectionHandler, ParallelPlayable } from 'ojs/ojdvt-toolkit';
...
Obj.createSubclass(DvtTreeNode, Obj);
...
DvtTreeNode.__NODE_SELECTED_SHADOW = new Shadow(Math.sqrt(2), Math.sqrt(2), 5/3, 'rgba(0, 0, 0, 0.5)');
...
DvtTreeNode.prototype.animateUpdate = function(handler, oldNode) {
  ...
  if (!ArrayUtils.equals(startState, endState)) {
    ...
    nodePlayable = new CustomAnimation(this.getView().getCtx(), this, this.getView().__getAnimationDuration());
    nodePlayable.getAnimator().addProp(Animator.TYPE_NUMBER_ARRAY, this, this.GetAnimationParams, this.SetAnimationParams, endState);
    ...
    var bColorChanged = (ColorUtils.getRGBA(this._color) != ColorUtils.getRGBA(oldNode._color));
...
DvtTreeNode.prototype.animateInsert = function(handler) {
  ...
  if (!handler.isDrillAnimation() || !handler.isAncestorInsert(this)) {
    ...
    var anim = new AnimFadeIn(this.getView().getCtx(), this, this.getView().__getAnimationDuration());
...
DvtTreeNode.prototype.animateDelete = function(handler, container) {
  ...
  var anim = new AnimFadeOut(this.getView().getCtx(), this, this.getView().__getAnimationDuration());
...
DvtTreeNode.prototype.GetFill = function() {
  if (this.hasPattern())
    return new PatternFill(this._pattern, this._color);
  else
    return new SolidFill(this._color);
};
...
DvtTreeNode.prototype.isRootNode = function() {
  return Obj.compareValues(this.getView().getCtx(), this.getId(), this.getView().getRootNode().getId()) || this.isArtificialRoot();
};
...
Obj.createSubclass(DvtTreePeer, SimpleObjPeer);
...
Obj.createSubclass(DvtTreeUtils, Obj);
...
Obj.createSubclass(DvtTreeAnimationHandler, DataAnimationHandler);
...
DvtTreeAnimationHandler.prototype.isAncestorInsert = function(node) {
  if (this._bDrill)
    return Obj.compareValues(this.getCtx(), this._oldRoot.getId(), node.getId()) ||
...
Obj.createSubclass(DvtBreadcrumbsDefaults, Obj);
...
DvtBreadcrumbsDefaults.calcOptions = function(userOptions) {
  ...
  if (!userOptions)
    ...
    return JsonUtils.merge(userOptions, defaults);
};
...
Obj.createSubclass(DvtBreadcrumbsPeer, SimpleObjPeer);
...
Obj.createSubclass(DvtBreadcrumbsEventManager, EventManager);
...
Obj.createSubclass(DvtBreadcrumbsRenderer, Obj);
...
DvtBreadcrumbsRenderer._createButtonState = function(context, text, cssStyle) {
  ...
  var state = new Rect(context, 0, 0, labelDims.w + padLeft + padRight, labelDims.h + padTop + padBottom);
...
DvtBreadcrumbsRenderer._createLabel = function(context, textStr, options, bEnabled) {
  ...
  if (bEnabled && (options.labelStyleOver || options.labelStyleDown)) {
    ...
    if (cssStyle.getPadding(CSSStyle.PADDING_LEFT) || cssStyle.getPadding(CSSStyle.PADDING_RIGHT) || cssStyle.getPadding(CSSStyle.PADDING_TOP) || cssStyle.getPadding(CSSStyle.PADDING_BOTTOM)) {
...
DvtBreadcrumbsRenderer._truncateLabels = function(label, maxWidth) {
  if (label instanceof Button) {
    var ena = label.upState;
    var text = ena.getChildAt(0);
    TextUtils.fitText(text, Math.max(maxWidth - text.getTranslateX(), 0), Infinity, text.getParent());
...
DvtBreadcrumbsRenderer._positionLabels = function(breadcrumbs, container, availWidth, labels, peers) {
  ...
  for (var i = 0; i < labels.length; i++) {
    ...
    if (Agent.isTouchDevice()) {
      var rect = new Rect(container.getCtx(), -DvtBreadcrumbsRenderer._TOUCH_BUFFER, -DvtBreadcrumbsRenderer._TOUCH_BUFFER,
    ...
    if (x + dims.w > availWidth) {
      ...
      if (i < peers.length)
        ...
        eventManager.associate(labels[i], new SimpleObjPeer(labelString));
 ...
 */
const DvtBreadcrumbs = function(context, callback, callbackObj, options) {
  this.Init(context, callback, callbackObj, options);
};

Obj.createSubclass(DvtBreadcrumbs, Container);
...
Obj.createSubclass(DvtSunburstNode, DvtTreeNode);
...
/** @const **/
DvtSunburstNode.LARGE_DATASET_THRESHOLD = 1000;
...
DvtSunburstNode.prototype.render = function(container) {
  ...
  if (this._showRootContent() && template) {
    ...
    if (aw > 0 && ah > 0) {
      ...
      var afRoot = AfComponentFactory.parseAndLayout(afContext, template, this._shape);
    ...
    if (this._text != null) {
      ...
      if (this.hasPattern() || backgroundColor) {
        ...
        this._textBackground = new Rect(this.getView().getCtx(), dims.x, dims.y, dims.w, dims.h);
        if (backgroundColor)
...
DvtSunburstNode.prototype.renderChildren = function(container) {
  ...
  var children = this.getChildNodes();
  var hasLargeNodeCount = this.getView().__getNodeCount() >= DvtSunburstNode.LARGE_DATASET_THRESHOLD;
  if (children != null) {
    var prevEndCoord = 0;
    for (var i = 0; i < children.length; i++) {
  ...
  switch (keyCode)
  {
    case KeyboardEvent.UP_ARROW:
      ...
      if (this === root || midAngle > 180)
      {
        ...
        next = KeyboardHandler.getNextAdjacentNavigable(this, event, this.getDescendantNodes());
      }
...
DvtSunburstNode.prototype._createShapeNode = function() {
  ...
  if (shape.getCtx().getThemeBehavior() !== 'redwood') {
    ...
    shape.setSelectedStroke(new Stroke(nodeDefaults['selectedInnerColor'], 1, 1.5), new Stroke(nodeDefaults['selectedOuterColor'], 1, 3.5));  
    shape.setSelectedHoverStroke(new Stroke(nodeDefaults['hoverColor'], 1, 3));
  }
  else {
    var hoverColor = this.getColor();
    shape.setHoverStroke(new Stroke(nodeDefaults['selectedInnerColor'], 1, 1), new Stroke(hoverColor, 1, 3.5));
    shape.setSelectedStroke(new Stroke(nodeDefaults['selectedInnerColor'], 1, 1), new Stroke(nodeDefaults['selectedOuterColor'], 1, 3.5));
    shape.setSelectedHoverStroke(new Stroke(nodeDefaults['selectedInnerColor'], 1, 1), new Stroke(nodeDefaults['selectedOuterColor'], 1, 3.5));
  }
...
DvtSunburstNode.prototype._createPathCmd = function() {
  ...
  if (this._angleExtent < DvtSunburstNode.TWO_PI)
  {
    ...
    cmd = PathUtils.moveTo(p1.x, p1.y) +
          PathUtils.arcTo(this._outerRadius, this._outerRadius, this._angleExtent, 1, p2.x, p2.y) +
          PathUtils.lineTo(p3.x, p3.y) +
          PathUtils.arcTo(this._innerRadius, this._innerRadius, this._angleExtent, 0, p4.x, p4.y) +
          PathUtils.closePath();
  }
  else
  {
    ...
    cmd = PathUtils.moveTo(p1.x, p1.y) +
          PathUtils.arcTo(this._outerRadius, this._outerRadius, this._angleExtent / 2, 1, p2.x, p2.y) +
          PathUtils.arcTo(this._outerRadius, this._outerRadius, this._angleExtent / 2, 1, p1.x, p1.y);

    // Add the inner segment for a hollow center
    if (this._innerRadius > 0)
      cmd += PathUtils.moveTo(p4.x, p4.y) +
             PathUtils.arcTo(this._innerRadius, this._innerRadius, this._angleExtent / 2, 0, p3.x, p3.y) +
             PathUtils.arcTo(this._innerRadius, this._innerRadius, this._angleExtent / 2, 0, p4.x, p4.y);
...
DvtSunburstNode.prototype._createTextNode = function(container) {
  ...
  if (this._labelDisplay == 'auto') {
    ...
    // in those browsers.
    if (Agent.browser !== 'ie' && Agent.browser !== 'edge' && Agent.os == 'windows' && !Agent.isEnvironmentTest())
      bRotated = false;
    else
      bRotated = true;
  }
...
DvtSunburstNode.prototype._createTextHoriz = function(container) {
  ...
  // Now create the text
  if (textAnchor) {
    var text = new OutputText(this.getView().getCtx(), this._textStr, textAnchor.x, textAnchor.y, null);
    ...
    return TextUtils.fitText(text, usableSpace, textDims.h, container, labelMinLength) ? text : null;
  }
};
...
DvtSunburstNode.prototype._createTextRotated = function(container) {
  ...
  if (!TextUtils.fitText(text, availWidth, availHeight, container))
    return null;
...
DvtSunburstNode.prototype.getDropSiteFeedback = function() {
  if (this._shape instanceof Circle)
    return new Circle(this.getView().getCtx(), this._shape.getCx(), this._shape.getCy(), this._shape.getRadius());
...
DvtSunburstNode.prototype._getButtonHelper = function(resourceKey) {
  ...
  var button = new IconButton(context, 'outlined', {style: iconStyle, size: DvtSunburstNode._EXPAND_ICON_SIZE}, background, null, this.expandCollapse, this);
  return button;
};
 ...
 */
DvtSunburstNode.prototype.__isIsolated = function() {
...
DvtSunburstNode.prototype.UpdateAriaLabel = function() {
  if (!Agent.deferAriaCreation() && this._shape) // need the null check bc it may fail in unit test (TreemapSelectionTest)
...
DvtSunburstNode.prototype._evaluateExpanded = function(sunburst, props) {
  ...
  if (!props['bArtificialRoot'] && !props['_expanded']) {
    ...
    else if (expanded instanceof Array && !ArrayUtils.hasAnyMapItem(options['_expandedNodes'], [props['id']]))
      this.setDisclosed(false);
  }
};
...
var DvtTreemapNode = function(treemap, props) {
  ...
  this._headerLabelStyle = headerOptions['labelStyle'] ? new CSSStyle(headerOptions['labelStyle']) : null;
  ...
  if (this._isolate == 'auto')
    this._isolate = Agent.isTouchDevice() ? 'off' : 'on';

  // TODO  Switch to determining dynamically, which involves changing how isolate/restore is handled
  this._bIsolated = (options['isolatedNode'] != null && Obj.compareValues(this.getView().getCtx(), options['isolatedNode'], this.getId()));
};

// Make DvtTreemapNode a subclass of DvtTreeNode
Obj.createSubclass(DvtTreemapNode, DvtTreeNode);
...
DvtTreemapNode.prototype.setSelected = function(selected) {
  ...
  if (this.isSelected()) {
    ...
    if (isRedwood && this._textStyle !== DvtTreemapNode.TEXT_STYLE_HEADER) {
      this._selectionOuter = new Rect(this.getView().getCtx(), x + .5, y + .5, Math.max(w - 1, 0), Math.max(h - 1, 0));
      this._selectionInner = new Rect(this.getView().getCtx(), x + 1.5, y + 1.5, Math.max(w - 3, 0), Math.max(h - 3, 0));
    }
    else {
      this._selectionOuter = new Rect(this.getView().getCtx(), x, y, w, h);
      this._selectionInner = new Rect(this.getView().getCtx(), x + 1, y + 1, Math.max(w - 2, 0), Math.max(h - 2, 0));
    }
...
DvtTreemapNode.prototype.showHoverEffect = function() {
  ...
  if (this._textStyle == DvtTreemapNode.TEXT_STYLE_HEADER) {
    ...
    stroke = new Stroke(nodeHeaderDefaults['hoverInnerColor'], DvtTreemapNode.GROUP_HOVER_INNER_OPACITY, DvtTreemapNode.GROUP_HOVER_INNER_WIDTH);
    ...
    if (this._shape.getCtx().getThemeBehavior() === 'redwood') {
      x = this._x + 1.5;
      y = this._y + 1.5;
      w = this._width - 3 - DvtTreemapNode._LINE_FUDGE_FACTOR;
      h = this._height - 3 - DvtTreemapNode._LINE_FUDGE_FACTOR;
      ...
      stroke = new Stroke(nodeDefaults['hoverColor'], DvtTreemapNode.NODE_HOVER_OPACITY, DvtTreemapNode.NODE_SELECTION_WIDTH);
    }
    //  - treemap visualization issues
    h = Math.max(h, 0);
    w = Math.max(w, 0);
...
DvtTreemapNode.prototype.getNextNavigable = function(event) {
  ...
  if ((keyCode == KeyboardEvent.UP_ARROW && event.altKey) || keyCode == KeyboardEvent.CLOSE_BRACKET) {
  ...
  else if ((keyCode == KeyboardEvent.DOWN_ARROW && event.altKey) || keyCode == KeyboardEvent.OPEN_BRACKET) {
...
DvtTreemapNode.prototype._getIsolateAnimation = function() {
  if (this._oldState) {
    ...
    var playable = new CustomAnimation(this.getView().getCtx(), this, DvtTreemapNode._ANIMATION_ISOLATE_DURATION);
    playable.getAnimator().addProp(Animator.TYPE_NUMBER_ARRAY, this, this.GetAnimationParams, this.SetAnimationParams, this.GetAnimationParams());
...
DvtTreemapNode.prototype._createShapeNode = function() {
  ...
  if (this._textStyle == DvtTreemapNode.TEXT_STYLE_HEADER) {
    ...
    if (bNodeGaps) {
      ...
      shape = new Rect(context, rects._shape.x, rects._shape.y, rects._shape.w, rects._shape.h);
      this._innerShape = new Rect(context, rects._innerShape.x, rects._innerShape.y, rects._innerShape.w, rects._innerShape.h);

      // Also draw a background shape to prevent bleed through during isolate
      this._backgroundShape = new Rect(context, rects._backgroundShape.x, rects._backgroundShape.y, rects._backgroundShape.w, rects._backgroundShape.h);
      ...
      this._innerShape = new Rect(context, this._x + 1, this._y + 1, this._width - 2, this._height - 2);
    }
    ...
    if (bNodeGaps) {
      ...
      if (bVisualEffects && this._width >= DvtTreemapNode.MIN_SIZE_FOR_BORDER && this._height >= DvtTreemapNode.MIN_SIZE_FOR_BORDER) {
        ...
        var topLeft = new Stroke(DvtTreemapNode.DEFAULT_NODE_TOP_BORDER_COLOR);
        var bottomRight = new Stroke(DvtTreemapNode.DEFAULT_NODE_BOTTOM_BORDER_COLOR, DvtTreemapNode.DEFAULT_NODE_BORDER_OPACITY);
        if (this.hasPattern()) {
          topLeft = new Stroke(this._color, DvtTreemapNode.DEFAULT_NODE_PATTERN_BORDER_OPACITY);
          bottomRight = topLeft;
        }

        // Retrieve the bevel colors and blend with the fill color to get the desired effect
        var fillColor = this.getColor();
        var topLeftColor = ColorUtils.interpolateColor(DvtTreemapNode.DEFAULT_NODE_TOP_BORDER_COLOR, fillColor, 1 - DvtTreemapNode.DEFAULT_NODE_BORDER_OPACITY);
        var bottomRightColor = ColorUtils.interpolateColor(DvtTreemapNode.DEFAULT_NODE_BOTTOM_BORDER_COLOR, fillColor, 1 - DvtTreemapNode.DEFAULT_NODE_BORDER_OPACITY);
        ...
        if (minDim >= 4) {
          ...
          this._topLeftShape = new Rect(context, this._x, this._y, this._width - 1, this._height - 1);
          ...
          this._fillShape = new Rect(context, this._x + 1, this._y + 1, this._width - 2, this._height - 2);
          ...
          this._fillShape = new Rect(context, this._x, this._y, this._width - 1, this._height - 1);
          this._fillShape.setFill(fill);
          this._fillShape.setMouseEnabled(false);
          shape.addChild(this._fillShape);
        }
...
DvtTreemapNode.prototype._createIsolateRestoreButton = function(container) {
  ...
  if (availIconWidth > DvtTreemapNode._ISOLATE_ICON_SIZE) {
    ...
    // Add a buffer to make the objects easier to interact with on touch devices
    if (Agent.isTouchDevice()) {
      var rect = new Rect(container.getCtx(), - DvtTreemapNode._ISOLATE_TOUCH_BUFFER, - DvtTreemapNode._ISOLATE_TOUCH_BUFFER, DvtTreemapNode._ISOLATE_ICON_SIZE + 2 * DvtTreemapNode._ISOLATE_TOUCH_BUFFER, DvtTreemapNode._ISOLATE_ICON_SIZE + 2 * DvtTreemapNode._ISOLATE_TOUCH_BUFFER);
      rect.setInvisibleFill();
...
DvtTreemapNode.prototype._createTextNode = function(container) {
  ...
  if (this._textStyle == DvtTreemapNode.TEXT_STYLE_NODE && availHeight > textHeight * 2 && computedTextStyle.getStyle(CSSStyle.WHITE_SPACE) != 'nowrap' &&
      TextUtils.getTextStringWidth(this.getView().getCtx(), this._textStr, computedTextStyle) > availWidth) {
    text = new MultilineText(this.getView().getCtx(), this._textStr);
  }
  else
    text = new OutputText(this.getView().getCtx(), this._textStr);
  ...
  if (this._textStyle == DvtTreemapNode.TEXT_STYLE_NODE) {
    ...
    var chromeAdjustment = (Agent.browser === 'safari' || Agent.engine === 'blink') ? DvtTreemapNode._LINE_FUDGE_FACTOR : 0;
  ...
  if (text != null) {
    ...
    // Truncate the text if necessary
    var labelMinLength = this.getView().getOptions()['nodeDefaults']['labelMinLength'];
    var renderText = TextUtils.fitText(text, availWidth, availHeight, container, labelMinLength);
...
DvtTreemapNode.prototype.ApplyHeaderStyle = function(shape, innerShape) {
  ...
  if (this._bHeaderUseNodeColor) {
    ...
    var borderColor = ColorUtils.interpolateColor(nodeHeaderDefaults['borderColor'], fillColor, 1 - DvtTreemapNode.DEFAULT_HEADER_WITH_NODE_COLOR_ALPHA);
...
DvtTreemapNode.prototype.ApplyHeaderTextStyle = function(text, styleType) {
  ...
  textStyle.push(new CSSStyle(this.getView().getOptions()['nodeDefaults']['header']["labelStyle"]));
...
DvtTreemapNode.prototype._getGeometriesWithGaps = function() {
  ...
  if (this._textStyle == DvtTreemapNode.TEXT_STYLE_HEADER) {
    ...
    ret._innerShape = new Rectangle(this._x + 1, this._y + 1, this._width - 3, this._titleBarHeight - 1);
    ...
    ret._backgroundShape = new Rectangle(this._x, this._y + this._titleBarHeight, adjustedBackgroundShapeWidth, adjustedBackgroundShapeHeight);
  }
  else {
    // Non-header node with children, make 0 height so it doesn't bleed through the gaps.
    if (this.hasChildren())
      ret._shape = new Rectangle(this._x, this._y, 0, 0);
    else {
...
DvtTreemapNode.prototype.getDropSiteFeedback = function() {
  if (this._shape) {
    return new Rect(this.getView().getCtx(), this._shape.getX(), this._shape.getY(), this._shape.getWidth(), this._shape.getHeight());
  }
  else
    return null;
};
...
DvtTreemapNode.prototype._getButtonHelper = function(resourceKey, callback) {
  ...
  var iconStyle = ToolkitUtils.getIconStyle(context, iconClass);
  // Create button and hook up click listener
  var button = new IconButton(context, 'borderless', {style: iconStyle, size: DvtTreemapNode._ISOLATE_ICON_SIZE});
...
Obj.createSubclass(DvtTreeAutomation, Automation);
...
Obj.createSubclass(DvtTreeBreadcrumbsRenderer, Obj);
...
Obj.createSubclass(DvtTreeEventManager, EventManager);
...
Obj.createSubclass(DvtTreeKeyboardHandler, KeyboardHandler);
...
Obj.createSubclass(DvtTreeLegendRenderer, Obj);
  ...
  if (sizeValueStr || colorValueStr) {
    ...
    if (sizeWidth + colorWidth > availWidth) {
      ...
      if (sizeWidth > widthPerSection && colorWidth > widthPerSection) {
        ...
        if (TextUtils.fitText(sizeValueLabel, availWidth - colorWidth - sizeLabelWidth - DvtTreeLegendRenderer._LEGEND_LABEL_GAP, Infinity, labelContainer)) {
          sizeValueLabelWidth = sizeValueLabel.getDimensions().w;
          eventManager.associate(sizeValueLabel, new SimpleObjPeer(sizeValueStr));
        }
        else {
        ...
        if (TextUtils.fitText(colorValueLabel, availWidth - sizeWidth - colorLabelWidth - DvtTreeLegendRenderer._LEGEND_LABEL_GAP, Infinity, labelContainer)) {
          colorValueLabelWidth = colorValueLabel.getDimensions().w;
          eventManager.associate(colorValueLabel, new SimpleObjPeer(colorValueStr));
        }
        else {
...
Obj.createSubclass(DvtTreeView, BaseComponent);
...
DvtTreeView.prototype.render = function(options, width, height)
{
  ...
  if (this._templates) {
    this._afContext = new AfContext(this.getCtx(), this.EventManager);
  ...
  if (!this._container) {
    ...
    if (BlackBoxAnimationHandler.isSupported(animationOnDataChange)) {
      // Black Box Animation
      this.Animation = BlackBoxAnimationHandler.getCombinedAnimation(this.getCtx(), animationOnDataChange, this._container, container, bounds, this.AnimationDuration);
...
DvtTreeView.prototype.RenderEmptyText = function(container) {
  ...
  this.renderEmptyText(container, emptyText, new Rectangle(0, 0, this.Width, this.Height), this.getEventManager(), options['_statusMessageStyle']);
};
...
DvtTreeView.prototype.GetDisplayAnimation = function(container, bounds) {
  ...
  if (BlackBoxAnimationHandler.isSupported(animationOnDisplay))
    return BlackBoxAnimationHandler.getInAnimation(this.getCtx(), animationOnDisplay, container, bounds, this.AnimationDuration);
  else
    return null;
};
...
DvtTreeView.prototype.highlight = function(categories) {
  ...
  CategoryRolloverHandler.highlight(categories, DvtTreeUtils.getAllNodes(this._root), this.getOptions()['highlightMatch'] == 'any');
};
...
DvtTreeView.prototype.ApplyParsedProperties = function(props) {
  ...
  if (options['selectionMode'] == 'none')
    ...
    this._nodeSelection = SelectionHandler.TYPE_MULTIPLE;
  ...
  if (templates) {
    ...
    for (var templateKey in templates) {
      var afComponent = AfComponentFactory.parseJsonElement(templates[templateKey]);
...
DvtTreeView.prototype.__drill = function(id, bDrillUp) {
  ...
  if (bDrillUp && this._root && id == this._root.getId() && this._ancestors.length > 0) {
    ...
    this.dispatchEvent(EventFactory.newTreeDrillEvent(this._ancestors[0].id, DvtTreeUtils.findRootAndAncestors(this.getCtx(), this.getOptions()['_nodes'], this._ancestors[0].id, [])['root'], component));
  }
  else if (!bDrillUp) // Fire the event
    this.dispatchEvent(EventFactory.newTreeDrillEvent(id, DvtTreeUtils.findRootAndAncestors(this.getCtx(), this.getOptions()['_nodes'], id, [])['root'], component));
...
Obj.createSubclass(DvtTreeDefaults, BaseComponentDefaults);
...
DvtTreeDefaults.SKIN_ALTA = {
  ...
  'styleDefaults': {
    '_attributeTypeTextStyle': new CSSStyle(BaseComponentDefaults.FONT_FAMILY_ALTA_BOLD_12 + 'color:#4F4F4F'),
...
DvtTreeDefaults.prototype.Init = function(defaultsMap, context) {
  ...
  var ret = {
    'alta': JsonUtils.merge(defaultsMap['alta'], DvtTreeDefaults.SKIN_ALTA)
  };
...
Obj.createSubclass(DvtSunburstDefaults, DvtTreeDefaults);
...
Obj.createSubclass(DvtSunburstEventManager, DvtTreeEventManager);
...
Obj.createSubclass(DvtSunburstLayout, Obj);
...
DvtSunburstLayout._layout = function(radiusPerDepth, node, startAngle, angleExtent, sorting, innerRadius)
{
  ...
  if (children != null && node.isDisclosed()) {
    ...
    if (Agent.isRightToLeft(node.getView().getCtx()))
 ...
 * @extends {DvtTreeView}
 */
const Sunburst = function(context, callback, callbackObj) {
  this.Init(context, callback, callbackObj);
};

// Make Sunburst a subclass of DvtTreeView
Obj.createSubclass(Sunburst, DvtTreeView);

/** @const @private **/
Sunburst._ROTATION_SHAPE_RADIUS = 15; // size of additional radius for mouse rotation detection
/** @const @private **/
Sunburst._ROTATION_SHAPE_RADIUS_TOUCH = 60; // size of additional radius for mouse rotation detection
/** @const @private **/
Sunburst._ANIMATION_TYPE_FAN = 'fan';
/** @const @private **/
Sunburst._BACKGROUND_INLINE_DEFAULT = '';

// Layout Constants
/** @const @private **/
Sunburst._BUFFER_SPACE = 3;
/** @const @private **/
Sunburst._MIN_BUFFER_SPACE = 2; // Minimum buffer for very small sunbursts

/**
 * Returns a new instance of Sunburst.
 * @param {dvt.Context} context The rendering context.
 * @param {string} callback The function that should be called to dispatch component events.
 * @param {object} callbackObj The optional object instance on which the callback function is defined.
 * @return {Sunburst}
 */
Sunburst.newInstance = function(context, callback, callbackObj) {
 ...
 * @override
 */
Sunburst.prototype.Init = function(context, callback, callbackObj) {
  Sunburst.superclass.Init.call(this, context, callback, callbackObj);
 ...
 * @override
 */
Sunburst.prototype.ApplyParsedProperties = function(props) {
  Sunburst.superclass.ApplyParsedProperties.call(this, props);
 ...
 * @override
 */
Sunburst.prototype.Layout = function(availSpace) {
  // Update available space
  var bufferSpace = Math.max(Math.ceil(Sunburst._BUFFER_SPACE * Math.min(availSpace.w, availSpace.h) / 400), Sunburst._MIN_BUFFER_SPACE);
 ...
 * @override
 */
Sunburst.prototype.Render = function(container, bounds) {
  // Background
  this.RenderBackground(container, Sunburst._BACKGROUND_INLINE_DEFAULT);
  ...
  if (this.__isRotationEnabled() && this.HasValidData()) {
    var buffer = (Agent.isTouchDevice()) ? Sunburst._ROTATION_SHAPE_RADIUS_TOUCH : Sunburst._ROTATION_SHAPE_RADIUS;
    var rotationShape = new Circle(this.getCtx(), bounds.x + bounds.w / 2, bounds.y + bounds.h / 2, this._totalRadius + buffer);
 ...
 * @override
 */
Sunburst.prototype.CreateEventManager = function(view, context, callback, callbackObj)
{
 ...
 * @override
 */
Sunburst.prototype.GetDisplayAnimation = function(container, bounds) {
  if (this.getOptions()['animationOnDisplay'] === Sunburst._ANIMATION_TYPE_FAN && this.HasValidData()) {
    // Set up the initial state
    this._animateAngleExtent(0);

    // Create and return the animation
    var anim = new CustomAnimation(this.getCtx(), this, this.AnimationDuration);
    anim.getAnimator().addProp(Animator.TYPE_NUMBER, this, this.__getAngleExtent, this._animateAngleExtent, 2 * Math.PI);
    return anim;
  }
  else
    return Sunburst.superclass.GetDisplayAnimation.call(this, container, bounds);
};
 ...
 * @override
 */
Sunburst.prototype.GetDeleteContainer = function() {
  var ret = Sunburst.superclass.GetDeleteContainer.call(this);
  if (ret) {
 ...
 * @override
 */
Sunburst.prototype.OnAnimationEnd = function() {
  ...
  Sunburst.superclass.OnAnimationEnd.call(this);
};
 ...
 */
Sunburst.prototype.__moveToHoverLayer = function(displayable) {
 ...
 * @param {dvt.Displayable} displayable The object to be moved.
 */
Sunburst.prototype.__moveToSelectedLayer = function(displayable) {
 ...
 */
Sunburst.prototype.__getAngleExtent = function() {
 ...
 * @private
 */
Sunburst.prototype._animateAngleExtent = function(extent) {
 ...
 */
Sunburst.prototype.__startRotation = function(mouseX, mouseY) {
 ...
 */
Sunburst.prototype.__setRotationAnchor = function(angle) {
 ...
 */
Sunburst.prototype.__continueRotation = function(mouseX, mouseY) {
 ...
 */
Sunburst.prototype.__rotate = function(newAngle) {
 ...
 */
Sunburst.prototype.__endRotation = function() {
  // Clear flags
 ...
 * @override
 */
Sunburst.prototype.SetOptions = function(options) {
  Sunburst.superclass.SetOptions.call(this, options);
 ...
 */
Sunburst.prototype.expandCollapseNode = function(id) {
  ...
  this.dispatchEvent(new EventFactory.newExpandCollapseEvent(bDisclosed ? 'expand' : 'collapse', id, DvtTreeUtils.findRootAndAncestors(this.getCtx(), this.getOptions()['_nodes'], id, [])['root'], this.getOptions()['_widgetConstructor'], expanded));
  this.__setNavigableIdToFocus(id);
};
 ...
 * @private
 */
Sunburst.prototype._calcAngle = function(x, y) {
 ...
 * @private
 */
Sunburst.prototype._updateStartAngle = function(change) {
 ...
 * @override
 */
Sunburst.prototype.__getNodeUnderPoint = function(x, y) {
 ...
 * @override
 */
Sunburst.prototype.__showDropSiteFeedback = function(node) {
  var feedback = Sunburst.superclass.__showDropSiteFeedback.call(this, node);
  if (feedback) {
 ...
 * @override
 */
Sunburst.prototype.CreateNode = function(nodeOptions) {
...
Obj.createSubclass(DvtTreemapDefaults, DvtTreeDefaults);
...
DvtTreemapDefaults.SKIN_ALTA = {
  ...
  'nodeDefaults': {
    'header': {
      ...
      'isolate': 'auto',
      'labelHalign': 'start',
      '_labelStyle': new CSSStyle(BaseComponentDefaults.FONT_FAMILY_ALTA_12 + 'color:#252525;'),
...
Obj.createSubclass(DvtTreemapEventManager, DvtTreeEventManager);
...
Obj.createSubclass(DvtTreemapKeyboardHandler, DvtTreeKeyboardHandler);
...
DvtTreemapKeyboardHandler.prototype.isNavigationEvent = function(event)
{
  ...
  {
    var keyCode = event.keyCode;
    if (keyCode == KeyboardEvent.OPEN_BRACKET ||
...
Obj.createSubclass(DvtTreemapLayoutBase, Obj);
...
Obj.createSubclass(DvtTreemapLayoutSliceAndDice, DvtTreemapLayoutBase);
...
DvtTreemapLayoutSliceAndDice.prototype._layout = function(isHoriz, view, node, x, y, width, height, isRoot)
{
  ...
  if (children != null) {
    ...
    // BIDI Support: For horizontal layout, reverse the order of the nodes
    if (isHoriz && Agent.isRightToLeft(view.getCtx()))
      children = children.slice(0).reverse();
...
Obj.createSubclass(DvtTreemapLayoutSquarifying, DvtTreemapLayoutBase);
 ...
 * @extends {DvtTreeView}
 */
const Treemap = function(context, callback, callbackObj) {
  this.Init(context, callback, callbackObj);
};

// Make Treemap a subclass of DvtTreeView
Obj.createSubclass(Treemap, DvtTreeView);

/** @const @private **/
Treemap._BUFFER_SPACE = 7; // This is in addition to gap between groups

/** @const @private **/
Treemap._MIN_BUFFER_SPACE = 1; // Minimum buffer for very small treemaps

/** @const @private **/
Treemap._BACKGROUND_FILL_COLOR = '#EBEFF5';

/** @const @private **/
Treemap._BACKGROUND_BORDER_COLOR = '#DBE0EA';

/** @const @private **/
Treemap._BACKGROUND_INLINE_DEFAULT = 'background-color:' + Treemap._BACKGROUND_FILL_COLOR + ';' +
    'border-color:' + Treemap._BACKGROUND_BORDER_COLOR + ';' +
    'border-width:2px';
 ...
 * @return {Treemap}
 */
Treemap.newInstance = function(context, callback, callbackObj) {
 ...
 * @override
 */
Treemap.prototype.Init = function(context, callback, callbackObj) {
  Treemap.superclass.Init.call(this, context, callback, callbackObj);
...
Treemap.prototype.Layout = function(availSpace) {
  ...
  bufferSpace = Math.max(Math.ceil(bufferSpace * Math.min(availSpace.w, availSpace.h) / 400), Treemap._MIN_BUFFER_SPACE);
  availSpace.x += bufferSpace;
  availSpace.y += bufferSpace;
  availSpace.w -= 2 * bufferSpace;
  availSpace.h -= 2 * bufferSpace;
 ...
 * @override
 */
Treemap.prototype.Render = function(container, bounds) {
  // Background
  this.RenderBackground(container, Treemap._BACKGROUND_INLINE_DEFAULT);
 ...
 * @override
 */
Treemap.prototype.OnAnimationEnd = function() {
  ...
  // Delegate to the superclass to clear common things
  Treemap.superclass.OnAnimationEnd.call(this);
};
 ...
 * @override
 */
Treemap.prototype.CreateKeyboardHandler = function(manager)
{
 ...
 * @override
 */
Treemap.prototype.CreateEventManager = function(view, context, callback, callbackObj)
{
 ...
 * @override
 */
Treemap.prototype.GetInitialFocusedItem = function(root)
{
 ...
 */
Treemap.prototype.__showHoverEffect = function(x, y, w, h, stroke) {
 ...
 */
Treemap.prototype.__hideHoverEffect = function() {
  this._hoverEffect.setVisible(false);
};
 ...
 * @return {dvt.Container}
 */
Treemap.prototype.__getGroupTextLayer = function() {
 ...
 */
Treemap.prototype.__moveToSelectedLayer = function(rect) {
 ...
 * @override
 */
Treemap.prototype.__getNodeUnderPoint = function(x, y) {
 ...
 */
Treemap.prototype.__isolate = function(node) {
 ...
 */
Treemap.prototype.__restore = function() {
 ...
 */
Treemap.prototype.__getLastIsolatedNode = function()
{
 ...
 * @private
 */
Treemap.prototype._processInitialIsolate = function(isolateRowKey) {
 ...
 */
Treemap.prototype.__getDefaultNavigable = function(navigableItems)
{
 ...
 * @override
 */
Treemap.prototype.CreateNode = function(nodeOptions) {
 ...
 * @protected
 */
Treemap.prototype.getIsolatedNodes = function() {
 ...
 * @protected
 */
Treemap.prototype.getNodeContent = function() {
