...
import { unwrap, removeResizeListener, addResizeListener, setScrollLeft, isTouchSupported, isMetaKeyPressed } from 'ojs/ojdomutils';
...
StaticContentHandler.prototype.unsetAriaProperties = function (item) {
  enableAllFocusableElements(item);
...
TreeDataProviderContentHandler.prototype._handleFetchError = function (status) {
  ...
  if (this.m_widget == null) {
    info('handleFetchError: widget has already been destroyed');
    return;
  }
...
ContentHandlerMixin.call(IteratingDataProviderContentHandler.prototype);
...
const _ojListView = _ListViewUtils.clazz(Object, /** @lends oj._ojListView.prototype */ {
  ...
  _applyExitAnimation(cards) {
    ...
    cards.forEach(function (card) {
      slideOut(card, { offsetY: '300px', duration: '300ms', persist: 'all' });
      promise = fadeOut(card, { duration: '300ms', persist: 'all' });
    });
  ...
  destroy: function () {
    ...
    unwrap(this.element, this.getListContainer());
  },
  ...
  _clearBusyState: function () {
    if (this.readinessStack && this.readinessStack.length > 0) {
      warn('ListView did not end with a clean state, this could happen if ListView is detached before fetch is complete.  State: ' +
        this.readinessStack);
  ...
  HandleSelectionOption: function (options) {
    if (options.selection != null || options.selected != null) {
      ...
      if (selected != null && selected.isAddAll()) {
        ...
        // eslint-disable-next-line no-param-reassign
        options.selection = KeySetUtils.toArray(selected);
      } else {
  ...
  CreateDataContentHandler: function (data) {
    ...
    if (typeof oj.TableDataSource !== 'undefined' && data instanceof oj.TableDataSource) {
      ...
      contentHandler =
        new IteratingDataProviderContentHandler(this,
          this.element[0],
      ...
      contentHandler = new IteratingDataProviderContentHandler(this,
        this.element[0], data);
    } else {
   ...
   */
  GetSlotMap: function () {
    return CustomElementUtils.getSlotMap(this.GetRootElement()[0]);
  },
  ...
  renderComplete: function () {
        ...
        this._validateSelection(selection).then(function (validRowKeyData) {
          ...
          if (validRowKeyData.validKeys.length === 0) {
            ...
            self._setSelectionOption(
              KeySetUtils.toKeySet(validRowKeyData.validKeys),
    ...
    if (this._isAnimateCards()) {
      ...
      this.element[0].querySelectorAll('.oj-listview-card:not(.oj-listview-card-animated)').forEach(function (card) {
        ...
        if (isInitial) {
          slideIn(card, { offsetY: '300px', delay: delay + 'ms', duration: '300ms' });
          _animationPromise = fadeIn(card, { delay: delay + 'ms', duration: '300ms' });
          delay = Math.min(1000, delay + increment);
        } else {
          fadeIn(card, { duration: '150ms' });
        }
      });
    }
  ...
  _validateKeyForScroll: function (key, skipLocal) {
    ...
    if (this.m_contentHandler instanceof IteratingDataProviderContentHandler) {
      var dataProvider = this.m_contentHandler.getDataProvider();
  ...
  StartAnimation: function (elem, action, effect) {
    ...
    var promise = startAnimation(elem, action, effect, this.ojContext);
  ...
  _enableAllTabbableElements: function (elem) {
    var elems = $(enableAllFocusableElements(elem[0]));
  ...
  _cleanupTabbableElementProperties: function (elem) {
    $(elem).find('[' + _DATA_OJ_TABMOD + ']')
      .removeAttr('tabIndex')
      .removeAttr(_DATA_OJ_TABMOD)
  ...
  */
  ...
  _isNodeFocusable: function (node) {
    ...
    if (node === null) {
      return false;
    } else if (node.hasAttribute(_DATA_OJ_TABMOD)) {
      return true;
  ...
  _isClickthroughDisabled: function (elem) {
    return ((elem.length > 0 && isClickthroughDisabled(elem.get(0))) ||
      elem.hasClass('oj-clickthrough-disabled'));
  },
  ...
  _setSelectionOption: function (newValue, event, selectedElems, firstSelectedItemData) {
    ...
    var selection = KeySetUtils.toArray(newValue);
  ...
  HandleSelectionOrActiveKeyDown: function (event) {
    ...
    if (this._isActionableMode()) {
      ...
      if (key === 'Escape' || key === 'Esc' || key === this.ESC_KEY) {
        ...
        if (event.shiftKey) {
          processed = handleActionablePrevTab(event, focusElem);
        } else {
          processed = handleActionableTab(event, focusElem);
        }

        if ((!event.shiftKey && processed) || (event.shiftKey && !processed)) {
          var firstFocuasble = getFocusableElementsInNode(focusElem)[0];
  ...
  _getOptionDefaults: function () {
    if (this.defaultOptions == null) {
      this.defaultOptions =
        parseJSONFromFontFamily(this.getOptionDefaultsStyleClass());
    }
  ...
  getWidgetConstructor: function () {
    return __GetWidgetConstructor(this.element);
  },
...
oj.__registerWidget('oj.ojListView', $.oj.baseComponent, {
  ...
  getIndexerModel: function () {
    if (this.indexerModel == null && ListViewIndexerModel) {
      this.indexerModel = new ListViewIndexerModel(this.listview);
    }
...
setDefaultOptions({
  ojListView: {
    gridlines: createDynamicPropertyGetter(function () {
      return { item: (parseJSONFromFontFamily('oj-listview-option-defaults') || {}).gridlines.item };
    })
  }
});
