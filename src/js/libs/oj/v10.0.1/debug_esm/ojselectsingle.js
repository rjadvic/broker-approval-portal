...
import CachingDataProvider from 'ojs/ojcachingdataprovider';
...
AbstractLovBase.prototype._usingHandler = function (pos, props) {
  ...
  if (oj$1.PositionUtils.isAligningPositionClipped(props)) {
    ...
    var availableSpace = oj$1.PositionUtils.calcAvailablePopupSize(pos, props);
...
AbstractLovBase.prototype._runQuery = function (term, focusFirstElem, initial) {
  ...
  if (this.hasData()) {
    ...
    fetchPromise.then(function () {
      ...
      // ignore old responses
      if (fetchPromise === this._lastDataProviderPromise) {
        warn('Select: _fetchFromDataProvider promise was rejected: ' + reason);
        this._handleQueryResultsFetch(focusFirstElem);
      }
    }.bind(this));
  }
  ...
  if (term) {
    if (this._dataProvider) {
      ...
      if (!filterCapability || !filterCapability.textFilter) {
        error('Select: DataProvider does not support text filter.  ' +
    ...
    filterCriteria = oj$1.FilterFactory.getFilter({ filterDef: { text: term } });
  }

  var retPromise = new Promise(function (resolve, reject) {
    // fetch data from dataProvider
    var renderPromise = this._lovDropdown.renderResults(term, filterCriteria,
      LovUtils.isValueForPlaceholder(this._value) ? null : this._value, initial);
    ...
    renderPromise.then(function () {
      ...
      // ignore old responses
      if (retPromise === this._lastDataProviderPromise) {
        warn('Select: renderResults promise was rejected: ' + reason);
      }
      afterRenderPromiseFunc();
    });
  }.bind(this));
...
LovDropdown._highlighter = function (unhighlightedText, searchText) {
  if (searchText && searchText.length > 0) {
    ...
    return unhighlightedText.replace(new RegExp(escSearchText, 'gi'),
...
LovDropdown.prototype._defaultItemRenderer = function (listViewItemContext) {
  ...
  var nodes = stringToNodeArray('<span>' + str + '</span>');
...
LovDropdown.prototype.renderResults = function (searchText, filterCriteria,
  ...
  return retPromise.then(function () {
    ...
    // ignore old responses
    if (retPromise === this._lastRenderResultsPromise) {
      warn('Select: LovDropdown.renderResults retPromise rejected: ' + reason);
    }
    afterRetPromiseFunc();
  }.bind(this));
};
...
LovDropdown.prototype._fetchFirstFlatData = function () {
  ...
  var fetchPromise = this._dataProvider.fetchByOffset({ offset: 0, size: 1 });
  var result = null;
  var parseResults = function (fetchResults) {
    if (fetchResults != null) {
      let results = fetchResults.results;
      if (results.length > 0) {
...
LovDropdown.prototype._fetchFirstLeafData = function () {
  ...
  var fetchListParams = { size: 1 };
  var self = this;
  var done = false;
  var fetchDataRecursively = function (dataProvider) {
    let result = null;
    let asyncIterator = dataProvider.fetchFirst(fetchListParams)[Symbol.asyncIterator]();
    let processChunk = function (fetchResults) {
      let data = fetchResults.value.data;
      let metadata = fetchResults.value.metadata;

      let fetchChild = function (childIndex) {
        if (childIndex < data.length) {
          if (!done) {
            let itemData = data[childIndex];
            let itemMetadata = metadata[childIndex];

            let childDataProvider = dataProvider.getChildDataProvider(itemMetadata.key);
            ...
            let item = { data: itemData, metadata: itemMetadata };
      ...
      let fetchChildPromise = fetchChild(0);
...
LovDropdown.prototype._defaultCollectionRenderer = function (context) {
  ...
  if (!this._resultsElem) {
    ...
    busyContext.whenReady().then(function () {
      ...
      if (context.data && oj$1.DataProviderFeatureChecker
        && oj$1.DataProviderFeatureChecker.isTreeDataProvider(context.data)) {
...
LovDropdown.prototype._handleDataProviderEvent = function (event) {
  ...
  busyContext.whenReady().then(function () {
    ...
    warn('Select: LovDropdown.handleDataProviderEvent busyContext promise rejected: ' +
      reason);
    resolveBusyState();
  });
};
...
LovDropdown.prototype.close = function () {
  ...
  if (isAncestor(this.getElement(), activeElem)) {
    activeElem.blur();
  }
...
LovDropdown.prototype._handleCollectionCurrentRowKeyChanged = function (rowKey) {
  ...
  // whenever the rowKey is updated, we need to update the selected property
...
LovDropdown.prototype._createCollectionTemplateContext = function (templateEngine, context) {
  ...
  if (templateEngine) {
    ...
    let currentRow = {};
    templateEngine.defineTrackableProperty(currentRow, 'rowKey', undefined,
      context.currentRowKeyChangedListener);
    templateEngine.defineTrackableProperty(templateContext, 'currentRow', currentRow,
    ...
    error('JET Select: template engine not available when creating context' +
...
LovDropdown.prototype._setCollectionSelectedKeySet = function (rowKey) {
  ...
  var selected;
  if (rowKey != null) {
    selected = new KeySetImpl([rowKey]);
  } else {
...
LovDropdown.prototype._addListenerForRowKeyProperty = function (currentRow) {
  ...
  if (this._templateEngine) {
    this._templateEngine.defineTrackableProperty(currentRow, 'rowKey', rowKey,
      context.currentRowKeyChangedListener);
  }
};
...
LovDropdown.prototype._handleCollectionFocusinOnce = function (data) {
  ...
  if (!recentPointer()) {
    var resolveBusyState = this._addBusyStateFunc(
      'LovDropdown setting selected KeySet on focusin');
...
const FilteringDataProviderView = function (dataProvider) {
  ...
  this.setFilterCriterion = function (filterCriterion) {
    ...
    if (!_compareFilterCriterion(oldFilterCriterion, this._filterCriterion)) {
      var event = new DataProviderRefreshEvent();
...
oj$1.__registerWidget('oj.ojSelect2', $.oj.editableValue, {
  ...
  _ComponentCreate: function () {
    ...
    this._fullScreenPopup = (getDeviceRenderMode() === 'phone');
  ...
  _getTemplateSlot: function (name) {
    var slots = CustomElementUtils.getSlotMap(this.OuterWrapper);
  ...
  _setupSelectResources: function (cachedMainFieldInputElem) {
    ...
    var lovDropdown = new LovDropdown();
    ...
    this._initLovDropdownFunc = function (afterInitFunc) {
      ...
      this._initLovDropdown(idSuffix, inputType, elemId, getTranslatedStringFunc, addBusyStateFunc,
  ...
  _handleContainerKeyDown: function (event) {
    ...
    var lovDropdown = this._lovDropdown;
    // don't call lovDropdown.getValueItemForSelection() to get the currentSelectedItem until after
  ...
  _handleContainerMouseDown: function (event) {
    ...
    if (mainButton && !this.options.readOnly) {
      if (this._fullScreenPopup) {
        ...
        if (clearValueElem && isAncestorOrSelf(clearValueElem, event.target)) {
          return;
        }
      }
      ...
      if (this._lovEnabled) {
        if (!abstractLovBase.isDropdownOpen()) {
          ...
          var clickedMainIcon = isAncestorOrSelf(mainDropdownIcon, event.target);
          ...
          if (isAncestorOrSelf(filterDropdownIcon, event.target)) {
            this._closeDropdown();
          }
        }
      }
   ...
   * @private
   */
  _fetchDataAndSelect: function (valueItem, event) {
   ...
   * @private
   */
  _handleSelection: function (valueItem, event) {
    var context;
  ...
  _closeDropdown: function () {
    ...
    if (dropdownElem != null) {
      ...
      // explicitly blur the focused element in the dropdown before closing it
      var activeElem = document.activeElement;
      if (activeElem && isAncestor(dropdownElem, activeElem)) {
        activeElem.blur();
      }
  ...
  _createClearValueIcon: function () {
    ...
    var a = document.createElement('a');
    var styleClasses = className + '-clear-value ' + className + '-clear-value-icon ' +
      className + '-icon oj-component-icon oj-clickable-icon-nocontext';
    a.setAttribute('class', styleClasses);
    ...
    this._lovMainFieldInputEventListeners = {
      focus: function (event) {
        ...
        if (!this._fullScreenPopup && !this.options.readOnly) {
          ...
          this._mainFieldFocusHandlerTimer = getTimer(0);
   ...
   * @private
   */
  _transferSelectionRangeToFilterField: function () {
    var mainInputElem = this._lovMainField.getInputElem();
    var filterInputElem = this._getFilterInputElem();
    if (filterInputElem) {
      filterInputElem.setSelectionRange(
        mainInputElem.selectionStart,
        mainInputElem.selectionEnd,
        mainInputElem.selectionDirection
      );
    }
  },
  ...
  _createFilterInputText: function (className, idSuffix) {
    ...
    if (!this._fullScreenPopup) {
      ...
      innerIcon.setAttribute('class', className + '-back-icon ' + className +
        '-icon oj-component-icon oj-clickable-icon-nocontext');
    ...
    filterInputText.addEventListener('blur', function (event) {
      ...
      if (!this._fullScreenPopup) {
        ...
        if (!event.relatedTarget ||
            !isAncestor(this._lovDropdown.getElement(), event.relatedTarget)) {
          this._showMainField();
        }
      }
    }.bind(this));
   ...
   * @private
   */
  _initLovDropdown: function (idSuffix, inputType, containerId,
  ...
  _fetchByKeysFromDataProvider: function (arKeys) {
    ...
    if (this._cachedFetchByKeys && this._cachedFetchByKeys.promise &&
        oj$1.Object.compareValues(arKeys, this._cachedFetchByKeys.key)) {
      ...
      fetchPromise.then(function () {
        ...
        warn('Select: fetchByKeys promise was rejected: ' + reason);
        afterFetchPromiseFunc();
      });
    }
  ...
  _setUiLoadingState: function (state) {
    if (state === 'start') {
      ...
      this._loadingIndicatorTimer = getTimer(this._showIndicatorDelay);
