  ...
  oj.__registerWidget('oj.ojToolbar', $.oj.baseComponent, {
    ...
    _InitOptions: function (originalDefaults, constructorOptions) {
      ...
      if ('disabled' in constructorOptions) {
        warn("Caller attempted to set the 'disabled' option on Toolbar, but Toolbar does not support the 'disabled' option.  See API doc.");
      }
    },
    ...
    _ComponentCreate: function () {
      ...
      elem.setAttribute(_OJ_CONTAINER_ATTR, this.widgetName); // @HTMLUpdateOK
    ...
    _setOption: function (key, value) { // eslint-disable-line no-unused-vars
      ...
      if (key === 'disabled') {
        warn("Caller attempted to set the 'disabled' option on Toolbar, but Toolbar does not support the 'disabled' option.  See API doc.  Ignoring the call.");
        return;
      }
              ...
              if (mutation.addedNodes) {
                for (i = 0; i < mutation.addedNodes.length; i++) {
                  if (ojElementNames.indexOf(mutation.addedNodes[i].nodeName) >= 0) {
                    mutation.addedNodes[i].addEventListener(disabledChangedEventType,
              ...
              if (mutation.removedNodes) {
                for (i = 0; i < mutation.removedNodes.length; i++) {
                  if (ojElementNames.indexOf(mutation.removedNodes[i].nodeName) >= 0) {
                    mutation.removedNodes[i].removeEventListener(disabledChangedEventType,
                      self._disabledChangedListener);
                  }
                }
              }
...
//        console.log("mapped button and last button are:");  console.log(button);  console.log(last);  console.log(".");
    ...
    _destroy: function () {
      ...
      elem.removeAttribute(_OJ_CONTAINER_ATTR);
      ...
      if (!this._IsCustomElement()) {
        ...
        for (var i = 0; i < this.topLevelChildren.length; i++) {
          ...
          if (child.tagName === 'OJ-BUTTON' || child.tagName === 'OJ-MENU-BUTTON') {
            ...
            if (__GetWidgetConstructor(child, 'ojButtonset')) {
              child.refresh();
            }
          }
        }
...
setDefaultOptions({
  ...
  ojToolbar: {
    chroming: createDynamicPropertyGetter(function () {
      return (parseJSONFromFontFamily('oj-toolbar-option-defaults') || {}).chroming;
    })
  }
});
