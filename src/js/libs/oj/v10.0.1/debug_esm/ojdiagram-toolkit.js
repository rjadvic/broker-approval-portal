 ...
 * @ignore
 */
import { Obj, CSSStyle, Rectangle, PathUtils, BaseDiagram, ArrayUtils, Point, Container, Agent, TextUtils, SelectionHandler, Matrix, BaseComponentDefaults, CategoryRolloverHandler, SelectionEffectUtils, ToolkitUtils, BaseComponent, JsonUtils, Rect, Stroke, ImageMarker, SimpleMarker, PatternFill, MultilineText, OutputText, BackgroundMultilineText, BackgroundOutputText, Displayable, KeyboardEvent, MouseEvent, KeyboardHandler, CustomAnimation, Animator, AnimScaleTo, AnimFadeOut, Playable, AnimFadeIn, SolidFill, EventManager, Path, IconButton, Polygon, Polyline, Circle, AfComponentFactory, ResourceUtils, EventFactory, ClipPath, Dimension, Automation, DataAnimationHandler, BlackBoxAnimationHandler } from 'ojs/ojdvt-toolkit';
import { PanZoomComponent, PanZoomCanvasEventManager, PanZoomCanvasKeyboardHandler, PanZoomCanvas } from 'ojs/ojdvt-panzoomcanvas';
...
Obj.createSubclass(DvtDiagramLinkUtils, Obj, 'DvtDiagramLinkUtils');
 ...
 */
DvtDiagramLinkUtils.GetLinkBounds = function(linkObj) {
  var linkBounds = new Rectangle(Number.MAX_VALUE, Number.MAX_VALUE, - Number.MAX_VALUE, - Number.MAX_VALUE);
  ...
  if (labelPos && labelBounds) {
    ...
    if (labelRotAngle != null) {
      labelBounds = BaseDiagram.RotateBounds(labelBounds, labelRotAngle, labelRotPoint);
    }
...
Obj.createSubclass(DvtDiagramPoint, Obj, 'DvtDiagramPoint');
...
Obj.createSubclass(DvtDiagramDataUtils, Obj);
 ...
 */
DvtDiagramDataUtils.compareValues = function(ctx, obj1, obj2) {
  if (!obj1 && !obj2) {
    return true;
  }
...
Obj.createSubclass(DvtDiagramLayoutContext, Obj, 'DvtDiagramLayoutContext');
...
Obj.createSubclass(DvtDiagramRectangle, Obj, 'DvtDiagramRectangle');
...
Obj.createSubclass(DvtDiagramLayoutContextNode, Obj, 'DvtDiagramLayoutContextNode');
...
Obj.createSubclass(DvtDiagramLayoutUtils, Obj, 'DvtDiagramLayoutUtils');
...
DvtDiagramLayoutUtils.convertDiagramRectToRect = function(diagramRect) {
  if (!diagramRect) {
    ...
    return new Rectangle(diagramRect['x'], diagramRect['y'], diagramRect['w'], diagramRect['h']);
  }
};
...
Obj.createSubclass(DvtDiagramLayoutContextLink, Obj, 'DvtDiagramLayoutContextLink');
...
DvtDiagramLayoutContextLink.prototype.setPoints = function(points) {
  if (typeof points == 'string')
    this._points = PathUtils.createPathArray(points);
  else
    this._points = points;
  this.setDirty();
};
...
Obj.createSubclass(DvtBaseDiagram, PanZoomComponent, 'DvtBaseDiagram');
 ...
 */
DvtBaseDiagram.prototype.GetAllRoots = function() {
  return [];
};
...
DvtBaseDiagram.prototype.RefreshEmptyText = function(emptyDiagram) {
  if (emptyDiagram && this.getEmptyText()) {
    if (!this._emptyTextDisplay) {
      ...
      TextUtils.fitText(this._emptyTextDisplay, this.getWidth() - 2 * TextUtils.EMPTY_TEXT_BUFFER, Infinity, this);
    }
  }
  else {
    if (this._emptyTextDisplay) {
...
DvtBaseDiagram.prototype.CreateEmptyText = function(text) {
  ...
  return this.renderEmptyText(this, text, new Rectangle(0, 0, this.getWidth(), this.getHeight()), this.getEventManager(), options['_statusMessageStyle']);
};
 ...
 */
DvtBaseDiagram.prototype.getTouchEventPane = function() {
 ...
 */
DvtBaseDiagram.prototype.getBottomPane = function() {
  if (!this._bottomPane) {
 ...
 */
DvtBaseDiagram.prototype.ApplyLayoutContext = function(layoutContext, bSaveOffset) {
  ...
  if (topContainerPadding) {
    ...
    if (containerId) {
      ...
      if (containerNode) {
        containerNode.setContainerPadding(DvtBaseDiagram.getContainerPadding(topContainerPadding));
      }
    }
  }
  ...
  for (var ni = 0; ni < layoutContext.getNodeCount(); ni++) {
    ...
    if (node.isDisclosed()) {
      ...
      if (containerPadding) {
        node.setContainerPadding(DvtBaseDiagram.getContainerPadding(containerPadding));
      }
    }
  }
  ...
  if (layoutViewport) {
    this._layoutViewport = new Rectangle(layoutViewport['x'] + tx, layoutViewport['y'] + ty, layoutViewport['w'], layoutViewport['h']);
  }

  if (bSaveOffset)
    this._layoutOffset = new Point(tx, ty);
};
 ...
 * @protected
 */
DvtBaseDiagram.prototype.GetLinkTranslationOffset = function(link, lc, lopt) {
 ...
 * @protected
 */
DvtBaseDiagram.prototype.GetLayoutViewport = function() {
 ...
 * @protected
 */
DvtBaseDiagram.prototype.AdjustMinZoom = function() {
 ...
 * @protected
 */
DvtBaseDiagram.prototype.GetViewBounds = function() {
 ...
 * @protected
 */
DvtBaseDiagram.prototype.ApplyLabelPosition = function(objc, obj, pos) {
 ...
 */
DvtBaseDiagram.prototype.getCustomObjKeyboardBoundingBox = function(obj) {
...
Obj.createSubclass(DvtDiagramDefaults, BaseComponentDefaults, 'DvtDiagramDefaults');
...
DvtDiagramDefaults.SKIN_ALTA = {
  ...
  'styleDefaults': {
    ...
    'nodeDefaults': {
      '_containerStyle' : new CSSStyle('border-color:#abb3ba;background-color:#f9f9f9;border-width:.5px;border-radius:1px;padding-top:20px;padding-left:20px;padding-bottom:20px;padding-right:20px;'),
      'labelStyle': new CSSStyle(BaseComponentDefaults.FONT_FAMILY_ALTA_BOLD_12 + 'color:#383A47'),
...
Obj.createSubclass(DvtDiagramCategoryRolloverHandler, CategoryRolloverHandler, 'DvtDiagramCategoryRolloverHandler');
 ...
 * @param {Diagram} diagram parent diagram component
...
Obj.createSubclass(DvtBaseDiagramNode, Container, 'DvtBaseDiagramNode');
 ...
 */
DvtBaseDiagramNode.prototype.getDisplayable = function() {
  return this;
};
 ...
 */
DvtBaseDiagramNode.prototype.getLayoutBounds = function(forceDims) {
 ...
 */
DvtBaseDiagramNode.prototype.getContentBounds = function(forceDims) {
  //subclasses should override
  return null;
};
...
DvtBaseDiagramNode.prototype.setLabelPosition = function(pos) {
  if (pos) {
    this._labelPos = pos;
    DvtBaseDiagramNode.PositionLabel(this._labelObj, pos, this.getLabelBounds(true),
 ...
 */
DvtBaseDiagramNode.prototype.getLabelBounds = function(forceDims) {
  return null;
};
 ...
 */
DvtBaseDiagramNode.prototype.setContainerPadding = function(top, right, bottom, left) {
};
...
DvtBaseDiagramNode.prototype.setLabelRotationAngle = function(angle) {
  this._labelRotAngle = angle;
  DvtBaseDiagramNode.PositionLabel(this._labelObj, this.getLabelPosition(), this.getLabelBounds(true), angle, this.getLabelRotationPoint());
};
...
DvtBaseDiagramNode.prototype.setLabelRotationPoint = function(point) {
  this._labelRotPoint = point;
  DvtBaseDiagramNode.PositionLabel(this._labelObj, this.getLabelPosition(), this.getLabelBounds(true), this.getLabelRotationAngle(), point);
};
 ...
 * @protected
 */
DvtBaseDiagramNode.prototype.GetNodeBounds = function() {
 ...
 * @protected
 */
DvtBaseDiagramNode.prototype.SetPosition = function(xx, yy) {
 ...
 * @protected
 */
DvtBaseDiagramNode.prototype.GetPosition = function()
{
...
Obj.createSubclass(DvtDiagramNode, DvtBaseDiagramNode, 'DvtDiagramNode');
 ...
 * @param {Diagram} diagram the parent diagram
...
DvtDiagramNode.prototype._applyCustomNodeContent = function(renderer, state, prevState) {
  ...
  if (nodeContent && nodeContent.namespaceURI === ToolkitUtils.SVG_NS) {
    if (!this._customNodeContent) {
      ToolkitUtils.appendChildElem(this.getContainerElem(), nodeContent);
      this._customNodeContent = nodeContent;
    }
  }
  else if (nodeContent && Array.isArray(nodeContent)) {
    if (!this._customNodeContent) {
      nodeContent.forEach(function(node) {ToolkitUtils.appendChildElem(this.getContainerElem(), node);}.bind(this));
      this._customNodeContent = nodeContent;
    }
  }
...
DvtDiagramNode._renderNodeBackground = function(diagram, nodeData, container) {
  ...
  var backgroundProps = [CSSStyle.WIDTH,
    CSSStyle.HEIGHT,
  ...
  if (!backgroundStyle.isEmpty()) {
    ...
    var borderRadius = CSSStyle.toNumber(backgroundStyle.getStyle(CSSStyle.BORDER_RADIUS));
    ...
    if (styleObj)
      backgroundProps.forEach(function(entry) {delete styleObj[CSSStyle.cssStringToObjectProperty(entry)];});
...
DvtDiagramNode._renderNodeIcon = function(diagram, nodeData, container) {
  ...
  if (icon) {
    ...
    var iconMarker;
    if (icon['source']) {
      iconMarker = new ImageMarker(diagram.getCtx(), iconWidth / 2, iconHeight / 2, iconWidth, iconHeight, iconBorderRadius,
          icon['source'], icon['sourceSelected'], icon['sourceHover'], icon['sourceHoverSelected']);
    }
    else {
      iconMarker = new SimpleMarker(diagram.getCtx(), icon['shape'], iconWidth / 2, iconHeight / 2, iconWidth, iconHeight, iconBorderRadius);
    }
...
DvtDiagramNode._renderNodeLabels = function(diagram, nodeData, container) {
  ...
  if (labelString) {
    ...
    var halign = bMultiline ? (rtl ? MultilineText.H_ALIGN_RIGHT : MultilineText.H_ALIGN_LEFT) : (rtl ? OutputText.H_ALIGN_RIGHT : OutputText.H_ALIGN_LEFT);
...
DvtDiagramNode.createText = function(ctx, strText, style, halign, valign, bMultiline) {
  ...
  if (style.hasBackgroundStyles())
    text = bMultiline ? new BackgroundMultilineText(ctx, strText, 0, 0, style, null, true) : new BackgroundOutputText(ctx, strText, 0, 0, style);
  else {
    text = bMultiline ? new MultilineText(ctx, strText, 0, 0, null, true) : new OutputText(ctx, strText, 0, 0);
...
DvtDiagramNode._addHoverSelectionDefaultStrokes = function(selectionShape, nodeData) {
  ...
  if (isRedwood) {
    if (nodeData.icon && nodeData.icon.color)
      var dataColor = nodeData.icon.color;
    else
      var dataColor = hoverOuterColor;
    var his = new Stroke(hoverInnerColor, 1, 1, true);
    var hos = new Stroke(dataColor, 1, 3, true);
    var sis = new Stroke(hoverInnerColor, 1, 1, true);
    var sos = new Stroke(selectionColor, 1, 3, true);
    selectionShape.setHoverStroke(his, hos).setSelectedStroke(sis, sos);
  }
  else {
    var his = new Stroke(hoverInnerColor, 1, 4, true);
    var hos = new Stroke(hoverOuterColor, 1, 8, true);
    var sis = new Stroke(hoverInnerColor, 1, 2, true);
    var sos = new Stroke(selectionColor, 1, 6, true);
...
DvtDiagramNode._addHoverSelectionOuterStrokes = function(selectionShape, nodeData) {
  ...
  if (isRedwood) {
    var his = new Stroke(hoverInnerColor, 1, 1, true);
    var hos = new Stroke(hoverOuterColor, 1, 3, true);
    var sis = new Stroke(hoverInnerColor, 1, 1, true);
    var sos = new Stroke(selectionColor, 1, 3, true);
    selectionShape.setHoverStroke(his, hos).setSelectedStroke(sis, sos);
  }
    ...
    var his = new Stroke(hoverInnerColor, 1, 2, true);
    var hos = new Stroke(hoverOuterColor, 1, 2, true);
    var sis = new Stroke(hoverInnerColor, 1, 1, true);
    var sos = new Stroke(selectionColor, 1, 2, true);
...
DvtDiagramNode.prototype.getSelectionShape = function() {
  if (!this._selectionShape) { // create selection shape in necessary
    ...
    if (contentDims) {
      var selectionShape = new Rect(this._diagram.getCtx(), contentDims.x, contentDims.y, contentDims.w, contentDims.h);
...
DvtDiagramNode.prototype.getNextNavigable = function(event) {
  ...
  else if ((event.keyCode == KeyboardEvent.OPEN_ANGLED_BRACKET || KeyboardEvent.CLOSE_ANGLED_BRACKET) &&
      event.altKey) {
  ...
  else if (event.keyCode == KeyboardEvent.OPEN_BRACKET) { //next node down in container hierarchy
    if (this.isDisclosed()) {
  ...
  else if (event.keyCode == KeyboardEvent.CLOSE_BRACKET) {//next node up in container hierarchy
...
DvtDiagramNode.prototype.animateUpdate = function(animationHandler, oldNode) {
  ...
  var playable = new CustomAnimation(this.getCtx(), null, animationHandler.getAnimationDuration());
  ...
  if (!this.isDisclosed() && !oldNode.isDisclosed()) {
    ...
    if (oldNode._shape && this._shape) {
      playable.getAnimator().addProp(Animator.TYPE_RECTANGLE, this._shape, this._shape.getCenterDimensions, this._shape.setCenterDimensions, this._shape.getCenterDimensions());
    ...
    if (oldNode._labelObj && this._labelObj) {
      ...
      if (!oldMat.equals(newMat)) {
        ...
        playable.getAnimator().addProp(Animator.TYPE_MATRIX, this._labelObj, this._labelObj.getMatrix, this._labelObj.setMatrix, newMat);
      }
    }
    //background and icon
  ...
  // 2. both nodes are disclosed containers - node size, background and child nodes could change
    ...
    if (this._getCustomRenderer('renderer')) {
      ...
      if (Agent.isRightToLeft(this.getCtx()) && oldNode._containerButton && this._containerButton) {
        ...
        if (oldButtonTx != newButtonTx) {
          ...
          playable.getAnimator().addProp(Animator.TYPE_NUMBER, this._containerButton,
...
DvtDiagramNode.prototype._animateCustomUpdate = function(animationHandler, oldNode) {
  ...
  var oldNodeCenter = new Point(oldNode.getTranslateX() + oldBounds.w * .5, oldNode.getTranslateY() + oldBounds.h * .5);
  ...
  playable.getAnimator().addProp(Animator.TYPE_NUMBER, this, this.getTranslateX, this.setTranslateX, newTx);
  playable.getAnimator().addProp(Animator.TYPE_NUMBER, this, this.getTranslateY, this.setTranslateY, newTy);
  animationHandler.add(playable, DvtDiagramDataAnimationPhase.UPDATE);

  // animate scale and fade in new node
  this.setScaleX(scaleFrom);
  this.setScaleY(scaleFrom);
  var scaleNewToOld = new AnimScaleTo(this.getCtx(), this, new Point(1, 1), animationHandler.getAnimationDuration());
  ...
  if (oldNodeContent) {
    ...
    var newNodeCenter = new Point(this.getTranslateX() + newBounds.w * .5, this.getTranslateY() + newBounds.h * .5);
    playable.getAnimator().addProp(Animator.TYPE_NUMBER, oldNodeContent, oldNode.getTranslateX, oldNodeContent.setTranslateX, newNodeCenter.x - oldBounds.w * scaleTo * .5);
    playable.getAnimator().addProp(Animator.TYPE_NUMBER, oldNodeContent, oldNode.getTranslateY, oldNodeContent.setTranslateY, newNodeCenter.y - oldBounds.h * scaleTo * .5);

    // animate scale and fade out old node
    var scaleOldToNew = new AnimScaleTo(this.getCtx(), oldNodeContent, new Point(scaleTo, scaleTo), animationHandler.getAnimationDuration());
    animationHandler.add(scaleOldToNew, DvtDiagramDataAnimationPhase.UPDATE);

    var fadeOut = new AnimFadeOut(this.getCtx(), oldNodeContent, animationHandler.getAnimationDuration());
    animationHandler.add(fadeOut, DvtDiagramDataAnimationPhase.UPDATE);
    var thisRef = this;
    Playable.appendOnEnd(fadeOut, function() {thisRef.getParent().removeChild(oldNodeContent);});

    this.setAlpha(0);
    var fadeIn = new AnimFadeIn(this.getCtx(), this, animationHandler.getAnimationDuration());
...
DvtDiagramNode._animatePosition = function(playable, oldNode, newNode) {
  if (newNode && oldNode) {
    ...
    if (oldTx != newTx) {
      ...
      playable.getAnimator().addProp(Animator.TYPE_NUMBER, newNode, newNode.getTranslateX, newNode.setTranslateX, newTx);
    }
    if (oldTy != newTy) {
      newNode.setTranslateY(oldTy);
      playable.getAnimator().addProp(Animator.TYPE_NUMBER, newNode, newNode.getTranslateY, newNode.setTranslateY, newTy);
    }
  }
};
...
DvtDiagramNode._animateContainer = function(playable, oldDisplayable, newDisplayable) {
  if (newDisplayable && oldDisplayable) {
    ...
    playable.getAnimator().addProp(Animator.TYPE_NUMBER, newDisplayable, newDisplayable.getWidth, newDisplayable.setWidth, newDisplayable.getWidth());
    newDisplayable.setWidth(oldDisplayable.getWidth());
    playable.getAnimator().addProp(Animator.TYPE_NUMBER, newDisplayable, newDisplayable.getHeight, newDisplayable.setHeight, newDisplayable.getHeight());
...
DvtDiagramNode.prototype.animateDelete = function(animationHandler, deleteContainer) {
  ...
  var playable = new AnimFadeOut(this.getCtx(), this, animationHandler.getAnimationDuration());
...
DvtDiagramNode.prototype.animateInsert = function(animationHandler) {
  this.setAlpha(0);
  animationHandler.add(new AnimFadeIn(this.getCtx(), this, animationHandler.getAnimationDuration()), DvtDiagramDataAnimationPhase.INSERT);
};
...
DvtDiagramNode.prototype.setLabelAlignments = function(halign, valign) {
  var isMultiline = this._labelObj instanceof MultilineText || this._labelObj instanceof BackgroundMultilineText;
  if (valign == 'baseline')
...
DvtDiagramNode._renderContainer = function(diagram, nodeData, container) {
  ...
  var containerProps = [CSSStyle.BACKGROUND_COLOR,
                        CSSStyle.BORDER_COLOR,
                        CSSStyle.BORDER_WIDTH,
                        CSSStyle.BORDER_RADIUS];
  ...
  var containerShape = new Rect(diagram.getCtx(), 0, 0,
  ...
  //Parse out the CSS properties which are already applied on the DOM
  if (styleObj)
    containerProps.forEach(function(entry) {delete styleObj[CSSStyle.cssStringToObjectProperty(entry)];});
...
DvtDiagramNode._renderContainerButton = function(diagram, nodeData, container) {
  ...
  var containerButton = new IconButton(diagram.getCtx(), 'outlined', {style: iconStyle, size: iconResources.width, pos: {x: xPos, y: yPos}}, background, null, container.handleDisclosure, container);
...
DvtDiagramNode.prototype._createDropEffect = function(styleClass) {
  ...
  if (this._customNodeContent || this.isDisclosed() || this._background ||
    ...
    var contentDims = this.getContentBounds(true);
    if (contentDims) {
      dropEffectShape = new Rect(this._diagram.getCtx(), contentDims.x, contentDims.y,
...
DvtDiagramNode.prototype._cleanUp = function(saveContainer) {
  ...
  if (this._customNodeContent) {
    ...
    if (this._customNodeContent.namespaceURI === ToolkitUtils.SVG_NS) {
      this.getContainerElem().removeChild(this._customNodeContent);
      saveContainer && ToolkitUtils.appendChildElem(saveContainer.getContainerElem(), this._customNodeContent);
    }
    else if (Array.isArray(this._customNodeContent)) {
      for (var i = 0; i < this._customNodeContent.length; i++) {
        this.getContainerElem().removeChild(this._customNodeContent[i]);
        saveContainer && ToolkitUtils.appendChildElem(saveContainer.getContainerElem(), this._customNodeContent[i]);
      }
    }
...
Obj.createSubclass(DvtDiagramLinkConnectorUtils, Obj, 'DvtDiagramLinkConnectorUtils');
...
DvtDiagramLinkConnectorUtils.CreateCustomConnector = function(context, connectorTemplate, parentLink) {
  ...
  var connector = AfComponentFactory.parseAndLayout(afContext, connectorTemplate, parentLink);
  return connector;
};
...
Obj.createSubclass(DvtDiagramLinkUnderlay, Container, 'DvtDiagramLinkUnderlay');
...
DvtDiagramLinkUnderlay.prototype.Init = function(context, points, stroke, styleObj, styleClass)
{
  ...
  this._stroke = stroke;
  if (!this._stroke)
    this._stroke = new Stroke('#ffffff', 1, 1, true);
...
DvtDiagramLinkUnderlay.prototype.CreateConnectorUnderlay = function(points, connectorType, connectorTemplate, parentLink, connectorPos) {
  ...
  var stroke = new Stroke(this._stroke.getColor(), this._stroke.getAlpha(), this._stroke.getWidth(),
    false, this._stroke.getDashProps());

  var connectorUnderlay = DvtDiagramLinkConnectorUtils.CreateConnectorShape(this.getCtx(), connectorType, connectorTemplate, stroke, parentLink);
...
Obj.createSubclass(DvtBaseDiagramLink, Container, 'DvtBaseDiagramLink');
 ...
 * @return {dvt.Displayable} displayable
 */
DvtBaseDiagramLink.prototype.getDisplayable = function() {
  return this;
};
...
DvtBaseDiagramLink.prototype.CreateConnector = function(points, connectorType, connectorPos, connectorTemplate) {
  ...
  if (!connectorTemplate) {
    ...
    stroke = new Stroke(origStroke.getColor(), origStroke.getAlpha(), origStroke.getWidth());
  }
...
DvtBaseDiagramLink.prototype.setPoints = function(points) {
  ...
  //a link
  if (this._hitDetectionUnderlay && this._hitDetectionUnderlay.getUnderlay() instanceof Path) {
 ...
 * @protected
 */
DvtBaseDiagramLink.prototype.CreateUnderlay = function(strokeColor, strokeAlpha, strokeWidthOffset, styleObj, styleClass) {
 ...
 * @protected
 */
DvtBaseDiagramLink.prototype.CreateFeedbackUnderlay = function(strokeColor, strokeAlpha, strokeWidthOffset, styleObj, styleClass) {
 ...
 * @protected
 */
DvtBaseDiagramLink.prototype.GetStyledLinkStroke = function(strokeToCopy, color, width) {
  ...
  var stroke = new Stroke(color, strokeToCopy.getAlpha(), width, true, dashProps);
  return stroke;
};
...
DvtBaseDiagramLink.prototype.ReplaceConnectorColor = function(connector, stroke) {
  ...
  if (color) {
    ...
    if (conStroke) {
      connector.setStroke(new Stroke(color, conStroke.getAlpha(), conStroke.getWidth(), conStroke.isFixedWidth(), conStroke.getDashProps()));
    }
    if (conFill) {
...
Obj.createSubclass(DvtDiagramLink, DvtBaseDiagramLink, 'DvtDiagramLink');
...
DvtDiagramLink.prototype._showFeedback = function(bHovered, bSelected) {
  ...
  if (bHovered && (!isRedwood || !bSelected)) {
    ...
    var hoverStroke = new Stroke(this.getData()['hoverInnerColor'],
  ...
  if (linkStyle && linkStyle instanceof Object) {
    ...
    if (strokeType == DvtDiagramLink.CUSTOM_STYLE) {
      var strokeDashOffset = (linkStyle['strokeDashoffset'] != null) ?
          (CSSStyle.toNumber(linkStyle['strokeDashoffset']) + 1) : 1;
      dashProps = {
...
DvtDiagramLink.prototype.getShortDesc = function() {
  ...
  if (this.isPromoted()) {
    ...
    return ResourceUtils.format(translations[linkCount > 1 ? 'promotedLinks' : 'promotedLink'], [linkCount]);
  }
  return this.getData()['shortDesc'];
};
...
DvtDiagramLink.prototype.getNextNavigable = function(event)
{
  ...
  else if (event.keyCode == KeyboardEvent.UP_ARROW || event.keyCode == KeyboardEvent.DOWN_ARROW) {
...
DvtDiagramLink.prototype._movingToStart = function(direction) {
  ...
  if ((direction == KeyboardEvent.RIGHT_ARROW && linkDirectionL2R) ||
...
DvtDiagramLink.prototype.animateUpdate = function(animationHandler, oldLink, bCleanUp) {
  ...
  if (oldLink.getShape() && this.getShape()) {
    ...
    if (oldStroke && newStroke &&
      ...
      playable.getAnimator().addProp(Animator.TYPE_STROKE, this.getShape(), this.getShape().getStroke, this.getShape().setStroke, newStroke);
    }
  }
  ...
  if (this._getCustomRenderer('renderer')) {
    ...
    if (animateFrom && animateTo) {
      var oldPoints = ToolkitUtils.getAttrNullNS(animateFrom, 'd');
      var newPoints = ToolkitUtils.getAttrNullNS(animateTo, 'd');

      ToolkitUtils.setAttrNullNS(animateTo, 'd', oldPoints);
      ...
      playable.getAnimator().addProp(Animator.TYPE_PATH, animateTo, pointsGetter, pointsSetter, PathUtils.createPathArray(newPoints));
    }
    ...
    var playableExtra = new CustomAnimation(this.getCtx(), null, animationHandler.getAnimationDuration());
    for (var ix = 0; ix < fadeInItems.length; ix++) {
      ...
      var opacitySetter = function(val) {
        ToolkitUtils.setAttrNullNS(this, 'opacity', val);
      }.bind(item);
      var opacityGetter = function(){
        var value = ToolkitUtils.getAttrNullNS(this, 'opacity');
        return Number(value);
      }.bind(item);
      playableExtra.getAnimator().addProp(Animator.TYPE_NUMBER, item, opacityGetter, opacitySetter, 1);
    }
  ...
  if (bCleanUp) {
    var thisRef = this;
    Playable.appendOnEnd(playable, function() {thisRef.getParent().removeChild(thisRef);});
  }
  animationHandler.add(playable, DvtDiagramDataAnimationPhase.UPDATE);
};
...
DvtDiagramLink._processLinkContent = function(linkContent) {
  ...
  // updated content from customer defined callback that was initially wrapped into svg element
    ...
    for (var i = 0; i < linkContent.length; i++) {
      if (linkContent[i].namespaceURI === ToolkitUtils.SVG_NS && linkContent[i].tagName === 'svg') {
        // Using Array.prototype.forEach.call() since IE11 does not support NodeList.forEach()
...
Obj.createSubclass(DvtDiagramStyleUtils, Obj);
...
DvtDiagramStyleUtils.getNodeStyles = function(comp, nodeData, nodeDefaults) {
  if (comp.isDataProviderMode()) {
    return JsonUtils.merge(nodeData, nodeDefaults, {'_itemData': true, 'id': true, 'nodes': true});
  }
...
DvtDiagramStyleUtils.getAnimationDuration = function(diagram) {
  return CSSStyle.getTimeMilliseconds(diagram.getOptions()['styleDefaults']['animationDuration']) / 1000;
};
...
Obj.createSubclass(DvtDiagramEventManager, EventManager, 'DvtDiagramEventManager');
...
DvtDiagramEventManager.prototype.ProcessKeyboardEvent = function(event)
{
  ...
  if (keyCode != KeyboardEvent.TAB && this._bPassOnEvent) {
  ...
  else if ((keyCode == KeyboardEvent.ZERO || keyCode == KeyboardEvent.NUMPAD_ZERO) &&
    ...
    if (focusObj) {
      ...
      var animator = DvtDiagramStyleUtils.getAnimationOnDataChange(this._diagram) != 'none' ?
              new Animator(this.getCtx(), this._diagram.getAnimationDuration()) : null;
      pzc.zoomToFit(animator, finBounds);
      animator && animator.play();
    }
  }
  else {
    if (keyCode == KeyboardEvent.TAB && focusObj) {//make sure focused obj in on screen
...
DvtDiagramEventManager.prototype.HandleTouchClickInternal = function(event) {
  return this.GetEventInfo(event, PanZoomCanvasEventManager.EVENT_INFO_PANNED_KEY);
};
...
DvtDiagramEventManager.prototype.StoreInfoByEventType = function(key) {
  if (key == PanZoomCanvasEventManager.EVENT_INFO_PANNED_KEY) {
    return false;
  }
  ...
  if (this._diagram.isSelectionSupported() && this._diagram.getSelectionHandler().getSelectedCount() > 1) {
    ...
    for (var i = 0; i < selection.length; i++) {
      if (selection[i] instanceof DvtDiagramNode) { //don't drag links yet
        ...
        context['nodeOffset'] = new Point((bounds.x - contentPosition.x) / zoom, (bounds.y - contentPosition.y) / zoom);
        if (bSanitize)
          ToolkitUtils.cleanDragDataContext(context);
...
DvtDiagramEventManager.prototype.GetDropOffset = function(event) {
  ...
  if (contentPosition) {
    ...
    var zoom = this._diagram.getPanZoomCanvas().getZoom();
    return new Point((contentPosition.x - relPos.x) / zoom, (contentPosition.y - relPos.y) / zoom);
  }
  return null;
};
...
DvtDiagramEventManager.prototype.ShowDropEffect = function(event) {
  ...
  if (dropTargetType === 'background') {
    ...
    ToolkitUtils.addClassName(obj.__dropPort, 'oj-active-drop');
  }
};
...
DvtDiagramEventManager.prototype.ShowRejectedDropEffect = function(event) {
  ...
  if (dropTargetType === 'background') {
    ...
    ToolkitUtils.addClassName(obj.__dropPort, 'oj-invalid-drop');
  }
};
...
DvtDiagramEventManager.prototype.ClearDropEffect = function() {
  ...
  if (obj && obj.__dropPort) {
    ToolkitUtils.removeClassName(obj.__dropPort, 'oj-active-drop');
    ToolkitUtils.removeClassName(obj.__dropPort, 'oj-invalid-drop');
  }
...
Obj.createSubclass(DvtBaseDiagramKeyboardHandler, PanZoomCanvasKeyboardHandler, 'DvtBaseDiagramKeyboardHandler');
...
DvtBaseDiagramKeyboardHandler.prototype.getFirstNavigableLink = function(node, event, listOfLinks) {
  ...
  for (var i = 0; i < listOfLinks.length; i++)
  {
    ...
    if ((direction == KeyboardEvent.OPEN_ANGLED_BRACKET && linkCenterX <= nodeCenterX) ||
...
Obj.createSubclass(DvtDiagramKeyboardHandler, DvtBaseDiagramKeyboardHandler, 'DvtDiagramKeyboardHandler');
...
Obj.createSubclass(DvtDiagramOverviewUtils, Obj, 'DvtDiagramOverviewUtils');
...
DvtDiagramOverviewUtils.CalcViewportFromMatrix = function(diagram) {
  ...
  var adjViewport = new Rectangle(newX, newY, newWidth, newHeight);
  return adjViewport;
};
 ...
 */
DvtDiagramOverviewUtils.ConfigureOverviewWindow = function(diagram, overview) {
...
DvtDiagramOverviewUtils._adjustNodePositionsForAspectRatio = function (diagram, overview, width, height) {
  if (diagram.Options.overview.preserveAspectRatio !== 'meet') {
    ...
    var ovPadding = 2 * CSSStyle.toNumber(diagram.Options.styleDefaults._overviewStyles.overviewContent.padding);
...
DvtDiagramOverviewUtils._calcOverviewScale = function(diagram, ztfBounds, width, height, overview)
{
  ...
  if (diagram.Options.overview.fitArea === 'canvas') {
    ...
    var diagramPaddingW = 2 * contentPadding * stretchFactorH * vz; // 40 = 2 * PanZoomCanvas.DEFAULT_PADDING;
    var diagramPaddingH = 2 * contentPadding * stretchFactorV * vz; // 40 = 2 * PanZoomCanvas.DEFAULT_PADDING;
...
DvtDiagramOverviewUtils._createOverviewNodeShape = function(diagram, node) {
  ...
  var ovNode = new SimpleMarker(diagram.getCtx(), iconShape, iconWidth / 2, iconHeight / 2, iconWidth, iconHeight, 0);
...
DvtDiagramOverviewUtils._positionOverviewWindow = function(diagram, overview) {
  ...
  var padding = CSSStyle.toNumber(diagram.Options.styleDefaults._overviewStyles.overview.padding);
...
Obj.createSubclass(DvtDiagramOverview, Overview);
 ...
 * @param {DvtDiagramDataAnimationHandler} animationHandler the animation handler, which can be used to chain animations.
 */
DvtDiagramOverview.prototype.animateInsert = function(animationHandler) {
  // do nothing
};
 ...
 * @param {dvt.Container} deleteContainer
 */
DvtDiagramOverview.prototype.animateDelete = function(animationHandler, deleteContainer) {
  // do nothing
};
...
DvtDiagramOverview.prototype.animateUpdate = function(animationHandler, oldDiagramOverview) {
  ...
  animationHandler.add(new AnimFadeIn(this.getCtx(), this.Content, animationHandler.getAnimationDuration()), DvtDiagramDataAnimationPhase.UPDATE);
  ...
  var fadeOutAnim = new AnimFadeOut(this.getCtx(), oldDiagramOverview.Content, animationHandler.getAnimationDuration());
  Playable.appendOnEnd(fadeOutAnim, removeFunc);
  animationHandler.add(fadeOutAnim, DvtDiagramDataAnimationPhase.UPDATE);

  //animate viewport
  var customContentAnim = new CustomAnimation(this.getCtx(), null, animationHandler.getAnimationDuration());
  ...
  customContentAnim.getAnimator().addProp(Animator.TYPE_NUMBER_ARRAY, this, this.GetAnimationParams, this.SetAnimationParams, newAnimationParams);
...
DvtDiagramOverview.prototype.CreateAnimationClone = function() {
  var context = this.getCtx();
  var overviewClone = new Container(context, this.getId());
  overviewClone.ContentBounds = this.Diagram.GetViewBounds();
  overviewClone.setMouseEnabled(false);
  var ovContentClone = new Container(context, 'ovContentClone');
...
Obj.createSubclass(DvtDiagramAutomation, Automation, 'DvtDiagramAutomation');
...
Obj.createSubclass(DvtDiagramDataAnimationState, Obj, 'DvtDiagramDataAnimationState');
...
Obj.createSubclass(DvtDiagramDataAnimationHandler, DataAnimationHandler, 'DvtDiagramDataAnimationHandler');
 ...
 * @constructor
 */
var Diagram = function(context, callback, callbackObj) {
  this.Init(context, callback, callbackObj);
};

Obj.createSubclass(Diagram, DvtBaseDiagram);
 ...
 * @param {object} callbackObj The optional object instance on which the callback function is defined.
 */
Diagram.prototype.Init = function(context, callback, callbackObj) {
  Diagram.superclass.Init.call(this, context, callback, callbackObj);
 ...
 * @return {Diagram}
 */
Diagram.newInstance = function(context, callback, callbackObj) {
 ...
 * @protected
 */
Diagram.prototype.ResetNodesAndLinks = function() {
  var context = this.getCtx();
 ...
 * @private
 */
Diagram.prototype._onAnimationEnd = function() {
 ...
 * @override
 */
Diagram.prototype.animateUpdate = function(animationHandler, oldDiagramState) {
  var playable = new CustomAnimation(this.getCtx(), null, animationHandler.getAnimationDuration());
  ...
  if (!oldMat.equals(newMat)) {
    ...
    playable.getAnimator().addProp(Animator.TYPE_MATRIX, this.getPanZoomCanvas().getContentPane(), this.getPanZoomCanvas().getContentPane().getMatrix, this.getPanZoomCanvas().getContentPane().setMatrix, newMat);
  }
 ...
 * @private
 */
Diagram.prototype._resolveDeferredData = function(dataSource, nodeData) {
 ...
 * @private
 */
Diagram.prototype._renderDeferredData = function(renderCount, dataSource, nodeData, resolvedData) {
 ...
 * @private
 */
Diagram.prototype._findUnresolvedLinks = function(data, deepWalk) {
 ...
 */
Diagram.prototype._discoverLinks = function(dataSource, containerData) {
 ...
 * @private
 */
Diagram.prototype._setDataSourceListeners = function(dataSource, attach) {
  if (!dataSource)
    return;
 ...
 */
Diagram.prototype.addDataSourceEventListeners = function() {
 ...
 */
Diagram.prototype.removeDataSourceEventListeners = function() {
  this._setDataSourceListeners(this.getOptions()['data'], false);
};
 ...
 * @private
 */
Diagram.prototype._fetchDataProviderData = function(rootDataProvider, nodesArray) {
 ...
 */
Diagram.prototype.isDataProviderMode = function() {
 ...
 */
Diagram.prototype.isTreeDataProvider = function() {
...
Diagram.prototype.Render = function() {
  Diagram.superclass.Render.call(this);
 ...
 * @protected
 */
Diagram.prototype.RenderComponentInternal = function() {
 ...
 * @private
 */
Diagram.prototype._processContent = function(bEmptyDiagram) {
  var calcViewBounds;
  if (!bEmptyDiagram) {
  ...
  if (!this._bRendered && animationOnDisplay !== 'none' && !this._oldDataAnimState) {
    //initial animation
    this.Animation = BlackBoxAnimationHandler.getInAnimation(this.getCtx(),
                     BlackBoxAnimationHandler.ALPHA_FADE, this, null,
                     DvtDiagramStyleUtils.getAnimationDuration(this));
  }
  else if (animationOnDataChange !== 'none' && this._oldDataAnimState) {
    this._deleteContainer = new Container(this.getCtx(), 'Delete Container');
 ...
 * @private
 */
Diagram.prototype._fitContent = function() {
  var pzc = this.getPanZoomCanvas();
  if (!this._bRendered) {
    this.AdjustMinZoom();
    ...
    pzc.setZoomToFitPadding(bLayoutViewport ? 0 : PanZoomCanvas.DEFAULT_PADDING);
...
Diagram.prototype.SetOptions = function(options) {
  Diagram.superclass.SetOptions.call(this, options);
  //initial setup
  if (!this.isDataProviderMode()) {
    this.Options['nodes'] = [];
    this.Options['links'] = [];
 ...
 */
Diagram.getDefaults = function(skin) {
 ...
 * @override
 */
Diagram.prototype.getMaxZoom = function() {
 ...
 * @override
 */
Diagram.prototype.getMinZoom = function() {
 ...
 * @override
 */
Diagram.prototype.getPanDirection = function() {
  return this.getOptions()['panDirection'];
};
/**
 * @override
 */
Diagram.prototype.getAnimationDuration = function() {
 ...
 */
Diagram.prototype.processEvent = function(event, source) {
  var type = event['type'];
 ...
 */
Diagram.prototype.prepareNodes = function(nodesData) {
  if (!nodesData)
    return;
 ...
 */
Diagram.prototype.renderLinks = function(linksData) {
  if (!linksData)
    return;
...
Diagram.prototype.layout = function() {
  ...
  if (this.Options.panZoomState != null) {
    ...
    var viewport = new Rectangle(x, y, w, h);
 ...
 */
Diagram.prototype.setAlphas = function(alpha) {
  if (!this._bRendered) {
 ...
 * @override
 */
Diagram.prototype.getLinkById = function(id) {
  return this._links.get(id);
};
 ...
 * @override
 */
Diagram.prototype.GetAllLinks = function() {
 ...
 * @protected
 */
Diagram.prototype.GetAllLinksMap = function() {
 ...
 * @protected
 */
Diagram.prototype.GetAllLinkObjects = function() {
  var allLinks = [];
  if (this._links) {
    this._links.forEach(function(link, linkId, map){
      allLinks.push(link);
 ...
 * @override
 */
Diagram.prototype.GetAllNodes = function() {
 ...
 * @protected
 */
Diagram.prototype.GetAllNodesMap = function() {
 ...
 * @protected
 */
Diagram.prototype.GetAllNodeObjects = function() {
  var allNodes = [];
  if (this._nodes) {
    this._nodes.forEach(function(value, key, map){
      allNodes.push(value);
 ...
 * @override
 */
Diagram.prototype.GetAllRoots = function() {
 ...
 * @protected
 */
Diagram.prototype.GetRootNodeObjects = function() {
  var roots = [];
 ...
 * @override
 */
Diagram.prototype.HandlePanEvent = function(event) {
  Diagram.superclass.HandlePanEvent.call(this, event);
  if (this.Overview) {
 ...
 * @override
 */
Diagram.prototype.HandleZoomEvent = function(event) {
  Diagram.superclass.HandleZoomEvent.call(this, event);
  if (this.Overview) {
    this.Overview.UpdateViewport();
  }
  var subtype = event.subtype;
 ...
 */
Diagram.prototype.getNavigableLinksForNodeId = function(nodeId) {
  var links = [];
  this._links.forEach(function(link, linkId, map){
 ...
 * @private
 */
Diagram.prototype._processInitialSelections = function() {
 ...
 * @private
 */
Diagram.prototype._processHighlighting = function(handleEventListeners) {
  ...
  this._nodes.forEach(function(node, nodeId, map){
    var match = bAnyMatched ? ArrayUtils.hasAnyItem(node.getCategories(), categories) :
  ...
  this._links.forEach(function(link, linkId, map){
    var match = bAnyMatched ? ArrayUtils.hasAnyItem(link.getCategories(), categories) :
 ...
 * @private
 */
Diagram.prototype._processNodeConnections = function(highlightedNodes) {
 ...
 * @private
 */
Diagram.prototype._processLinkConnections = function(highlightedLinks) {
 ...
 * @private
 */
Diagram.prototype._updateAlphas = function(bHighlight, highlightedObjects) {
 ...
 * @override
 */
Diagram.prototype.highlight = function(categories) {
  // Update the options
 ...
 * @override
 */
Diagram.prototype.select = function(selection) {
  // Update the options
 ...
 */
Diagram.prototype.getAutomation = function() {
  if (!this.Automation)
 ...
 * @protected
 */
Diagram.prototype.Log = function(message, level) {
 ...
 * @param {boolean} hovered true to show hover effect
 */
Diagram.prototype.processDefaultHoverEffect = function(nodeId, hovered) {
 ...
 */
Diagram.prototype.processDefaultSelectionEffect = function(nodeId, selected) {
  var node = this.getNodeById(nodeId);
  if (node)
 ...
 * @param {boolean} focused true to show keyboard focus effect
 */
Diagram.prototype.processDefaultFocusEffect = function(nodeId, focused) {
  var node = this.getNodeById(nodeId);
  if (node)
...
Diagram.prototype.updateNodeDims = function(nodeContext) {
 ...
 * @param {DvtDiagramLayoutContextNode} nodeContext
 */
Diagram.prototype.renderLeafNodeFromContext = function(nodeContext) {
  var node = this.getNodeById(nodeContext.getId());
  if (node.isDisclosed()) {
 ...
 */
Diagram.prototype.renderNodeFromContext = function(nodeContext, forceDims) {
  if (!nodeContext.IsRendered) {
    var node = this.getNodeById(nodeContext.getId());
    if (node.isDisclosed()) {
 ...
 * @private
 */
Diagram.prototype._prepareNodes = function(parent, nodesData) {
  if (!nodesData)
    return;
 ...
 */
Diagram.prototype.setNodeDisclosed = function(nodeId, disclosed) {
  if (disclosed != this._isNodeDisclosed(nodeId))
    this.dispatchEvent(new EventFactory.newEvent(disclosed ? 'beforeExpand' : 'beforeCollapse', nodeId));
};
 ...
 * @param {String} nodeId node id
 */
Diagram.prototype.expand = function(nodeId) {
 ...
 * @param {String} nodeId node id
 */
Diagram.prototype.collapse = function(nodeId) {
  var triggerEvent = false;
  var expanded = this.getOptions()['expanded'];
  if (expanded && expanded['has'] && expanded['has'](nodeId)) { // key set - component created a custom element
    triggerEvent = true;
 ...
 * @private
 */
Diagram.prototype._addLayoutContextForChildNodes = function(parentNode, lcParentNode, layoutContext, bRenderAfter) {
 ...
 * @private
 */
Diagram.prototype._appendLayoutContextForChildNodes = function(parentNode, lcParentNode, layoutContext, nodes, index) {
  if (parentNode.isDisclosed()) {
    ...
    lcParentNode.SetContainerPaddingObj(DvtBaseDiagram.getLayoutContainerPadding(parentNode.getContainerPadding()));
 ...
 * @private
 */
Diagram.prototype._createLayoutContextForChildNodes = function(lcParentNode, layoutContext, arChildIds, bRenderAfter) {
  var lcChildNodes = [];
 ...
 */
Diagram.prototype._isNodeDisclosed = function(nodeId) {
  var expanded = this.getOptions()['expanded'];
 ...
 */
Diagram.prototype._isLinkPromoted = function(linkData) {
 ...
 * @private
 */
Diagram.prototype._addToCollapsedArray = function(node) {
 ...
 * @private
 */
Diagram.prototype._findFirstVisibleAncestor = function(childId) {
 ...
 * @private
 */
Diagram.prototype._updateKeyboardFocusEffect = function() {
 ...
 */
Diagram.prototype.clearDisclosedState = function() {
 ...
 */
Diagram.prototype.ShowLinkCreationFeedback = function(event) {
      ...
      if (obj instanceof DvtDiagramNode) {
        ...
        if (styleCallback && typeof styleCallback === 'function') {
          ...
          if (linkFeedbackStyle) {
            linkDefaults['style'] = JsonUtils.merge(linkFeedbackStyle['svgStyle'], linkDefaults['style']);
 ...
 * @protected
 */
Diagram.prototype.HideLinkCreationFeedback = function() {
 ...
 * @param {Object} event event data
 */
Diagram.prototype.handleDataSourceChangeEvent = function(type, event) {
  var animationOnDataChange = DvtDiagramStyleUtils.getAnimationOnDataChange(this);
 ...
 * @private
 */
Diagram.prototype._updateOverview = function(type, event) {
  if (this.Overview) {
 ...
 * @private
 */
Diagram.prototype._updateLayoutContext = function(type, event) {
 ...
 */
Diagram.prototype._getLayoutContext = function() {
  if (!this._layoutContext) {
    var layoutContext = this.CreateEmptyLayoutContext();
 ...
 */
Diagram.prototype._removeNodes = function(parent, nodesData) {
 ...
 */
Diagram.prototype._removeLinks = function(linksData) {
  ...
  for (var i = 0; i < linksData.length; i++) {
    ...
    if (link) {
      ...
      if (data && data.length == 1 && data[0]['id'] == linkId) {
        ...
        for (var i = 0; i < data.length; i++) {
          if (DvtDiagramDataUtils.compareValues(this.getCtx(), data[i]['id'], linkId)) {
            data.splice(i, 1);
          }
        }
        this._linkToPromotedMap.delete(linkId);
 ...
 */
Diagram.prototype.handleTouchStart = function() {
 ...
 */
Diagram.prototype.handleTouchEnd = function() {
 ...
 */
Diagram.prototype.storeTouchEventContent = function(content, diagramObj) {
  ...
  if (content.namespaceURI === ToolkitUtils.SVG_NS) {
    ...
    content.forEach(function(node) {ToolkitUtils.appendChildElem(this.getTouchEventPane().getElem(), node);}.bind(this));
  }
};
 ...
 */
Diagram.prototype._clearTouchEventContent = function() {
