...
Obj.createSubclass(DvtPictoChartImageMarker, ImageMarker);
...
DvtPictoChartImageMarker.prototype.animateUpdate = function(handler, oldMarker) {
  var animation = new CustomAnimation(this.getCtx(), this, this._picto.getAnimationDuration() * 0.75);
  ...
  animator.addProp(Animator.TYPE_NUMBER_ARRAY, this, this._getAnimationParams, this._setAnimationParams, endParams);
...
DvtPictoChartImageMarker.prototype.animateDelete = function(handler) {
  handler.add(new AnimFadeOut(this.getCtx(), this, this._picto.getAnimationDuration() * 0.5), 0);
};
...
DvtPictoChartImageMarker.prototype.animateInsert = function(handler) {
  this.setAlpha(0);
  handler.add(new AnimFadeIn(this.getCtx(), this, this._picto.getAnimationDuration() * 0.5), 2);
};
...
Obj.createSubclass(DvtPictoChartShapeMarker, SimpleMarker);
...
DvtPictoChartShapeMarker.prototype.animateUpdate = function(handler, oldMarker) {
  ...
  if (!startFill.equals(endFill)) {
    ...
    animator.addProp(Animator.TYPE_FILL, this, this.getFill, this.setFill, endFill);
  }
  ...
  DvtPictoChartShapeMarker.prototype.setDataColor = function (dataColor, bSkipStroke) {
    ...
    if (isRedwood) {
      ...
      if (!bSkipStroke) {
        ...
        this.setHoverStroke(new Stroke(innerColor, 1, 1.5, true), new Stroke(hoverColor, 1, 4.5, true));
...
Obj.createSubclass(DvtPictoChartKeyboardHandler, KeyboardHandler);
...
DvtPictoChartKeyboardHandler.getNextNavigable = function(picto, currentNavigable, event, originalNavigable) {
  ...
  var isForward =
      (event.keyCode == KeyboardEvent.LEFT_ARROW && isOriginRight) ||
      (event.keyCode == KeyboardEvent.RIGHT_ARROW && !isOriginRight) ||
      (event.keyCode == KeyboardEvent.UP_ARROW && isOriginBottom) ||
      (event.keyCode == KeyboardEvent.DOWN_ARROW && !isOriginBottom);

  var isDirectional =
      (event.keyCode == KeyboardEvent.LEFT_ARROW && isVertical) ||
      (event.keyCode == KeyboardEvent.RIGHT_ARROW && isVertical) ||
      (event.keyCode == KeyboardEvent.UP_ARROW && !isVertical) ||
      (event.keyCode == KeyboardEvent.DOWN_ARROW && !isVertical);

  var nextIdx = navigableItems.indexOf(currentNavigable) + (isForward ? 1 : -1);

  if (isDirectional)
    nextItem = KeyboardHandler.getNextNavigable(currentNavigable, event, navigableItems);
...
DvtPictoChartKeyboardHandler.prototype.processKeyDown = function(event) {
  ...
  if (currentNavigable && event.keyCode == KeyboardEvent.ENTER) {
...
Obj.createSubclass(DvtPictoChartItem, Container);
...
DvtPictoChartItem.prototype.Init = function(picto, item) {
  ...
  this._keyboardTooltipLocation = new Point(0, 0);
...
DvtPictoChartItem.prototype.getDatatip = function(target) {
  ...
  if (name)
    tds.push(HtmlTooltipManager.createElement('td', options['_tooltipLabelStyle'], name, ['oj-dvt-datatip-label']));
  tds.push(HtmlTooltipManager.createElement('td', options['_tooltipValueStyle'], this._getCountString(), ['oj-dvt-datatip-value']));
  var tr = HtmlTooltipManager.createElement('tr', null, tds);
  var table = HtmlTooltipManager.createElement('table', null, [tr], ['oj-dvt-datatip-table']);
  return table;
};
...
DvtPictoChartItem.prototype._getCountString = function() {
  return ResourceUtils.format(this._picto.getOptions().translations.labelCountWithTotal, [this.getCount(), this._picto.getTotalCount()]);
};
...
Obj.createSubclass(DvtPictoChartAutomation, Automation);
...
Obj.createSubclass(DvtPictoChartDefaults, BaseComponentDefaults);
...
DvtPictoChartDefaults.VERSION_1 = {
  ...
  '_gapRatio': 0.25,
  '_textStyle': new CSSStyle(BaseComponentDefaults.FONT_FAMILY_ALTA_13 + 'color: #252525;'),
  '_statusMessageStyle': new CSSStyle(BaseComponentDefaults.FONT_FAMILY_ALTA_13 + 'color: #252525;'),
  '_tooltipLabelStyle': new CSSStyle(''),
  '_tooltipValueStyle': new CSSStyle('')
};
...
Obj.createSubclass(DvtPictoChartEventManager, EventManager);
...
Obj.createSubclass(DvtPictoChartRenderer, Obj);
...
DvtPictoChartRenderer.render = function(picto, container, availSpace, info) {
  ...
  var background = new Rect(context, availSpace.x, availSpace.y, availSpace.w, availSpace.h);
  ...
  for (var i = 0; i < info.items.length; i++) {
    ...
    while (count > 0) {
      ...
      if (item.getSource()) {
        ...
        var hitArea = new Rect(context, rectX, rectY, rectW, rectH);
      ...
      if (isFirstMarker) {
        item.setKeyboardTooltipLocation(new Point(x, y));
        isFirstMarker = false;
      }
...
DvtPictoChartRenderer.getInfo = function(picto, width, height) {
  ...
  var categoryMap = ArrayUtils.createBooleanMap(options['hiddenCategories']);
  ...
  for (var i = 0; i < itemObjs.length; i++) {
    ...
    if (categoryMap && ArrayUtils.hasAnyMapItem(categoryMap, item.getCategories()))
      continue;
...
DvtPictoChartRenderer.renderEmptyText = function(picto, container, availSpace) {
  ...
  var emptyText = picto.renderEmptyText(container, emptyTextStr, availSpace.clone(),
 ...
 * @constructor
 */
const PictoChart = function(context, callback, callbackObj) {
  this.Init(context, callback, callbackObj);
};

Obj.createSubclass(PictoChart, BaseComponent);
...
PictoChart.prototype.Init = function(context, callback, callbackObj) {
  PictoChart.superclass.Init.call(this, context, callback, callbackObj);
...
PictoChart.prototype.render = function(options, width, height) {
  ...
  if (!width && !height) {
    ...
    ToolkitUtils.setSvgSize(context, preferredSize.w, preferredSize.h);
    var outerDivSize = ToolkitUtils.getOuterDivSize(context);
    ...
    if (preferredSize.w == outerDivSize.w && preferredSize.h != outerDivSize.h) {
      ...
      ToolkitUtils.setSvgSize(context, preferredSize.w, this.Height);
      this.Width = ToolkitUtils.getOuterDivSize(context).w;
    }
      ...
      ToolkitUtils.setSvgSize(context, this.Width, preferredSize.h);
      this.Height = ToolkitUtils.getOuterDivSize(context).h;
    }
    else {
  ...
  DvtPictoChartRenderer.render(this, this._container, new Rectangle(0, 0, this.Width, this.Height), this._info);
  ...
  if (!this._oldContainer) {
    ...
    var animHandler = new DataAnimationHandler(context, null);
  ...
  if (this.Animation) {
    ...
    ToolkitUtils.setSvgSize(context, Math.max(oldWidth, this.Width), Math.max(oldHeight, this.Height));
...
PictoChart.prototype._onRenderEnd = function() {
  ...
  ToolkitUtils.setSvgSize(this.getCtx(), this.Width, this.Height);
  ...
  CategoryRolloverHandler.highlight(this.Options['highlightedCategories'], this._items, this.Options['highlightMatch'] == 'any');
...
PictoChart.prototype.getAnimationDuration = function() {
  return CSSStyle.getTimeMilliseconds(this.Options['animationDuration']) / 1000;
};
...
PictoChart.prototype._getAnimationOnDisplay = function() {
  ...
  if (BlackBoxAnimationHandler.isSupported(animOnDisplay))
    return BlackBoxAnimationHandler.getInAnimation(context, animOnDisplay, this._container, bounds, duration);

  var playables = [];
  if (animOnDisplay == 'popIn') {
    for (var i = 0; i < this._markers.length; i++) {
      var marker = this._markers[i];
      playables.push(new AnimPopIn(context, marker, true, duration));
    }
  }
    ...
    for (var i = 0; i < this._markers.length; i++) {
      var marker = this._markers[i];
      var playable = new CustomAnimation(context, marker, duration);
      ...
      if (DvtPictoChartUtils.isVertical(this)) {
        ...
        playable.getAnimator().addProp(Animator.TYPE_NUMBER, marker, marker.getCx, marker.setCx, endState);
      }
      else {
        startState = DvtPictoChartUtils.isOriginBottom(this) ? this.Height : 0;
        endState = marker.getCy();
        marker.setCy(startState);
        playable.getAnimator().addProp(Animator.TYPE_NUMBER, marker, marker.getCy, marker.setCy, endState);
      }
      playables.push(playable);
    }
    // Add fade in to hide the ugly overlaps at the beginning
    playables.push(BlackBoxAnimationHandler.getInAnimation(context, BlackBoxAnimationHandler.ALPHA_FADE, this._container, bounds, duration));
  }

  if (playables.length > 0)
    return new ParallelPlayable(context, playables);
  else
    return null;
};
...
PictoChart.prototype.highlight = function(categories) {
  ...
  options['highlightedCategories'] = JsonUtils.clone(categories);

  // Perform the highlighting and propagate to children
  CategoryRolloverHandler.highlight(categories, this.getItems(), options['highlightMatch'] == 'any');
};
