...
Obj.createSubclass(DvtTimelineTooltipUtils, Obj);
 ...
 * @param {Timeline} timeline The owning timeline instance.
 * @param {boolean} isTabular Whether the datatip is in a table format.
...
DvtTimelineTooltipUtils._processDatatip = function(datatipRows, timeline, isTabular) {
  ...
  if (isTabular)
    return HtmlTooltipManager.createElement('table', null, datatipRows, ['oj-dvt-datatip-table']);
  else
...
DvtTimelineTooltipUtils._addDatatipRow = function(datatipRows, timeline, type, defaultLabel, value, isTabular, index) {
  ...
  if (isTabular) {
    var tds = [HtmlTooltipManager.createElement('td', options['tooltipLabelStyle'], tooltipLabel, ['oj-dvt-datatip-label']),
               HtmlTooltipManager.createElement('td', options['tooltipValueStyle'], value, ['oj-dvt-datatip-value'])];
    datatipRows.push(HtmlTooltipManager.createElement('tr', null, tds));
  }
  else {
    datatipRows.push((datatipRows.length > 0 ? '<br>' : '') + ResourceUtils.format(translations.labelAndValue, [tooltipLabel, value]));
  }
};
...
DvtTimelineSeriesNode.prototype.getAriaLabel = function()
{
  ...
  if (this.isSelectable())
    states.push(this._timeline.getOptions().translations[this.isSelected() ? 'stateSelected' : 'stateUnselected']);
...
{
  if (isPublic) {
    return TimeComponent.sanitizeData(this._data, 'item');
  }
  return this._data;
};
 ...
 */
DvtTimelineSeriesNode.prototype.getRendererContext = function()
{
 ...
 */
DvtTimelineSeriesNode.prototype.changeState = function(state, value, override)
{
  if (override) {
...
Obj.createSubclass(DvtTimelineAutomation, Automation);
...
Obj.createSubclass(DvtTimelineDefaults, BaseComponentDefaults);
...
Obj.createSubclass(DvtTimelineOverviewDefaults, BaseComponentDefaults);
...
Obj.createSubclass(DvtTimelineOverviewNode, Obj);


/**
 * @param {TimelineOverview} overview The TimelineOverview that owns this node.
 * @param {object} props The properties for the node.
 * @protected
 */
...
Obj.createSubclass(DvtTimelineOverviewStyleUtils, Obj);
...
Obj.createSubclass(DvtTimelineOverviewParser, OverviewParser);


/**
 * @param {TimelineOverview} timelineOverview
 * @protected
 */
 ...
 */
const TimelineOverview = function(context, callback, callbackObj) {
  this.Init(context, callback, callbackObj);
};

Obj.createSubclass(TimelineOverview, Overview);

// state
TimelineOverview.ENABLED_STATE = '_';
TimelineOverview.HOVER_STATE = '_h';
TimelineOverview.SELECTED_STATE = '_s';
TimelineOverview.ACTIVE_SELECTED_STATE = '_as';

// property
TimelineOverview.BORDER_STYLE = 'bs';
TimelineOverview.BORDER_COLOR = 'bc';
TimelineOverview.BORDER_WIDTH = 'bw';
TimelineOverview.DURATION_BORDER_STYLE = 'dbs';
TimelineOverview.DURATION_BORDER_COLOR = 'dbc';
TimelineOverview.DURATION_BORDER_WIDTH = 'dbw';
TimelineOverview.BORDER_OFFSET = 'bof';
TimelineOverview.BORDER_OPACITY = 'bo';
TimelineOverview.GLOW_COLOR = 'gc';
TimelineOverview.GLOW_OPACITY = 'go';

TimelineOverview.FILL_COLOR = 'fc';

/**
 * Returns a new instance of TimelineOverview.
 * @param {dvt.Context} context The rendering context.
 * @param {string} callback The function that should be called to dispatch component events.
 * @param {object} callbackObj The optional object instance on which the callback function is defined.
 * @return {TimelineComponent}
 */
TimelineOverview.newInstance = function(context, callback, callbackObj)
{
  return new TimelineOverview(context, callback, callbackObj);
};
 ...
 * @override
 */
TimelineOverview.prototype.Init = function(context, callback, callbackObj)
{
  TimelineOverview.superclass.Init.call(this, context, callback, callbackObj);


  // default fills
  var colors = [ColorUtils.getPound(ColorUtils.getBrighter('#aadd77', 0.35)), '#aadd77', ColorUtils.getPound(ColorUtils.getDarker('#aadd77', 0.5))];
  ...
  this._markerBorderFill = SolidFill.invisibleFill();
 ...
 * @override
 */
TimelineOverview.prototype.initDefaults = function()
{
 ...
 * @override
 */
TimelineOverview.prototype.getParser = function(obj)
{
 ...
 * @override
 */
TimelineOverview.prototype._applyParsedProperties = function(props)
{
  TimelineOverview.superclass._applyParsedProperties.call(this, props);
  ...
  if (props.labelStyle)
    this._labelStyle = new CSSStyle(props.labelStyle);
  ...
  this._radialFill = new LinearGradientFill(250, this._defColors, this._defAlphas);
  this._linearFill = new LinearGradientFill(180, this._defColors, this._defAlphas);
  var borderOpacity = this.isVertical() ? 0 : 1;
  this._border = new Stroke('#aadd77', borderOpacity);
};
 ...
 */
TimelineOverview.prototype.getSeriesIds = function()
{
...
TimelineOverview.prototype.getDrawableById = function(id)
{
  ...
  for (var childIndex = 0; childIndex < numChildren; childIndex++) {
    ...
    if (drawable && drawable._node && Obj.compareValues(this.getCtx(), id, drawable._node.getId()))
      return drawable;
  }
  return null;
};

TimelineOverview.prototype.getItemId = function(drawable)
{
  if (drawable._node)
    return drawable._node.getId();
  else
    return drawable.getId().substr(5);
};

TimelineOverview.prototype.getStyle = function(state, style)
{
  return this._borderStyles[state + style];
};

TimelineOverview.prototype.getX = function(drawable)
{
...
TimelineOverview.prototype.getY = function(drawable)
{
  if (drawable._node != null)
...
TimelineOverview.prototype.getScaleX = function(node)
{
...
TimelineOverview.prototype.getScaleY = function(node)
{
...
TimelineOverview.prototype.renderData = function(width, height)
{
  TimelineOverview.superclass.renderData.call(this, width, height);
...
TimelineOverview.prototype.prepareDurations = function(durationMarkers)
{
 ...
 */
TimelineOverview.prototype.getDurationColorMap = function() {
...
TimelineOverview.prototype.calculateOptimalSize = function(start, end, width, height, size)
{
  ...
  for (var i = 0; i < this._markers.length; i++)
  {
    ...
    if (marker._endTime != null)
    {
      var x = OverviewUtils.getDatePosition(start, end, marker.getTime(), canvasSize);
...
TimelineOverview.prototype.addMarker = function(node, sz)
{
  ...
  var displayable = new SimpleMarker(this.getCtx(), marker, cx, cy, width, height, null, null, null, itemId);
  ...
  else
  {
    ...
    if (color != null)
    {
      ...
      {
        var lighter = ColorUtils.getBrighter(color, 0.50);
        var darker = ColorUtils.getDarker(color, 0.50);
...
TimelineOverview.prototype.addDurations = function(durationMarkers, start, end)
{
  ...
  for (var i = this._maxDurationY; i > 0; i--)
  {
    for (var j = 0; j < durationMarkers.length; j++)
    {
      ...
      if (i == node._durationLevel)
      {
        ...
        var x2 = OverviewUtils.getDatePosition(start, end, node.getEndTime(), this.isVertical() ? this.Height : this.Width);
        if (this.isVertical())
        {
          if (this.isRTL())
            var duration = new Rect(context, 0, x, durationY, x2 - x, durationId);
          else
            duration = new Rect(context, this.Width - durationY, x, durationY, x2 - x, durationId);
        }
        else
        {
          if (this.isRTL())
            duration = new Rect(context, this.Width - x2, this.Height - durationY - 20, x2 - x, durationY, durationId);
          else
            duration = new Rect(context, x, this.Height - durationY - 20, x2 - x, durationY, durationId);
        }
        duration.setFill(new SolidFill(node._durationFillColor));

        var feelerStroke = new Stroke(this.getStyle(TimelineOverview.ENABLED_STATE, TimelineOverview.DURATION_BORDER_COLOR), 1, 1);
...
TimelineOverview.prototype.calculateSize = function(node, start, end, size, hsz, result, maxHeight)
{
  ...
  var cx = OverviewUtils.getDatePosition(start, end, time, size);
  if (this.isHorizontalRTL())
    cx = size - cx - hszx * 2;
...
TimelineOverview.prototype.calculateY = function(overlappingMarkers, shape, cx, cy, hszx, hszy, maxy, hsz, maxHeight)
{
  // see if y intersects
...
TimelineOverview.prototype.calculateDurationY = function(item, durationMarkers)
{
...
/************************** event handling *********************************************/
TimelineOverview.prototype.HandleShapeMouseOver = function(event)
{
  // drawable will be null if it is handled by super
  var drawable = TimelineOverview.superclass.HandleShapeMouseOver.call(this, event);
  if (drawable == null)
    return;
  ...
  if (!isSelected)
  {
    ...
    var evt = EventFactory.newTimelineOverviewEvent('highlight', itemId);
...
TimelineOverview.prototype.HandleShapeMouseOut = function(event)
{
  // drawable will be null if it is handled by super
  var drawable = TimelineOverview.superclass.HandleShapeMouseOut.call(this, event);
  if (drawable == null)
    return;
  ...
  if (!this.isMovable(drawable))
  {
    ...
    if (!isSelected)
    {
      ...
      var evt = EventFactory.newTimelineOverviewEvent('unhighlight', itemId);
...
TimelineOverview.prototype.HandleShapeClick = function(event, pageX, pageY)
{
  // drawable will be null if it is handled by super
  var drawable = TimelineOverview.superclass.HandleShapeClick.call(this, event, pageX, pageY);
  if (drawable == null)
    return;

  // handle click on marker
  this.HandleMarkerClick(drawable, (event.ctrlKey || event.shiftKey || Agent.isTouchDevice()));
};

TimelineOverview.prototype.HandleMarkerClick = function(drawable, isMultiSelect)
{
  // if selection is disabled in Timeline then return
  if (!this.isItemSelectionEnabled())
    return;
...
/************************** marker highlight *********************************************/
TimelineOverview.prototype.highlightItem = function(itemId)
{
...
TimelineOverview.prototype.unhighlightItem = function(itemId)
{
  var drawable = this.getDrawableById(itemId);
  if (drawable != null)
    this.unhighlightMarker(drawable);
...
TimelineOverview.prototype.highlightMarker = function(drawable)
{
  if (this._selectedMarkers != null)
  {
    for (var i = 0; i < this._selectedMarkers.length; i++)
  ...
  // draw border
  this.applyState(drawable, TimelineOverview.HOVER_STATE);
};

TimelineOverview.prototype.unhighlightMarker = function(drawable)
{
  if (this._selectedMarkers != null)
  {
    for (var i = 0; i < this._selectedMarkers.length; i++)
  ...
  this.applyState(drawable, TimelineOverview.ENABLED_STATE);
};
/************************** end marker highlight *****************************************/


/************************** marker selection *********************************************/
TimelineOverview.prototype.selSelectItem = function(itemId)
{
  var drawable = this.getDrawableById(itemId);
  if (drawable != null)
...
TimelineOverview.prototype.selUnselectItem = function(itemId)
{
  var drawable = this.getDrawableById(itemId);
  if (drawable != null)
...
TimelineOverview.prototype.selectItem = function(drawable, isMultiSelect)
{
  var itemId = this.getItemId(drawable);

  // scroll timeline
  var evt = EventFactory.newTimelineOverviewEvent('selection', itemId, isMultiSelect);
  this.dispatchEvent(evt);
};

TimelineOverview.prototype.addSelectedMarker = function(drawable)
{
  if (this._selectedMarkers == null)
  ...
  if (lastSelectedMarker != null)
    this.applyState(lastSelectedMarker, TimelineOverview.SELECTED_STATE);

  this.applyState(drawable, TimelineOverview.ACTIVE_SELECTED_STATE);
};

TimelineOverview.prototype.removeSelectedMarker = function(drawable)
{
  if (this._selectedMarkers != null)
  {
    var index = -1;
...
TimelineOverview.prototype.removeAllSelectedMarkers = function()
{
  if (this._selectedMarkers != null)
  {
    for (var i = 0; i < this._selectedMarkers.length; i++)
...
TimelineOverview.prototype.applyState = function(drawable, state)
{
  ...
  {
    var borderWidth = parseInt(this.getStyle(state, TimelineOverview.BORDER_WIDTH), 10);
    var borderOffset = parseInt(this.getStyle(state, TimelineOverview.BORDER_OFFSET), 10);
    ...
    if (borderMarker == null)
    {
      ...
      borderMarker = new SimpleMarker(this.getCtx(), markerType, cx, cy, width, height, null, null, null, drawable.getId() + '_border');
    ...
    var stroke = new Stroke(borderColor, this.getStyle(state, TimelineOverview.BORDER_OPACITY), borderWidth);
    ...
    if (requiresGlowMarker)
    {
      if (glowMarker == null)
      {
        ...
        glowMarker = new SimpleMarker(this.getCtx(), markerType, cx, cy, width, height, null, null, null, drawable.getId() + '_glow');
      ...
      var glowStroke = new Stroke(glowColor, this.getStyle(state, TimelineOverview.GLOW_OPACITY), 4);
...
TimelineOverview.prototype.applyDurationState = function(drawable, state)
{
  var borderColor = this.getStyle(state, TimelineOverview.DURATION_BORDER_COLOR);
  if (borderColor == null)
    borderColor = '#000000';
  var width = parseInt(this.getStyle(state, TimelineOverview.DURATION_BORDER_WIDTH), 10);
  drawable.setStroke(new Stroke(borderColor, 1, width));
};
...
Obj.createSubclass(DvtTimelineStyleUtils, Obj);
...
DvtTimelineStyleUtils.getItemDescriptionStyle = function(item)
{
  ...
  if (style)
  {
    var cssStyle = new CSSStyle(style);
...
DvtTimelineStyleUtils.getItemFillColor = function(item)
{
  ...
  {
    var cssStyle = new CSSStyle(style);
    var fillColor = cssStyle.getStyle(CSSStyle.BACKGROUND_COLOR);
    if (fillColor)
      return fillColor;
  }
  var options = item._series.getOptions();
...
DvtTimelineStyleUtils.getItemStrokeColor = function(item)
{
  ...
  {
    var cssStyle = new CSSStyle(style);
    var strokeColor = cssStyle.getStyle(CSSStyle.BORDER_COLOR);
    if (strokeColor)
      return strokeColor;
  }
  var options = item._series.getOptions();
...
{
  return CSSStyle.getTimeMilliseconds(options['styleDefaults']['animationDuration']) / 1000;
};
...
Obj.createSubclass(DvtTimelineRenderer, Obj);
...
DvtTimelineRenderer._renderBackground = function(timeline)
{
  if (timeline._background)
  {
    ...
    timeline._background = new Rect(timeline.getCtx(), 0, 0, timeline._backgroundWidth, timeline._backgroundHeight, 'bg');
...
DvtTimelineRenderer._renderScrollableCanvas = function(timeline)
{
  if (timeline._canvas)
    return;
  timeline._canvas = new Container(timeline.getCtx(), 'canvas');
...
DvtTimelineRenderer._renderSeriesLabels = function(timeline)
{
  if (timeline._series)
  {
    ...
    var zoomControlSpacing = TransientButton._DEFAULT_RADIUS * 2 + 6;
    ...
    for (var i = 0; i < seriesCount; i++)
    {
      ...
      if (seriesLabel != null)
      {
        ...
        var seriesLabelBackgroundStyle = new CSSStyle(DvtTimelineStyleUtils.getSeriesLabelBackgroundStyle());
        if (series._style)
        {
          var backgroundColor = series._style.getStyle(CSSStyle.BACKGROUND_COLOR);
          if (backgroundColor)
            seriesLabelBackgroundStyle.setStyle(CSSStyle.BACKGROUND_COLOR, backgroundColor);
        }
        ...
        var backgroundRect = new Rect(context, 0, 0, width + seriesLabelPadding * 2, dim.h + seriesLabelPadding * 2, 'slb_s' + i);
        ...
        {
          if (isRTL)
            posX = Math.abs(i - 1) * (timeline._canvasSize - width - doubleLabelSpacing) + labelSpacing + timeline._startX + (i - 1) * zoomControlSpacing;
          else
        ...
        TextUtils.fitText(seriesLabelElem, width, Infinity, timeline);
...
DvtTimelineRenderer._renderSeriesTimeAxis = function(timeline, startPos, endPos, container, length)
{
  ...
  if (timeline._seriesScale)
  {
    ...
    else
    {
      ...
      var startDate = TimeAxisUtils.getPositionDate(start, end, startPos, length);
      var currentDate = timeline._seriesTimeAxis.adjustDate(startDate);
      var currentPos = TimeAxisUtils.getDatePosition(start, end, currentDate, length);
      ...
      while (currentPos < endPos)
      {
        ...
        currentPos = TimeAxisUtils.getDatePosition(start, end, currentDate, length);
    ...
    var seriesAxisLabelBackgroundStyle = new CSSStyle(DvtTimelineStyleUtils.getSeriesAxisLabelBackgroundStyle());
    if (timeline._series[0] && timeline._series[0]._style)
    {
      var backgroundColor = timeline._series[0]._style.getStyle(CSSStyle.BACKGROUND_COLOR);
      if (backgroundColor)
        seriesAxisLabelBackgroundStyle.setStyle(CSSStyle.BACKGROUND_COLOR, backgroundColor);
    }
...
DvtTimelineRenderer._renderOverview = function(timeline)
{
  ...
  if (timeline._overviewCanvas == null)
  {
    ...
    timeline._overviewCanvas = new Container(context, 'oCanvas');
  }
  else
  {
...
DvtTimelineRenderer._renderScrollbars = function(timeline)
{
  ...
  if (timeline.isTimeDirScrollbarOn())
  {
    if (timeline.isVertical())
    {
      if (Agent.isRightToLeft(timeline.getCtx()))
  ...
  {
    var sbOptions = {};
    sbOptions['color'] = timeline.timeDirScrollbarStyles.getStyle(CSSStyle.COLOR);
    sbOptions['backgroundColor'] = timeline.timeDirScrollbarStyles.getStyle(CSSStyle.BACKGROUND_COLOR);
    ...
    {
      sbOptions['isReversed'] = Agent.isRightToLeft(context);
  ...
  if (timeline.contentDirScrollbar)
  {
    ...
    {
      sbOptions = {};
      sbOptions['color'] = timeline.contentDirScrollbarStyles.getStyle(CSSStyle.COLOR);
      sbOptions['backgroundColor'] = timeline.contentDirScrollbarStyles.getStyle(CSSStyle.BACKGROUND_COLOR);
...
{
  timeline.setTimeDirScrollbar(new SimpleScrollbar(timeline.getCtx(), timeline.HandleEvent, timeline));
  container.addChild(timeline.timeDirScrollbar);
  if (timeline.isVertical())
  {
    var location = 'right';
    var width = CSSStyle.toNumber(timeline.timeDirScrollbarStyles.getWidth());
  ...
  else
  {
    location = 'bottom';
    width = availSpace.w;
    height = CSSStyle.toNumber(timeline.timeDirScrollbarStyles.getHeight());
  }
  LayoutUtils.position(availSpace, location, timeline.timeDirScrollbar, width, height, 0);
...
{
  timeline.setContentDirScrollbar(new SimpleScrollbar(timeline.getCtx(), timeline.HandleEvent, timeline), index);
  container.addChild(timeline.contentDirScrollbar[index]);
  if (timeline.isVertical())
  {
    var location = 'bottom';
    var width = availSpace.w;
    var height = CSSStyle.toNumber(timeline.contentDirScrollbarStyles.getHeight());
  }
  else
  {
    location = 'right';
    width = CSSStyle.toNumber(timeline.contentDirScrollbarStyles.getWidth());
    height = availSpace.h;
  }
  LayoutUtils.position(availSpace, location, timeline.contentDirScrollbar[index], width, height, 0);
  return new Dimension(width, height);
};
...
DvtTimelineRenderer._renderEmptyText = function(timeline)
{
  ...
  if (timeline.Options['series'])
    ...
    emptyTextStr = timeline.Options.translations.labelNoData;

  timeline.clearComponent();
  timeline.renderEmptyText(timeline._canvas, emptyTextStr,
      new Rectangle(0, 0, timeline._backgroundWidth, timeline._backgroundHeight),
...
DvtTimelineRenderer._addLabel = function(context, container, pos, text, maxLength, y, labelStyle, id, renderBackground, backgroundLabelStyle, backgroundLabelOpacity, labelPadding, labelList, isRTL)
{
  ...
  if (renderBackground)
  {
    ...
    var backgroundRect = new Rect(context, x - labelPadding, y - labelPadding, width, dim.h + labelPadding * 2, 'ob_' + id);
  ...
  TextUtils.fitText(label, maxLength, Infinity, container);
  if (labelList)
...
Obj.createSubclass(DvtTimelineSeriesItem, Container);
...
Obj.createSubclass(DvtTimelineSeriesItemRenderer, Obj);
...
DvtTimelineSeriesItemRenderer._createBubble = function(item, series, index)
{
  ...
  if (customRenderer) {
    ...
    width = Math.max(0, content._w + content._x + marginStart * 2);
    height = content._h + content._y + marginTop * 2;
  } else {
    width = content._w + marginStart * 2;
    height = content._h + marginTop * 2;
  }
...
DvtTimelineSeriesItemRenderer._renderBubble = function(item, series, container, animationElems)
{
  ...
  if (!isRTL) {
    ...
    if (customRenderer) {
      content.setTranslate(nodeWidth - marginStart - 2 * content._x, marginTop);
    } else {
      content.setTranslate(marginStart, marginTop);
    }
  }
...
DvtTimelineSeriesItemRenderer._displayBubble = function(item, series, overflowOffset, animator, resetState)
{
  ...
  if (animator)
  {
    ...
    animator.addProp(Animator.TYPE_NUMBER, bubbleContainer, bubbleContainer.getTranslateX, bubbleContainer.setTranslateX, transX);
    animator.addProp(Animator.TYPE_NUMBER, bubbleContainer, bubbleContainer.getTranslateY, bubbleContainer.setTranslateY, transY);
  }
  else
...
DvtTimelineSeriesItemRenderer._getBubbleContent = function(item, series)
{
  ...
  var container = new Container(context);
  var offsetX = 0;
  var offsetY = 0;
  var maxWidth = 0;
  var maxHeight = 0;
  ...
  if (!isRTL)
  {
    ...
    if (thumbnail)
    {
      ...
      var thumbImage = new Image(context, thumbnail, 0, 0, thumbWidth, thumbHeight, '_tn');
    ...
    if (title)
    {
      var titleText = new OutputText(context, title, offsetX, 0);
    ...
    if (desc)
    {
      var descText = new OutputText(context, desc, offsetX, offsetY);
  ...
  else
  {
    ...
    if (title)
    {
      titleText = new OutputText(context, title, 0, 0);
      titleText.setCSSStyle(DvtTimelineStyleUtils.getItemTitleStyle(item));
    ...
    if (desc)
    {
      descText = new OutputText(context, desc, 0, offsetY);
      descText.setCSSStyle(DvtTimelineStyleUtils.getItemDescriptionStyle(item));
      dim = descText.getDimensions();
      var width = dim.w;
...
DvtTimelineSeriesItemRenderer._renderFeeler = function(item, series, container, overflowOffset, animationElems)
{
  ...
  if (isRTL)
    ...
    feelerX = loc;
  var feeler = new Line(context, feelerX, feelerY, feelerX, feelerHeight, feelerId);
  if (animationElems)
  {
    feeler.setAlpha(0);
  ...
  var stroke = new Stroke(feelerColor, 1, feelerWidth);
...
DvtTimelineSeriesItemRenderer._updateFeeler = function(item, series, overflowOffset, animator)
{
  ...
  if (animator)
  {
    ...
    animator.addProp(Animator.TYPE_NUMBER, feeler, feeler.getX1, feeler.setX1, feelerX);
    animator.addProp(Animator.TYPE_NUMBER, feeler, feeler.getY1, feeler.setY1, feelerY);
    animator.addProp(Animator.TYPE_NUMBER, feeler, feeler.getX2, feeler.setX2, feelerX);
    animator.addProp(Animator.TYPE_NUMBER, feeler, feeler.getY2, feeler.setY2, feelerHeight);
  }
  else
  {
...
DvtTimelineSeriesItemRenderer._renderDuration = function(item, series, container, overflowOffset, animationElems)
{
  ...
  {
    if (!isRTL && !series.isInverted() || isRTL && series.isInverted())
      var duration = new Rect(context, series._size - durationSize + 5, loc, durationSize, endLoc - loc, durationId);
    else
    {
      duration = new Rect(context, -5, loc, durationSize, endLoc - loc, durationId);
      overflowOffset = 0;
    }
  ...
  else
  {
    ...
    if (!series.isInverted())
    {
      duration = new Rect(context, transX, series._size - durationSize + 5, width, durationSize, durationId);
      duration.setTranslateY(overflowOffset);
    }
    else
    {
      duration = new Rect(context, transX, -5, width, durationSize, durationId);
  ...
  var feelerWidth = DvtTimelineStyleUtils.getItemStrokeWidth();
  var feelerColor = DvtTimelineStyleUtils.getItemStrokeColor(item);
  var feelerStroke = new Stroke(feelerColor, 1, feelerWidth);
  duration.setStroke(feelerStroke);
...
DvtTimelineSeriesItemRenderer._updateDuration = function(item, series, overflowOffset, animator)
{
  ...
  if (duration != null)
  {
    ...
    if (animator)
    {
      ...
      animator.addProp(Animator.TYPE_NUMBER, duration, duration.getTranslateX, duration.setTranslateX, durationTransX);
      animator.addProp(Animator.TYPE_NUMBER, duration, duration.getTranslateY, duration.setTranslateY, durationTransY);
      animator.addProp(Animator.TYPE_NUMBER, duration, duration.getX, duration.setX, durationX);
      animator.addProp(Animator.TYPE_NUMBER, duration, duration.getY, duration.setY, durationY);
      animator.addProp(Animator.TYPE_NUMBER, duration, duration.getWidth, duration.setWidth, durationWidth);
      animator.addProp(Animator.TYPE_NUMBER, duration, duration.getHeight, duration.setHeight, durationHeight);
    }
    else
    {
 ...
 * @extends {dvt.Obj}
 */
var DvtTimelineSeriesParser = function(context)
{
    this._context = context;
};

Obj.createSubclass(DvtTimelineSeriesParser, Obj);
...
Obj.createSubclass(DvtTimelineSeriesRenderer, Obj);
...
DvtTimelineSeriesRenderer.renderSeries = function(series, width, height)
{
  ...
  if (series._blocks.length == 0)
  {
    ...
    var block = new Container(context, 'itemBlock_' + series._fetchStartPos + '_' + series._fetchEndPos);
    block.startPos = series._fetchStartPos;
    block.endPos = series._fetchEndPos;

    var feelerBlock = new Container(context, 'feelers');
    block.addChild(feelerBlock);
    block.feelers = feelerBlock;

    var durationBlock = new Container(context, 'durations');
  ...
  else
  {
    ...
    {
      series._frAnimationElems = [];
      series._mvAnimator = new Animator(series.getCtx(), DvtTimelineStyleUtils.getAnimationDuration(series.Options), 0, Easing.cubicInOut);
  ...
  if (series._callbackObj.SelectionHandler)
    block.setCursor(SelectionEffectUtils.getSelectingCursor());
...
DvtTimelineSeriesRenderer._renderBackground = function(series, width, height)
{
  ...
  {
    addBackground = true;
    series._background = new Rect(series.getCtx(), 0, 0, width, height, 'bg');
  }
...
DvtTimelineSeriesRenderer._renderSeriesTicks = function(series)
{
  ...
  {
    var separatorStyle = new CSSStyle(DvtTimelineStyleUtils.getSeriesAxisSeparatorStyle());
    ...
    series._separatorStroke = new Stroke(separatorStyle.getStyle(CSSStyle.COLOR), 1, 1, false, {dashArray: '3'});
...
DvtTimelineSeriesRenderer._updateSeriesTicks = function(series)
{
  ...
  for (var i = 0; i < series._seriesTicksArray.length; i++)
  {
    ...
    if (!series.isVertical() && isRTL)
      var pos = series._length - TimeAxisUtils.getDatePosition(series._start, series._end, tick.time, series._length);
    else
      pos = TimeAxisUtils.getDatePosition(series._start, series._end, tick.time, series._length);
    if (series.isVertical())
    {
      tick.setX1(0);
      tick.setY1(pos);
...
DvtTimelineSeriesRenderer._renderSeriesTimeAxis = function(series, startPos, endPos, container)
{
  ...
  {
    dates = [];
    var startDate = TimeAxisUtils.getPositionDate(start, end, startPos, series._length);
    var currentDate = series._timeAxis.adjustDate(startDate);
    var currentPos = TimeAxisUtils.getDatePosition(start, end, currentDate, series._length);
    dates.push(currentDate.getTime());
    while (currentPos < endPos)
    {
      currentDate = series._timeAxis.getNextDate(currentDate.getTime());
      currentPos = TimeAxisUtils.getDatePosition(start, end, currentDate, series._length);
      dates.push(currentDate.getTime());
    }
  }
  ...
  {
    var currentTime = dates[i];
    currentPos = TimeAxisUtils.getDatePosition(start, end, currentTime, series._length);
    if (!series.isVertical() && isRTL)
      ...
      if (refObject)
      {
        ...
        if (series._renderedReferenceObjects.length == 0)
        {
          ...
          else
          {
            ...
            ref = new Line(context, pos, 0, pos, series._maxOverflowValue, 'zoomOrder[i]');
          }
          var referenceObjectStroke = new Stroke(DvtTimelineStyleUtils.getReferenceObjectColor(series.Options));
        ...
        else
        {
          ...
          ref.date = refObject;
          pos = TimeAxisUtils.getDatePosition(series._start, series._end, ref.date, series._length);
          if (series.isVertical())
          {
            ref.setX1(0);
            ref.setY1(pos);
...
DvtTimelineSeriesRenderer._updateReferenceObjects = function(series)
{
  ...
  for (var i = 0; i < series._renderedReferenceObjects.length; i++)
  {
    ...
    var pos = TimeAxisUtils.getDatePosition(series._start, series._end, ref.date, series._length);
    if (series.isVertical())
    {
      ref.setX1(0);
      ref.setY1(pos);
...
Obj.createSubclass(DvtTimelineSeries, BaseComponent);
...
DvtTimelineSeries.prototype.triggerAnimations = function()
{
  ...
  if (this._rmAnimationElems && this._rmAnimationElems.length != 0)
  {
    ...
    var fadeOutAnimator = new ParallelPlayable(context, new AnimFadeOut(context, this._rmAnimationElems, DvtTimelineStyleUtils.getAnimationDuration(this.Options)));
    Playable.appendOnEnd(fadeOutAnimator, this._onRmAnimationEnd, this);
  ...
  else if (this._mvAnimator && this._hasMvAnimations)
  {
    // Disable event listeners temporarily
    this._callbackObj.EventManager.removeListeners(this._callbackObj);
    Playable.appendOnEnd(this._mvAnimator, this._onMvAnimationEnd, this);
...
DvtTimelineSeries.prototype.render = function(options, width, height)
{
  ...
  if (desc) {
    ToolkitUtils.setAttrNullNS(this.getElem(), 'role', 'img');
    ToolkitUtils.setAttrNullNS(this.getElem(), 'aria-label', AriaUtils.processAriaLabel(desc));
  }
};
...
DvtTimelineSeries.prototype.GetComponentDescription = function()
{
  ...
  return ResourceUtils.format(translations.labelAndValue, seriesDescArray);
};
...
{
  this._style = new CSSStyle(DvtTimelineStyleUtils.getSeriesStyle());
...
DvtTimelineSeries.prototype._applyParsedProperties = function(props)
{
  ...
  if (this._emptyText == null)
    this._emptyText = this.Options.translations.labelNoData;
...
DvtTimelineSeries.prototype.getData = function(isPublic)
{
  if (isPublic) {
    ...
    return TimeComponent.sanitizeData(publicData, 'series');
  }
  return this._data;
};
...
{
  var line = new Line(this.getCtx(), x1, y1, x2, y2, id);
 ...
 */
const Timeline = function(context, callback, callbackObj)
{
  this.Init(context, callback, callbackObj);
};

Obj.createSubclass(Timeline, TimeComponent);
 ...
 * @return {Timeline}
 */
Timeline.newInstance = function(context, callback, callbackObj)
{
 ...
 * @protected
 */
Timeline.prototype.Init = function(context, callback, callbackObj)
{
  Timeline.superclass.Init.call(this, context, callback, callbackObj);
...
Timeline.prototype.Parse = function(options)
{
...
Timeline.prototype._applyParsedProperties = function(props)
{
  ...
  if (this._seriesScale)
  {
    ...
    this._seriesTimeAxis = new TimeAxis(this.getCtx(), null, null);
  ...
  Timeline.superclass._applyParsedProperties.call(this, props);
};
 ...
 */
Timeline.prototype.getTimeAxis = function()
{
  return this._timeAxis;
};

Timeline.prototype.getTimeAxisSize = function()
{
 ...
 * @override
 */
Timeline.prototype.select = function(selection)
{
  // Update the options
 ...
 * @private
 */
Timeline.prototype._bundleTimeAxisOptions = function(options)
{
 ...
 * @param {number} height The height of the component.
 */
Timeline.prototype.render = function(options, width, height)
{
  if (!options) {
    this._handleResize(width, height);
    return;
  }
  Timeline.superclass.render.call(this, options, width, height);
  ...
  if (this._scale)
  {
    ...
    if (!this._timeAxis)
      this._timeAxis = new TimeAxis(this.getCtx(), null, null);
 ...
 */
Timeline.prototype.hasValidSeriesItems = function()
{
 ...
 * @override
 */
Timeline.prototype.GetComponentDescription = function()
{
  if (this._shortDesc)
    return this._shortDesc;
  else
 ...
 *
 */
Timeline.prototype.applyStyleValues = function()
{
  this._style = new CSSStyle(DvtTimelineStyleUtils.getTimelineStyle());
  if (this.Options['styleDefaults'])
  {
  ...
  if (this._hasOverview)
  {
    ...
    if (overviewStyle)
    {
      var overviewCSSStyle = new CSSStyle(overviewStyle);
      var overviewSize = this._isVertical ? overviewCSSStyle.getWidth() : overviewCSSStyle.getHeight();
      if (overviewSize != null)
        this._overviewSize = CSSStyle.toNumber(overviewSize);
    }
  }
  Timeline.superclass.applyStyleValues.call(this);
  ...
  if (this._isVertical)
  {
    ...
    if (this.isContentDirScrollbarOn())
      this._backgroundHeight = this._backgroundHeight - CSSStyle.toNumber(this.contentDirScrollbarStyles.getHeight()) - scrollbarPadding;
    ...
    else
    {
      if (this.isTimeDirScrollbarOn())
        this._backgroundWidth = this._backgroundWidth - CSSStyle.toNumber(this.timeDirScrollbarStyles.getWidth()) - scrollbarPadding;
  ...
  else
  {
    // The size of the canvas viewport
    if (this.isContentDirScrollbarOn())
      this._backgroundWidth = this._backgroundWidth - CSSStyle.toNumber(this.contentDirScrollbarStyles.getWidth()) - scrollbarPadding;
    ...
    {
      if (this.isTimeDirScrollbarOn())
        this._backgroundHeight = this._backgroundHeight - CSSStyle.toNumber(this.timeDirScrollbarStyles.getHeight()) - scrollbarPadding;
 ...
 *
 */
Timeline.prototype.applyAxisStyleValues = function()
{
  if (this._seriesStyleDefaults && this._seriesStyleDefaults['backgroundColor'])
  {
    ...
    var r = ColorUtils.getRed(bgColor);
    var g = ColorUtils.getGreen(bgColor);
    var b = ColorUtils.getBlue(bgColor);
 ...
 */
Timeline.prototype.onAnimationEnd = function()
{
...
Timeline.prototype._getOverviewObject = function()
{
...
Timeline.prototype._getOverviewAxisOptions = function()
{
  var axisTicks = [];
  if (this._seriesTimeAxis)
  {
    ...
    else
    {
      ...
      var startDate = TimeAxisUtils.getPositionDate(start, end, this._fetchStartPos, length);
...
Timeline.prototype._getOverviewMarkerOptions = function()
{
  if (this._series)
  {
    var overviewMarkers = [];
...
Timeline.prototype.handleZoomWheel = function(newLength, time, compLoc, triggerViewportChangeEvent)
{
  ...
  Timeline.superclass.handleZoomWheel.call(this, newLength, time, compLoc, triggerViewportChangeEvent);
...
Timeline.prototype.updateSeries = function()
{
  if (this._series)
  {
    var seriesCount = this._series.length;
    ...
    for (var i = 0; i < seriesCount; i++)
    {
      ...
      if (this._isVertical)
      {
        ...
        else
        {
          ...
          posMatrix = new Matrix(1, 0, 0, 1, key * (this._seriesSize + axisSize), 0);
        }
...
Timeline.prototype._handleResize = function(width, height)
{
  this.Width = width;
  this.Height = height;
 ...
 * Ends panning.
 */
Timeline.prototype.endPan = function()
{
...
Timeline.prototype.applyInitialSelections = function()
{
...
Timeline.prototype._findDrawable = function(target)
{
  if (target)
  {
    var id = target.getId();
 ...
 */
Timeline.prototype.getAutomation = function()
{
  if (!this.Automation)
 ...
 * @override
 */
Timeline.prototype.clearComponent = function()
{
  Timeline.superclass.clearComponent.call(this);
  this.clearOverview();
};
 ...
 */
Timeline.prototype.clearOverview = function()
{
 ...
 * @override
 */
Timeline.prototype.isContentDirScrollbarOn = function()
{
  return true;
};
 ...
 */
Timeline.prototype.getBackgroundXOffset = function()
{
 ...
 */
Timeline.prototype.setBackgroundXOffset = function(backgroundX)
{
