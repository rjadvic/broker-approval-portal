      ...
      if (typeof value === 'undefined') {
        ...
        if (!cssDiv) {
          if (styleClass === 'oj-dvt-color-ramp') {
            cssDiv = ColorAttributeGroupHandler.__createAttrDiv();
          } else {
  ...
  for (var i = 0; i < definitions.length; i++) {
    ...
    if (property) {
      ...
      if (typeof value === 'undefined' && cssDiv) {
        ...
        value = styleClass === 'oj-dvt-color-ramp' ? ColorAttributeGroupHandler.__processAttrDiv(cssDiv)
...
TemplateHandler.prototype.getDataSetBoolean = function (templateName, attr) {
  ...
  var prop = AttributeUtils.attributeToPropertyName(attr);
  var propValue = templateDataSet && templateDataSet[prop];
  return propValue != null && AttributeUtils.coerceBooleanValue(this.getTemplate(templateName),
...
TemplateHandler.prototype.getTemplates = function () {
  return CustomElementUtils.getSlotMap(this._component.element[0]);
};
...
oj.__registerWidget('oj.dvtBaseComponent', $.oj.baseComponent, {
  ...
  _ComponentCreate: function () {
    ...
    if (parentElement && parentElement._dvtcontext) {
      ...
      this._context = new Context(this.element[0], null, this._referenceDiv[0]);
    }
    ...
    this._context.setTooltipAttachedCallback(subtreeAttached);
    this._context.setOverlayAttachedCallback(subtreeAttached);
  ...
  _addResizeListener: function () {
    if (!this._resizeListener) {
      ...
      addResizeListener(this.element[0], this._resizeListener, 250);
    }
  },
  ...
  _removeResizeListener: function () {
    if (this._resizeListener) {
      removeResizeListener(this.element[0], this._resizeListener);
  ...
  _Render: function (isResize) {
    ...
    if (!this._context.isReadyToRender() || (this._renderNeeded && isResize)) {
      ...
      this.options._locale = Agent.isEnvironmentTest() && this.options._locale
        ? this.options._locale : getLocale();
  ...
  _GetDvtComponent: function (element) {
    var widget = __GetWidgetConstructor(element)('instance');
    if (widget) {
  ...
  _resolveDeferredDataItems: function () {
    ...
    this._optionsCopy = JsonUtils.clone(this.options, null, this._GetComponentNoClonePaths());
  ...
  _NotReady: function () {
    ...
    if (this._numDeferredObjs === 0) {
      ...
      if (!this._readyResolveFunc) {
        var busyContext = Context$1.getContext(this.element[0]).getBusyContext();
        var options = {
...
export default DvtAttributeUtils;
