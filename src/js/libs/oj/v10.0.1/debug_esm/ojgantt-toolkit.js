 ...
 * @ignore
 */
import { Obj, BaseComponentDefaults, CSSStyle, Agent, AnimFadeIn, Easing, AnimFadeOut, CustomAnimation, ParallelPlayable, Playable, ToolkitUtils, Animator, Container, Path, Stroke, PathUtils, KeyboardEvent, MouseEvent, ResourceUtils, OutputText, IconButton, Displayable, SelectionEffectUtils, ColorUtils, Rectangle, AriaUtils, EventFactory, Point, EventManager, TextUtils, SolidFill, JsonUtils, Automation, Rect, Line, ClipPath, SvgShapeUtils, SimpleScrollbar, LayoutUtils, Dimension, MarqueeHandler, Matrix, SelectionHandler } from 'ojs/ojdvt-toolkit';
...
Obj.createSubclass(DvtGanttStyleUtils, Obj);
 ...
 * @private
 */
DvtGanttStyleUtils._DEFAULT_TIMEAXES_LABEL_STYLE = BaseComponentDefaults.FONT_FAMILY_ALTA_12 + 'color: #333333;';
...
Obj.createSubclass(DvtGanttAnimationManager, Obj);
...
Obj.createSubclass(DvtGanttDependencyNode, Container);
...
Obj.createSubclass(DvtGanttRowLabelContent, Obj);
...
DvtGanttRowLabelContent.prototype.getAriaLabel = function()
{
  ...
  return Displayable.generateAriaLabel(translations.labelLevel
...
Obj.createSubclass(DvtGanttTaskShape, Path);
...
Obj.createSubclass(DvtGanttTooltipUtils, Obj);
...
Obj.createSubclass(DvtGanttEventManager, TimeComponentEventManager);
...
DvtGanttEventManager.prototype.ProcessMarqueeEvent = function(event)
{
  ...
  if (this._gantt.isMarqueeSelectEnabled()) {
    ...
    if (event.subtype === 'start') {
      ...
      var layoutObjs = this._gantt.getDataLayoutManager().getLayoutObjectsInBBox(new Rectangle(event.x, event.y, event.w, event.h));
...
DvtGanttEventManager.prototype._dragStartSetup = function()
{
  ...
  // Disable pointer-events on reference objects (which lay on top of the row drop targets and interferes)
...
DvtGanttEventManager.prototype._dragCancelCleanup = function()
{
  ...
  // Re-enable pointer-events on referece objects
...
DvtGanttEventManager.prototype._handleKeyboardDnDChronologically = function(event, direction)
{
  ...
  // Update the aria live region with text that the screenreader should read.
  this._gantt.updateLiveRegionText(AriaUtils.processAriaLabel(DvtGanttTooltipUtils.getDatatip(this._keyboardDnDSourceObj, false)));
};
...
DvtGanttEventManager.prototype.ShowDropEffect = function(event) {
  ...
  if (dropObj && dropObj.nodeType === 'row')
  {
    ...
    ToolkitUtils.addClassName(targetElem, this._gantt.GetStyleClass('activeDrop'));
  };
};
...
DvtGanttEventManager.prototype.ShowRejectedDropEffect = function(event) {
  ...
  {
    var targetElem = dropObj.getBackground().getElem();
    ToolkitUtils.addClassName(targetElem, this._gantt.GetStyleClass('invalidDrop'));
  };
};
...
DvtGanttEventManager.prototype.ClearDropEffect = function() {
  ...
  if (prevDropObj && prevDropObj.nodeType === 'row')
  {
    ...
    ToolkitUtils.removeClassName(prevTargetElem, this._gantt.GetStyleClass('invalidDrop'));
    ToolkitUtils.removeClassName(prevTargetElem, this._gantt.GetStyleClass('activeDrop'));
  }
};
        ...
        if (self._lastKnownDragOverEvent.getNativeEvent().defaultPrevented)
        {
          ...
          if (Agent.isTouchDevice())
          {
            ...
            if (self._gantt.isSelectionSupported() && obj.isSelected())
            {
              ...
              if (totalSelected > 1)
              {
                screenReaderDragText += '. ' + ResourceUtils.format(self._gantt.getOptions().translations.taskMoveSelectionInfo, [totalSelected - 1]);
              }
            }
...
DvtGanttEventManager.prototype.toggleRowExpandCollapse = function(event, rowObj) {
  ...
  var rowData = rowObj['data'];
  // Today, only hierarchical task is supported, so itemData is the row data of task-data data provider
  ...
  var evt = EventFactory.newGanttExpandCollapseEvent(type, taskId, rowData, itemData, newExpandedKeySet);
  this._gantt.dispatchEvent(evt);
};
...
Obj.createSubclass(DvtGanttKeyboardHandler, TimeComponentKeyboardHandler);
...
Obj.createSubclass(DvtGanttTask, Obj);
...
Obj.createSubclass(DvtGanttTaskLabel, Obj);
...
DvtGanttTaskLabel.prototype._evaluatePosition = function(labelPosition)
{
  ...
  if (labelWidth > availableWidth)
  {
    TextUtils.fitText(this._labelOutputText, availableWidth, Infinity, this._container, 1);
  }
      ...
      onRenderEnd = function() {
        ...
        switch (self._effectiveLabelPosition)
        {
          ...
          case 'oProgress':
            ...
            labelColor = ColorUtils.getContrastingTextColor(fillColor ? fillColor['computedFill'] : null);
            if (labelColor != null)
            {
...
Obj.createSubclass(DvtGanttTaskNode, Container);
  ...
  if (this._draggedObj && eventManager)
  {
    ...
    switch (dragSourceType)
    {
      case 'tasks':
        ...
        // render feedback
        this._renderTaskMoveDragFeedback(referenceFinalLocalX, referenceFinalLocalY, artifactsContainer);
...
DvtGanttTaskNode.prototype._renderTaskMoveDragFeedback = function(referenceFinalLocalX, referenceFinalLocalY, container)
{
  ...
  else
  {
    ...
    var referenceDragFeedback = new DvtGanttTaskShape(this._gantt.getCtx(),
    ...
    if (this._gantt.isSelectionSupported() && this.isSelected() && this._gantt.getSelectionHandler().getSelectedCount() > 1)
    {
      ...
      for (i = 0; i < selection.length; i++)
      {
        ...
        if (selectionObj && selectionObj.nodeType === 'task' && selectionObj !== this)
        {
          ...
          dragFeedback = new DvtGanttTaskShape(this._gantt.getCtx(),
                                               dragFeedbackLocalX,
                                               dragFeedbackLocalY,
                                               mainShape.getFinalWidth(),
                                               mainShape.getFinalHeight(),
                                               mainShape.getBorderRadius(),
                                               selectionObj.getTask(),
                                               'mainDragFeedback');
...
DvtGanttTaskNode.prototype.getAriaLabel = function()
{
  ...
  if (this._predecessors != null || this._successors != null)
  {
    ...
    {
      depDesc = ResourceUtils.format(translations.accessiblePredecessorInfo, [this._predecessors.length]);
    ...
    if (this._successors != null && this._successors.length > 0)
    {
      if (depDesc.length > 0)
        depDesc = depDesc + ', ';
      depDesc = depDesc + ResourceUtils.format(translations.accessibleSuccessorInfo, [this._successors.length]);
...
DvtGanttTaskNode.prototype.getNextNavigable = function(event)
{
  ...
  if (event.type == MouseEvent.CLICK || keyboardHandler.isMultiSelectEvent(event))
    next = this;
  else if ((event.keyCode == KeyboardEvent.OPEN_ANGLED_BRACKET || KeyboardEvent.CLOSE_ANGLED_BRACKET) && event.altKey)
  {
    ...
    {
      if (Agent.isRightToLeft(this.getGantt().getCtx()))
        var type = event.keyCode == KeyboardEvent.CLOSE_ANGLED_BRACKET ? 'predecessor' : 'successor';
      else
  ...
  else if (keyboardHandler.isNavigationEvent(event) && this._gantt.getEventManager().getKeyboardDnDMode() == null)
...
Obj.createSubclass(DvtGanttAutomation, Automation);
...
Obj.createSubclass(DvtGanttRowNode, Container);
...
DvtGanttRowNode.prototype._finalizeRowLabelRender = function(gantt)
{
  ...
  else
  {
    ...
    TextUtils.fitText(labelDisplayable, rowAxis.getWidth() - 2 * rowLabelPadding, height, rowAxis, 1);
...
Obj.createSubclass(DvtGanttDataLayoutManager, Obj);
...
DvtGanttDataLayoutManager.prototype.renderViewport = function(yMin, yMax, bFromScrolling)
{
  ...
  if (databody)
  {
    ...
    var eventManager = this._gantt.getEventManager();
    if (!(TimeAxisUtils.supportsTouch() && eventManager && eventManager.isDnDDragging()))
    {
...
Obj.createSubclass(DvtGanttDefaults, BaseComponentDefaults);
...
Obj.createSubclass(DvtGanttParser, Obj);
...
Obj.createSubclass(DvtGanttRowAxis, Container);
...
Obj.createSubclass(DvtGanttReferenceObjects, Container);
...
DvtGanttReferenceObjects.prototype._applyStyles = function () {
  ...
  var defaultStyleClass = this._type === 'line'
    ? this._gantt.GetStyleClass('referenceObjectLine')
    : this._gantt.GetStyleClass('referenceObjectArea');
...
DvtGanttReferenceObjects.prototype._calcCmds = function (offset) {
  ...
  if (this._type === 'line') {
    this._refObjs.forEach(function (refObj) {
      ...
      cmds += PathUtils.moveTo(pos, this._gantt.getDatabodyStart())
        + PathUtils.verticalLineTo(this._gantt.getDatabodyStart() + this._gantt._canvasSize - this._gantt.getAxesHeight());
    }, this);
  } else {
    this._refObjs.forEach(function (refObj) {
      ...
      cmds += PathUtils.moveTo(leftPos, this._gantt.getDatabodyStart())
        + PathUtils.horizontalLineTo(rightPos)
        + PathUtils.verticalLineTo(this._gantt.getDatabodyStart() + this._gantt._canvasSize - this._gantt.getAxesHeight())
        + PathUtils.horizontalLineTo(leftPos)
        + PathUtils.verticalLineTo(this._gantt.getDatabodyStart());
    }, this);
  }
...
DvtGanttReferenceObjects.prototype.render = function (container) {
  ...
  this._ref = new Path(this._gantt.getCtx(), this._calcCmds());
  if (this._type === 'line') {
    // For reference lines, there's a light "border" counterpart for contrast
    this._partnerRef = new Path(this._gantt.getCtx(), this._calcCmds(1));
...
Obj.createSubclass(DvtGanttRenderer, Obj);
...
DvtGanttRenderer._renderScrollbars = function(gantt, container)
{
  ...
  if (gantt._scrollbarsCanvas == null)
  {
    gantt._scrollbarsCanvas = new Container(context, 'sbCanvas');
...
DvtGanttRenderer._renderScrollableCanvas = function(gantt)
{
  if (gantt._canvas)
    return;
  gantt._canvas = new Container(gantt.getCtx(), 'canvas');
...
DvtGanttRenderer._renderEmptyText = function(gantt, noData)
{
  ...
  var text = gantt.renderEmptyText(gantt._canvas, emptyTextStr,
      new Rectangle(0, 0, gantt._backgroundWidth, gantt._backgroundHeight),
...
DvtGanttRenderer._renderDatabody = function(gantt, container)
{
  ...
  if (databody == null)
  {
    databody = new Container(gantt.getCtx(), 'db');
...
DvtGanttRenderer._renderVerticalGridline = function(gantt, container)
{
  ...
  if (gantt.isVerticalGridlinesVisible())
  {
    ...
    {
      var pos = ToolkitUtils.getAttrNullNS(lines[i], 'x1');
...
DvtGanttRenderer._createMarker = function(gantt, id, width, height, pathCmd)
{
  ...
  ToolkitUtils.setAttrNullNS(elem, 'class', statusClassName);
...
Obj.createSubclass(Gantt, TimeComponent);
...
Gantt.prototype.setDataRegionTranslateY = function(translateY, bFromScrolling)
{
  ...
  if (rowIndex !== -1)
  {
    ...
    var evt = EventFactory.newGanttScrollPositionChangeEvent(y, rowIndex, offsetY);
