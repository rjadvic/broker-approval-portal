  ...
  const _ojListView = _ListViewUtils.clazz(Object, /** @lends oj._ojListView.prototype */ {
    ...
    _applyExitAnimation(cards) {
      ...
      cards.forEach(function (card) {
        AnimationUtils.slideOut(card, { offsetY: '300px', duration: '300ms', persist: 'all' });
        promise = AnimationUtils.fadeOut(card, { duration: '300ms', persist: 'all' });
      });
    ...
    renderComplete: function () {
          ...
          this._validateSelection(selection).then(function (validRowKeyData) {
            ...
            if (validRowKeyData.validKeys.length === 0) {
              ...
              self._setSelectionOption(
                ojkeyset.KeySetUtils.toKeySet(validRowKeyData.validKeys),
      ...
      if (this._isAnimateCards()) {
        ...
        this.element[0].querySelectorAll('.oj-listview-card:not(.oj-listview-card-animated)').forEach(function (card) {
          ...
          if (isInitial) {
            AnimationUtils.slideIn(card, { offsetY: '300px', delay: delay + 'ms', duration: '300ms' });
            _animationPromise = AnimationUtils.fadeIn(card, { delay: delay + 'ms', duration: '300ms' });
    ...
    */
    ...
    _isClickthroughDisabled: function (elem) {
      return ((elem.length > 0 && DataCollectionUtils.isClickthroughDisabled(elem.get(0))) ||
