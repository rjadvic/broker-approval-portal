...
define(['exports', 'ojs/ojdvt-toolkit', 'ojs/ojtranslation'], function (exports, dvt, Translations) { 'use strict';
  ...
  DvtNBoxDataUtils.isNodeHighlighted = function(nbox, node) {
    ...
    if (options[NBoxConstants.HIGHLIGHT_MATCH] == 'all')
      return dvt.ArrayUtils.hasAllMapItems(options['__highlightedMap'], node[NBoxConstants.CATEGORIES]);
    else
      return dvt.ArrayUtils.hasAnyMapItem(options['__highlightedMap'], node[NBoxConstants.CATEGORIES]);
  };
    ...
    if (icon) {
      ...
      if (iconSource) {
        ...
        if (hasSource) {
          ...
          var backgroundMarker =  new dvt.ImageMarker(nbox.getCtx(),
            iconX,
            iconY,
    ...
    if (cellData) {
      ...
      if (DvtNBoxDataUtils.isCellMaximized(nbox, cellIndex)) {
        ...
        if (cellRect.getWidth() - cellPadding - cellPadding > closeWidth) {
          ...
          var closeButton = new dvt.IconButton(context, 'borderless', {style: iconStyle, size: closeWidth}, null, null, cellContainer.handleDoubleClick, cellContainer);
  ...
  DvtNBoxDrawerRenderer._renderHeader = function(nbox, data, drawerContainer) {
    ...
    if (drawerBounds.w > closeWidth) {
      ...
      var context = nbox.getCtx();
      var iconStyle = dvt.ToolkitUtils.getIconStyle(context, close['class']);
      var closeButton = new dvt.IconButton(context, 'borderless', {style: iconStyle, size: closeWidth}, null, null, drawerContainer.closeDrawer, drawerContainer);
  ...
  DvtNBoxNodeOverflow.prototype.render = function(container, width, height) {
    ...
    this._button = new dvt.IconButton(ctx, 'borderless', {style: iconStyle, size: Math.max(w, h), pos: {x: width / 2, y: height / 2}}, background, null, this.HandleClick, this);
  ...
  DvtNBoxDefaults.SKIN_ALTA = {
    ...
    'styleDefaults': {
      ...
      'cellDefaults' : {'_style': new dvt.CSSStyle(),
        '_borderRadius': 0,
        ...
        for (var i = 0; i < categoryNodeCount; i++) {
          ...
          if (nodeData[NBoxConstants.CATEGORIES]) {
            if (nbox.getOptions()[NBoxConstants.HIGHLIGHT_MATCH] == 'all' ?
                dvt.ArrayUtils.hasAllItems(categories, nodeData[NBoxConstants.CATEGORIES]) :
                dvt.ArrayUtils.hasAnyItem(categories, nodeData[NBoxConstants.CATEGORIES]))
              return true;
          }
        }
      }
