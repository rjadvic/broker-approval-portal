 ...
 * @ignore
 */
define(['ojs/ojcustomelement', 'ojs/ojcustomelement-utils'], function (ojcustomelement, ojcustomelementUtils) {
  'use strict';
  /**
   * @license
  ...
  var ChildMutationObserver = function ChildMutationObserver(element, handler) {
    var _element = element;
    var _handler = handler;

    var _trackOption = ojcustomelementUtils.CustomElementUtils.isElementRegistered(element.tagName) ? ojcustomelementUtils.CustomElementUtils.getElementState(element).getTrackChildrenOption() : 'none';
    /**
    ...
    var _filterMutations = function _filterMutations(mutations) {
      ...
      for (var i = 0; i < mutations.length; i++) {
        ...
        while (testElement) {
          if (testElement === _element) {
            ...
            testElement = null;
          } else if (_trackOption === 'nearestCustomElement' && !ojcustomelementUtils.ElementUtils.isValidCustomElementName(testElement.localName)) {
    ...
    var _internalHandler = function _internalHandler(mutations) {
