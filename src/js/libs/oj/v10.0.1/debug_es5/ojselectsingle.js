...
define(['ojs/ojeditablevalue', 'ojs/ojlistview', 'ojs/ojpopupcore', 'ojs/ojinputtext', 'ojs/ojcore-base', 'jquery', 'ojs/ojcontext', 'ojs/ojdomutils', 'ojs/ojlogger', 'ojs/ojconfig', 'ojs/ojfocusutils', 'ojs/ojkeyset', 'ojs/ojhtmlutils', 'ojs/ojcomponentcore', 'ojs/ojthemeutils', 'ojs/ojcachingdataprovider', 'ojs/ojlistdataproviderview', 'ojs/ojtreedataproviderview', 'ojs/ojdataprovider', 'ojs/ojtimerutils', 'ojs/ojcustomelement-utils'], function (ojeditablevalue, ojlistview, ojpopupcore, ojinputtext, oj$1, $, Context, DomUtils, Logger, Config, FocusUtils, ojkeyset, HtmlUtils, Components, ThemeUtils, CachingDataProvider, ListDataProviderView, TreeDataProviderView, ojdataprovider, TimerUtils, ojcustomelementUtils) {
  ...
  CachingDataProvider = CachingDataProvider && Object.prototype.hasOwnProperty.call(CachingDataProvider, 'default') ? CachingDataProvider['default'] : CachingDataProvider;
  ...
  LovMainField.prototype._createInnerDom = function (options) {
    ...
    textFieldContainer.setAttribute('class', 'oj-text-field-container oj-searchselect-main-field');
    ...
    input.setAttribute('id', className + '-input-' + idSuffix); // JET-37990 - oj-select-single: autocomplete is not disabled in chrome browser
  ...
  AbstractLovBase.prototype._usingHandler = function (pos, props) {
    ...
    if (oj$1.PositionUtils.isAligningPositionClipped(props)) {
      ...
      var $dropdownElem = $(dropdownElem); // JET-34367 - remove code to update popup position on refresh
      // get the space available to the popup after the popup service has run its collision logic

      var availableSpace = oj$1.PositionUtils.calcAvailablePopupSize(pos, props); // constrain the dropdown size to the available space

      var dropdownElemStyle = dropdownElem.style; // don't change width if pos.left is < 0 because it can result in the dropdown flickering

      if (pos.left >= 0) {
        dropdownElemStyle.maxWidth = availableSpace.width + 'px';
      }

      dropdownElemStyle.maxHeight = availableSpace.height + 'px'; // apply same position, because the popup service should put it in the same place after
      // running collision logic (exclude using callback, because we're already in it and we don't
      // need to loop again)

      var dropdownPosition = this.getDropdownPosition(true); // reposition the dropdown
  ...
  AbstractLovBase.prototype._handleQueryResultsFetch = function (focusFirstElem) {
    ...
    var dropdownClassList = dropdownElem.classList;
    dropdownClassList.remove('oj-listbox-initial-open'); // var results = this._lovDropdown.findHighlightableOptionElems();
    ...
    this.updateLiveRegion(translation); // JET-39385 - SELECT SINGLE HAS NO OPTION TO SET TITLE IN MOBILE SEARCH
  ...
  AbstractLovBase.prototype._fetchFromDataProvider = function (term, initial) {
    ...
    if (term) {
      ...
      filterCriteria = oj$1.FilterFactory.getFilter({
        filterDef: {
          text: term
        }
      });
  ...
  LovDropdown.prototype.init = function (options) {
    ...
    this._filterInputText = options.filterInputText;
    this._getThrottlePromiseFunc = options.getThrottlePromiseFunc; // this._maxDisplayCount = 50;
    // this._fetchSize = 50;
    ...
    this._containerElem = containerElem; // JET-34367 - remove code to update popup position on refresh
  ...
  LovDropdown.prototype._createDropdownPositioningProxyElem = function (options) {
    var idSuffix = options.idSuffix; // JET-34367 - remove code to update popup position on refresh
    // Position a proxy element instead of the real dropdown so that we don't have to
    // change the size of the dropdown before calculating the position, because changing
    // the size of the dropdown before positioning may reset the results' scroll position and
    // prevent the user from scrolling down.
  ...
  LovDropdown.prototype.renderResults = function (searchText, filterCriteria, selectedValue, initial) {
    ...
    this._getThrottlePromiseFunc().then(function () {
  ...
  LovDropdown.prototype._configureResults = function (searchText, selectedValue, initial) {
    ...
    this._setCurrentFirstItem(null); // JET-38215 - SELECT SINGLE - KEYBOARD NAVIGATION FOR COLLECTION TEMPLATE USAGE
  ...
  LovDropdown.prototype._fetchFirstResult = function () {
    if (oj$1.DataProviderFeatureChecker.isTreeDataProvider(this._dataProvider)) {
        ...
        var fetchChild = function fetchChild(childIndex) {
          if (childIndex < data.length) {
            if (!done) {
              ...
              var childDataProvider = dataProvider.getChildDataProvider(itemMetadata.key);
  ...
  LovDropdown.prototype._defaultCollectionRenderer = function (context) {
    ...
    if (!this._resultsElem) {
      ...
      busyContext.whenReady().then(function () {
        ...
        if (context.data && oj$1.DataProviderFeatureChecker && oj$1.DataProviderFeatureChecker.isTreeDataProvider(context.data)) {
  ...
  LovDropdown.prototype.close = function () {
    ...
    this._lastRenderResultsPromise = null; //  - firefox: can't arrow into dropdown after clicking outside lov
    ...
    this._clearSelection(); // JET-38215 - SELECT SINGLE - KEYBOARD NAVIGATION FOR COLLECTION TEMPLATE USAGE
    ...
    var psOptions = {};
    psOptions[oj$1.PopupService.OPTION.POPUP] = this._containerElem;
    oj$1.PopupService.getInstance().close(psOptions); // this._containerElem.removeAttr('id');
  ...
  LovDropdown.prototype.open = function () {
    var containerElem = this._containerElem; // JET-37797 - can't close dropdown while list is loading
    // remove the 'no results found' style class until we know it's needed

    this._containerElem.removeClass(this._NO_RESULTS_FOUND_CLASSNAME); // if (this._searchElem) {
    ...
    psEvents[oj$1.PopupService.EVENT.POPUP_CLOSE] = function () {
      this._dispatchEvent('closeDropdown', {
        trigger: 'popupCloseEvent'
      });
    }.bind(this);

    psEvents[oj$1.PopupService.EVENT.POPUP_REMOVE] = this._surrogateRemoveHandler.bind(this);
    psEvents[oj$1.PopupService.EVENT.POPUP_AUTODISMISS] = this._clickAwayHandler.bind(this);

    psEvents[oj$1.PopupService.EVENT.POPUP_REFRESH] = function () {
      this._dispatchEvent('sizeDropdown'); // JET-34367 - remove code to update popup position on refresh
      // Position a proxy element instead of the real dropdown so that we don't have to
      // change the size of the dropdown before calculating the position, because changing
      // the size of the dropdown before positioning may reset the results' scroll position and
      // prevent the user from scrolling down.
    ...
    var psOptions = {};
    psOptions[oj$1.PopupService.OPTION.POPUP] = containerElem;
    ...
    psOptions[oj$1.PopupService.OPTION.CUSTOM_ELEMENT] = true;

    if (this._fullScreenPopup) {
      psOptions[oj$1.PopupService.OPTION.MODALITY] = oj$1.PopupService.MODALITY.MODAL;
    }
  ...
  LovDropdown.prototype._handleCollectionCurrentRowChanged = function (currentRow) {
    ...
    if (this._duringListViewInitialization || currentRow == null || this._handlingCollectionFocusinOnce) {
      return;
  ...
  //     selection = [val];
  //   }
  //   this._setSelectedKeySet(new KeySetImpl(selection));
  // };
  ...
  LovDropdown.prototype._handleCollectionFocusinOnce = function (data) {
    ...
    this._focusinListener = null; // only focus the first row if focus was transferred via the keyboard

    if (!DomUtils.recentPointer()) {
      var resolveBusyState = this._addBusyStateFunc('LovDropdown setting selected KeySet on focusin'); // JET-39993 - Select2 options qunit tests fail using chrome and safari on Mac for master branch
      ...
      this._setCollectionSelectedKeySet(data.key);
  ...
  var FilteringDataProviderView = function FilteringDataProviderView(dataProvider) {
    ...
    this.setFilterCriterion = function (filterCriterion) {
      ...
      if (!_compareFilterCriterion(oldFilterCriterion, this._filterCriterion)) {
        var event = new ojdataprovider.DataProviderRefreshEvent(); // use defineProperty to create the filterCriterionChanged prop so that it's not enumerable
    ...
    if (dataProvider && oj$1.DataProviderFeatureChecker.isTreeDataProvider(dataProvider)) {
  ...
  * @memberof! oj.ojSelectSingle
   ...
   * it is up to the application developer to associate an <code class="prettyprint">oj-label</code>
   * with the Select element.
   * For accessibility, you should associate an oj-label element with the Select element
   * by putting an <code>id</code> on the <code class="prettyprint">oj-select-single</code> element,
   * and then setting the <code>for</code> attribute on the <code class="prettyprint">oj-label</code>
   * to be the component's id.
   * </p>
   * <p>
  ...
  oj$1.__registerWidget('oj.ojSelect2', $.oj.editableValue, {
    ...
    _cleanUpMainFieldInputElement: function _cleanUpMainFieldInputElement() {
      ...
      if (agentInfo.browser === oj$1.AgentUtils.BROWSER.IE) {
    ...
    _setFilterFieldText: function _setFilterFieldText(text) {
      ...
      this._filterInputText.value = text; // if the filter field is in the mobile dropdown and the dropdown is not currently in
      ...
      switch (event.which) {
        ...
        case LovUtils.KEYS.DOWN:
          if (abstractLovBase.isDropdownOpen()) {
            ...
            FocusUtils.focusFirstTabStop(lovDropdown.getElement()); // keep oj-focus on root so that inside label is still shifted up and field looks focused
        ...
        case LovUtils.KEYS.ENTER:
          ...
          //  - select and combobox stop keyboard event propegation
          event.preventDefault(); // JET-40375 - SELECT SINGLE - DEFAULTED VALUE OVERRIDES USER INPUT
          // clear flag that the user has typed filter text so that we can override it when setting
          // the selected label

          this._userHasTypedFilterText = false; // on desktop, if the user clears all the text and presses Enter, clear the LOV value
    ...
    _handleContainerMouseDown: function _handleContainerMouseDown(event) {
      ...
      var $OuterWrapper = $(OuterWrapper); // only open dropdown on left (main) button click
      ...
      if (mainButton) {
        if (!this._fullScreenPopup && this._lovEnabled) {
          var filterInputElem = this._getFilterInputElem(); //  - help.instruction text not always shown
      ...
      switch (detail.subtype) {
        case 'closeDropdown':
          if (detail.trigger === 'escKeyDown') {
            ...
            // clear flag that the user has typed filter text so that we can override it when setting
            // the selected label
            this._userHasTypedFilterText = false; // explicitly show main field again in case focus is in the dropdown
        ...
        case 'tabOut':
          ...
          // clear flag that the user has typed filter text so that we can override it when setting
          // the selected label
          this._userHasTypedFilterText = false; // If no control item focused and going forward, then put focus on the
        ...
        case 'adjustDropdownPosition':
          ...
          if (this._fullScreenPopup) {
            ...
            var $resultsElem = $popupElem.find('.oj-select-results'); // size the proxy elem width the same as the dropdown width

            proxyElem.style.width = '' + $popupElem.outerWidth() + 'px'; // size the proxy results elem height the same as the dropdown results max-height
        ...
        case 'openPopup':
          ...
          psOptions[oj$1.PopupService.OPTION.LAUNCHER] = this.element;
          psOptions[oj$1.PopupService.OPTION.POSITION] = abstractLovBase.getDropdownPosition();
    ...
    _handleUserSelectedValueItem: function _handleUserSelectedValueItem(valueItem, event, context) {
      ...
      var afterSetValue = function (valueSet) {
        ...
        this._userSelectedValueItem = null; // JET-36443 - fire an ojValueAction event whenever the user selects a value, even if the
      ...
      if (ret instanceof Promise) {
        ret.then(afterSetValue, function () {
          afterSetValue(false);
        });
      } else {
        afterSetValue(ret);
    ...
    _createFilterInputText: function _createFilterInputText(className, idSuffix) {
      ...
      filterInputText.setAttribute('clear-icon', this._fullScreenPopup ? 'always' : 'never'); // JET-37990 - oj-select-single: autocomplete is not disabled in chrome browser
      // Chrome used to ignore "off" value, but as of Chrome 81 it works more reliably
      // (https://stackoverflow.com/questions/57367813/2019-chrome-76-approach-to-autocomplete-off)
      ...
      filterInputText.setAttribute('disabled', options.disabled); // if (options.labelEdge) {
      ...
      filterInputText.addEventListener('blur', function (event) {
        ...
        if (!this._fullScreenPopup) {
          ...
          if (!event.relatedTarget || !DomUtils.isAncestor(this._lovDropdown.getElement(), event.relatedTarget)) {
            this._showMainField();
          }
        }
    ...
    _setOptions: function _setOptions(options, flags) {
      ...
      var hasValueItem = options.hasOwnProperty('valueItem'); // JET-38612 - Select Single | Value, disable bug
      // check whether any of the options being set will require a refresh

      var requiresRefresh = false; // iterate over the set of options being applied because that set is most likely smaller than
    ...
    _updateInputElemValue: function _updateInputElemValue(valueItem) {
      ...
      if (text !== null) {
        ...
        if ((!this._fullScreenPopup && this._filterInputText.style.visibility !== 'hidden' || this._fullScreenPopup && this._abstractLovBase.isDropdownOpen()) && !this._userHasTypedFilterText) {
    ...
    _fetchByKeysFromDataProvider: function _fetchByKeysFromDataProvider(arKeys) {
      ...
      if (this._cachedFetchByKeys && this._cachedFetchByKeys.promise && oj$1.Object.compareValues(arKeys, this._cachedFetchByKeys.key)) {
     ...
     * @private
     */
    _fireValueActionEvent: function _fireValueActionEvent(valueItem, previousValue) {
      var detail = {
        value: valueItem.key,
        itemContext: valueItem,
        previousValue: previousValue
      };
      var valueActionEvent = new CustomEvent('ojValueAction', {
        detail: detail
      });
     ...
     * @private
     */
    _selectItemByValue: function _selectItemByValue(value, event) {
      var fetchDataAndSelectPromise = new Promise(function (resolve) {
