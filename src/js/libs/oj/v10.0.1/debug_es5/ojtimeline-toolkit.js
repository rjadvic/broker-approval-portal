 ...
 * @ignore
 */
define(['exports', 'ojs/ojdvt-toolkit', 'ojs/ojdvt-timecomponent', 'ojs/ojtimeaxis-toolkit', 'ojs/ojdvt-overview'], function (exports, dvt, ojdvtTimecomponent, ojtimeaxisToolkit, ojdvtOverview) {
  'use strict';
  /**
   * @license
   * Copyright (c) 2014, 2021, Oracle and/or its affiliates.
  ...
  dvt.Obj.createSubclass(DvtTimelineOverviewParser, ojdvtOverview.OverviewParser);
  /**
  ...
  var TimelineOverview = function TimelineOverview(context, callback, callbackObj) {
    this.Init(context, callback, callbackObj);
  };

  dvt.Obj.createSubclass(TimelineOverview, ojdvtOverview.Overview); // state

  TimelineOverview.ENABLED_STATE = '_';
  TimelineOverview.HOVER_STATE = '_h';
  TimelineOverview.SELECTED_STATE = '_s';
  TimelineOverview.ACTIVE_SELECTED_STATE = '_as'; // property
  ...
  TimelineOverview.prototype.Init = function (context, callback, callbackObj) {
    TimelineOverview.superclass.Init.call(this, context, callback, callbackObj); // default fills
    ...
    if (ojdvtOverview.OverviewUtils.supportsTouch()) colors = ['#aadd77']; // fill in duration Colors here to pull from style ramp
  ...
  TimelineOverview.prototype._applyParsedProperties = function (props) {
    ...
    if (this.getStyle(TimelineOverview.ENABLED_STATE, TimelineOverview.BORDER_STYLE) == 'solid') var _eOffset = parseInt(this.getStyle(TimelineOverview.ENABLED_STATE, TimelineOverview.BORDER_OFFSET), 10);else _eOffset = minMarkerSpacing;
    if (this.getStyle(TimelineOverview.ACTIVE_SELECTED_STATE, TimelineOverview.BORDER_STYLE) == 'solid') var _asOffset = parseInt(this.getStyle(TimelineOverview.ACTIVE_SELECTED_STATE, TimelineOverview.BORDER_OFFSET), 10);else _asOffset = minMarkerSpacing;
    if (this.getStyle(TimelineOverview.SELECTED_STATE, TimelineOverview.BORDER_STYLE) == 'solid') var _sOffset = parseInt(this.getStyle(TimelineOverview.SELECTED_STATE, TimelineOverview.BORDER_OFFSET), 10);else _sOffset = minMarkerSpacing;
  ...
  TimelineOverview.prototype.addMarker = function (node, sz) {
    ...
    if (color == null && opacity == this._defOpacity && isGradient == null) {
      ...
      if (color != null) {
        if (ojdvtOverview.OverviewUtils.supportsTouch()) colors = [color];else {
    ...
    this.applyState(displayable, TimelineOverview.ENABLED_STATE); // Do not antialias markers if specified or vertical
    ...
    for (var i = this._maxDurationY; i > 0; i--) {
      for (var j = 0; j < durationMarkers.length; j++) {
        ...
        if (i == node._durationLevel) {
          ...
          var x2 = ojdvtOverview.OverviewUtils.getDatePosition(start, end, node.getEndTime(), this.isVertical() ? this.Height : this.Width);
          ...
          var feelerStroke = new dvt.Stroke(this.getStyle(TimelineOverview.ENABLED_STATE, TimelineOverview.DURATION_BORDER_COLOR), 1, 1);
  ...
  TimelineOverview.prototype.calculateSize = function (node, start, end, size, hsz, result, maxHeight) {
    ...
    if (!this.isVertical()) {
      ...
      if (borderStyle == 'solid') borderOffset = parseInt(this.getStyle(TimelineOverview.ENABLED_STATE, TimelineOverview.BORDER_WIDTH), 10);
  ...
  TimelineOverview.prototype.addSelectedMarker = function (drawable) {
    ...
    if (lastSelectedMarker != null) this.applyState(lastSelectedMarker, TimelineOverview.SELECTED_STATE);
  ...
  TimelineOverview.prototype.removeSelectedMarker = function (drawable) {
    if (this._selectedMarkers != null) {
      ...
      if (index != -1) {
        ...
        this.applyState(drawable, TimelineOverview.ENABLED_STATE); // fix the array
  ...
  TimelineOverview.prototype.applyState = function (drawable, state) {
    ...
    if (requiresBorderMarker) {
      ...
      var stroke = new dvt.Stroke(borderColor, this.getStyle(state, TimelineOverview.BORDER_OPACITY), borderWidth);
      ...
      if (requiresGlowMarker) {
        ...
        var glowStroke = new dvt.Stroke(glowColor, this.getStyle(state, TimelineOverview.GLOW_OPACITY), 4);
  ...
  DvtTimelineRenderer.renderTimeline = function (timeline) {
    ...
    if (timeline.hasValidOptions()) {
      ...
      if (ojtimeaxisToolkit.TimeAxisUtils.supportsTouch()) timeline._setAriaProperty('flowto', timeline._series[0].getId());
  ...
  DvtTimelineRenderer._renderSeriesTimeAxis = function (timeline, startPos, endPos, container, length) {
    ...
    if (timeline._seriesScale) {
      ...
      if (timeline._customTimeScales && timeline._customTimeScales[timeline._seriesScale]) {
        ...
        dates = [];
        labels = [];
        var startDate = ojtimeaxisToolkit.TimeAxisUtils.getPositionDate(start, end, startPos, length);

        var currentDate = timeline._seriesTimeAxis.adjustDate(startDate);

        var currentPos = ojtimeaxisToolkit.TimeAxisUtils.getDatePosition(start, end, currentDate, length);
        ...
        while (currentPos < endPos) {
          ...
          currentPos = ojtimeaxisToolkit.TimeAxisUtils.getDatePosition(start, end, currentDate, length); // the last currentTime added in this loop is outside of the time range, but is needed
  ...
  DvtTimelineRenderer._renderEmptyText = function (timeline) {
    ...
    timeline.renderEmptyText(timeline._canvas, emptyTextStr, new dvt.Rectangle(0, 0, timeline._backgroundWidth, timeline._backgroundHeight), timeline.EventManager, DvtTimelineStyleUtils.getEmptyTextStyle(timeline.Options));
  };
  /**
  ...
  DvtTimelineSeriesItemRenderer._renderBubble = function (item, series, container, animationElems) {
    ...
    if (ojtimeaxisToolkit.TimeAxisUtils.supportsTouch()) dvt.ToolkitUtils.setAttrNullNS(bubbleContainer._elem, 'id', bubbleContainer._id); // associate the node with the marker
  ...
  DvtTimelineSeriesRenderer._updateItemsForZoom = function (items, series) {
    ...
    for (var i = 0; i < items.length; i++) {
      var item = items[i];
      var startTime = item.getStartTime();
      var loc = ojtimeaxisToolkit.TimeAxisUtils.getDatePosition(series._start, series._end, startTime, series._length); // offset position if a duration bar is rendered as well
  ...
  DvtTimelineSeriesRenderer._updateSeriesTicks = function (series) {
    ...
    for (var i = 0; i < series._seriesTicksArray.length; i++) {
      ...
      if (!series.isVertical() && isRTL) var pos = series._length - ojtimeaxisToolkit.TimeAxisUtils.getDatePosition(series._start, series._end, tick.time, series._length);else pos = ojtimeaxisToolkit.TimeAxisUtils.getDatePosition(series._start, series._end, tick.time, series._length);
      ...
      for (var i = 0; i < maxRefObjects; i++) {
        ...
        if (refObject) {
          ...
          if (series._renderedReferenceObjects.length == 0) {
            ...
            ref.date = refObject;
            pos = ojtimeaxisToolkit.TimeAxisUtils.getDatePosition(series._start, series._end, ref.date, series._length);
  ...
  DvtTimelineSeriesRenderer._updateReferenceObjects = function (series) {
    ...
    for (var i = 0; i < series._renderedReferenceObjects.length; i++) {
      ...
      var pos = ojtimeaxisToolkit.TimeAxisUtils.getDatePosition(series._start, series._end, ref.date, series._length);
  ...
  DvtTimelineSeries.prototype.render = function (options, width, height) {
    ...
    if (desc) {
      ...
      dvt.ToolkitUtils.setAttrNullNS(this.getElem(), 'aria-label', dvt.AriaUtils.processAriaLabel(desc));
    }
  };
  /**
   * @override
  ...
  DvtTimelineSeries.prototype.prepareItems = function (items) {
    ...
    for (var i = 0; i < items.length; i++) {
      var item = items[i];
      var loc = ojtimeaxisToolkit.TimeAxisUtils.getDatePosition(this._start, this._end, item.getStartTime(), this._length); // offset position if a duration bar is rendered as well
  ...
  var Timeline = function Timeline(context, callback, callbackObj) {
    this.Init(context, callback, callbackObj);
  };

  dvt.Obj.createSubclass(Timeline, ojdvtTimecomponent.TimeComponent);
  /**
  ...
  Timeline.prototype.Init = function (context, callback, callbackObj) {
    Timeline.superclass.Init.call(this, context, callback, callbackObj); // Create the defaults object
  ...
  Timeline.prototype._applyParsedProperties = function (props) {
    var orientation = this.Options['orientation'];
    if (orientation && orientation == ojtimeaxisToolkit.TimeAxis.ORIENTATION_VERTICAL) this._isVertical = true;else this._isVertical = false;
  ...
  Timeline.prototype.render = function (options, width, height) {
    ...
    Timeline.superclass.render.call(this, options, width, height); // Animation Support
    ...
    if (this._scale) {
      ...
      if (!this._timeAxis) this._timeAxis = new ojtimeaxisToolkit.TimeAxis(this.getCtx(), null, null); // Axis border visibility needs to be reset in case of orientation changes.
  ...
  Timeline.prototype.applyStyleValues = function () {
    ...
    Timeline.superclass.applyStyleValues.call(this); // double border width to account for stroke width rendering
  ...
  Timeline.prototype._getOverviewAxisOptions = function () {
    ...
    if (this._seriesTimeAxis) {
      ...
      if (this._customTimeScales && this._customTimeScales[this._seriesScale]) {
        ...
        var startDate = ojtimeaxisToolkit.TimeAxisUtils.getPositionDate(start, end, this._fetchStartPos, length);
        ...
        while (currentPos < this._fetchEndPos) {
          ...
          currentPos = ojtimeaxisToolkit.TimeAxisUtils.getDatePosition(start, end, currentDate, length);
        }
      }
