 ...
 * @ignore
 */
define(['ojs/ojcore-base', 'ojs/ojcustomelement-utils', 'ojs/ojcontext', 'ojs/ojlogger', 'ojs/ojcomponentcore'], function (oj, ojcustomelementUtils, Context, Logger, Components) {
  'use strict';
  ...
  function ojValidationGroup(context) {
    ...
    self.updateDOM = function () {
      ...
      // property.
      var busyContext = Context.getContext(_ojDiv).getBusyContext(); // Create a busy state on the element which is resolved after the valid state
      // has been set. This allows anything waiting on the whenReady Promise to run
      // after the state is set.

      var validStateResolve = Context.getContext(element).getBusyContext().addBusyState({
        description: "oj-validation-group#".concat(element.id, " initial valid state")
      });
