...
define(['exports', 'ojs/ojcore-base', 'ojs/ojcomponentcore', 'jquery', 'ojs/ojlabel', 'ojs/ojthemeutils', 'ojs/ojcontext', 'ojs/ojlogger', 'ojs/ojvalidator-required', 'ojs/ojlabelledbyutils', 'ojs/ojtranslation', 'ojs/ojmessaging', 'ojs/ojconverterutils', 'ojs/ojvalidation-error', 'ojs/ojpopup', 'hammerjs', 'ojs/ojjquery-hammer', 'ojs/ojdomutils', 'ojs/ojanimation', 'ojs/ojfocusutils'], function (exports, oj, Components, $, ojlabel, ojthemeutils, Context, Logger, RequiredValidator, LabelledByUtils, Translations, Message, ConverterUtils, ojvalidationError, ojpopup, Hammer, ojjqueryHammer, DomUtils, AnimationUtils, FocusUtils) {
  ...
  var BaseInsideLabelStrategy = function BaseInsideLabelStrategy(options) {
    this.Init(options);
  ...
  BaseInsideLabelStrategy.prototype._createOjLabelElement = function (element, component, container, labelHint, showRequired, helpHintsAttrValue, showUserAssistanceOnLabel) {
    ...
    span.textContent = labelHint; // add to dom and create event handlers
  ...
  BaseInsideLabelStrategy.prototype._CreateEventHandlers = function (span, element, ojlabel, component) {
    this._labelHintChangedCallback = BaseInsideLabelStrategy._labelHintChangedHandler.bind(this, span);
    element.addEventListener('labelHintChanged', this._labelHintChangedCallback);
    this._requiredChangedCallback = BaseInsideLabelStrategy._requiredChangedHandler.bind(this, ojlabel, component);
    element.addEventListener('requiredChanged', this._requiredChangedCallback);
    this._helpHintsChangedCallback = BaseInsideLabelStrategy._helpHintsChangedHandler.bind(this, ojlabel, component);
  ...
  BaseInsideLabelStrategy.prototype._DestroyLabel = function () {
    ...
    var ojlabel = element.querySelector("[id='".concat(labelId, "']"));
  ...
  var EditableValueUtils = {}; // S T A T I C    V A R S
  ...
  */
  ...
  EditableValueUtils.initializeOptionsFromDom = function (props, constructorOptions, comp, postprocessCallback) {
    ...
    for (var i = 0; i < props.length; i++) {
      ...
      if (constructorOptions[option] === undefined) {
        ...
        result = EditableValueUtils.getAttributeValue(element, attribute); // if we are using domValue then coerce the dom value before writing to options and trigegr
  ...
  EditableValueUtils._initInputIdLabelForConnection = function (contentElement, componentId, labelledBy) {
    if (componentId) {
  ...
  EditableValueUtils._createOrUpdateReadonlyDiv = function (input) {
  ...
  */
  ...
  oj.__registerWidget('oj.editableValue', $.oj.baseComponent, {
    ...
    showMessages: function showMessages() {
      ...
      for (i = 0; i < msgsHidden.length; i++) {
        msgHidden = msgsHidden[i]; // The Message and ComponentMessage distinction is important in a few places in
    ...
    _setOption: function _setOption(name, value, flags) {
      ...
      switch (name) {
        ...
        case 'displayOptions':
          ...
          var resetNeeded = value !== null && value.validatorHint === undefined && value.converterHint === undefined && value.messages === undefined;
          ...
          if (resetNeeded) {
            ...
            var useUserAssistanceDensity = (ojthemeutils.parseJSONFromFontFamily('oj-form-control-option-defaults') || {}).useUserAssistanceOptionDefault === 'use';
    ...
    */
    ...
    _cloneMessagesBeforeSet: function _cloneMessagesBeforeSet(value) {
      ...
      var msg;
      var msgsClone = [];
      var val; // we want all messages to be an instance of Message. So clone array
    ...
    _getResolvedUserAssistance: function _getResolvedUserAssistance() {
      ...
      var defaultOptions = ojthemeutils.parseJSONFromFontFamily('oj-form-control-option-defaults'); // this will return 'use' or 'ignore'. This tells us whether we should use the
    ...
    _UseReadonlyDiv: function _UseReadonlyDiv() {
      // ThemeUtils caches the result so no need to do it here.
      var defaultOptions = ojthemeutils.parseJSONFromFontFamily('oj-form-control-option-defaults');
    ...
    _afterConverterFormatFailure: function _afterConverterFormatFailure(e) {
      var newMsgs; // Turn this into Array of ComponentMessage instances.
      // This is what we set on 'messagesShown'
        ...
        try {
          ...
          if (validateReturned instanceof Promise) {
            return validateReturned.then(function () {}, function (e) {
              newMsgs = self._processValidationErrors(e, context, Message.ComponentMessage.DISPLAY.HIDDEN);
          ...
          newMsgs = this._processValidationErrors(e, context, Message.ComponentMessage.DISPLAY.HIDDEN);
    ...
    _asyncValidateValue: function _asyncValidateValue(value, event, context) {
      ...
      if (valMsgs.length > 0) {
        ...
        newMsgs = this._processValidationErrors(ve, context); // turn this into Array of ComponentMessage instances.
        // This is what we set on 'messagesShown'
    ...
    editableValue: // properties for all editableValue components
    {
      displayOptions: Components.createDynamicPropertyGetter(function (context) {
        ...
        if (useUserAssistanceDensity) {
          ...
          displayOptions = {
            messages: context.containers.indexOf('ojDataGrid') >= 0 || context.containers.indexOf('ojTable') >= 0 ? ['notewindow'] : _sDisplayOptionsAltaDefaults.MESSAGES,
  ...
  var PopupMessagingStrategy = function PopupMessagingStrategy(displayOptions) {
  ...
  var PopupMessagingStrategyPoolUtils = {};
  /**
   * @ignore
   */

  var PopupMessagingStrategyUtils = {};
  /**
  ...
  oj.ComponentMessaging.registerMessagingStrategy(oj.ComponentMessaging._STRATEGY_TYPE.NOTEWINDOW, PopupMessagingStrategy); // Subclass from MessagingStrategy
  ...
  PopupMessagingStrategy.prototype._setActionResolver = function (messagingContentRoot, eventType, resolvePromise) {
    ...
    function doOpen(resolve) {
      ...
      if (this._canOpenPopup()) {
        ...
        if (!oj.StringUtils.isEmptyOrUndefined(latestContent)) {
          ...
          domNode = PopupMessagingStrategyPoolUtils.getPopupContentNode(messagingContentRoot); // latestContent is includes content that may come from app. It is scrubbed for illegal tags
    ...
    if (this._isPopupInitialized()) {
      ...
      if (isPopupOpen) {
        ...
        if (contentToShow) {
          ...
          domNode = PopupMessagingStrategyPoolUtils.getPopupContentNode(messagingContentRoot); // contentToShow is includes content that may come from app. It is scrubbed for illegal tags
          // before setting to innerHTML
  ...
  PopupMessagingStrategy.prototype._registerLauncherEvents = function () {
    ...
    events = compDefaults ? compDefaults.events : PopupMessagingStrategy._DEFAULTS_BY_COMPONENT.default.events; // 1. associate the ojPopup component to wrapper <div> for popup content
    ...
    if (events.open) {
      ...
      nonPressOpenEvents = this._getNamespacedEvents(events.open.replace('press', ''), PopupMessagingStrategy._OPEN_NAMESPACE);
  ...
  PopupMessagingStrategy.prototype._getPopupPosition = function () {
    ...
    compDefaults = PopupMessagingStrategy._DEFAULTS_BY_COMPONENT[this.GetComponent().widgetName];
    compDefaultPosition = compDefaults ? compDefaults.position : PopupMessagingStrategy._DEFAULTS_BY_COMPONENT.default.position;
  ...
  PopupMessagingStrategy.prototype._buildMessagesHtml = function (document) {
    ...
    if (this.HasMessages()) {
      ...
      content = PopupMessagingStrategyUtils.buildMessagesHtml(document, messages, maxSeverity, renderSeveritySelectors);
    }
  ...
  PopupMessagingStrategy.prototype._buildHintsHtml = function (document) {
    ...
    if (this.ShowConverterHint()) {
      ...
      hintsHtml += PopupMessagingStrategyUtils.buildHintHtml(document, PopupMessagingStrategy._SELECTOR_FORMCONTROL_HINT_CONVERTER, hint, false, PopupMessagingStrategy._SELECTOR_FORMCONTROL_HINT);
    }
    ...
    if (this.ShowValidatorHint()) {
      ...
      for (i = 0; i < hints.length; i++) {
        hintsHtml += PopupMessagingStrategyUtils.buildHintHtml(document, PopupMessagingStrategy._SELECTOR_FORMCONTROL_HINT_VALIDATOR, hints[i], false, PopupMessagingStrategy._SELECTOR_FORMCONTROL_HINT);
      }
    }

    if (this.ShowTitle()) {
      hintsHtml += PopupMessagingStrategyUtils.buildHintHtml(document, PopupMessagingStrategy._SELECTOR_FORMCONTROL_HINT_TITLE, this.GetTitle(), true, PopupMessagingStrategy._SELECTOR_FORMCONTROL_HINT);
    }
  ...
  PopupMessagingStrategyUtils.buildHintHtml = function (document, selector, hintText, htmlAllowed, formControlSelectors) {
    var titleDom;
    ...
    if (hintText) {
      ...
      PopupMessagingStrategyUtils._appendTextDom(titleDom, PopupMessagingStrategyUtils.GetTextDom(document, hintText, htmlAllowed));
    }
  ...
  PopupMessagingStrategyUtils.buildMessagesHtml = function (document, messages, maxSeverity, renderSeveritySelectors) {
    var content = '';
    var detail;
    var i;
    var j;
    ...
    for (i = maxSeverity; i >= Message.SEVERITY_LEVEL.CONFIRMATION; i--) {
      ...
      for (j = 0; j < messagesByType.length; j++) {
        ...
        content = content.concat(PopupMessagingStrategyUtils.buildMessageHtml(document, summary, detail, severityLevel, renderSeveritySelectors));
      }
    }
  ...
  PopupMessagingStrategyUtils.buildMessageHtml = function (document, summary, detail, severityLevel, addSeverityClass) {
    var msgContent;
    var msgDetail;
    var msgDom;
    var msgIcon;
    var msgSummary;
    var severityStr = PopupMessagingStrategyUtils.getSeverityTranslatedString(severityLevel); // build message
    // (x) <Summary Text>
    // <Detail Text>
  ...
  PopupMessagingStrategyUtils._SELECTOR_MESSAGE_CONFIRMATION_ICON = 'oj-message-confirmation-icon'; // new theming keys so that we can style the different types of messages differently. Like,
  ...
  PopupMessagingStrategyPoolUtils._getPopupContentHtml = function () {
    return '<div class="' + PopupMessagingStrategyPoolUtils._SELECTOR_MESSAGING + '">' + '<div class="' + PopupMessagingStrategyPoolUtils._SELECTOR_MESSAGING_CONTAINER + '"></div>' + '</div>';
  };
  /**
   * @const
   * @private
  ...
  InlineHelpHintsStrategy.prototype.reactivate = function (newOptions, parentElement) {
    ...
    this._userAssistanceDivElement = parentElement; // sets up dom and focusin handlers
  ...
  InlineHelpHintsStrategy.prototype.shouldUpdate = function (content) {
    ...
    var updateConverterHint = !!(content && content.converterHint !== undefined);
  ...
  InlineHelpHintsStrategy.prototype._addHelpHintsContent = function (component) {
    ...
    if (helpInstruction) {
      var helpInstructionDom = PopupMessagingStrategyUtils.GetTextDom(document, helpInstruction, true); // turn it into a string
    ...
    var source = helpHints ? helpHints.source : null;
    var helpSourceDom;
  ...
  InlineHelpHintsStrategy.prototype._getHelpSourceDom = function (source) {
    ...
    var helpSourceText = Translations.getTranslatedString('oj-ojEditableValue.helpSourceText');
  ...
  InlineHelpHintsStrategy.prototype._updateInlineHelpHints = function () {
    var showRightAway = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;
    var component = this.GetComponent();
  ...
  InlineHelpHintsStrategy._focusinHandler = function (component, event) {
    var rootElem = event.currentTarget; // if we are focusing in from already within the component, return
    ...
    this._focusIn = true; // add 'oj-focus' on parent

    this._userAssistanceDivElement.classList.add('oj-focus'); // if the content we fetched earlier is still up-to-date, like no
    ...
    if (this.containerRoot && this._isFocusInContentCurrent()) {
      ...
      var hasContent = this._addHelpHintsContent(component);
  ...
  InlineHelpHintsStrategy._focusoutHandler = function (event) {
    ...
    var rootElem = event.currentTarget;
    var relatedTarget = event.relatedTarget;
    ...
    }.bind(this); // if we have helphint content, then animate it closed on focusout.
    ...
    if (this.containerRoot && this.containerRoot.hasChildNodes()) {
      ...
      var effect = defaults.close; // e.g., {effect: 'fadeOut', duration: '200ms'}

      this._setBusyState();

      var self = this;
      AnimationUtils.startAnimation(this.containerRoot, 'inline-hints-' + action, effect, this.GetComponent()).then(function () {
  ...
  InlineHelpHintsStrategy.prototype._animateOpen = function () {
    if (this.containerRoot && this.containerRoot.hasChildNodes()) {
      ...
      var effect = defaults.open; // e.g., {effect: 'fadeIn', duration: '200ms'}

      var action = 'open';
      AnimationUtils.startAnimation(this.containerRoot, 'inline-hints-' + action, effect, this.GetComponent());
    }
  };
  /**
  ...
  InlineHelpHintsStrategy.prototype._setBusyState = function () {
    ...
    if (!this._resolveBusyState) {
      ...
      var domElem = component._getRootElement();

      var busyContext = Context.getContext(domElem).getBusyContext();
      var description = 'The page is waiting for inline help hints ';
  ...
  var InlineMessagingStrategy = function InlineMessagingStrategy(displayOptions, parentElement) {
    this.Init(displayOptions);
  ...
  oj.ComponentMessaging.registerMessagingStrategy(oj.ComponentMessaging._STRATEGY_TYPE.INLINE, InlineMessagingStrategy); // Subclass from MessagingStrategy

  oj.Object.createSubclass(InlineMessagingStrategy, oj.MessagingStrategy, 'oj.InlineMessagingStrategy');
  /**
  ...
  InlineMessagingStrategy.prototype.activate = function (cm) {
    InlineMessagingStrategy.superclass.activate.call(this, cm); // We do not create the inline messaging dom until we get messages to show.
  };
  /**
   * Reinitializes with the new display options and updates component messaging using the new content.
   *
  ...
  InlineMessagingStrategy.prototype._determineAnimation = function (rootElem, newContent) {
    ...
    if (defaults) {
      ...
      if (action !== 'noanimation') {
        effect = defaults[action]; // An example of effect is 'fadeIn' or 'expand',
  ...
  InlineMessagingStrategy.prototype._queueAction = function (contentToShow) {
    ...
    this._setBusyState(); // add styleclass as soon as we start animating open so that
  ...
  oj.ComponentMessaging.registerMessagingStrategy(oj.ComponentMessaging._STRATEGY_TYPE.USER_ASSISTANCE_INLINE, InlineUserAssistanceStrategy); // Subclass from MessagingStrategy
  ...
  InlineUserAssistanceStrategy.prototype._activateContainerStrategies = function (cm, options) {
    ...
    if (this._inlineMessagingStrategy === undefined) {
      this._inlineMessagingStrategy = new InlineMessagingStrategy(this._displayOptions, this.containerRoot);
    ...
    if (options.required !== undefined && this._inlineRequiredStrategy === undefined) {
      this._inlineRequiredStrategy = new InlineRequiredStrategy(this._displayOptions, this.containerRoot);
    ...
    if (this._inlineHelpHintsStrategy === undefined) {
      ...
      this._inlineHelpHintsStrategy = new InlineHelpHintsStrategy(this._displayOptions, this.containerRoot);
  ...
  oj.ComponentMessaging.registerMessagingStrategy(oj.ComponentMessaging._STRATEGY_TYPE.LABEL_EDGE_INSIDE_FORM_CNTRL, InsideFormControlLabelStrategy); // Subclass from BaseInsideLabelStrategy

  oj.Object.createSubclass(InsideFormControlLabelStrategy, BaseInsideLabelStrategy, 'InsideFormControlLabelStrategy');
  /**
   * @param {Object} cm a reference to an instance of ComponentMessaging that provides access to
   * the latest messaging content.
   * @public
  ...
  var InsideLabelPlaceholderStrategy = function InsideLabelPlaceholderStrategy(options) {
    this.Init(options);
  }; // Subclass from MessagingStrategy


  oj.Object.createSubclass(InsideLabelPlaceholderStrategy, oj.MessagingStrategy, 'oj.InsideLabelPlaceholderStrategy');
  /**
  ...
  var InsideLabelStrategy = function InsideLabelStrategy(displayOptions) {
    this.Init(displayOptions);
    this._displayOptions = displayOptions;
  };
  /**
  ...
  oj.ComponentMessaging.registerMessagingStrategy(oj.ComponentMessaging._STRATEGY_TYPE.LABEL_EDGE_INSIDE, InsideLabelStrategy); // Subclass from BaseInsideLabelStrategy
  ...
  InsideLabelStrategy.prototype._activatePlaceholderStrategyIfNeeded = function (cm) {
    ...
    if (this._placeholderStrategy === undefined) {
      var createInsideLabelPlaceholderStrategy = function createInsideLabelPlaceholderStrategy() {
        var Callback = InsideLabelPlaceholderStrategy;
      ...
      var needsInsidePlaceholder = compOptions.placeholder || component._getResolvedUserAssistance() === 'displayOptions' && InsideLabelPlaceholderStrategy.ShowConverterHintAsPlaceholder(component);
