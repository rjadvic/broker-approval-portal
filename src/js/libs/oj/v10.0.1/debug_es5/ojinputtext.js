 ...
 * @ignore
 */
define(['ojs/ojcore', 'jquery', 'ojs/ojthemeutils', 'ojs/ojeditablevalue', 'ojs/ojcore-base', 'ojs/ojdomutils', 'ojs/ojfilter-length', 'ojs/ojvalidator-regexp', 'ojs/ojcustomelement-utils'], function (ojcore, $, ojthemeutils, ojeditablevalue, oj, DomUtils, LengthFilter, RegExpValidator, ojcustomelementUtils) {
  'use strict';
  ...
  (function () {
    ...
    var __oj_text_area_metadata = {
      "properties": {
        ...
        "resizeBehavior": {
          "type": "string",
          "enumValues": ["both", "horizontal", "none", "vertical"],
          "value": "none"
        },
        "rows": {
          "type": "number",
  ...
  */
  ...
  oj.__registerWidget('oj.ojInputPassword', $.oj.inputBase, {
     ...
     */
    _ComponentCreate: function _ComponentCreate() {
      var retVal = this._super(); // add in the mask visibility icon if needed
     ...
     * @instance
     */
    _processMaskVisibilityIcon: function _processMaskVisibilityIcon() {
      var wrapperElem = this._GetContentWrapper().parentElement;
      ...
      if (this.options.disabled || this.options.readOnly) {
        ...
        if (this.options.maskIcon === 'visible') {
          ...
          maskVisibilityIconBtn.setAttribute('aria-label', this.getTranslatedString(this._MASK_ICON_SHOW_PASSWORD_LABEL_KEY));
          ...
          maskVisibilityIconBtn.addEventListener(this._CLICK_HANDLER_KEY, this._onMaskVisibilityIconClickHandler.bind(this));
          maskVisibilityIconBtn.addEventListener(this._KEYDOWN_HANDLER_KEY, this._onMaskVisibilityIconKeyDownHandler.bind(this));
        }
     ...
     * @memberof! oj.ojInputPassword
     */
    _onMaskVisibilityIconClickHandler: function _onMaskVisibilityIconClickHandler() {
      var elem = this.element[0]; // Get the input element

      var wrapperElem = this._GetContentWrapper().parentElement;

      var maskVisibilityIconBtn = wrapperElem.querySelector('a.oj-component-icon');
      var maskVisibilityIcon = maskVisibilityIconBtn.children[0]; // toggle the input type and the icon class
      ...
      if (elem.type === 'password') {
        ...
        maskVisibilityIconBtn.setAttribute('aria-label', this.getTranslatedString(this._MASK_ICON_HIDE_PASSWORD_LABEL_KEY));
      } else {
     ...
     * @memberof! oj.ojInputPassword
     */
    _onMaskVisibilityIconKeyDownHandler: function _onMaskVisibilityIconKeyDownHandler(event) {
  ...
  * @memberof! oj.ojInputText
  ...
  oj.__registerWidget('oj.ojInputText', $.oj.inputBase, {
    ...
    _onBlurHandler: function _onBlurHandler(event) {
      ...
      if (!(target && target.classList.contains('oj-inputtext-clear-icon-btn') && target.parentElement && DomUtils.isAncestorOrSelf(wrapperNode, target.parentElement))) {
        this._super(event);
      } else {
  ...
  * @memberof! oj.ojTextArea
  ...
  oj.__registerWidget('oj.ojTextArea', $.oj.inputBase, {
    ...
    _AfterCreate: function _AfterCreate() {
      ...
      this.widget()[0].querySelector('.' + this._INPUT_CONTAINER_CLASS).style.resize = this.options.resizeBehavior;
    ...
    _onKeyDownHandler: function _onKeyDownHandler(event) {
      if (event.keyCode === 13 || event.key === 'Enter') {
        if (event.altKey) {
          ...
          textarea.setSelectionRange(cursorPosition, cursorPosition); // Using blur()/focus() to bring the cursor position so that it's visible in the textarea
