 ...
 * @ignore
 */
define(['ojs/ojcore', 'ojs/ojeditablevalue', 'ojs/ojcore-base', 'jquery', 'ojs/ojdomutils', 'ojs/ojfocusutils'], function (ojcore, ojeditablevalue, oj, $, DomUtils, FocusUtils) {
  'use strict';
    ...
    oj.__registerWidget('oj.ojSwitch', $.oj.editableValue, {
      ...
      _SetAriaInfo: function _SetAriaInfo(target) {
        ...
        if (this._IsCustomElement()) {
          ...
          if (this.options.labelledBy) {
            ...
            labelElementId = ojeditablevalue.EditableValueUtils._getOjLabelAriaLabelledBy(this.options.labelledBy, defaultLabelId);
          }
        } else {
      ...
      _switchEvents: {
        ...
        touchend: function touchend(event) {
          ...
          this._SetValue(!this.option('value'), event); // Click JET-37778 - Clicking switch track should move focus to thumb
          // This is to make sure that the component retains the focus after one toggles the
          // value by clicking on the track. Otherwise the Dynamic form may fail to update the
          // model, since the focus would not be inside the form
