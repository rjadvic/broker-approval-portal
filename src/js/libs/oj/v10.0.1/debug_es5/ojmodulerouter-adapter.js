 ...
 * @ignore
 */
define(['ojs/ojlogger', 'ojs/ojmodule-element-utils', 'ojs/ojmoduleanimations', 'knockout'], function (Logger, ModuleUtils, ModuleAnimations, ko) {
  'use strict';
  /**
   * @license
   * Copyright (c) 2014, 2021, Oracle and/or its affiliates.
    ...
    function onStateChange(args) {
      ...
      configPromise.then(function (config) {
        if (_configPromise === configPromise) {
          ...
          _currentState = args.state; // JET-38828 - ensure that a new view is being returned every time the oj-module is re-rendered.

          var clonedViewConfig = {
            viewModel: config.viewModel
          };
          Object.defineProperty(clonedViewConfig, 'view', {
            enumerable: true,
            get: function get() {
              return config.view.map(function (node) {
