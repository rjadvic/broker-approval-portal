    ...
    oj.__registerWidget('oj.ojMenu', $.oj.baseComponent, {
      ...
      _customOptionRenderer: function _customOptionRenderer(optionDom) {
        ...
        if (!/[^\-\u2014\u2013\s]/.test(ojOption.text())) {
          ...
          var slots = ojcustomelementUtils.CustomElementUtils.getSlotMap(optionDom); // handle startIcon slot
      ...
      _beforeCloseHandler: function _beforeCloseHandler(psOptions) {
        ...
        if (this._IsCustomElement()) {
          ...
          if (selectUi && selectUi.item.length) {
            ...
            params.bubbles = true; // dispatch old ojAction(deprecated)

            var ojActionEvent = new CustomEvent('ojAction', params);
            itemElement.dispatchEvent(ojActionEvent); // dispatch new ojMenuAction with selecteValue in the payload
      ...
      _beforeOpenHandler: function _beforeOpenHandler(psOptions) {
        ...
        rootElement.show(); // If the menu is dropdown, check if we need to re-calculate the position.
      ...
      _initVoiceOverAssist: function _initVoiceOverAssist(initialFocus) {
        ...
        this._focusSkipLink = new ojpopupcore.PopupSkipLink(firstItem, message, callback, focusSkipLinkId, options);
      },
      ...
      _setWhenReady: function _setWhenReady(operation) {
        ...
        this._whenReadyMediator = new ojpopupcore.PopupWhenReadyMediator(this.element, operation, 'ojMenu', this._IsCustomElement());
      },
