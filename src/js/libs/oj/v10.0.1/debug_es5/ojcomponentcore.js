...
define(['exports', 'jqueryui-amd/widget', 'jqueryui-amd/unique-id', 'jqueryui-amd/keycode', 'jqueryui-amd/focusable', 'jqueryui-amd/tabbable', 'ojs/ojcore', 'jquery', 'ojs/ojmessaging', 'ojs/ojmetadatautils', 'ojs/ojcore-base', 'ojs/ojdomutils', 'ojs/ojcustomelement', 'ojs/ojcustomelement-utils', 'ojs/ojlogger', 'ojs/ojdefaultsutils', 'ojs/ojtranslation', 'ojs/ojfocusutils', 'ojs/ojgestureutils'], function (exports, widget, uniqueId, keycode, focusable, tabbable, oj, $, Message, MetadataUtils, oj$1, DomUtils, ojcustomelement, ojcustomelementUtils, Logger, ojdefaultsutils, Translations, ojfocusutils, GestureUtils) {
  ...
  var ComponentMessaging = function ComponentMessaging(component) {
    this.Init(component);
  ...
  ComponentMessaging.prototype._createMessagingStrategy = function (type, artifactsForType) {
    var Callback = ComponentMessaging._STRATEGY_TYPE_TO_CALLBACK[type] || ComponentMessaging._STRATEGY_TYPE_TO_CALLBACK[ComponentMessaging._STRATEGY_TYPE.NONE]; // dynamically instantiate the strategy objects.
  ...
  ComponentMessaging.prototype._getUserAssistanceStrategyToArtifactsObj = function () {
    ...
    for (var i = 0; i < keys.length; i++) {
      ...
      if (typeof strategyTypes === 'string' && strategyTypes === 'none') {
        ...
        artifactPiecesToStrategyObj[artifact] = ComponentMessaging._STRATEGY_TYPE.USER_ASSISTANCE_INLINE;
      }
    } // update the label creation strategy
    ...
    Object.keys(ComponentMessaging._STRATEGY_TYPE).forEach(function (key) {
      var value = ComponentMessaging._STRATEGY_TYPE[key];
  ...
  ComponentMessaging.prototype._resolveStrategyTypeForArtifact = function (artifact, strategyType, options, $messagingPreferences) {
    var artifactStrategyTypeResolved = false;
  ...
  ComponentMessaging.prototype._initializeMessagingStrategies = function () {
    ...
    var displayInline = strategyToArtifacts[ComponentMessaging._STRATEGY_TYPE.INLINE]; // these are input components

    var displayLabelEdgeInsideTextField = strategyToArtifacts[ComponentMessaging._STRATEGY_TYPE.LABEL_EDGE_INSIDE]; // these are form controls that aren't inputs, like radioset

    var displayLabelEdgeInsideFormControl = strategyToArtifacts[ComponentMessaging._STRATEGY_TYPE.LABEL_EDGE_INSIDE_FORM_CNTRL]; // these are for user assistance inline that is used in the Redwood theme

    var displayUserAssistanceInline = strategyToArtifacts[ComponentMessaging._STRATEGY_TYPE.USER_ASSISTANCE_INLINE];
    ...
    if (displayInNoteWindow.length > 0) {
      ...
      messagingStrategies[ComponentMessaging._STRATEGY_TYPE.NOTEWINDOW] = this._createMessagingStrategy(ComponentMessaging._STRATEGY_TYPE.NOTEWINDOW, displayInNoteWindow);
    }
    ...
    if (displayInPlaceholder.length > 0 && displayLabelEdgeInsideTextField.length === 0) {
      ...
      // e.g., 'converterHints'
      messagingStrategies[ComponentMessaging._STRATEGY_TYPE.PLACEHOLDER] = this._createMessagingStrategy(ComponentMessaging._STRATEGY_TYPE.PLACEHOLDER, displayInPlaceholder);
    }

    if (displayInline.length > 0) {
      // displayInline is an array of the artifacts that want to be displayed in inline
      // e.g., 'messages'
      messagingStrategies[ComponentMessaging._STRATEGY_TYPE.INLINE] = this._createMessagingStrategy(ComponentMessaging._STRATEGY_TYPE.INLINE, displayInline);
    }

    if (displayUserAssistanceInline.length > 0) {
      messagingStrategies[ComponentMessaging._STRATEGY_TYPE.USER_ASSISTANCE_INLINE] = this._createMessagingStrategy(ComponentMessaging._STRATEGY_TYPE.USER_ASSISTANCE_INLINE, displayUserAssistanceInline);
    } // Create one of these strategies for whatever labelEdge we are using currently


    if (displayLabelEdgeInsideTextField.length > 0) {
      messagingStrategies[ComponentMessaging._STRATEGY_TYPE.LABEL_EDGE_INSIDE] = this._createMessagingStrategy(ComponentMessaging._STRATEGY_TYPE.LABEL_EDGE_INSIDE, displayLabelEdgeInsideTextField);
    } else if (displayLabelEdgeInsideFormControl.length > 0) {
      messagingStrategies[ComponentMessaging._STRATEGY_TYPE.LABEL_EDGE_INSIDE_FORM_CNTRL] = this._createMessagingStrategy(ComponentMessaging._STRATEGY_TYPE.LABEL_EDGE_INSIDE_FORM_CNTRL, displayLabelEdgeInsideFormControl);
    }

    messagingStrategies[ComponentMessaging._STRATEGY_TYPE.NONE] = this._createMessagingStrategy(ComponentMessaging._STRATEGY_TYPE.NONE, displayNone);
  ...
  var MessagingStrategy = function MessagingStrategy(displayOptions) {
  ...
  MessagingStrategy.prototype.update = function () {}; // P R O T E C T E D  M E T H O D S
  ...
  var DefaultMessagingStrategy = function DefaultMessagingStrategy(displayOptions) {
    this.Init(displayOptions);
  };

  ComponentMessaging.registerMessagingStrategy(ComponentMessaging._STRATEGY_TYPE.NONE, DefaultMessagingStrategy); // TODO: Need to retrieve style selectors from a Style Manager

  DefaultMessagingStrategy._SELECTOR_STATE_INVALID = 'oj-invalid';
  DefaultMessagingStrategy._SELECTOR_STATE_WARNING = 'oj-warning';
  oj.Object.createSubclass(DefaultMessagingStrategy, MessagingStrategy, 'oj.DefaultMessagingStrategy');
  ...
  DefaultMessagingStrategy.prototype.deactivate = function () {
    ...
    jqRoot.removeClass(DefaultMessagingStrategy._SELECTOR_STATE_INVALID).removeClass(DefaultMessagingStrategy._SELECTOR_STATE_WARNING);
  ...
  var PlaceholderMessagingStrategy = function PlaceholderMessagingStrategy(displayOptions) {
    this.Init(displayOptions);
  };

  ComponentMessaging.registerMessagingStrategy(ComponentMessaging._STRATEGY_TYPE.PLACEHOLDER, PlaceholderMessagingStrategy); // Subclass from oj.MessagingStrategy

  oj.Object.createSubclass(PlaceholderMessagingStrategy, MessagingStrategy, 'oj.PlaceholderMessagingStrategy');
  ...
  var ComponentValidity = function ComponentValidity(valid, messages) {
  ...
  */
  ...
  function _getComponentElementByNode(node, mtAccessCompositeInternals) {
    ...
    var containingComposite = oj$1.Composite && !mtAccessCompositeInternals ? oj$1.Composite.getContainingComposite(node) : null;
  ...
  */
  ...
  Components.callComponentMethod = function (componentElement, method, methodArguments) {
    if (!_isComponentElement(componentElement)) {
      ...
      return Components.__GetWidgetConstructor(componentElement).apply($(componentElement), [].slice.call(arguments, 1));
    }
      ...
      function set(value, bOuterSet) {
        ...
        if (!this._BRIDGE.SaveEarlyPropertySet(this._ELEMENT, property, value)) {
          ...
          if (!oj.BaseCustomElementBridge.__CompareOptionValues(property, propertyMeta, value, previousValue)) {
            ...
            Logger.info(ojcustomelementUtils.CustomElementUtils.getElementInfo(this._ELEMENT) + ": Ignoring property set for property '" + property + "' with same value.");
          }
        }
      }
  ...
  var WidgetState = /*#__PURE__*/function (_ojcustomelementUtils) {
    _inherits(WidgetState, _ojcustomelementUtils);

    var _super = _createSuper(WidgetState);

    function WidgetState() {
      _classCallCheck(this, WidgetState);

      return _super.apply(this, arguments);
    }

    _createClass(WidgetState, [{
      key: "IsTransferAttribute",
      value: function IsTransferAttribute(attrName) {
        var bridge = ojcustomelementUtils.CustomElementUtils.getElementBridge(this.Element);
        var transferAttrs = bridge._EXTENSION._GLOBAL_TRANSFER_ATTRS;
        return bridge._WIDGET_ELEM && transferAttrs && transferAttrs.includes(attrName);
      }
    }, {
      key: "GetDescriptiveTransferAttributeValue",
      value: function GetDescriptiveTransferAttributeValue(name) {
        var bridge = ojcustomelementUtils.CustomElementUtils.getElementBridge(this.Element);
    ...
    return WidgetState;
  }(ojcustomelementUtils.ElementState);
  /**
   * @license
   * Copyright (c) 2014, 2021, Oracle and/or its affiliates.
   * The Universal Permissive License (UPL), Version 1.0
  ...
  oj.CollectionUtils.copyInto(CustomElementBridge.proto, {
    ...
    getThrottlePromise: function getThrottlePromise() {
    ...
    AddComponentMethods: function AddComponentMethods(proto) {
      ...
      proto.setProperty = function (prop, value) {
        ...
        if (!bridge.SaveEarlyPropertySet(this, prop, value)) {
          if (!bridge._setEventProperty(this, prop, value) && !bridge._validateAndSetCopyProperty(this, prop, value, null)) {
            ...
            var meta = oj.BaseCustomElementBridge.__GetPropertyMetadata(prop, ojcustomelementUtils.CustomElementUtils.getElementProperties(this)); // For non component specific properties, just set directly on the element instead.
      ...
      proto.getProperty = function (prop) {
        ...
        var meta = oj.BaseCustomElementBridge.__GetPropertyMetadata(prop, ojcustomelementUtils.CustomElementUtils.getElementProperties(this)); // For event listeners and non component specific properties, return the property from the element.
      ...
      proto.focus = function () {
        var bridge = ojcustomelementUtils.CustomElementUtils.getElementBridge(this); // If focus is called before the component has been created there
    ...
    CreateComponent: function CreateComponent(element) {
      ...
      this._WIDGET_ELEM = CustomElementBridge._getWidgetElement(element, innerDomFun ? innerDomFun(element) : this._EXTENSION._INNER_ELEM); // Transfer global attributes and copy tagged properties to child element if one exists
      ...
      Components.unmarkPendingSubtreeHidden(element); // Initialize jQuery object with options and pass element as wrapper if needed
    ...
    DefinePropertyCallback: function DefinePropertyCallback(proto, property, propertyMeta) {
      ...
      Object.defineProperty(proto, property, {
        ...
        set: function set(value) {
          var bridge = ojcustomelementUtils.CustomElementUtils.getElementBridge(this); // Properties can be set before the component is created. These early
          // sets are actually saved until after component creation and played back.
     ...
     * @private
     */
    _setOption: function _setOption(property, value, meta, element) {
      ...
      if (value === undefined && property.indexOf('.') === -1) {
        ...
        value = MetadataUtils.getDefaultValue(meta); // Usually the widget logic fires the property changed events, but in this case
        ...
        if (this.State.isComplete) {
          var previousValue = element[property];

          oj.BaseCustomElementBridge.__FirePropertyChangeEvent(element, property, undefined, previousValue, 'external');
        }
      }
    ...
    _processProperties: function _processProperties(element) {
    ...
    _validateAndSetCopyProperty: function _validateAndSetCopyProperty(elem, prop, value, propMeta) {
      ...
      if (isCopy) {
        ...
        if (this._WIDGET_ELEM) {
          if (!propMeta) {
            // eslint-disable-next-line no-param-reassign
            propMeta = oj.BaseCustomElementBridge.__GetPropertyMetadata(prop, ojcustomelementUtils.CustomElementUtils.getElementProperties(elem));
          }
  ...
  CustomElementBridge.register = function (tagName, descriptor) {
    ...
    var bridgeProto = ext && ext._WIDGET_NAME ? CustomElementBridge.proto : DefinitionalElementBridge.proto;
    var stateClass = ext && ext._WIDGET_NAME ? WidgetState : ojcustomelementUtils.ElementState; // Create component to element property alias mapping for easy optionChange lookup and stash it in the extension object
  ...
  CustomElementBridge._getPropertyAccessor = function (element, property, flags) {
    function optionAccessor(value) {
      var bridge = ojcustomelementUtils.CustomElementUtils.getElementBridge(element); // option set case
  ...
  (function () {
    ...
    $.widget('oj.' + _BASE_COMPONENT, {
      ...
      destroy: function destroy() {
        ...
        DomUtils.dispatchEvent(this.element[0], new CustomEvent('_ojDestroy')); // allow subcomponent to release resources they hold.
      ...
      option: function option(optionName, value) {
        ...
        for (i = 0; i < optionKeys.length; i++) {
          ...
          if (changed || !this._CompareOptionValues(option, oldValue, newValue)) {
            ...
            Logger.info(ojcustomelementUtils.CustomElementUtils.getElementInfo(this.element[0]) + ": Ignoring property set for property '" + option + "' with same value.");
          }
        }
      ...
      _triggerCustomEvent: function _triggerCustomEvent(type, event, data) {
        ...
        if (type === 'optionChange') {
          ...
          eventName = ojcustomelementUtils.AttributeUtils.propertyNameToChangeEventType(property); // Copy over component specific optionChange event properties, promoting those exposed
       ...
       * @protected
       */
      _GetThrottlePromise: function _GetThrottlePromise() {
        if (this._IsCustomElement()) {
  ...
  var subtreeAttached = Components.subtreeAttached;
  var subtreeDetached = Components.subtreeDetached;
  var subtreeHidden = Components.subtreeHidden;
  var subtreeShown = Components.subtreeShown;
  var createDynamicPropertyGetter = Components.createDynamicPropertyGetter;
  var setDefaultOptions = Components.setDefaultOptions;
  var getDefaultOptions = Components.getDefaultOptions;
  var __GetWidgetConstructor = Components.__GetWidgetConstructor;
  var setComponentOption = Components.setComponentOption;
  var getComponentOption = Components.getComponentOption;
  var getWidgetConstructor = Components.getWidgetConstructor;
  var isComponentInitialized = Components.isComponentInitialized;
  var markPendingSubtreeHidden = Components.markPendingSubtreeHidden;
  var unmarkPendingSubtreeHidden = Components.unmarkPendingSubtreeHidden;
  var __getDefaultOptions = Components.__getDefaultOptions;
  var getComponentElementByNode = Components.getComponentElementByNode;
  var getSubIdByNode = Components.getSubIdByNode;
  var getNodeBySubId = Components.getNodeBySubId;
  var callComponentMethod = Components.callComponentMethod;
  var _OJ_CONTAINER_ATTR = Components._OJ_CONTAINER_ATTR;
