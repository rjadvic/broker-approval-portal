 ...
 * @ignore
 */
define(['exports', 'ojs/ojdvt-toolkit', 'ojs/ojtranslation'], function (exports, dvt, Translations) {
  'use strict';
  /**
   * @license
  ...
  var NBoxConstants = {};
  dvt.Obj.createSubclass(NBoxConstants, dvt.Obj);
  /**
   * @const
   */
  ...
  var DvtNBoxUtils = {
     ...
     */
    getKeyboardBoundingBox: function getKeyboardBoundingBox(obj, targetCoordinateSpace) {
     ...
     */
    createText: function createText(ctx, strText, style, halign, valign) {
     ...
     */
    getGlobalMatrix: function getGlobalMatrix(displayable) {
     ...
     */
    positionText: function positionText(text, x, y, angle) {
      text.setX(x);
      text.setY(y);
     ...
     */
    setFill: function setFill(displayable, fillString) {
     ...
     */
    setDisplayable: function setDisplayable(nbox, dataObject, displayable, key) {
     ...
     */
    getDisplayable: function getDisplayable(nbox, dataObject, key) {
      if (!dataObject) {
        return null;
      }
  ...
  DvtNBoxDataUtils.processDataObject = function (nbox) {
    ...
    for (var n = 0; n < DvtNBoxDataUtils.getNodeCount(nbox); n++) {
      ...
      if (!grouping && (nodeObj[NBoxConstants.GROUP_CATEGORY] || nodeObj['_groupCategories'] || nodeObj['_groupLabels'])) grouping = true;
    }

    options['__nodeMap'] = nodeMap;
    options['__grouping'] = options[NBoxConstants.GROUP_BEHAVIOR] != 'none' ? grouping : false; // Disable maximize if we're grouping across cells
    ...
    if (options[NBoxConstants.MAXIMIZED_ROW] && isNaN(rowMap[options[NBoxConstants.MAXIMIZED_ROW]]) || options[NBoxConstants.MAXIMIZED_COLUMN] && isNaN(columnMap[options[NBoxConstants.MAXIMIZED_COLUMN]])) {
      options[NBoxConstants.MAXIMIZED_ROW] = null;
      DvtNBoxDataUtils.fireOptionChangeEvent(nbox, NBoxConstants.MAXIMIZED_ROW, null);
      options[NBoxConstants.MAXIMIZED_COLUMN] = null;
      DvtNBoxDataUtils.fireOptionChangeEvent(nbox, NBoxConstants.MAXIMIZED_COLUMN, null);
  ...
  DvtNBoxDataUtils.hasValidData = function (nbox) {
    ...
    if (!options || !options[NBoxConstants.ROWS] || options[NBoxConstants.ROWS].length < 1 || !options[NBoxConstants.COLUMNS] || options[NBoxConstants.COLUMNS].length < 1) {
      return false;
    }
  ...
  DvtNBoxDataUtils.getColumnLabel = function (nbox, colValue) {
    ...
    if (col && col[NBoxConstants.LABEL]) return col[NBoxConstants.LABEL];
    return null;
  };
  /**
  ...
  DvtNBoxDataUtils.getRowLabel = function (nbox, rowValue) {
    ...
    if (row && row[NBoxConstants.LABEL]) return row[NBoxConstants.LABEL];
    return null;
  };
  /**
  ...
  DvtNBoxDataUtils.getNodeGroupId = function (node) {
    if (node[NBoxConstants.GROUP_CATEGORY]) return node[NBoxConstants.GROUP_CATEGORY];
  ...
  DvtNBoxDataUtils.getCategoryNodeLabels = function (nbox, categoryNode) {
    ...
    if (node[NBoxConstants.GROUP_CATEGORY]) return categoryNode['__labels'] = [node[NBoxConstants.GROUP_CATEGORY]]; // Set labels to group categories
  ...
  DvtNBoxDataUtils.isNodeHidden = function (nbox, node) {
    ...
    if (options[NBoxConstants.HIDDEN_CATEGORIES] && !options['__hiddenMap']) {
      options['__hiddenMap'] = dvt.ArrayUtils.createBooleanMap(options[NBoxConstants.HIDDEN_CATEGORIES]);
    } // Null checking done by util function


    return dvt.ArrayUtils.hasAnyMapItem(options['__hiddenMap'], node[NBoxConstants.CATEGORIES]);
  };
  /**
  ...
  DvtNBoxDataUtils.isNodeHighlighted = function (nbox, node) {
    ...
    if (options[NBoxConstants.HIGHLIGHTED_CATEGORIES] && !options['__highlightedMap']) {
      options['__highlightedMap'] = dvt.ArrayUtils.createBooleanMap(options[NBoxConstants.HIGHLIGHTED_CATEGORIES]);
    } // Null checking done by util functions


    if (options[NBoxConstants.HIGHLIGHT_MATCH] == 'all') return dvt.ArrayUtils.hasAllMapItems(options['__highlightedMap'], node[NBoxConstants.CATEGORIES]);else return dvt.ArrayUtils.hasAnyMapItem(options['__highlightedMap'], node[NBoxConstants.CATEGORIES]);
  };
  /**
  ...
  DvtNBoxDataUtils.buildAriaDesc = function (nbox, object, datatip, selected) {
    ...
    var baseDesc = object.nboxType === 'categoryNode' || object.nboxType === 'drawer' ? dvt.ResourceUtils.format(translations.labelAndValue, [translations.labelGroup, datatip]) : datatip;
    var states = [];
  ...
  DvtNBoxStyleUtils.getAnimationDuration = function (nbox) {
    return dvt.CSSStyle.getTimeMilliseconds(nbox.getOptions()[NBoxConstants.STYLE_DEFAULTS][NBoxConstants.ANIMATION_DURATION]) / 1000;
  };
  /**
  ...
  DvtNBoxStyleUtils.getColumnLabelStyle = function (nbox, columnIndex) {
    ...
    if (column[NBoxConstants.LABEL_STYLE]) {
      return dvt.JsonUtils.merge(new dvt.CSSStyle(column[NBoxConstants.LABEL_STYLE]), defaults);
    }
  ...
  DvtNBoxStyleUtils.getRowLabelStyle = function (nbox, rowIndex) {
    ...
    if (row[NBoxConstants.LABEL_STYLE]) {
      return dvt.JsonUtils.merge(new dvt.CSSStyle(row[NBoxConstants.LABEL_STYLE]), defaults);
    }
  ...
  DvtNBoxStyleUtils.getCellLabelStyle = function (nbox, cellIndex) {
    ...
    if (cell[NBoxConstants.LABEL_STYLE]) {
      return dvt.JsonUtils.merge(new dvt.CSSStyle(cell[NBoxConstants.LABEL_STYLE]), defaults);
    }
  ...
  DvtNBoxStyleUtils.getCellShowCount = function (nbox, cell) {
    return cell[NBoxConstants.SHOW_COUNT] ? cell[NBoxConstants.SHOW_COUNT] : nbox.getOptions()[NBoxConstants.STYLE_DEFAULTS][NBoxConstants.CELL_DEFAULTS][NBoxConstants.SHOW_COUNT];
  };
  /**
  ...
  DvtNBoxStyleUtils.getNodeLabel = function (nbox, node) {
    ...
    label = DvtNBoxUtils.createText(nbox.getCtx(), node[NBoxConstants.LABEL], DvtNBoxStyleUtils.getNodeLabelStyle(nbox, node), halign, dvt.OutputText.V_ALIGN_CENTRAL);
  ...
  DvtNBoxStyleUtils.getNodeSecondaryLabel = function (nbox, node) {
    ...
    var rtl = dvt.Agent.isRightToLeft(nbox.getCtx());
    var halign = rtl ? dvt.OutputText.H_ALIGN_RIGHT : dvt.OutputText.H_ALIGN_LEFT;
    secondaryLabel = DvtNBoxUtils.createText(nbox.getCtx(), node[NBoxConstants.SECONDARY_LABEL], DvtNBoxStyleUtils.getNodeSecondaryLabelStyle(nbox, node), halign, dvt.OutputText.V_ALIGN_CENTRAL);
  ...
  DvtNBoxStyleUtils.getNodeColor = function (nbox, node) {
    if (node[NBoxConstants.COLOR]) return node[NBoxConstants.COLOR];
  ...
  DvtNBoxStyleUtils.getNodeBorderColor = function (nbox, node) {
    var color = node[NBoxConstants.BORDER_COLOR] ? node[NBoxConstants.BORDER_COLOR] : nbox.getOptions()[NBoxConstants.STYLE_DEFAULTS][NBoxConstants.NODE_DEFAULTS][NBoxConstants.BORDER_COLOR];
  ...
  DvtNBoxStyleUtils.getNodeBorderWidth = function (nbox, node) {
    var width = node[NBoxConstants.BORDER_WIDTH] ? node[NBoxConstants.BORDER_WIDTH] : nbox.getOptions()[NBoxConstants.STYLE_DEFAULTS][NBoxConstants.NODE_DEFAULTS][NBoxConstants.BORDER_WIDTH];
  ...
  DvtNBoxStyleUtils.getNodeIndicatorColor = function (nbox, node) {
    if (node[NBoxConstants.INDICATOR_COLOR]) return node[NBoxConstants.INDICATOR_COLOR];
  ...
  DvtNBoxStyleUtils.getIndicatorIcon = function (nbox, node) {
    if (node[NBoxConstants.INDICATOR_ICON]) return DvtNBoxStyleUtils.getStyledIndicatorIcon(nbox, node[NBoxConstants.INDICATOR_ICON]);
    return null;
  };
  /**
  ...
  DvtNBoxStyleUtils.getStyledIcon = function (nbox, icon) {
    return dvt.JsonUtils.merge(icon, nbox.getOptions()[NBoxConstants.STYLE_DEFAULTS][NBoxConstants.NODE_DEFAULTS][NBoxConstants.ICON_DEFAULTS]);
  };
  /**
  ...
  DvtNBoxStyleUtils.getStyledIndicatorIcon = function (nbox, indicatorIcon) {
    return dvt.JsonUtils.merge(indicatorIcon, nbox.getOptions()[NBoxConstants.STYLE_DEFAULTS][NBoxConstants.NODE_DEFAULTS][NBoxConstants.INDICATOR_ICON_DEFAULTS]);
  };
  /**
  ...
  DvtNBoxStyleUtils.getCategoryNodeColor = function (nbox, categoryNode) {
    ...
    if (!color) return nbox.getOptions()[NBoxConstants.STYLE_DEFAULTS][NBoxConstants.NODE_DEFAULTS][NBoxConstants.COLOR];

    for (var i = 1; i < nodeIndices.length; i++) {
      var node = DvtNBoxDataUtils.getNode(nbox, nodeIndices[i]);
      if (color != node[NBoxConstants.COLOR]) return nbox.getOptions()[NBoxConstants.STYLE_DEFAULTS][NBoxConstants.NODE_DEFAULTS][NBoxConstants.COLOR];
    }
  ...
  DvtNBoxStyleUtils.getCategoryNodeIndicatorColor = function (nbox, categoryNode) {
    ...
    if (!color) return nbox.getOptions()[NBoxConstants.STYLE_DEFAULTS][NBoxConstants.NODE_DEFAULTS][NBoxConstants.INDICATOR_COLOR];

    for (var i = 1; i < nodeIndices.length; i++) {
      var node = DvtNBoxDataUtils.getNode(nbox, nodeIndices[i]);
      if (color != node[NBoxConstants.INDICATOR_COLOR]) return nbox.getOptions()[NBoxConstants.STYLE_DEFAULTS][NBoxConstants.NODE_DEFAULTS][NBoxConstants.INDICATOR_COLOR];
    }
  ...
  DvtNBoxStyleUtils.getStyledCategoryIndicatorIcon = function (nbox, categoryNode) {
    ...
    if (!DvtNBoxDataUtils.getGroupAttributes(nbox) || DvtNBoxDataUtils.getGroupAttributes(nbox).indexOf('indicatorIconShape') != -1) {
      ...
      for (var i = 1; i < nodeIndices.length; i++) {
        ...
        if (nodeIcon && shape != nodeIcon[NBoxConstants.SHAPE]) match = false;
      }
    ...
    if (!DvtNBoxDataUtils.getGroupAttributes(nbox) || DvtNBoxDataUtils.getGroupAttributes(nbox).indexOf('indicatorIconColor') != -1) {
      ...
      for (var j = 1; j < nodeIndices.length; j++) {
        ...
        if (nodeIcon && color != nodeIcon[NBoxConstants.COLOR]) match = false;
      }
    ...
    if (!DvtNBoxDataUtils.getGroupAttributes(nbox) || DvtNBoxDataUtils.getGroupAttributes(nbox).indexOf('indicatorIconPattern') != -1) {
      ...
      for (var k = 1; k < nodeIndices.length; k++) {
        ...
        if (nodeIcon && pattern != nodeIcon[NBoxConstants.PATTERN]) match = false;
      }
  ...
  DvtNBoxStyleUtils.getLabelHalign = function (nbox, data) {
    ...
    if (!halign) halign = nbox.getOptions()[NBoxConstants.STYLE_DEFAULTS][NBoxConstants.CELL_DEFAULTS][NBoxConstants.LABEL_HALIGN];
    var rtl = dvt.Agent.isRightToLeft(nbox.getCtx());
    ...
    if (maximizedRow && maximizedColumn) {
      ...
      if (node[NBoxConstants.LABEL]) {
        ...
        if (!labelVisible) {
          ...
          if (node[NBoxConstants.COLOR]) {
            if ((!indicatorIcon || DvtNBoxStyleUtils.getNodeIndicatorColor(nbox, node)) && (!icon || icon[NBoxConstants.SOURCE])) {
              // Swatch needed
      ...
      if (simpleLayout['labelSectionWidth'] != null) {
        ...
        if (options['labelTruncation'] != 'ifRequired') {
          ...
          var cellArea = cell['__childArea'];
          var maxRows = Math.floor((cellArea.h + gridGap) / (nodeHeight + gridGap));
          var maxColumns = Math.floor((cellArea.w + gridGap) / (indicatorSectionWidth + iconSectionWidth + parseInt(options['__layout']['minimumLabelWidth']) + startLabelGap + nodeEndLabelGap + gridGap)); // use all vertical space to allow more horizontal space for labels
  ...
  DvtNBoxNodeRenderer._calculateSimpleNodeLayout = function (nbox) {
    ...
    if (icon) {
      var preferredSize = Math.max(nodeHeight, node[NBoxConstants.LABEL] ? dvt.Agent.isTouchDevice() ? icon['preferredSizeTouch'] : icon['preferredSize'] : parseInt(icon['preferredSizeNoLabel']));
  ...
  DvtNBoxNodeRenderer._renderNodeIndicator = function (nbox, node, nodeContainer, nodeLayout) {
    ...
    if (indicatorIcon) {
      ...
      if (indicatorIcon[NBoxConstants.SOURCE]) {
        indicatorMarker = new dvt.ImageMarker(nbox.getCtx(), indicatorIconX, indicatorIconY, indicatorIconWidth, indicatorIconHeight, indicatorIconBorderRadius, indicatorIcon[NBoxConstants.SOURCE]);
        indicatorMarker.setPreserveAspectRatio(DvtNBoxNodeRenderer._ASPECT_RATIO_SCALING);
      } else {
        indicatorMarker = new dvt.SimpleMarker(nbox.getCtx(), indicatorIcon[NBoxConstants.SHAPE], indicatorIconX, indicatorIconY, indicatorIconWidth, indicatorIconHeight, indicatorIconBorderRadius);
  ...
  DvtNBoxNodeRenderer._renderNodeIcon = function (nbox, node, nodeContainer, nodeLayout) {
    ...
    if (icon) {
      ...
      if (iconSource) {
        ...
        iconMarker.setClassName('oj-nbox-node-initials-bg-color' + (bgColor === 'neutral' ? ' oj-nbox-node-initials-neutral' : ' oj-avatar-bg-' + bgColor + ' oj-nbox-node-initials-' + bgColor));
        ...
        iconInitials = DvtNBoxUtils.createText(nbox.getCtx(), icon[NBoxConstants.INITIALS], null, dvt.OutputText.H_ALIGN_CENTER, dvt.OutputText.V_ALIGN_CENTRAL);
        DvtNBoxUtils.positionText(iconInitials, iconX, iconY); // Use small initials if there's only primary label or no height specified for no labels

        node[NBoxConstants.LABEL] && !node[NBoxConstants.SECONDARY_LABEL] || !node[NBoxConstants.LABEL] && parseInt(options[NBoxConstants.STYLE_DEFAULTS][NBoxConstants.NODE_DEFAULTS]['height']) > 0 ? iconInitials.setClassName('oj-nbox-node-initials') : iconInitials.setClassName('oj-nbox-node-initials oj-nbox-node-initials-lg');
      } else {
        iconMarker = new dvt.SimpleMarker(nbox.getCtx(), icon[NBoxConstants.SHAPE], iconX, iconY, iconWidth - iconBorderWidth, iconHeight - iconBorderWidth, iconBorderRadius);
      }
  ...
  DvtNBoxNodeRenderer.animateDelete = function (animationHandler, oldNode) {
    ...
    if (!isNaN(newNodeIndex)) {
      ...
      if (!DvtNBoxDataUtils.isNodeHidden(newNBox, newNode)) {
        ...
        else if (oldData[NBoxConstants.ROW] != newNode[NBoxConstants.ROW] || oldData[NBoxConstants.COLUMN] != newNode[NBoxConstants.COLUMN]) {
            var oldGlobalMatrix = DvtNBoxUtils.getGlobalMatrix(oldNode);
  ...
  DvtNBoxNodeRenderer._addAccessibilityAttributes = function (nbox, object) {
    ...
    if (!dvt.Agent.deferAriaCreation()) {
      ...
      if (desc) object.setAriaProperty(NBoxConstants.LABEL, desc);
    }
  };
  /**
   * @license
  ...
  DvtNBoxNode.prototype.UpdateAccessibilityAttributes = function () {
    if (!dvt.Agent.deferAriaCreation()) {
      ...
      if (desc) this.setAriaProperty(NBoxConstants.LABEL, desc);
    }
  };
  /**
   * @override
  ...
  DvtNBoxNode.prototype.getKeyboardFocusDisplayable = function () {
    ...
    if (node) {
      ...
      if (lastNode.nboxType === 'overflow') return lastNode; // return the group node if there is one
  ...
  DvtNBoxCellRenderer.render = function (nbox, cellData, cellContainer, availSpace) {
    ...
    if (labelHeight) {
      if (DvtNBoxCellRenderer._isLabelVertical(nbox, cellData)) {
        childArea = new dvt.Rectangle(headerSize, cellPadding, cellRect.getWidth() - headerSize - cellPadding, cellRect.getHeight() - cellPadding - cellPadding);
      } else {
        childArea = new dvt.Rectangle(cellPadding, headerSize, cellRect.getWidth() - cellPadding - cellPadding, cellRect.getHeight() - headerSize - cellPadding);
      }

      if (childContainer instanceof dvt.SimpleScrollableContainer) childContainer.setTranslate(0, headerSize);
    } else {
      childArea = new dvt.Rectangle(cellPadding, cellPadding, cellRect.getWidth() - cellPadding - cellPadding, cellRect.getHeight() - cellPadding - cellPadding);
    }
    ...
    if (cellData) {
      ...
      if (cellData[NBoxConstants.LABEL]) {
        ...
        if (DvtNBoxCellRenderer._isLabelVertical(nbox, cellData)) {
          // Vertical Label
          if (countText) {
            countLabel = DvtNBoxUtils.createText(nbox.getCtx(), countText, DvtNBoxStyleUtils.getCellCountLabelStyle(nbox, cellIndex), dvt.OutputText.H_ALIGN_CENTER, dvt.OutputText.V_ALIGN_CENTRAL);
            countLabel.setClassName('oj-typography-semi-bold');

            if (dvt.TextUtils.fitText(countLabel, cellRect.getHeight() - cellPadding - cellPadding, cellRect.getWidth() - 2 * cellPadding, cellContainer)) {
          ...
          if (!skipLabel) {
            var label = DvtNBoxUtils.createText(nbox.getCtx(), cellData[NBoxConstants.LABEL], DvtNBoxStyleUtils.getCellLabelStyle(nbox, cellIndex), dvt.OutputText.H_ALIGN_CENTER, dvt.OutputText.V_ALIGN_CENTRAL);

            if (dvt.TextUtils.fitText(label, cellRect.getHeight() - cellPadding - cellPadding - countLabelWidthWithGap, cellRect.getWidth() - 2 * cellPadding, cellContainer)) {
          ...
          if (countText) {
            countLabel = DvtNBoxUtils.createText(nbox.getCtx(), countText, DvtNBoxStyleUtils.getCellCountLabelStyle(nbox, cellIndex), halign, dvt.OutputText.V_ALIGN_CENTRAL);
            countLabel.setClassName('oj-typography-semi-bold');

            if (dvt.TextUtils.fitText(countLabel, cellRect.getWidth() - cellPadding - cellPadding - cellCloseWidthWithGap, cellRect.getHeight() - 2 * cellPadding, cellContainer)) {
              DvtNBoxUtils.setDisplayable(nbox, cellData, countLabel, 'countLabel');
              addedHeader = true;
              countLabelWidth = countLabel.getDimensions().w;
              countLabelWidthWithGap = countLabelWidth + countLabelGap; // Count label will get offset after rendering the cell label
          ...
          if (!skipLabel) {
            var label = DvtNBoxUtils.createText(nbox.getCtx(), cellData[NBoxConstants.LABEL], DvtNBoxStyleUtils.getCellLabelStyle(nbox, cellIndex), halign, dvt.OutputText.V_ALIGN_CENTRAL);

            if (dvt.TextUtils.fitText(label, cellRect.getWidth() - cellPadding - cellPadding - cellCloseWidthWithGap - countLabelWidthWithGap, cellRect.getHeight() - 2 * cellPadding, cellContainer)) {
              DvtNBoxUtils.setDisplayable(nbox, cellData, label, NBoxConstants.LABEL);
              var labelWidth = label.getDimensions().w;
              addedHeader = true;
              var labelX;
  ...
  DvtNBoxCellRenderer.renderBodyCountLabels = function (nbox, cellIndices) {
    ...
    for (var i = 0; i < maximizedCellIndices.length; i++) {
      ...
      maximizedLabels[i] = DvtNBoxUtils.createText(nbox.getCtx(), '' + count, DvtNBoxStyleUtils.getCellBodyCountLabelStyle(nbox), dvt.OutputText.H_ALIGN_CENTER, dvt.OutputText.V_ALIGN_CENTRAL);
    }

    for (var i = 0; i < minimizedCellIndices.length; i++) {
      var cellIndex = minimizedCellIndices[i];
      var count = cellCounts['total'][cellIndex];
      minimizedLabels[i] = DvtNBoxUtils.createText(nbox.getCtx(), '' + count, DvtNBoxStyleUtils.getCellBodyCountLabelStyle(nbox), dvt.OutputText.H_ALIGN_CENTER, dvt.OutputText.V_ALIGN_CENTRAL);
  ...
  DvtNBoxCellRenderer._calculateCellLayout = function (nbox) {
    ...
    for (var i = 0; i < cellCount; i++) {
      ...
      if (cellData && cellData[NBoxConstants.LABEL]) {
        ...
        var label = DvtNBoxUtils.createText(nbox.getCtx(), cellData[NBoxConstants.LABEL], DvtNBoxStyleUtils.getCellLabelStyle(nbox, i), halign, dvt.OutputText.V_ALIGN_CENTRAL);
        var cellLabelHeight = label.getDimensions().h;

        if (DvtNBoxStyleUtils.getCellShowCount(nbox, cellData) == 'on') {
          var count = DvtNBoxUtils.createText(nbox.getCtx(), cellCounts['total'][i], DvtNBoxStyleUtils.getCellCountLabelStyle(nbox, i), halign, dvt.OutputText.V_ALIGN_CENTRAL);
  ...
  DvtNBoxCellRenderer.animateUpdate = function (animationHandler, oldCell, newCell) {
    ...
    DvtNBoxCellRenderer._animateLabels(animationHandler, oldCell, newCell, NBoxConstants.LABEL); // Close button
  ...
  DvtNBoxCellRenderer._addAccessibilityAttributes = function (nbox, cellData, cellContainer) {
    var object = dvt.Agent.isTouchDevice() ? DvtNBoxUtils.getDisplayable(nbox, cellData, 'background') : cellContainer;
    object.setAriaRole('img');
  ...
  DvtNBoxCell.prototype.handleDoubleClick = function () {
    ...
    if (!oldFocus) this._nbox.EventManager.setFocusObj(this);else {
      if (oldFocus.nboxType === 'node') otherCell = this.getCellIndex() != DvtNBoxDataUtils.getCellIndex(this._nbox, oldFocus.getData());else if (oldFocus.nboxType === 'overflow') otherCell = this != oldFocus.getParentCell();else if (oldFocus.nboxType === 'cell') otherCell = this.getCellIndex() != oldFocus.getCellIndex();else if (oldFocus.nboxType === 'categoryNode') otherCell = this.getCellIndex() != oldFocus.getData()['cell'];
  ...
  DvtNBoxCell.prototype.getNode = function (index) {
    ...
    if (nodes) {
      ...
      for (var idx = 0; idx < nodes.length; idx++) {
        if (this.getData()[NBoxConstants.ROW] == nodes[idx][NBoxConstants.ROW] && this.getData()[NBoxConstants.COLUMN] == nodes[idx][NBoxConstants.COLUMN] && !DvtNBoxDataUtils.isNodeHidden(this._nbox, nodes[idx])) {
  ...
  DvtNBoxCell.prototype.getKeyboardFocusDisplayable = function () {
    // return the cell
    if (DvtNBoxDataUtils.getGroupBehavior(this._nbox) != NBoxConstants.GROUP_BEHAVIOR_ACROSS_CELLS || !this._nbox.getOptions()['__groups']) {
  ...
  DvtNBoxCategoryNodeRenderer._renderNodeIndicator = function (nbox, node, nodeContainer, scale, gap) {
    ...
    if (indicatorIcon) {
      var indicatorIconColor = indicatorIcon[NBoxConstants.COLOR] ? indicatorIcon[NBoxConstants.COLOR] : contrastColor;
      var indicatorMarker = new dvt.SimpleMarker(nbox.getCtx(), indicatorIcon[NBoxConstants.SHAPE], (rtl ? 1 : -1) * (side - indicatorWidth) / 2, 0, indicatorIcon[NBoxConstants.WIDTH] * indicatorIconScale, indicatorIcon[NBoxConstants.HEIGHT] * indicatorIconScale, indicatorIcon[NBoxConstants.BORDER_RADIUS]);
      if (indicatorIcon[NBoxConstants.BORDER_WIDTH]) indicatorMarker.setSolidStroke(indicatorIcon[NBoxConstants.BORDER_COLOR], null, indicatorIcon[NBoxConstants.BORDER_WIDTH]);

      if (indicatorIcon[NBoxConstants.PATTERN] && indicatorIcon[NBoxConstants.PATTERN] != 'none') {
        indicatorMarker.setFill(new dvt.PatternFill(indicatorIcon[NBoxConstants.PATTERN], indicatorIconColor, color));
      } else {
  ...
  DvtNBoxCategoryNodeRenderer._renderNodeCount = function (nbox, node, nodeContainer, scale, bIndicator, gap) {
    ...
    if (label) label.setTextString('' + count);else {
      label = DvtNBoxUtils.createText(nbox.getCtx(), '' + count, DvtNBoxStyleUtils.getCategoryNodeLabelStyle(nbox), dvt.OutputText.H_ALIGN_CENTER, dvt.OutputText.V_ALIGN_CENTRAL);
  ...
  DvtNBoxDrawerRenderer._renderHeader = function (nbox, data, drawerContainer) {
    ...
    var halign = rtl ? dvt.OutputText.H_ALIGN_RIGHT : dvt.OutputText.H_ALIGN_LEFT;
    var countLabel = DvtNBoxUtils.createText(nbox.getCtx(), '' + nodeCount, DvtNBoxStyleUtils.getDrawerCountLabelStyle(nbox), halign, dvt.OutputText.V_ALIGN_CENTRAL);
    ...
    if (drawerBounds.w - closeWidth - 2 * drawerButtonGap - drawerStartGap > countIndicatorWidth) {
      ...
      if (countIndicatorSectionWidth > 0) {
        ...
        if (indicatorIcon) {
          var indicatorMarker = new dvt.SimpleMarker(nbox.getCtx(), indicatorIcon[NBoxConstants.SHAPE], indicatorX + countIndicatorSectionWidth / 2, countIndicatorHeight / 2, indicatorIcon[NBoxConstants.WIDTH] * scale, indicatorIcon[NBoxConstants.HEIGHT] * scale, indicatorIcon[NBoxConstants.BORDER_RADIUS]);
          if (indicatorIcon[NBoxConstants.BORDER_WIDTH]) indicatorMarker.setSolidStroke(indicatorIcon[NBoxConstants.BORDER_COLOR], null, indicatorIcon[NBoxConstants.BORDER_WIDTH]);

          if (indicatorIcon[NBoxConstants.PATTERN] && indicatorIcon[NBoxConstants.PATTERN] != 'none') {
            indicatorMarker.setFill(new dvt.PatternFill(indicatorIcon[NBoxConstants.PATTERN], indicatorIconColor, indicatorColor ? indicatorColor : countColor));
          } else {
            indicatorMarker.setSolidFill(indicatorIconColor);
          } //Set the Indicator marker custom style and class name
    ...
    var categoryLabel = DvtNBoxUtils.createText(nbox.getCtx(), categoryText, DvtNBoxStyleUtils.getDrawerLabelStyle(nbox), halign, dvt.OutputText.V_ALIGN_CENTRAL);
    var labelOffset = 0;
  ...
  DvtNBoxDrawerRenderer._renderBody = function (nbox, data, drawerContainer) {
    ...
    DvtNBoxUtils.setDisplayable(nbox, data, body, 'background'); // Render the nodes
  ...
  DvtNBoxDrawerRenderer.calculateNodeOrders = function (nbox, data) {
    ...
    for (var p = 0; p < orderPasses.length; p++) {
      for (var n = 0; n < nodeCount; n++) {
        ...
        if (orderPasses[p] == 'normal' || orderPasses[p] == 'highlighted-selected' && highlightedMap[node[NBoxConstants.ID]] && selectedMap[node[NBoxConstants.ID]] || orderPasses[p] == 'highlighted-unselected' && highlightedMap[node[NBoxConstants.ID]] && !selectedMap[node[NBoxConstants.ID]] || orderPasses[p] == 'unhighlighted-selected' && !highlightedMap[node[NBoxConstants.ID]] && selectedMap[node[NBoxConstants.ID]] || orderPasses[p] == 'unhighlighted-unselected' && !highlightedMap[node[NBoxConstants.ID]] && !selectedMap[node[NBoxConstants.ID]]) {
          nodes.push(node);
        }
      }
    }
  ...
  DvtNBoxDrawerRenderer._addAccessibilityAttributes = function (nbox, data, drawerContainer) {
    var object = dvt.Agent.isTouchDevice() ? DvtNBoxUtils.getDisplayable(nbox, data, 'background') : drawerContainer;
    object.setAriaRole('img');
  ...
  DvtNBoxDrawer.prototype.UpdateAccessibilityAttributes = function () {
    if (!dvt.Agent.deferAriaCreation()) {
      ...
      if (desc) {
        var object = dvt.Agent.isTouchDevice() ? DvtNBoxUtils.getDisplayable(this._nbox, this.getData(), 'background') : this;
  ...
  DvtNBoxAutomation.prototype.getGroupNode = function (groupInfo) {
    if (groupInfo && DvtNBoxDataUtils.getGrouping(this._nBox) && DvtNBoxDataUtils.getGroupBehavior(this._nBox) == NBoxConstants.GROUP_BEHAVIOR_ACROSS_CELLS) {
  ...
  DvtNBoxAutomation.prototype.getCellGroupNode = function (cellData, groupInfo) {
    if (groupInfo && DvtNBoxDataUtils.getGrouping(this._nBox) && DvtNBoxDataUtils.getGroupBehavior(this._nBox) == NBoxConstants.GROUP_BEHAVIOR_WITHIN_CELL) {
  ...
  DvtNBoxEventManager.prototype.OnClickInternal = function (event) {
    ...
    if (obj.nboxType === 'categoryNode' && !obj.isSelectable()) obj.openDrawer();
  };
  /**
   * @override
   */
  ...
  DvtNBoxEventManager.prototype.ProcessKeyboardEvent = function (event) {
    ...
    if (keyCode == dvt.KeyboardEvent.ENTER || keyCode == dvt.KeyboardEvent.ESCAPE) {
      ...
      //multi-select event
      if (navigable.nboxType === 'categoryNode' || navigable.nboxType === 'node') eventConsumed = DvtNBoxEventManager.superclass.ProcessKeyboardEvent.call(this, event);
  ...
  DvtNBoxCategoryNode.prototype.Init = function (nbox, data) {
    DvtNBoxCategoryNode.superclass.Init.call(this, nbox.getCtx(), null, isNaN(data[NBoxConstants.CELL]) ? data[NBoxConstants.ID] : data[NBoxConstants.CELL] + ':' + data[NBoxConstants.ID]); // TODO for : Passing non container params weird
  ...
  DvtNBoxCategoryNode.prototype.openDrawer = function () {
    DvtNBoxDataUtils.fireOptionChangeEvent(this._nbox, NBoxConstants.DRAWER, {
      id: this.getId()
    });
  ...
  DvtNBoxCategoryNode.prototype.HandleKeyboardEvent = function (event) {
    if (event.keyCode == dvt.KeyboardEvent.ENTER) {
      this.openDrawer();
    } else if (event.keyCode == dvt.KeyboardEvent.ESCAPE && DvtNBoxDataUtils.getGroupBehavior(this._nbox) == NBoxConstants.GROUP_BEHAVIOR_WITHIN_CELL) {
  ...
  DvtNBoxCategoryNode.prototype.getNextNavigable = function (event) {
    ...
    if (event.keyCode == dvt.KeyboardEvent.SPACE && event.ctrlKey) {
      ...
      // care of the selection
      return this;
    } else if (event.keyCode == dvt.KeyboardEvent.CLOSE_BRACKET && DvtNBoxDataUtils.getGroupBehavior(this._nbox) == NBoxConstants.GROUP_BEHAVIOR_WITHIN_CELL) {
      ...
      if (DvtNBoxDataUtils.getGroupBehavior(this._nbox) == NBoxConstants.GROUP_BEHAVIOR_ACROSS_CELLS) {
        ...
        for (var i = 0; i < container.getNumChildren(); i++) {
          if (container.getChildAt(i).nboxType === 'categoryNode') nodes.push(container.getChildAt(i));
        }
  ...
  DvtNBoxRenderer._renderTitles = function (nbox, container, availSpace) {
    ...
    if (options[NBoxConstants.COLUMNS_TITLE]) {
      columnsTitle = DvtNBoxUtils.createText(nbox.getCtx(), options[NBoxConstants.COLUMNS_TITLE], options[NBoxConstants.STYLE_DEFAULTS][NBoxConstants.COLUMNS_TITLE_STYLE], dvt.OutputText.H_ALIGN_CENTER, dvt.OutputText.V_ALIGN_MIDDLE);
      container.addChild(columnsTitle);
      columnsTitleHeight = columnsTitle.getDimensions().h;
    }

    if (options[NBoxConstants.ROWS_TITLE]) {
      rowsTitle = DvtNBoxUtils.createText(nbox.getCtx(), options[NBoxConstants.ROWS_TITLE], options[NBoxConstants.STYLE_DEFAULTS][NBoxConstants.ROWS_TITLE_STYLE], dvt.OutputText.H_ALIGN_CENTER, dvt.OutputText.V_ALIGN_MIDDLE);
    ...
    for (var i = 0; i < columnCount; i++) {
      ...
      if (column[NBoxConstants.LABEL]) {
        var columnLabel = DvtNBoxUtils.createText(nbox.getCtx(), column[NBoxConstants.LABEL], DvtNBoxStyleUtils.getColumnLabelStyle(nbox, i), dvt.OutputText.H_ALIGN_CENTER, dvt.OutputText.V_ALIGN_MIDDLE);
    ...
    for (var i = 0; i < rowCount; i++) {
      ...
      if (row[NBoxConstants.LABEL]) {
        var rowLabel = DvtNBoxUtils.createText(nbox.getCtx(), row[NBoxConstants.LABEL], DvtNBoxStyleUtils.getRowLabelStyle(nbox, i), dvt.OutputText.H_ALIGN_CENTER, dvt.OutputText.V_ALIGN_MIDDLE);
    ...
    if (rowsTitle) {
      if (dvt.TextUtils.fitText(rowsTitle, availSpace.h, rowsTitleWidth, container)) {
        DvtNBoxUtils.positionText(rowsTitle, availSpace.x + (rtl ? availSpace.w : 0) + (rtl ? 1 : -1) * (rowHeaderWidth - rowsTitleWidth / 2), availSpace.y + availSpace.h / 2, rtl ? Math.PI / 2 : -Math.PI / 2);
    ...
    for (var i = 0; i < rowCount; i++) {
      if (rowLabels[i]) {
        ...
        if (dvt.TextUtils.fitText(rowLabels[i], cellDims.h, rowLabelsWidth, container)) {
          DvtNBoxUtils.positionText(rowLabels[i], availSpace.x + (rtl ? availSpace.w : 0) + (rtl ? 1 : -1) * (rowTitleComponentGap + rowLabelsWidth / 2), cellDims.y + cellDims.h / 2, rtl ? Math.PI / 2 : -Math.PI / 2);
  ...
  DvtNBoxRenderer._renderEmptyText = function (nbox, container, availSpace) {
    ...
    nbox.renderEmptyText(container, emptyTextStr, new dvt.Rectangle(availSpace.x, availSpace.y, availSpace.w, availSpace.h), nbox.getEventManager(), options['_statusMessageStyle']);
  };
  /**
   * @license
   * Copyright (c) 2013 2021, Oracle and/or its affiliates.
  ...
  var NBox = function NBox(context, callback, callbackObj) {
    this.Init(context, callback, callbackObj);
  };

  dvt.Obj.createSubclass(NBox, dvt.BaseComponent);
  ...
  NBox.prototype.Init = function (context, callback, callbackObj) {
    NBox.superclass.Init.call(this, context, callback, callbackObj); // Create the defaults object
    ...
    var hasCategory = function hasCategory(disp) {
      if (disp.nboxType === 'node') {
        if (disp.getCategories()) {
          return nbox.getOptions()[NBoxConstants.HIGHLIGHT_MATCH] == 'all' ? dvt.ArrayUtils.hasAllItems(categories, disp.getCategories()) : dvt.ArrayUtils.hasAnyItem(categories, disp.getCategories());
        }
        ...
        for (var i = 0; i < categoryNodeCount; i++) {
          ...
          if (nodeData[NBoxConstants.CATEGORIES]) {
            if (nbox.getOptions()[NBoxConstants.HIGHLIGHT_MATCH] == 'all' ? dvt.ArrayUtils.hasAllItems(categories, nodeData[NBoxConstants.CATEGORIES]) : dvt.ArrayUtils.hasAnyItem(categories, nodeData[NBoxConstants.CATEGORIES])) return true;
          }
        }
      }
  ...
  exports.NBox = NBox;
  exports.NBoxConstants = NBoxConstants;
