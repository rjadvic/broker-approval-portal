...
define(['exports', 'ojs/ojdvt-toolkit'], function (exports, dvt) {
  ...
  var DvtTimeAxisCalendar = function DvtTimeAxisCalendar(options) {
    this.Init(options);
  };
  ...
  DvtTimeAxisCalendar.prototype.adjustDate = function (date, scale) {
    ...
    if (scale == 'weeks') {
      ...
      if (roll_amt > 0) _adjustedDate.setTime(_adjustedDate.getTime() - roll_amt * this._dayInMillis);
      ...
      if (roll_amt > 0) _adjustedDate.setMonth(_adjustedDate.getMonth() + roll_amt);
  ...
  DvtTimeAxisCalendar.prototype.getAdjacentDate = function (time, scale, direction) {
    ...
    if (scale == 'milliseconds') return new Date(time + directionSign * 1);else if (scale == 'seconds') return new Date(time + directionSign * 1000);else if (scale == 'minutes') return new Date(time + directionSign * 60000);else if (scale == 'hours') return new Date(time + directionSign * 3600000); // for larger scales, no set amount of time can be added

    var _adjacentDate = new Date(time);

    if (scale == 'days') _adjacentDate.setDate(_adjacentDate.getDate() + directionSign * 1);else if (scale == 'weeks') _adjacentDate.setDate(_adjacentDate.getDate() + directionSign * 7);else if (scale == 'months') _adjacentDate.setMonth(_adjacentDate.getMonth() + directionSign * 1);else if (scale == 'quarters') _adjacentDate.setMonth(_adjacentDate.getMonth() + directionSign * 3);else if (scale == 'halfyears') _adjacentDate.setMonth(_adjacentDate.getMonth() + directionSign * 6);else if (scale == 'years') _adjacentDate.setFullYear(_adjacentDate.getFullYear() + directionSign * 1);else if (scale == 'twoyears') _adjacentDate.setFullYear(_adjacentDate.getFullYear() + directionSign * 2);else {
      // circuit breaker
  ...
  var DvtTimeAxisDefaults = function DvtTimeAxisDefaults(context) {
    this.Init({
      'alta': DvtTimeAxisDefaults.VERSION_1
    }, context);
  };
  ...
  var DvtTimeAxisFormatter = function DvtTimeAxisFormatter(type, dateFormatStrings, locale) {
    ...
    if (mask != null) {
      ...
      if (timeZoneOffsets) {
        ...
        for (var i = 0; i < dates.length; i++) {
          if (timeInMS >= parseInt(dates[i], 10)) offset = timeZoneOffsets[dates[i]];
        }
      ...
      if (mask.indexOf(':') != -1) var separator = ':';else if (mask.indexOf('/') != -1) separator = '/';
      ...
      if (separator) {
        ...
        return newString;
      } else return this.getDateFormatValue(date, mask, isUTC);
    } else {
      ...
      try {
        ...
        if (e.name === 'RangeError') return date.toLocaleString();
      }
  ...
  var DvtTimeAxisParser = function DvtTimeAxisParser() {};
  ...
  var TimeAxisUtils = {
     ...
     */
    getDatePosition: function getDatePosition(startTime, endTime, time, width) {
     ...
     */
    getPositionDate: function getPositionDate(startTime, endTime, pos, width) {
  ...
  DvtTimeAxisRenderer._renderAxisBlock = function (timeAxis, axisStart, axisSize, separatorStyle) {
    ...
    if (timeAxis._axis == null) {
      ...
      if (timeAxis.isVertical()) {
        timeAxis._axis = new dvt.Path(context, dvt.PathUtils.roundedRectangle(axisStart, -timeAxis.getBorderWidth('top'), axisSize, timeAxis.getAxisLength(), 0, 0, 0, 0), 'axis');
        cp.addRect(axisStart, 0, axisSize, timeAxis._contentLength);
      } else {
        timeAxis._axis = new dvt.Path(context, dvt.PathUtils.roundedRectangle(-timeAxis.getBorderWidth('left'), axisStart, timeAxis.getAxisLength(), axisSize, 0, 0, 0, 0), 'axis');
        cp.addRect(0, axisStart, timeAxis._contentLength, axisSize);
      }

      timeAxis._axis.setCSSStyle(timeAxis._axisStyle); // setCSSStyle doesn't actually apply styles for dvt.Path. Adopt the logic from dvt.Rect:
      ...
      if (axisClass) timeAxis._axis.getElem().setAttribute('class', axisClass);
    } else {
      ...
      if (timeAxis.isVertical()) {
        timeAxis._axis.setCmds(dvt.PathUtils.roundedRectangle(axisStart, -timeAxis.getBorderWidth('top'), axisSize, timeAxis.getAxisLength(), 0, 0, 0, 0));

        cp.addRect(axisStart, 0, axisSize, timeAxis._contentLength);
      } else {
        timeAxis._axis.setCmds(dvt.PathUtils.roundedRectangle(-timeAxis.getBorderWidth('left'), axisStart, timeAxis.getAxisLength(), axisSize, 0, 0, 0, 0));
    ...
    timeAxis._axis.removeChildren(); // apply stroke dash array to turn on/off border sides accordingly
  ...
  DvtTimeAxisRenderer._renderAxisTicksLabels = function (timeAxis, startPos, endPos, container, length, axisEnd, tickStart, labelStart) {
    ...
    var maxLabelHeight = timeAxis.getContentSize(); // the last date in dates is past the end time, and only used as the last 'next' date
    ...
    for (var i = 0; i < dates.length - 1; i++) {
      ...
      if (currentPos != 0) {
        if (timeAxis.isVertical()) var tickElem = DvtTimeAxisRenderer._addTick(context, block, axisEnd, tickStart, currentPos, currentPos, timeAxis._separatorStroke, 's_tick' + date, separatorClass);else if (!isRTL) tickElem = DvtTimeAxisRenderer._addTick(context, block, currentPos, currentPos, axisEnd, tickStart, timeAxis._separatorStroke, 's_tick' + date, separatorClass);else tickElem = DvtTimeAxisRenderer._addTick(context, block, length - currentPos, length - currentPos, axisEnd, tickStart, timeAxis._separatorStroke, 's_tick' + date, separatorClass); // save the time associated with the element for dynamic resize

        tickElem.time = date;
      } // Label may have been truncated in the previous render. Start fresh for this render:


      if (label.isTruncated()) label.setTextString(label.getUntruncatedTextString());
        ...
        if (i === 0 && currentPos < 0) // first label currentPos is before the overall start time pos (at pos 0)
          {
            ...
            var maxLength = nextPos - Math.max(0, (nextPos - currentPos - labelWidth) / 2); // Can't simply take center of inter-tick region and alignCenter() for first label treatment if the label is going to be truncated
          ...
          } else if (i === dates.length - 2 && nextPos > length) // last label nextPos is after the overall end time pos (at pos length)
          {
            ...
            maxLength = length - currentPos - Math.max(0, (nextPos - currentPos - labelWidth) / 2); // Can't simply take center of inter-tick region and alignCenter() for last label treatment if the label is going to be truncated
            // Instead, align right and manually calculate label placement

            if (isRTL) label.alignRight();
            horizPos = Math.max(currentPos, currentPos + (nextPos - currentPos) / 2 - labelWidth / 2);
          } else {
          maxLength = nextPos - currentPos; // take center of inter-tick region and alignCenter()
  ...
  DvtTimeAxisRenderer._addAxisLabel = function (container, label, x, y, maxLength, maxHeight, labelClass, truncateStart) {
    ...
    if (labelClass) label.getElem().setAttribute('class', labelClass);
  };
  /**
   * Adds a tick mark.
  ...
  DvtTimeAxisRenderer._addTick = function (context, container, x1, x2, y1, y2, stroke, id, tickClass) {
    ...
    if (tickClass) line.getElem().setAttribute('class', tickClass);
    return line;
  };
  /**
   * @license
  ...
  var TimeAxis = function TimeAxis(context, callback, callbackObj) {
    this.Init(context, callback, callbackObj);
  };

  dvt.Obj.createSubclass(TimeAxis, dvt.BaseComponent);
  /**
  ...
  TimeAxis.prototype.Init = function (context, callback, callbackObj) {
    ...
    this._dateToIsoWithTimeZoneConverter = context.getLocaleHelpers()['dateToIsoWithTimeZoneConverter']; // Internationalization strings
  ...
  TimeAxis.prototype._applyParsedProperties = function (props) {
    ...
    if (orientation && orientation == TimeAxis.ORIENTATION_VERTICAL) this._isVertical = true;else this._isVertical = false;
    this.setIsVertical(this._isVertical);
    this._shortDesc = props.shortDesc; // TODO: update zoom implementation to handle longest value first
  ...
  TimeAxis.prototype.setContentLength = function (length, minLength) {
    ...
    if (minLength < length) this._contentLength = length;else this._contentLength = minLength;
  };
  /**
  ...
  TimeAxis.prototype.getPreferredLength = function (options, minViewPortLength) {
    ...
    if (firstDayOfWeek == null) firstDayOfWeek = 0; // default to sunday

    this._calendar.setFirstDayOfWeek(firstDayOfWeek);

    if (!this._dateToIsoWithTimeZoneConverter) this._dateToIsoWithTimeZoneConverter = this.getCtx().getLocaleHelpers()['dateToIsoWithTimeZoneConverter'];
  ...
  TimeAxis.prototype.applyStyleValues = function () {
    ...
    this._axisStyle.parseInlineStyle(this._inlineStyle); // double border width to account for stroke width rendering
  ...
  TimeAxis.prototype._setAxisDimensions = function () {
    if (this._canvasSize !== null) this.setContentSize(this._canvasSize - this.getSizeBorderWidth());
  ...
  TimeAxis.prototype.prepareTimeAxisZoomLevelIntervals = function (startTime, endTime, minViewPortLength) {
    ...
    if (this._isVertical) this.setDefaultConverter(this._resources['converterVert']);else this.setDefaultConverter(this._resources['converter']);
    if (this._timeZoneOffsets) this.setTimeZoneOffsets(this._timeZoneOffsets);
  ...
  TimeAxis.prototype._prepareScaleDatesLabels = function (scale, originalScale, startTime, endTime) {
    ...
    if (this._customTimeScales && this._customTimeScales[scale]) {
      ...
      while (currentTime < endTime) {
        ...
        currentTime = this.getNextDate(currentTime).getTime(); // the last currentTime added in this loop is outside of the time range, but is needed
    ...
    for (var j = 0; j < labelTexts.length; j++) {
      ...
      label.setCSSStyle(DvtTimeAxisStyleUtils.getAxisLabelStyle(this.Options)); // save the time associated with the element for dynamic resize

      label.time = currentTime;
      var nextTime = dates[j + 1]; // update maximum label width and height
      ...
      if (lengthFactor < minLength) minLength = lengthFactor;
      if (sizeDim > maxSize) maxSize = sizeDim;
      labels.push(label);
    }
  ...
  TimeAxis.prototype._prepareZoomLevelLengths = function (i, scale, startTime, endTime, minLength, minViewPortLength) {
    if (this._canvasSize !== null) //standalone timeaxis case
      ...
      } else zoomLevelLength = (endTime - startTime) / minLength;
    ...
    if (i == 0) this._maxContentLength = this._labels[this._labels.length - 1].length * minViewPortLength;
  };
  /**
  ...
  TimeAxis.prototype.getTimeAxisWidth = function () {
    // read from skin?
    if (this._timeAxisWidth == null) this._timeAxisWidth = 30;
  ...
  TimeAxis.prototype.formatDate = function (date, converter, converterType) {
    ...
    if (converterType == 'axis') {
      ...
      if (converter) {
        if (converter[scale]) converter = converter[scale];else if (converter['default']) converter = converter['default'];
        ...
        if (!converter) {
          ...
          if (defaultDateTimeConverter && defaultDateConverter) {
            ...
            try {
              ...
              if (e.name === 'RangeError') return date[localeStringMethod](); // fallback to using system locale rather than using app's locale
            }
  ...
  TimeAxis.prototype.setIsVertical = function (isVertical) {
    if (isVertical) this._contentSize = DvtTimeAxisStyleUtils.DEFAULT_INTERVAL_WIDTH;else this._contentSize = DvtTimeAxisStyleUtils.DEFAULT_INTERVAL_HEIGHT;
  };
  /**
   * Sets canvas size.
