...
define(['ojs/ojcore-base', 'ojs/ojdvt-toolkit', 'ojs/ojcontext', 'ojs/ojconfig', 'ojs/ojmap', 'ojs/ojlocaledata', 'ojs/ojcomponentcore', 'jquery', 'ojs/ojkeysetimpl', 'ojs/ojdomutils', 'ojs/ojattributegrouphandler', 'ojs/ojlogger', 'ojs/ojcustomelement', 'ojs/ojcustomelement-utils', 'ojdnd'], function (oj, dvt, Context, Config, ojMap, LocaleData, Components, $, KeySetImpl, DomUtils, attributeGroupHandler, Logger, ojcustomelement, ojcustomelementUtils, ojdnd) {
  ...
  var DvtStyleProcessor = function DvtStyleProcessor(context) {
  ...
  DvtStyleProcessor.prototype.processStyles = function (element, options, componentClasses, childClasses) {
    ...
    for (i = 0; i < styleClassKeys.length; i++) {
      ...
      var hasUncachedProperty = this._hasUncachedProperty(styleClass, definitions) || ignoreCache;
      ...
      if (!innerDummyDiv && !outerDummyDiv && hasUncachedProperty) {
        ...
        outerDummyDiv.attr('class', styleClasses); // Add an inner dummy div to overwrite inherited values and prevent populating options object with them
      ...
      if (hasUncachedProperty) {
        styleDivs[styleClass] = this._createStyleDivs(innerDummyDiv, styleClass, definitions);
      }
    }
    ...
    for (i = 0; i < styleClassKeys.length; i++) {
      ...
      this._processStyle(styleDivs[styleClass], options, styleClass, styleDefinitions[styleClass], ignoreCache);
  ...
  DvtStyleProcessor.prototype._processStyle = function (cssDiv, options, styleClass, definitions, ignoreCache) {
    for (var i = 0; i < definitions.length; i++) {
      var definition = definitions[i];
      var property = definition.property;
      ...
      if (property) {
        ...
        if (typeof value === 'undefined' && cssDiv) {
          ...
          value = styleClass === 'oj-dvt-color-ramp' ? attributeGroupHandler.ColorAttributeGroupHandler.__processAttrDiv(cssDiv) : DvtStyleProcessor._resolveStyle(cssDiv, property);
        ...
        if (value != null) {
          ...
          var path = new DvtJsonPath(options, stylePath); // If the value is an object, combine the value from the options with the value from the CSS.

          var optionsValue = path.getValue();
          var isObj = _typeof(optionsValue) === 'object' && !Array.isArray(optionsValue);
  ...
  DvtStyleProcessor.populateOptions = function (options, childClasses) {
    ...
    for (var i = 0; i < styleClassKeys.length; i++) {
      ...
      for (var j = 0; j < definitions.length; j++) {
        ...
        var stylePath = definition.path;
        var isObject = definition.property === 'TEXT' || definition.property === 'BACKGROUND'; // If it is a top level primitive, do nothing
      ...
      if (event.type === 'refresh') {
        ...
        if (removeDetail) {
          ...
          if (addDetail) {
            ...
            removedIDs = new Set(_toConsumableArray(removedIDs).filter(function (x) {
              return !addKeySet.has(x);
            }));
          }

          self._fireEvent('removeKeys', {
            keys: removedIDs
          });
  ...
  */
  ...
  TemplateHandler.prototype.getDataSetBoolean = function (templateName, attr) {
    ...
    return propValue != null && ojcustomelementUtils.AttributeUtils.coerceBooleanValue(this.getTemplate(templateName), attr, propValue, 'Boolean');
  };
  /**
  ...
  TemplateHandler.prototype.processNodeTemplate = function (dataProperty, templateEngine, template, templateElementName, context, nodeKey, isMainTemplate) {
    ...
    if (config) {
      ...
      if (getTemplateName && getTemplateElementName && resultPath) {
        ...
        var processDatum = function processDatum(nodeData, nodeKey, context, template, templateElementName) {
          ...
          try {
            if (template) {
              processedDatum = self.processNodeTemplate(dataProperty, templateEngine, template, templateElementName, context, nodeKey, true);
            } else {
  ...
  oj.__registerWidget('oj.dvtBaseComponent', $.oj.baseComponent, {
    ...
    _ComponentCreate: function _ComponentCreate() {
      ...
      this._context.setJetProperties(ref); // Set the reading direction on the context
      ...
      this._LoadResources(); // Pass the widget constructor through the options for JET specific behavior
    ...
    refresh: function refresh() {
      this._super(); // Clear the Styles Cache to force the component to recalc


      this._styleProcessor = null; // Update the reading direction on the context
     ...
     * @memberof oj.dvtBaseComponent
     */
    _ProcessStyles: function _ProcessStyles(options) {
      // Process selectors for this component
      this._styleProcessor = this._styleProcessor ? this._styleProcessor : new DvtStyleProcessor(this._component.getCtx());

      this._styleProcessor.processStyles(this.element, options, this._GetComponentStyleClasses(), this._GetChildStyleClasses());
    },
    ...
    _ReleaseResources: function _ReleaseResources() {
      ...
      this._renderNeeded = true; // Hide all component tooltips and remove references to the DvtContext

      this._context.hideTooltips();

      this._context.releaseTooltipResources(); // Remove DOM resize listener


      this._removeResizeListener();

      this._CleanAllTemplates(); // Remove any pending busy states


      this._MakeReady(); // Reset data provider state
    ...
    _destroy: function _destroy() {
      ...
      this._component = null; // Remove children and clean up DOM changes
    ...
    _Render: function _Render(isResize) {
      ...
      if (!this._context.isReadyToRender() || this._renderNeeded && isResize) {
        ...
        this._height = this._IsFlowingLayoutSupported() ? null : this.element.height(); // Set the root font-family and font-size

        this._context.setDefaultFontFamily(this._referenceDiv.css('font-family'));

        this._context.setDefaultFontSize(this._referenceDiv.css('font-size')); // Add the width, height, and locale as private fields in the options for debugging purposes


        this.options._width = this._width;
        this.options._height = this._height;
        this.options._locale = dvt.Agent.isEnvironmentTest() && this.options._locale ? this.options._locale : Config.getLocale(); // Add draggable attribute if DnD is supported

        if (this._IsDraggable()) {
          this.element.attr('draggable', true);
        }

        DvtStyleProcessor.populateOptions(this.options, this._GetChildStyleClasses()); // Skip the options on resize to suppress animation.
     ...
     * @memberof oj.dvtBaseComponent
     */
    _resolveDocumentFonts: function _resolveDocumentFonts() {
      var documentFonts = document.fonts;

      if (documentFonts && documentFonts.status === 'loading') {
        // document.fonts is not supported by IE
    ...
    _resolveDeferredDataItems: function _resolveDeferredDataItems() {
      ...
      this._optionsCopy = dvt.JsonUtils.clone(this.options, null, this._GetComponentNoClonePaths()); // Merge css styles with with json options object
     ...
     * @memberof oj.dvtBaseComponent
     */
    _RemoveKeys: function _RemoveKeys(removedKeys) {
