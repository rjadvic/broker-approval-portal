 ...
 * @ignore
 */
define(['ojs/ojcore-base', 'ojs/ojcomponentcore', 'ojs/ojcore', 'jquery'], function (oj, ojcomponentcore, oj$1, $) {
  'use strict';
  ...
  (function () {
    ...
    oj.CustomElementBridge.register('oj-progress', {
      metadata: __oj_progress_metadata
    });
  })();
  /**
   * @license
  ...
  (function () {
    ...
    oj$1.__registerWidget('oj.ojProgressbar', $.oj.baseComponent, {
      ...
      _ComponentCreate: function _ComponentCreate() {
        this._super(); // Constrain initial value
       ...
       * @private
       */
      _setUpProgressType: function _setUpProgressType() {
      ...
      _constrainedValue: function _constrainedValue(newValue) {
        ...
        this._indeterminate = newValue === -1; // sanitize value
      ...
      _setOptions: function _setOptions(options, flags) {
        ...
        var value = options.value; // eslint-disable-next-line no-param-reassign
       ...
       * @return {void}
       */
      _setupCircleSVG: function _setupCircleSVG() {
        // Clear existing SVG
        if (this.svg) {
          this.svg.remove();
        }
        ...
        if (!this._indeterminate) {
          ...
          svg.setAttribute('class', 'oj-progress-circle-transform'); // Track Circle

          bgCircle.setAttribute('r', radius * 0.9);
          bgCircle.setAttribute('cx', radius);
          bgCircle.setAttribute('cy', radius);
          bgCircle.setAttribute('class', 'oj-progress-circle-base'); // Value Circle
      ...
      _setOption: function _setOption(key, value, flags) {
        ...
        this._super(key, value, flags); // If changing the type of progress, want to remove the old progress
        // and rerender a new one
      ...
      _refreshValue: function _refreshValue() {
        ...
        if (isCircle) {
          ...
          this.valueDiv.toggle(this._indeterminate || value > this.min).width(percentage.toFixed(0) + '%');
      ...
      _destroy: function _destroy() {
        ...
        this.element.removeAttr('role').removeAttr('aria-valuemin').removeAttr('aria-valuemax').removeAttr('aria-valuenow');
