 ...
 * @ignore
 */
define(['exports', 'ojs/ojcore-base', 'ojs/ojdatasource-common', 'ojs/ojlogger', 'jquery'], function (exports, oj$1, ojdatasourceCommon, Logger, jquery) {
  'use strict';
  ...
  */
  ...
  var CollectionCellSet = function CollectionCellSet(startRow, endRow, startColumn, endColumn, columns) {
   ...
   * @memberof CollectionCellSet
   * @private
   */


  CollectionCellSet.prototype.setModels = function (models) {
    oj$1.Assert.assertArray(models); // make sure the array size is valid
  ...
  CollectionCellSet.prototype.getData = function (indexes) {
  ...
  CollectionCellSet.prototype.getMetadata = function (indexes) {
    // indexes are validated in _getModel
    var model = this._getModel(indexes);
  ...
  CollectionCellSet.prototype._getModel = function (indexes) {
    // make sure models is populated
    if (this.m_models == null) {
      return null;
    }

    oj$1.Assert.assertObject(indexes); // extract row and column index

    var row = indexes.row;
    var column = indexes.column; // make sure index are valid

    oj$1.Assert.assert(row >= this.m_startRow && row <= this.m_endRow && column >= this.m_startColumn && column <= this.m_endColumn);
  ...
  CollectionCellSet.prototype.getCount = function (axis) {
    if (axis === 'row') {
  ...
  CollectionCellSet.prototype.getStartRow = function () {
  ...
  CollectionCellSet.prototype.getEndRow = function () {
  ...
  CollectionCellSet.prototype.getStartColumn = function () {
  ...
  CollectionCellSet.prototype.getEndColumn = function () {
  ...
  CollectionCellSet.prototype.getColumns = function () {
  ...
  var CollectionDataGridDataSource = function CollectionDataGridDataSource(collection, options) {
  ...
  oj$1._registerLegacyNamespaceProp('CollectionDataGridDataSource', CollectionDataGridDataSource); // subclass of DataGridDataSource


  oj$1.Object.createSubclass(CollectionDataGridDataSource, oj$1.DataGridDataSource, 'oj.CollectionDataGridDataSource');
  /**
  ...
  CollectionDataGridDataSource.prototype.Init = function () {
    // call super
    CollectionDataGridDataSource.superclass.Init.call(this);
  ...
  CollectionDataGridDataSource.prototype._registerEventListeners = function () {
  ...
  CollectionDataGridDataSource.prototype.getCount = function (axis) {
    if (this.precision === undefined) {
      this.precision = {};
    }

    if (axis === 'row') {
      var totalSize = this._totalSize(); // 1. totalResults undefined
  ...
  CollectionDataGridDataSource.prototype.fetchHeaders = function (headerRange, callbacks, callbackObjects) {
  ...
  CollectionDataGridDataSource.prototype._handleHeaderFetchSuccess = function (headerRange, callbacks, callbackObjects, actualRange) {
    var axis = headerRange.axis;
    ...
    if (axis === 'column') {
      ...
      if (this.rowHeader != null) {
        ...
        headerSet = new oj$1.CollectionHeaderSet(start, end, this.columns, this.rowHeader); // resolve any promises before invoking callbacks

        this._resolveModels(start, end, headerSet, headerRange, callbacks, callbackObjects); // resolveModels will invoke callbacks
  ...
  CollectionDataGridDataSource.prototype._getRanges = function (cellRanges) {
    var rowStart;
    var rowCount;
    var colStart;
    var colCount; // extract the start and end row/column info from cellRanges (there should only be two, one for each axis)
  ...
  CollectionDataGridDataSource.prototype._handleCellFetchSuccess = function (cellRanges, callbacks, callbackObjects, actualRange) {
    ...
    var rowEnd; // use the actual rows returned by the server
    ...
    var colStart = ranges.colStart; // if there are no columns make sure we return an empty cellSet

    var colEnd = Math.min(this.columns == null ? 0 : this.columns.length, colStart + ranges.colCount); // resolves models at row range
  ...
  CollectionDataGridDataSource.prototype._resolveModels = function (rowStart, rowEnd, set, ranges, callbacks, callbackObjects) {
    var promises = [];

    for (var i = rowStart; i < rowEnd; i++) {
      promises.push(this.collection.at(i, {
        deferred: true
      }));
  ...
  CollectionDataGridDataSource.prototype.fetchCells = function (cellRanges, callbacks, callbackObjects) {
    // still fetching, just store the callback info
    if (callbacks != null) {
  ...
  CollectionDataGridDataSource.prototype._processPendingHeaderCallbacks = function (axis) {
    ...
    if (pendingCallback != null) {
      ...
      this._handleHeaderFetchSuccess(headerRange, callbacks, callbackObjects, actualRange); // clear any pending callback
  ...
  CollectionDataGridDataSource.prototype._processPendingCellCallbacks = function () {
    ...
    var actualRange = this.pendingCellCallback.actualRange; // handles success cell fetch
  ...
  CollectionDataGridDataSource.prototype._fetchCells = function (cellRanges) {
    var ranges = this._getRanges(cellRanges);

    var rowStart = ranges.rowStart;
    var rowCount = ranges.rowCount; // set the range local for the requested range
    ...
    this.collection.setRangeLocal(rowStart, rowCount).then(function (actual) {
      ...
      this._setActualCallbackRanges(actual.start, actual.count); // check if we need to poach columns from row


      if (this.columns === undefined) {
        this.collection.at(rowStart, {
          deferred: true
        }).then(function (model) {
          if (model != null) {
        ...
        }.bind(this)); // process must be done after columns are discovered
  ...
  CollectionDataGridDataSource.prototype._fetchCellsError = function (error) {
    // log the error
    Logger.error(error); // invoke any header error callbacks
  ...
  CollectionDataGridDataSource.prototype._processPendingHeaderErrorCallbacks = function (axis, error) {
    var pendingCallback = this.pendingHeaderCallback[axis];

    if (pendingCallback != null) {
      var callbacks = pendingCallback.callbacks;
      var callbackObjects = pendingCallback.callbackObjects;
      var headerRange = pendingCallback.headerRange; // invoke error callback
  ...
  CollectionDataGridDataSource.prototype._processPendingCellErrorCallbacks = function (error) {
    var callbacks = this.pendingCellCallback.callbacks;
    var callbackObjects = this.pendingCellCallback.callbackObjects;
    var cellRanges = this.pendingCellCallback.cellRanges; // invoke error callback
  ...
  CollectionDataGridDataSource.prototype._fetchCellsComplete = function (cellRanges) {
  ...
  CollectionDataGridDataSource.prototype._setActualCallbackRanges = function (start, count) {
    var actualRange = {
      start: start,
      count: count
    };
  ...
  CollectionDataGridDataSource.prototype._setupColumns = function (model) {
  ...
  CollectionDataGridDataSource.prototype.keys = function (indexes) {
    var rowIndex = indexes.row;
    var columnIndex = indexes.column;
    var self = this;
    return new Promise(function (resolve) {
      self.collection.at(rowIndex, {
        deferred: true
      }).then(function (rowModel) {
        if (rowModel == null) {
          resolve({
            row: null,
            column: null
  ...
  CollectionDataGridDataSource.prototype.indexes = function (keys) {
    var rowKey = keys.row;
    var columnKey = keys.column;
    var self = this;
    return new Promise(function (resolve) {
      self.collection.indexOf(rowKey, {
        deferred: true
      }).then(function (rowIndex) {
        var columnIndex = -1; // if the row exists but columns not yet set
  ...
  */
  ...
  CollectionDataGridDataSource.prototype.sort = function (criteria, callbacks, callbackObjects) {
    ...
    if (axis === 'column') {
      ...
      if (this.collection.IsVirtual()) {
        ...
        if (direction === 'ascending') {
          comparator = function comparator(_a, _b) {
            // Get the values from the model objects
            var a = _a.get(key);

            var b = _b.get(key); // Strings of numbers return false, so we can compare strings of numbers with numbers


            var as = isNaN(a);
            var bs = isNaN(b); // If they dates, turn them into sortable strings
  ...
  CollectionDataGridDataSource.prototype._resetSortOrder = function (callbacks, callbackObjects) {
  ...
  CollectionDataGridDataSource.prototype._setSortInfo = function (key) {
  ...
  CollectionDataGridDataSource.prototype.move = function (rowToMove, referenceRow, position, callbacks, callbackObjects) {
    this.collection.get(rowToMove, {
      deferred: true
    }).then(function (moveModel) {
      var indexPromise;
      ...
      if (referenceRow == null) {
        ...
        if (rowToMove === referenceRow) {
          indexPromise = this.collection.indexOf(referenceRow, {
            deferred: true
          });
          this.collection.remove(moveModel);
        } else {
  ...
  CollectionDataGridDataSource.prototype.moveOK = function (rowToMove, referenceRow, position) {
    return 'valid';
  }; // ////////////////////////////////// Event listeners /////////////////////////////////////
  ...
  CollectionDataGridDataSource.prototype._getModelEvent = function (operation, rowKey, columnKey, rowIndex, columnIndex) {
    var event = {};
    event.source = this;
  ...
  CollectionDataGridDataSource.prototype._handleCollectionRefresh = function () {
    this.data = null;
  ...
  CollectionDataGridDataSource.prototype._handleCollectionReset = function () {
    this.data = null;
  ...
  CollectionDataGridDataSource.prototype._size = function () {
  ...
  CollectionDataGridDataSource.prototype._totalSize = function () {
  ...
  CollectionDataGridDataSource.prototype.getCollection = function () {
  ...
  CollectionDataGridDataSource.prototype.getColumns = function () {
    return this.columns;
  };
  /**
  ...
  CollectionDataGridDataSource.prototype.getRowHeader = function () {
  ...
  CollectionDataGridDataSource.prototype.getData = function () {
    return this.data;
  };
  /**
   * @license
  ...
  var CollectionDataGridUtils = function CollectionDataGridUtils() {};
  ...
  CollectionDataGridUtils._getModelKey = function (model) {
    var key = model.GetId();
  ...
  var CollectionHeaderSet = function CollectionHeaderSet(start, end, headers, rowHeader, sortInfo) {
  ...
  CollectionHeaderSet.prototype.getData = function (index, level) {
    var model; // make sure index/level are valid

    oj$1.Assert.assert(index <= this.m_end && index >= this.m_start, 'index out of bounds');
    oj$1.Assert.assert(level == null || level === 0, 'level out of bounds'); // row or column header
  ...
  CollectionHeaderSet.prototype.getMetadata = function (index, level) {
    oj$1.Assert.assert(index <= this.m_end && index >= this.m_start, 'index out of bounds');
    oj$1.Assert.assert(level == null || level === 0, 'level out of bounds'); // row header case
    ...
    if (this.m_rowHeader != null) {
      ...
      return {
        key: oj$1.CollectionDataGridUtils._getModelKey(model)
      };
    }
    ...
    if (this.m_sortInfo.key === data) {
      return {
        key: data,
        sortDirection: this.m_sortInfo.direction
      };
    }
  ...
  CollectionHeaderSet.prototype.getLevelCount = function () {
    if (this.getCount() > 0) {
      return 1;
    }
  ...
  CollectionHeaderSet.prototype.getLabel = function () {
    return null;
  };
  /**
  ...
  CollectionHeaderSet.prototype.getCount = function () {
  ...
  CollectionHeaderSet.prototype.getStart = function () {
    return this.m_start;
  };
  /**
  ...
  CollectionHeaderSet.prototype.getEnd = function () {
    return this.m_end;
  };
  /**
  ...
  CollectionHeaderSet.prototype.getHeaders = function () {
  ...
  CollectionHeaderSet.prototype.getRowHeader = function () {
  ...
  exports.CollectionCellSet = CollectionCellSet;
  exports.CollectionDataGridDataSource = CollectionDataGridDataSource;
  exports.CollectionDataGridUtils = CollectionDataGridUtils;
  exports.CollectionHeaderSet = CollectionHeaderSet;
  exports.default = CollectionDataGridDataSource$1;
