 ...
 * @ignore
 */
define(['ojs/ojcore-base', 'ojs/ojrowexpander', 'ojs/ojdatasource-common'], function (oj$1, ojrowexpander, ojdatasourceCommon) {
  'use strict';
  ...
  var FlattenedTreeCellSet = function FlattenedTreeCellSet(startRow, endRow, startColumn, endColumn, nodeSet, columns) {
  ...
  oj$1._registerLegacyNamespaceProp('FlattenedTreeCellSet', FlattenedTreeCellSet);
  /**
   ...
   * @method
   * @instance
   * @memberof! FlattenedTreeCellSet
   */


  FlattenedTreeCellSet.prototype.getData = function (indexes) {
    ...
    oj$1.Assert.assert(row < this.m_nodeSet.getStart() + this.m_nodeSet.getCount() && column < this.m_columns.length);
  ...
  FlattenedTreeCellSet.prototype.getMetadata = function (indexes) {
    // convert to relative index
    var relIndex = this._getRelIndexes(indexes);
  ...
  FlattenedTreeCellSet.prototype._getRelIndexes = function (indexes) {
    ...
    oj$1.Assert.assertNumber(row, null);
    oj$1.Assert.assertNumber(column, null);
    oj$1.Assert.assert(row >= 0 && column >= 0);
    return [row, column];
  };
  /**
  ...
  FlattenedTreeCellSet.prototype.getStart = function (axis) {
    if (axis === 'row') {
  ...
  FlattenedTreeCellSet.prototype.getCount = function (axis) {
    if (axis === 'row') {
  ...
  var FlattenedTreeDataGridDataSource = function FlattenedTreeDataGridDataSource(treeDataSource, options) {
    FlattenedTreeDataGridDataSource.superclass.constructor.call(this, treeDataSource, options);
  };

  oj$1._registerLegacyNamespaceProp('FlattenedTreeDataGridDataSource', FlattenedTreeDataGridDataSource); // Subclass from FlattenedTreeDataSource
  ...
  FlattenedTreeDataGridDataSource.prototype.Init = function () {
    FlattenedTreeDataGridDataSource.superclass.Init.call(this);
    this.m_columns = FlattenedTreeDataGridDataSource.superclass.getOption.call(this, 'columns');
    this.m_rowHeader = FlattenedTreeDataGridDataSource.superclass.getOption.call(this, 'rowHeader');
  };
  /**
  ...
  FlattenedTreeDataGridDataSource.prototype.getCountPrecision = function (axis) {
  ...
  FlattenedTreeDataGridDataSource.prototype.getCount = function (axis) {
  ...
  FlattenedTreeDataGridDataSource.prototype.fetchHeaders = function (headerRange, callbacks, callbackObjects) {
    var headerSet;
    ...
    if (axis === 'column') {
      ...
      headerSet = new oj$1.FlattenedTreeHeaderSet(headerRange.start, columnCount, this.m_columns);
    } else if (axis === 'row') {
  ...
  FlattenedTreeDataGridDataSource.prototype.fetchCells = function (cellRanges, callbacks, callbackObjects) {
    var rowStart;
  ...
  FlattenedTreeDataGridDataSource.prototype.keys = function (indexes) {
    ...
    return new Promise(function (resolve) {
      ...
      if (rowIndex > FlattenedTreeDataGridDataSource.superclass.getFetchedRange.call(this).end || colIndex > this.m_columns.length) {
        resolve(null);
      } else {
        resolve({
  ...
  FlattenedTreeDataGridDataSource.prototype.indexes = function (keys) {
    var rowKey = keys.row;
    var colKey = keys.column;
    return new Promise(function (resolve) {
      // call helper method to find the flattened index
      var rowIndex = FlattenedTreeDataGridDataSource.superclass.getIndex.call(this, rowKey); // for column index, just search through the column keys array
  ...
  FlattenedTreeDataGridDataSource.prototype.sort = function (criteria, callbacks, callbackObjects) {
    var dataSource = FlattenedTreeDataGridDataSource.superclass.getWrappedDataSource.call(this); // delegates to the underlying TreeDataSource but intercept the success callback so that we can clear the cache
  ...
  FlattenedTreeDataGridDataSource.prototype._handleSortSuccess = function (callbacks, callbackObjects) {
    // reset state
  ...
  FlattenedTreeDataGridDataSource.prototype.move = function (rowToMove, referenceRow, position, callbacks) {
    var dataSource = FlattenedTreeDataGridDataSource.superclass.getWrappedDataSource.call(this); // delegates to the underlying TreeDataSource.  TreeDataSource is responsible for firing the appropriate model change
  ...
  FlattenedTreeDataGridDataSource.prototype.getCapability = function (feature) {
    var dataSource = FlattenedTreeDataGridDataSource.superclass.getWrappedDataSource.call(this); // just delegates to the underlying TreeDataSource
  ...
  FlattenedTreeDataGridDataSource.prototype.insertMetadata = function (key, metadata) {
    // just call super
    FlattenedTreeDataGridDataSource.superclass.insertMetadata.call(this, key, metadata);
  };
  /**
  ...
  FlattenedTreeDataGridDataSource.prototype._handleFetchRowsSuccess = function (nodeSet, cellRanges, callbacks, callbackObjects) {
    var rowStart;
    var rowCount;
    var columnStart;
    ...
    var cellSet = new oj$1.FlattenedTreeCellSet(rowStart, rowStart + rowCount, columnStart, columnStart + columnCount, nodeSet, this.m_columns); // invoke success callback
  ...
  FlattenedTreeDataGridDataSource.prototype._handleFetchRowsError = function (status, range, callbacks, callbackObjects) {
    // checks whether there is an outstanding fetch header request with the same range
    if (this.m_fetchHeaderRequest) {
      var headerRange = this.m_fetchHeaderRequest.range;
  ...
  FlattenedTreeDataGridDataSource.prototype._handleRowHeaderFetchSuccess = function (nodeSet, headerRange, callbacks, callbackObjects) {
    // create wrapper
    var headerSet = new oj$1.FlattenedTreeHeaderSet(headerRange.start, headerRange.start + headerRange.count, this.m_columns, nodeSet, this.m_rowHeader); // invoke success callback
  ...
  FlattenedTreeDataGridDataSource.prototype.insertRows = function (insertAtIndex, insertAtRowKey, nodeSet) {
    var headerSet = null;

    if (this.m_rowHeader) {
      headerSet = new oj$1.FlattenedTreeHeaderSet(insertAtIndex, insertAtIndex + nodeSet.getCount(), this.m_columns, nodeSet, this.m_rowHeader);
    } // create a CellSet that wraps around a RowSet


    var cellSet = new oj$1.FlattenedTreeCellSet(insertAtIndex, insertAtIndex + nodeSet.getCount(), 0, this.m_columns.length, nodeSet, this.m_columns);
    var keys = [];
    var indexes = [];
    ...
    FlattenedTreeDataGridDataSource.superclass.handleEvent.call(this, 'change', event);
  };
  /**
  ...
  FlattenedTreeDataGridDataSource.prototype.removeRows = function (rowKeys) {
    // extract the keys
    var keys = [];
    var indexes = [];
  ...
  FlattenedTreeDataGridDataSource.prototype.handleMaxCountReached = function (range, callbacks) {
    var empty = new oj$1.EmptyNodeSet(null, range.start);
  ...
  var FlattenedTreeHeaderSet = function FlattenedTreeHeaderSet(start, end, headers, nodeSet, rowHeader) {
  ...
  oj$1._registerLegacyNamespaceProp('FlattenedTreeHeaderSet', FlattenedTreeHeaderSet);
  /**
   ...
   * @method
   * @instance
   * @memberof! FlattenedTreeHeaderSet
   */


  FlattenedTreeHeaderSet.prototype.getData = function (index, level) {
  ...
  FlattenedTreeHeaderSet.prototype.getMetadata = function (index, level) {
    var data;
    oj$1.Assert.assert(index <= this.m_end && index >= this.m_start, 'index out of bounds');
  ...
  FlattenedTreeHeaderSet.prototype.getCount = function () {
    if (this.m_rowHeader != null && this.m_nodeSet != null) {
  ...
  FlattenedTreeHeaderSet.prototype.getLevelCount = function () {
    if (this.getCount() > 0) {
      return 1;
    }
  ...
  FlattenedTreeHeaderSet.prototype.getExtent = function (index, level) {
    oj$1.Assert.assert(index <= this.m_end && index >= this.m_start, 'index out of bounds');
    oj$1.Assert.assert(level == null || level === 0, 'level out of bounds');
    return {
      extent: 1,
  ...
  FlattenedTreeHeaderSet.prototype.getDepth = function (index, level) {
    oj$1.Assert.assert(index <= this.m_end && index >= this.m_start, 'index out of bounds');
    oj$1.Assert.assert(level == null || level === 0, 'level out of bounds');
    return 1;
  };
  ...
  FlattenedTreeHeaderSet.prototype.getLabel = function () {
    return null;
  };
  /**
   * @license
  ...
  FlattenedTreeDataGridDataSource$1.FlattenedTreeDataGridDataSource = oj.FlattenedTreeDataGridDataSource;
  FlattenedTreeDataGridDataSource$1.FlattenedTreeHeaderSet = oj.FlattenedTreeHeaderSet;
  FlattenedTreeDataGridDataSource$1.FlattenedTreeCellSet = oj.FlattenedTreeCellSet;
  return FlattenedTreeDataGridDataSource$1;
});
