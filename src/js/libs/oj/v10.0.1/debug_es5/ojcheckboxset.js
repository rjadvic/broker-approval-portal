 ...
 * @ignore
 */
define(['ojs/ojcomponentcore', 'ojs/ojeditablevalue', 'ojs/ojradiocheckbox', 'ojs/ojoption', 'ojs/ojcore-base', 'jquery', 'ojs/ojlogger', 'ojs/ojtranslation', 'ojs/ojlabelledbyutils'], function (ojcomponentcore, ojeditablevalue, ojradiocheckbox, ojoption, oj, $, Logger, Translations, LabeledByUtils) {
  'use strict';
    ...
    oj.__registerWidget('oj.ojCheckboxset', $.oj.editableValue, {
      ...
      */
      ...
      _ComponentCreate: function _ComponentCreate() {
        ...
        this.uiCheckboxset = $element.addClass('oj-checkboxset oj-component').attr('role', 'group'); // need to grab all elements using contents first and then do filter because jquery children will automatically
        // exclude all comment and text nodes

        var $childNodes = $element.contents().filter(function () {
        ...
        if ($childNodes.length > 0) {
          $childNodes.wrapAll("<div class='".concat(this._WRAPPER_CLASS_NAMES, "'></div>")); // @HTMLUpdateOK
        } else {
          $element.append("<div class='".concat(this._WRAPPER_CLASS_NAMES, "'></div>")); // @HTMLUpdateOK
      ...
      _processOjOptions: function _processOjOptions() {
        ...
        if (this._IsCustomElement()) {
          ...
          var renderer = this._customOptionRenderer.bind(this); // Get all the oj-option elements of the oj-checkboxset
          // Those that are direct child as well as those inside the wrapperDOM


          var options = this.element.children('oj-option').add(this.element.children('.oj-checkboxset-wrapper').find('oj-option'));
          var selectedOptionsArray = sortValuesInOptionsOrder(this.options.value, options);
          var numSelected = selectedOptionsArray.length; // If the wrapperDom isn't created yet, we use the component element as the wrapper

          if (!wrapperDom) {
            wrapperDom = domElem;
          }

          var novaluespan = domElem.querySelector('[data-no-value-span]'); // Remove the old no-value span when there is one and there is a current selection or not in readonly mode
       ...
       * @private
       */
      _processReadonlyOptions: function _processReadonlyOptions(ojOption, selectedOptionsArray) {
        var optionValue = ojOption.value;
        var selectedArrayLength = selectedOptionsArray.length;
        var element = this.element.get(0);
        var choiceItemSelector = 'span.oj-choice-item';
        var parentSpan = $(ojOption).parentsUntil(element, choiceItemSelector).get(0); // If the provided ojOption is present inside an oj-choice-item, it means
      ...
      */
      _initInputLabelFromOjOption: function _initInputLabelFromOjOption(elem) {
        ...
        if (!alreadyProcessed) {
          ...
          checkbox.setAttribute('id', checkboxId); // in readonly mode, if a option is selected, the <oj-option> will be surrounded by <label> tag
          ...
          if (ojoption.parentElement.nodeName === 'LABEL') {
            ...
            label.parentElement.insertBefore(checkbox, label); // remove 'aria-readonly' when toggle readonly to false

            label.parentElement.removeAttribute('aria-readonly'); // also we need to remove the oj-helper-hidden class from the parentElement
      ...
      _initReadonlyLabelFromOjOption: function _initReadonlyLabelFromOjOption(elem, parentSpan, isLastOption) {
        ...
        if (parentSpan) {
          ...
          if (checkboxExists) {
            checkbox.parentElement.classList.add('oj-helper-hidden'); // when we toggle readonly to true, we need to remove the checkbox element.

            var oldlabel = checkbox.parentElement.nextElementSibling;

            if (oldlabel !== null) {
              label.appendChild(ojoption); // Before removing the checkbox element, check if _ojRadioCheckbox is initialized
      ...
      _events: {
        ...
        'click .oj-checkboxset-wrapper': function clickOjCheckboxsetWrapper(event) {
          if (!this.widget()[0].classList.contains('oj-choice-direction-row') && event.target.tagName !== 'INPUT') {
        ...
        switch (key) {
          ...
          case 'readOnly':
            ...
            if (this.options.readOnly) {
              ...
              wrapperDom.removeAttribute('tabindex'); // remove oj-read-only class

              this.element.removeClass('oj-read-only');
            }

            this._ResetComponentState(); // when toggle readonly to false, we need to check the initial set values.
