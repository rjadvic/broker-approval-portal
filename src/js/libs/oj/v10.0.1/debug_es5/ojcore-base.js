 ...
 */
 ...
 define(['polyfills', 'ojs/ojlogger'], function (polyfills, Logger) {
  'use strict';
  /**
   * @license
   * Copyright (c) 2014, 2021, Oracle and/or its affiliates.
  ...
  var AgentUtils = function AgentUtils() {};
  ...
  AgentUtils.getAgentInfo = function (userAgent) {
    ...
    if (userAgent.indexOf('iphone') > -1 || userAgent.indexOf('ipad') > -1 || navigator.platform === 'MacIntel' && typeof navigator.standalone !== 'undefined') {
    ...
    if (os === AgentUtils.OS.ANDROID) {
      ...
      deviceType = userAgent.indexOf('mobile') > -1 ? AgentUtils.DEVICETYPE.PHONE : AgentUtils.DEVICETYPE.TABLET;
      ...
      deviceType = userAgent.indexOf('iphone') > -1 ? AgentUtils.DEVICETYPE.PHONE : AgentUtils.DEVICETYPE.TABLET;
    }
    ...
    if (Assert[_DEBUG]) {
      ...
      if (target != null) {
        ...
        if (!isPrototypeOf.call(thePrototype, target)) {
          Assert.assertionFailed("object '" + target + "' doesn't match prototype " + thePrototype, 1, reason);
        }
      } else {
  ...
  Assert.assertPrototypes = function (target, instanceOne, instanceTwo, reason) {
    if (Assert[_DEBUG]) {
      ...
      if (!(isPrototypeOf.call(thePrototype, target) || isPrototypeOf.call(thePrototypeTwo, target))) {
        Assert.assertionFailed("object '" + target + "' doesn't match prototype " + thePrototype + ' or ' + thePrototypeTwo, 1, reason);
      }
    }
  };
  /**
  ...
  (function () {
    var agentInfo = AgentUtils.getAgentInfo();
  ...
  CollectionUtils.mergeDeep = function (target) {
    for (var _len = arguments.length, sources = new Array(_len > 1 ? _len - 1 : 0), _key = 1; _key < _len; _key++) {
      sources[_key - 1] = arguments[_key];
    }

    if (!sources.length) return target;
    var isPlain = CollectionUtils.isPlainObject;
    var merge = CollectionUtils.mergeDeep;
  ...
  CollectionUtils._copyIntoImpl = function (target, source, keyConverter, recurse, maxRecursionDepth, currentLevel) {
    var targetKey;
    ...
    if (target && source && target !== source) {
      ...
      for (var i = 0; i < keys.length; i++) {
        ...
        if (recurse && currentLevel < maxRecursionDepth) {
          ...
          if (CollectionUtils.isPlainObject(sourceVal) && (targetVal == null || CollectionUtils.isPlainObject(targetVal))) {
            recursed = true; // eslint-disable-next-line no-param-reassign

            target[targetKey] = targetVal || {};

            CollectionUtils._copyIntoImpl(target[targetKey], sourceVal, keyConverter, true, maxRecursionDepth, currentLevel + 1);
          }
        }
  ...
  var OjObject = function OjObject() {
    this.Init();
  };
  ...
  OjObject._typeName = 'oj.Object'; // regular expressicloneon for stripping out the name of a function

  /**
   * @private
   */

  OjObject._GET_FUNCTION_NAME_REGEXP = /function\s+([\w$][\w$\d]*)\s*\(/; // oj.Object._TRIM_REGEXP = /(^\s*)|(\s*$)/g; this.replace(/(^\s*)|(\s*$)/g, "");
  ...
  OjObject.createSubclass = function (extendingClass, baseClass, typeName) {
    ...
    Assert.assert(extendingClass !== baseClass, "Class can't extend itself"); // use a temporary constructor to get our superclass as our prototype
  ...
  OjObject.createCallback = function (obj, func) {
    Assert.assertFunction(func); // All browsers supported by JET support bind() method
  ...
  var __isAmdLoaderPresent = function __isAmdLoaderPresent() {
