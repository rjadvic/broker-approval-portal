 ...
 * @ignore
 */
define(['exports', 'ojs/ojcore-base', 'ojs/ojhtmlutils', 'ojs/ojlogger', 'ojs/ojmetadatautils', 'ojs/ojcomposite-knockout', 'ojs/ojcustomelement', 'ojs/ojcustomelement-utils'], function (exports, oj, HtmlUtils, Logger, MetadataUtils, ojcompositeKnockout, ojcustomelement, ojcustomelementUtils) {
  'use strict';
  ...
  */
  ...
  CompositeInternal.getContainingComposite = function (node, stopBelow) {
    ...
    while (_node) {
      _node = oj.CompositeTemplateRenderer.getEnclosingComposite(_node); // : we should ignore oj-module component since it is not a relevant enclosing composite for this call

      if (_node && _node.nodeName.toLowerCase() !== 'oj-module') {
        // eslint-disable-next-line no-bitwise
        if (stopBelow && !(node.compareDocumentPosition(stopBelow) & 16
        /* contained by*/
        )) {
          break;
        }
  ...
  var CompositeState = /*#__PURE__*/function (_ojcustomelementUtils) {
    _inherits(CompositeState, _ojcustomelementUtils);

    var _super = _createSuper(CompositeState);

    function CompositeState() {
      _classCallCheck(this, CompositeState);

      return _super.apply(this, arguments);
    }

    _createClass(CompositeState, [{
  ...
  oj.CollectionUtils.copyInto(CompositeElementBridge.proto, {
    beforePropertyChangedEvent: function beforePropertyChangedEvent(element, property, detail) {
      ...
      oj.CompositeTemplateRenderer.invokeViewModelMethod(element, this._VIEW_MODEL, 'propertyChanged', [vmContext]);
    },
    AddComponentMethods: function AddComponentMethods(proto) {
      // Add subproperty getter/setter
      var setPropertyHelper = function setPropertyHelper(element, bridge, prop, value, propertyBag, isOuterSet) {
        if (!bridge.SaveEarlyPropertySet(element, prop, value)) {
          ...
          if (setResult.propertySet) {
            if (setResult.isSubproperty) {
              ...
              var propertyTracker = CompositeElementBridge._getPropertyTracker(bridge, setResult.property);
    ...
    CreateComponent: function CreateComponent(element) {
      ...
      var slotNodeCounts = {}; // Generate slot map before we update DOM with view nodes
      ...
      var model = ojcustomelementUtils.CustomElementUtils.getElementDescriptor(element.tagName)[CompositeElementBridge.DESC_KEY_VIEW_MODEL];
      ...
      if (typeof model === 'function') {
        ...
        model = oj.CompositeTemplateRenderer.invokeViewModelMethod(element, model, 'initialize', [vmContext]) || model;
      }

      this._VIEW_MODEL = model; // This method can return a Promise which will delay additional lifecycle phases until it is resolved.

      var activatedPromise = oj.CompositeTemplateRenderer.invokeViewModelMethod(element, model, 'activated', [vmContext]) || Promise.resolve(true);
      var bridge = this;
      return activatedPromise.then(function () {
        ...
        Object.defineProperty(element, CompositeInternal.__BINDING_PROVIDER, {
          value: 'knockout'
        ...
        var cache = ojcustomelementUtils.CustomElementUtils.getElementRegistration(element.tagName).cache; // Need to clone nodes first
    ...
    DefinePropertyCallback: function DefinePropertyCallback(proto, property, propertyMeta) {
      var set = function set(value, bOuterSet) {
      ...
      var innerSet = function innerSet(value) {
        set.bind(this)(value, false);
      }; // Called on the custom element


      var outerSet = function outerSet(value) {
        var bridge = ojcustomelementUtils.CustomElementUtils.getElementBridge(this);
        set.bind(bridge._PROPS)(value, true);
      };

      var get = function get(bOuterSet) {
        var propertyTracker = CompositeElementBridge._getPropertyTracker(this._BRIDGE, property); // If the attribute has not been set, return the default value
      ...
      var outerGet = function outerGet() {
        var bridge = ojcustomelementUtils.CustomElementUtils.getElementBridge(this);
    ...
    HandleDetached: function HandleDetached(element) {
      ...
      oj.BaseCustomElementBridge.proto.HandleDetached.call(this, element); // Detached is deprecated in 4.2.0 for disconnected

      oj.CompositeTemplateRenderer.invokeViewModelMethod(element, this._VIEW_MODEL, 'detached', [element]);
      oj.CompositeTemplateRenderer.invokeViewModelMethod(element, this._VIEW_MODEL, 'disconnected', [element]);
    },
    HandleReattached: function HandleReattached(element) {
      // Invoke callback on the superclass
      oj.BaseCustomElementBridge.proto.HandleReattached.call(this, element);
      oj.CompositeTemplateRenderer.invokeViewModelMethod(element, this._VIEW_MODEL, 'connected', [this._VM_CONTEXT]);
    },
    InitializeElement: function InitializeElement(element) {
      // Invoke callback on the superclass
      oj.BaseCustomElementBridge.proto.InitializeElement.call(this, element); // For upstream or indirect dependency we will still rely components being registered on the oj namespace.

      if (oj.Components) {
        oj.Components.markPendingSubtreeHidden(element);
      }

      var descriptor; // Cache the View
      ...
      if (!cache.view) {
        ...
        var view = descriptor[CompositeElementBridge.DESC_KEY_VIEW]; // when multiple instances of the same CCA are on the same page, because of the async
      ...
      if (!cache.css) {
        ...
        var css = descriptor[CompositeElementBridge.DESC_KEY_CSS]; // CSS is optional so we need to check if it was provided
    ...
    _getNodeBySubId: function _getNodeBySubId(locator) {
      ...
      if (match) {
        if (match.alias) {
          ...
          var component = match.node; // Check to see if we should call the method on the element or widget
    ...
    _getSubIdByNode: function _getSubIdByNode(node) {
      ...
      while (curNode !== this) {
        ...
        nodeKey = curNode.getAttribute('data-oj-subid-map') || curNode.getAttribute('data-oj-subid');
    ...
    _getViewModel: function _getViewModel() {
      if (!this._VIEW_MODEL) {
  ...
  CompositeElementBridge.register = function (tagName, descriptor) {
    var descrip = {};
    descrip[oj.BaseCustomElementBridge.DESC_KEY_META] = CompositeElementBridge._getResource(tagName, descriptor, oj.BaseCustomElementBridge.DESC_KEY_META);
    descrip[CompositeElementBridge.DESC_KEY_VIEW] = CompositeElementBridge._getResource(tagName, descriptor, CompositeElementBridge.DESC_KEY_VIEW);
    descrip[CompositeElementBridge.DESC_KEY_CSS] = CompositeElementBridge._getResource(tagName, descriptor, CompositeElementBridge.DESC_KEY_CSS);
    descrip[CompositeElementBridge.DESC_KEY_VIEW_MODEL] = CompositeElementBridge._getResource(tagName, descriptor, CompositeElementBridge.DESC_KEY_VIEW_MODEL);
    descrip[CompositeElementBridge.DESC_KEY_PARSE_FUN] = descriptor[CompositeElementBridge.DESC_KEY_PARSE_FUN];
    var registration = {
      descriptor: descrip,
    ...
    if (ojcustomelementUtils.CustomElementUtils.registerElement(tagName, registration)) {
      ...
      customElements.define(tagName, CompositeElementBridge.proto.getClass(descrip));
    }
  };
  /** ***************************/
  ...
  CompositeElementBridge._getDomNodes = function (content, element) {
    ...
    var bridge = ojcustomelementUtils.CustomElementUtils.getElementBridge(element); // TODO update this error message once we remove support for Array of DOM nodes and DocumentFragment

    bridge.State.throwError('The composite View is not one of the following supported types: string, Array of DOM nodes, DocumentFragment');
  ...
  CompositeElementBridge._generateSubIdMap = function (bridge, element) {
    if (!bridge._SUBID_MAP) {
      ...
      var subIdMap = {};
      var nodeMap = {}; // data-oj-subid or data-oj-subid-map attributes can be defined on nested objects so we need
      ...
      bridge._NODE_MAP = nodeMap; // eslint-disable-next-line no-param-reassign
  ...
  CompositeElementBridge._walkSubtree = function (subIdMap, nodeMap, node) {
    if (!node.hasAttribute('slot')) {
      CompositeElementBridge._addNodeToSubIdMap(subIdMap, nodeMap, node); // For upstream or indirect dependency we will still rely components being registered on the oj namespace.


      if (!ojcustomelementUtils.CustomElementUtils.isElementRegistered(node.tagName) && !oj.Components.__GetWidgetConstructor(node)) {
  ...
  CompositeElementBridge._addNodeToSubIdMap = function (subIdMap, nodeMap, node) {
    ...
    var nodeSubIdMapStr = node.getAttribute('data-oj-subid-map'); // We do not support an element having both attributes. If both are specified, -map takes precedence.

    if (nodeSubIdMapStr) {
      var parsedValue = JSON.parse(nodeSubIdMapStr);

      if (_typeof(parsedValue) === 'object' && !(parsedValue instanceof Array)) {
  ...
  CompositeElementBridge._getResource = function (tagName, descriptor, key) {
    ...
    if (resource != null) {
      ...
      if (hasOwnProperty.call(resource, 'inline')) {
        ...
        throw new Error('Error while registering ' + tagName + ". The resource type for descriptor key '" + key + "' is no longer supported." + ' The resource should be passed directly as the value instead.');
      } else {
        return resource;
      }
    }
  ...
  var Composite = {}; // this is the variable name that the AMD module will return in the require callback (used in a no-require environment)
  ...
  */
  ...
  var register = Composite.register;
  var getMetadata = Composite.getMetadata;
  var getComponentMetadata = Composite.getComponentMetadata;
  var getContainingComposite = Composite.getContainingComposite;
  var __COMPOSITE_PROP = Composite.__COMPOSITE_PROP;
