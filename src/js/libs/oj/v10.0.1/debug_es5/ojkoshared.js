 ...
 * @ignore
 */
define(['ojs/ojcore-base', 'knockout', 'ojs/ojconfig', 'ojs/ojlogger'], function (oj, ko, Config, Logger) {
  'use strict';
  ...
  var GlobalChangeQueue = function GlobalChangeQueue() {
    this.Init();
  ...
  GlobalChangeQueue.prototype.registerComponentChanges = function (tracker) {
    if (this._trackers.indexOf(tracker) === -1) {
      ...
      if (!this._delayTimer) {
        ...
        this._delayPromise = new Promise(function (resolve) {
  ...
  function _patchKoEvaluatorForCSP(cache) {
    ...
    if (!patched) {
      Logger.error('Unable to patch KO expression evaluation implementation. ' + 'If you have a custom binding provider, make sure it implements the getWrapped() method that returns the default binding provider instance.');
    }
  }
  ...
  function _getParseBindingsReplacement(original, cache) {
    return function (bindingsString, bindingContext, node, options) {
      ...
      return evaluate([bindingContext.$data || {}, bindingContext, {
        $element: node
      }]);
    };
  }
  ...
  function _createKoEvaluatorViaCache(expression, options, factory, cache) {
    ...
    if (!evaluate) {
      ...
      evaluate = factory.createEvaluator('{' + rewrittenBindings + '}').evaluate; // eslint-disable-next-line no-param-reassign
