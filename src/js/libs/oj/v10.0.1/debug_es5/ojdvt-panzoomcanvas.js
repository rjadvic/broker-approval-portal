...
define(['exports', 'ojs/ojdvt-toolkit'], function (exports, dvt) {
  ...
  var PanZoomCanvasEventManager = function PanZoomCanvasEventManager(context, callback, callbackObj) {
    this.Init(context, callback, callbackObj);
  };

  dvt.Obj.createSubclass(PanZoomCanvasEventManager, dvt.EventManager);
  /**
   * @const
   */

  PanZoomCanvasEventManager.EVENT_INFO_PANNED_KEY = 'panned';
  /**
   * @param {dvt.Context} context The rendering context
   * @param {function} callback The callback for this PanZoomCanvasEventManager
   * @param {Object} callbackObj The callback object for this PanZoomCanvasEventManager
  ...
  PanZoomCanvasEventManager.prototype.Init = function (context, callback, callbackObj) {
    PanZoomCanvasEventManager.superclass.Init.call(this, context, callback, callbackObj);
    this._pzc = callbackObj;
    this._zoomAnimator = null;
    this._bPanning = false;
    this._bPanned = false;
    this._bZooming = false;
    this._bDragging = false; //momentum-based panning (turned on for both touch and desktop)
  ...
  PanZoomCanvasEventManager._PAN_TOUCH_KEY = 'panTouch';
  /**
   * Key used saving zooming touch events in the touch manager
   * @private
   */

  PanZoomCanvasEventManager._ZOOM_TOUCH_KEY = 'zoomTouch';
  /**
   * Used for the momentum timer interval
   * @private
   */

  PanZoomCanvasEventManager._MOMENTUM_START_TIMER_INTERVAL = 50;
  /**
   * @override
   */

  PanZoomCanvasEventManager.prototype.addListeners = function (displayable) {
    PanZoomCanvasEventManager.superclass.addListeners.call(this, displayable);
  ...
  PanZoomCanvasEventManager.prototype.RemoveListeners = function (displayable) {
    PanZoomCanvasEventManager.superclass.RemoveListeners.call(this, displayable);
  ...
  PanZoomCanvasEventManager.prototype.OnMouseDown = function (event) {
    ...
    if (event.button != dvt.MouseEvent.RIGHT_CLICK_BUTTON) {
      ...
      this._px = this._mx;
      this._py = this._my;
      this._bDown = true; //for elastic constraints, save the original values
  ...
  PanZoomCanvasEventManager.prototype.OnMouseMove = function (event) {
    // Fixes for:
    ...
    if (this._bDown) {
      ...
      if (event.ctrlKey) {
        if (!this._bZooming) {
          ...
          this._bZooming = true; //turn on elastic constraints for the duration of this drag
  ...
  PanZoomCanvasEventManager.prototype.OnMouseUp = function (event) {
    this.PanZoomEnd();
    PanZoomCanvasEventManager.superclass.OnMouseUp.call(this, event);
  };
  /**
   * @override
   */
  ...
  PanZoomCanvasEventManager.prototype.OnClick = function (event) {
  ...
  PanZoomCanvasEventManager.prototype.OnMouseOut = function (event) {
    ...
    PanZoomCanvasEventManager.superclass.OnMouseOut.call(this, event);
  };
  /**
   * @protected
  ...
  PanZoomCanvasEventManager.prototype.OnMouseWheel = function (event) {
    if (!event.wheelDelta || event.wheelDelta === 0 || !this._callbackObj.isZoomingEnabled()) {
      return;
    }

    var currZoom = this._callbackObj.getZoom(); //TODO: re-enable animation after fixing how it works in conjunction with overview window
    ...
    if (this._zoomAnimator) {
      var oldZoomAnim = this._zoomAnimator; //stop the old animator

      this._zoomAnimator.stop(); //get the destination zoom level of the old animator so that we can add to it


      currZoom = this._callbackObj.getZoom(oldZoomAnim);
      this._zoomAnimator = null;
      oldZoomAnim = null; //change the easing function so that it's fast at the start to
    ...
    var delta = event.wheelDelta; //: divide by the absolute value of the delta so that the zoom only changes by the increment,
    //just like in native Flash

    var zz = currZoom * (1 + this._callbackObj.getZoomIncrement() * delta / Math.abs(delta));

    var pos = this._callbackObj.GetRelativeMousePosition(event); //cancel the mouse wheel event so that the browser doesn't scroll the page


    event.stopPropagation();
    event.preventDefault(); //elastic constraints don't work right for mouse wheel zooming
  ...
  PanZoomCanvasEventManager.prototype._clearZoomAnimator = function () {
    this._zoomAnimator = null;
  };
  /**
  ...
  PanZoomCanvasEventManager.prototype._handleMomentumStartTimer = function () {//do nothing
  };
  /**
  ...
  PanZoomCanvasEventManager.prototype._handleMomentumTimer = function () {
    ...
    var interval = this._momentumTimer.getInterval(); //PanZoomCanvasEventManager._MOMENTUM_START_TIMER_INTERVAL;
    //deltas to pan by for this iteration


    var dx = ratio * this._momentumXperMS * interval;
    var dy = ratio * this._momentumYperMS * interval; //add deltas to accumulated values

    this._momentumDx += dx;
    this._momentumDy += dy; //add accumulated deltas to the last drag pan position
    ...
    var bStop = false; //stop just before ratio goes to 0
    ...
    if (this._momentumIterCount >= 1 / fraction - 1) {
      ...
      var currY = this._callbackObj.getPanY(); //if the difference between the desired and actual pan positions is greater than the delta for this timer
      //iteration, we must be hitting the elastic constraints, so stop iterating


      var panDirection = this._pzc.getPanDirection();

      if (Math.abs(currX - newX) > Math.abs(dx) && panDirection != 'y' || Math.abs(currY - newY) > Math.abs(dy) && panDirection != 'x') {
        bStop = true;
      }
    }
    ...
    if (bStop) {
      ...
      this._momentumTimer.reset(); //turn off elastic constraints when momentum stops
  ...
  PanZoomCanvasEventManager.prototype.HandleImmediateTouchStartInternal = function (event) {
    if (this._callbackObj.isZoomingEnabled()) this.TouchManager.processAssociatedTouchAttempt(event, PanZoomCanvasEventManager._ZOOM_TOUCH_KEY, this.ZoomStartTouch, this);
    if (this._callbackObj.isPanningEnabled()) this.TouchManager.processAssociatedTouchAttempt(event, PanZoomCanvasEventManager._PAN_TOUCH_KEY, this.PanStartTouch, this);
  };
  /**
   * @override
   */
  ...
  PanZoomCanvasEventManager.prototype.HandleImmediateTouchMoveInternal = function (event) {
    if (this._callbackObj.isZoomingEnabled()) this.TouchManager.processAssociatedTouchMove(event, PanZoomCanvasEventManager._ZOOM_TOUCH_KEY);
    if (this._callbackObj.isPanningEnabled()) this.TouchManager.processAssociatedTouchMove(event, PanZoomCanvasEventManager._PAN_TOUCH_KEY); //  Only prevent default browser behavior if panning or zooming is enabled

    if (this._callbackObj.isZoomingEnabled() || this._callbackObj.isPanningEnabled()) event.preventDefault();
  };
  /**
   * @override
   */
  ...
  PanZoomCanvasEventManager.prototype.HandleImmediateTouchEndInternal = function (event) {
    if (this._callbackObj.isZoomingEnabled()) this.TouchManager.processAssociatedTouchEnd(event, PanZoomCanvasEventManager._ZOOM_TOUCH_KEY);
    if (this._callbackObj.isPanningEnabled()) this.TouchManager.processAssociatedTouchEnd(event, PanZoomCanvasEventManager._PAN_TOUCH_KEY);
  };
  /**
  ...
  PanZoomCanvasEventManager.prototype.ZoomStartTouch = function (event, touch) {
    var touchIds = this.TouchManager.getTouchIdsForObj(PanZoomCanvasEventManager._ZOOM_TOUCH_KEY);

    if (touchIds.length <= 1) {
      this.TouchManager.saveProcessedTouch(touch.identifier, PanZoomCanvasEventManager._ZOOM_TOUCH_KEY, null, PanZoomCanvasEventManager._ZOOM_TOUCH_KEY, PanZoomCanvasEventManager._ZOOM_TOUCH_KEY, this.ZoomMoveTouch, this.ZoomEndTouch, this);
      this._mx = touch.pageX;
      this._my = touch.pageY;
      this._px = this._mx;
      this._py = this._my; //for elastic constraints, save the original values

      this._origPanX = this._callbackObj.getPanX();
      this._origPanY = this._callbackObj.getPanY();
      this._origZoom = this._callbackObj.getZoom(); //: save the original finger distance

      this._origDist = null;
      touchIds = this.TouchManager.getTouchIdsForObj(PanZoomCanvasEventManager._ZOOM_TOUCH_KEY);
  ...
  PanZoomCanvasEventManager.prototype.ZoomMoveTouch = function (event, touch) {
    var touchIds = this.TouchManager.getTouchIdsForObj(PanZoomCanvasEventManager._ZOOM_TOUCH_KEY);
    ...
    if (touchIds.length == 2) {
      ...
      if (data) {
        ...
        var zz = this._origZoom * (data.dist / this._origDist); //the center point of the zoom needs to be converted to stage-relative coords
        ...
        this._callbackObj.zoomTo(zz, cp.x, cp.y); //the center point delta is in page coords, which should be okay for panning
  ...
  PanZoomCanvasEventManager.prototype.ZoomEndTouch = function (event, touch) {
    if (this._bZooming) {
    ...
    if (touchIds.length == 0) this._callback.call(this._callbackObj, dvt.EventFactory.newZoomEvent('zoomEnd', this._callbackObj.getZoom(), this._origZoom));
  };
  /**
  ...
  PanZoomCanvasEventManager.prototype.PanStartTouch = function (event, touch) {
    if (!this._bZooming) {
      var touchIds = this.TouchManager.getTouchIdsForObj(PanZoomCanvasEventManager._PAN_TOUCH_KEY);

      if (touchIds.length <= 1) {
        this.TouchManager.saveProcessedTouch(touch.identifier, PanZoomCanvasEventManager._PAN_TOUCH_KEY, null, PanZoomCanvasEventManager._PAN_TOUCH_KEY, PanZoomCanvasEventManager._PAN_TOUCH_KEY, this.PanMoveTouch, this.PanEndTouch, this);
        this._mx = touch.pageX;
        this._my = touch.pageY;
        this._px = this._mx;
        this._py = this._my; //for elastic constraints, save the original values
  ...
  PanZoomCanvasEventManager.prototype.PanMoveTouch = function (event, touch) {
    if (!this._bZooming) {
      var touchIds = this.TouchManager.getTouchIdsForObj(PanZoomCanvasEventManager._PAN_TOUCH_KEY);
  ...
  PanZoomCanvasEventManager.prototype.PanEndTouch = function (event, touch) {
  ...
  PanZoomCanvasEventManager.prototype._handleZoomEnd = function () {
    this._callback.call(this._callbackObj, dvt.EventFactory.newZoomEvent('dragZoomEnd'));

    this._bZooming = false; //turn off elastic constraints, which will animate a bounce back to constrained values, if necessary
  ...
  PanZoomCanvasEventManager.prototype._handlePanMove = function (xx, yy) {
    if (!this._bPanning) {
      this._callback.call(this._callbackObj, dvt.EventFactory.newPanEvent('dragPanBegin'));

      this._bPanning = true; //turn on elastic constraints for the duration of this drag
    ...
    if (this._bMomentumPanning) {
      ...
      this._currTime = new Date().getTime(); //create or reset the timer to determine if we should start momentum-based panning
      ...
      if (this._momentumStartTimer) {
        ...
        this._momentumStartTimer = new dvt.Timer(this._context, PanZoomCanvasEventManager._MOMENTUM_START_TIMER_INTERVAL, this._handleMomentumStartTimer, this, 1);
  ...
  PanZoomCanvasEventManager.prototype._handlePanEnd = function () {
    this._callback.call(this._callbackObj, dvt.EventFactory.newPanEvent('dragPanEnd'));

    this._bPanning = false;
    this._bPanned = true; //if the momentum-based panning start timer is still running, that means that the mouseup happened
    ...
    if (this._momentumStartTimer && this._momentumStartTimer.isRunning()) {
      ...
      this._momentumStartTimer.reset(); //create or reset the momentum panning timer
      ...
      if (!this._momentumTimer) {
        ...
        this._momentumTimer = new dvt.Timer(this._context, PanZoomCanvasEventManager._MOMENTUM_START_TIMER_INTERVAL, this._handleMomentumTimer, this);
      } else {
        this._momentumTimer.reset();
      ...
      this._momentumYperMS = dy / dt; //use timer interval similar to mousemove interval
      ...
      this._momentumDx = 0;
      this._momentumDy = 0; //We disable and enable panning on DnD operations. The check should prevent momentum panning when panning is temporary disabled

      if (this._pzc.isPanningEnabled()) this._momentumTimer.start();
    } else {
  ...
  PanZoomCanvasEventManager.prototype.PanZoomEnd = function () {
    this._bDown = false;
    this._bDragging = false;
  ...
  PanZoomCanvasEventManager.prototype.GetTouchResponse = function () {
    if (this._pzc.isPanningEnabled() || this._pzc.isZoomingEnabled()) return dvt.EventManager.TOUCH_RESPONSE_TOUCH_HOLD;
  ...
  PanZoomCanvasEventManager.prototype.StoreInfoByEventType = function (key) {
  ...
  var PanZoomCanvas = function PanZoomCanvas(context, ww, hh, view) {
    this.Init(context, ww, hh, view);
  };

  dvt.Obj.createSubclass(PanZoomCanvas, dvt.Container);
  PanZoomCanvas.DEFAULT_PAN_INCREMENT = 15;
  PanZoomCanvas.DEFAULT_ZOOM_INCREMENT = .05;
  PanZoomCanvas.DEFAULT_ANIMATION_DURATION = .5;
  PanZoomCanvas.DEFAULT_PADDING = 20;

  PanZoomCanvas.prototype.Init = function (context, ww, hh, view) {
    PanZoomCanvas.superclass.Init.call(this, context);
    this._view = view;
    this._ww = ww;
    this._hh = hh;
    this._px = 0;
    ...
    this._minZoom = .1;
    this._maxZoom = 1;
    this._panIncrement = PanZoomCanvas.DEFAULT_PAN_INCREMENT;
    this._zoomIncrement = PanZoomCanvas.DEFAULT_ZOOM_INCREMENT;
    ...
    this._animationDuration = PanZoomCanvas.DEFAULT_ANIMATION_DURATION;
    this._eventManager = new PanZoomCanvasEventManager(context, this.FireListener, this);

    this._eventManager.addListeners(this);

    this._clipIdSuffix = 1;
    this.SetClipRect(ww, hh); //flag indicating if constraints should be treated as elastic, so that overflow is possible, with bounce back

    this._bElasticConstraints = false; //anim for bouncing back elastic constraints
  ...
  PanZoomCanvas.prototype.setSize = function (ww, hh, bAdjustSizeOnly) {
    this._ww = ww;
    this._hh = hh; // Thematic Map uses this method to update canvas width temporarily before a zoom to fit to position the map within
  ...
  PanZoomCanvas.prototype.getSize = function () {
  ...
  PanZoomCanvas.prototype.SetClipRect = function (ww, hh) {
  ...
  PanZoomCanvas.prototype.getContentPane = function () {
    return this._contentPane;
  };

  PanZoomCanvas.prototype.setContentPane = function (contentPane) {
  ...
  PanZoomCanvas.prototype.getBackgroundPane = function () {
  ...
  PanZoomCanvas.prototype.getContentPaneMatrix = function (animator) {
    if (animator) {
  ...
  PanZoomCanvas.prototype.getZoom = function (animator) {
  ...
  PanZoomCanvas.prototype.getPanX = function (animator) {
  ...
  PanZoomCanvas.prototype.getPanY = function (animator) {
  ...
  PanZoomCanvas.prototype.setZoomToFitPadding = function (n) {
  ...
  PanZoomCanvas.prototype.getZoomToFitPadding = function () {
  ...
  PanZoomCanvas.prototype.panBy = function (dx, dy, animator, panEndFunc) {
    ...
    var fireStartEventFunc = function fireStartEventFunc() {
      thisRef.FirePanEvent('panning', newX, newY, oldX, oldY, animator);
    };

    var fireEndEventFunc = function fireEndEventFunc() {
      thisRef.FirePanEvent('panned', newX, newY, oldX, oldY, animator);
    };

    if (animator) {
      animator.addProp(dvt.Animator.TYPE_MATRIX, this._contentPane, this._contentPane.getMatrix, this._contentPane.setMatrix, mat);
  ...
  PanZoomCanvas.prototype.panTo = function (xx, yy, animator) {
    if (!this.isPanningEnabled()) {
      return;
    }
  ...
  PanZoomCanvas.prototype.zoomBy = function (dz, xx, yy, animator) {
    ...
    mat = mat.scale(deltaZoom, deltaZoom, xx, yy); // shift the update matrix back into bounds

    var xDiff = this.ConstrainPanX(mat.getTx()) - mat.getTx();
    var yDiff = this.ConstrainPanY(mat.getTy()) - mat.getTy();
    this.FireZoomEvent('adjustPanConstraints', newZoom, oldZoom, animator, xx, yy, xDiff, yDiff); // shift the update matrix back into bounds again in case the zooming listener changes the pan constraints
  ...
  PanZoomCanvas.prototype.zoomTo = function (zz, xx, yy, animator) {
    if (!this.isZoomingEnabled()) {
      return;
    }
  ...
  PanZoomCanvas.prototype.center = function (animator, fitBounds) {
  ...
  PanZoomCanvas.prototype.zoomToFit = function (animator, fitBounds) {
    ...
    try {
      ...
      if (!animator) fireStartEventFunc();else dvt.Playable.prependOnInit(animator, fireStartEventFunc);
  ...
  PanZoomCanvas.prototype.calcZoomToFitScale = function (bounds) {
    if (!bounds) {
  ...
  PanZoomCanvas.prototype.calcZoomToFitBounds = function () {
  ...
  PanZoomCanvas.prototype.getViewport = function () {
    ...
    return new dvt.Rectangle(topLeftLocal.x, topLeftLocal.y, bottomRightLocal.x - topLeftLocal.x, bottomRightLocal.y - topLeftLocal.y);
  };
  /**
   * @protected
  ...
  PanZoomCanvas.prototype.SetElasticConstraints = function (bElastic) {
    this._bElasticConstraints = bElastic; //if turning on, stop any previously running bounce back anim
    ...
    if (bElastic) {
      if (this._elasticConstraintsAnim) {
        ...
        if (this._bElasticPan || this._bElasticZoom) {
          this._elasticConstraintsAnim = new dvt.Animator(this.getCtx(), .4); //do cubicOut easing so that the anim happens fast at the beginning and slows down at the end,
          //to make it seem like an elastic

          this._elasticConstraintsAnim.setEasing(dvt.Easing.cubicOut); //if zoom beyond constraint, constrain it
          ...
          if (this._bElasticPan) this.FirePanEvent('elasticAnimBegin', null, null, null, null, this._elasticConstraintsAnim);
          if (this._bElasticZoom) this.FireZoomEvent('elasticAnimBegin', null, null, null, null, this._elasticConstraintsAnim);
  ...
  PanZoomCanvas.prototype.IsElasticConstraints = function () {
  ...
  PanZoomCanvas.prototype._elasticConstraintsAnimOnEnd = function () {
    this._elasticConstraintsAnim = null;
  ...
  PanZoomCanvas.prototype._panDampingFunc = function (delta, whole) {
  ...
  PanZoomCanvas.prototype._zoomDampingFunc = function (delta, whole) {
    //parabola centered at (0,0) expanding to the right: y ^ 2 = 4 * a * x
    var a = .002 * whole;
    return Math.sqrt(4 * a * delta);
  };
  ...
  PanZoomCanvas.prototype.ConstrainPanX = function (xx) {
    var offsetX = xx;
  ...
  PanZoomCanvas.prototype.ConstrainPanY = function (yy) {
    var offsetY = yy;
  ...
  PanZoomCanvas.prototype.ConstrainZoom = function (zz) {
  ...
  PanZoomCanvas.RoundFloatForCompare = function (n) {
  ...
  PanZoomCanvas.prototype.GetRelativeMousePosition = function (event) {
  ...
  PanZoomCanvas.prototype.FirePanEvent = function (subtype, newX, newY, oldX, oldY, animator) {
  ...
  PanZoomCanvas.prototype.FireZoomEvent = function (subtype, newZoom, oldZoom, animator, xx, yy, tx, ty) {
  ...
  PanZoomCanvas.prototype.zoomAndCenter = function () {
  ...
  PanZoomCanvas.prototype.getNextZoomLevel = function (currZoom) {
  ...
  PanZoomCanvas.prototype.getPrevZoomLevel = function (currZoom) {
    var zoomLevel = currZoom;
    zoomLevel -= this.getZoomIncrement();
  ...
  PanZoomCanvas.prototype.setZoomIncrement = function (n) {
  ...
  PanZoomCanvas.prototype.getZoomIncrement = function () {
  ...
  PanZoomCanvas.prototype.setPanIncrement = function (n) {
  ...
  PanZoomCanvas.prototype.getPanIncrement = function () {
  ...
  PanZoomCanvas.prototype.setMinZoom = function (n) {
    this._minZoom = n;
  };
  /**
  ...
  PanZoomCanvas.prototype.getMinZoom = function () {
  ...
  PanZoomCanvas.prototype.setMaxZoom = function (n) {
    if (n < 0) n = 1;
    this._maxZoom = n;
  };
  /**
  ...
  PanZoomCanvas.prototype.getMaxZoom = function () {
  ...
  PanZoomCanvas.prototype.setMinPanX = function (n) {
    this._minPanX = n;
  };
  /**
  ...
  PanZoomCanvas.prototype.getMinPanX = function () {
  ...
  PanZoomCanvas.prototype.setMaxPanX = function (n) {
    this._maxPanX = n;
  };
  /**
  ...
  PanZoomCanvas.prototype.getMaxPanX = function () {
  ...
  PanZoomCanvas.prototype.setMinPanY = function (n) {
    this._minPanY = n;
  };
  /**
  ...
  PanZoomCanvas.prototype.getMinPanY = function () {
  ...
  PanZoomCanvas.prototype.setMaxPanY = function (n) {
    this._maxPanY = n;
  };
  /**
  ...
  PanZoomCanvas.prototype.getMaxPanY = function () {
  ...
  PanZoomCanvas.prototype.setAnimationDuration = function (animationDuration) {
  ...
  PanZoomCanvas.prototype.getAnimationDuration = function () {
  ...
  PanZoomCanvas.prototype.setPanningEnabled = function (panningEnabled) {
  ...
  PanZoomCanvas.prototype.isPanningEnabled = function () {
  ...
  PanZoomCanvas.prototype.setPanDirection = function (panDirection) {
  ...
  PanZoomCanvas.prototype.getPanDirection = function () {
    return this._panDirection;
  };

  PanZoomCanvas.prototype.setZoomingEnabled = function (zoomingEnabled) {
    this._bZoomingEnabled = zoomingEnabled;
  };

  PanZoomCanvas.prototype.isZoomingEnabled = function () {
    return this._bZoomingEnabled;
  };

  PanZoomCanvas.prototype.setZoomToFitEnabled = function (zoomToFitEnabled) {
    this._bZoomToFitEnabled = zoomToFitEnabled;
  };

  PanZoomCanvas.prototype.isZoomToFitEnabled = function () {
  ...
  PanZoomCanvas.prototype.setCurrentTouchTargets = function (targets) {
  ...
  PanZoomCanvas.prototype.getCurrentTouchTargets = function () {
  ...
  PanZoomCanvas.prototype.resetTouchTargets = function () {
  ...
  PanZoomCanvas.prototype.panZoomEnd = function () {
  ...
  PanZoomCanvas.prototype.setInteractionEnabled = function (bEnabled) {
    if (!bEnabled) this._eventManager.removeListeners(this);else this._eventManager.addListeners(this);
  };
  /**
  ...
  PanZoomCanvas.prototype.destroy = function () {
    ...
    this.setClipPath(null); // Always call superclass last for destroy

    PanZoomCanvas.superclass.destroy.call(this);
  };
  /**
  ...
  PanZoomCanvas.prototype.getCursor = function (panOn) {
  ...
  var PanZoomCanvasKeyboardHandler = function PanZoomCanvasKeyboardHandler(component, manager) {
    this.Init(component, manager);
  };

  dvt.Obj.createSubclass(PanZoomCanvasKeyboardHandler, dvt.KeyboardHandler);
  /**
   * @override
   */

  PanZoomCanvasKeyboardHandler.prototype.Init = function (component, manager) {
    PanZoomCanvasKeyboardHandler.superclass.Init.call(this, manager);
  ...
  PanZoomCanvasKeyboardHandler.prototype.processKeyDown = function (event) {
    ...
    if (keyCode == dvt.KeyboardEvent.PAGE_UP) {
      ...
      if (event.ctrlKey || event.shiftKey) canvas.panBy(canvas.getPanIncrement(), 0);else canvas.panBy(0, canvas.getPanIncrement());
  ...
  var PanZoomComponent = function PanZoomComponent(context, callback, callbackObj) {
    this.Init(context, callback, callbackObj);
  };

  dvt.Obj.createSubclass(PanZoomComponent, dvt.BaseComponent);
  /**
  ...
  PanZoomComponent.prototype.Init = function (context, callback, callbackObj) {
    PanZoomComponent.superclass.Init.call(this, context, callback, callbackObj); // IE11 does not support vector-effects=non-scaling-stroke so we still need to set stroke width based on zoom
  ...
  PanZoomComponent.prototype.getPanZoomCanvas = function () {
  ...
  PanZoomComponent.prototype.render = function (options, width, height) {
    this.Width = width;
    this.Height = height;
    this._isResize = !options; // - buttons not responding in firefox if hierarchy viewer present
  ...
  PanZoomComponent.prototype.supportsVectorEffects = function () {
  ...
  PanZoomComponent.prototype.HandlePanEvent = function (event) {// subclasses should override
  };
  /**
  ...
  PanZoomComponent.prototype.HandleZoomEvent = function (event) {// subclasses should override
  };
  /**
  ...
  PanZoomComponent.prototype.IsResize = function () {
  ...
  PanZoomComponent.prototype.PreRender = function () {// subclasses should override
  };
  /**
  ...
  PanZoomComponent.prototype.Render = function () {
  ...
  PanZoomComponent.prototype.SetOptions = function (options) {
  ...
  PanZoomComponent.prototype.destroy = function () {
    if (this._panZoomCanvas) {
    ...
    PanZoomComponent.superclass.destroy.call(this);
  };
  /**
  ...
  PanZoomComponent.prototype.ensureObjInViewport = function (event, navigable) {
    ...
    var fireEndEventPanFunc = function fireEndEventPanFunc() {
  ...
  PanZoomComponent.prototype.isPanning = function () {
