 ...
 * @ignore
 */
define(['ojs/ojcore-base', 'jquery', 'ojs/ojthemeutils', 'ojs/ojcomponentcore', 'ojs/ojlogger', 'ojs/ojcontext', 'ojs/ojconfig', 'ojs/ojdomutils', 'touchr'], function (oj, $, ThemeUtils, Components, Logger, Context, Config, DomUtils, touchr) {
  'use strict';
  ...
  (function () {
    ...
    oj.CustomElementBridge.register('oj-conveyor-belt', {
      metadata: __oj_conveyor_belt_metadata
    });
  })();
  /**
   * @license
  ...
  */
  ...
  function ConveyorBeltCommon(elem, orientation, contentParent, bRtl, buttonInfo, callbackInfo, styleInfo, agentInfo) {
    this._elem = elem;
  ...
  ConveyorBeltCommon.prototype.setup = function () {
    var self = this;
    var cbcClass = ConveyorBeltCommon; // create the content and overflow containers

    this._createInnerContainers(); // create the next/prev buttons
    ...
    cbcClass._addBubbleEventListener(this._elem, 'wheel', this._mouseWheelListener, false); // handle swipe gestures on the overflow container, which excludes the next/prev buttons
    ...
    cbcClass._addBubbleEventListener(this._overflowContainer, 'touchstart', this._touchStartListener, true);

    this._touchMoveListener = function (event) {
      self._handleTouchMove(event);
    };

    cbcClass._addBubbleEventListener(this._overflowContainer, 'touchmove', this._touchMoveListener, false);
    ...
    this._origScroll = 0; // clear any old sizes so that new sizes will be calculated

    this._clearCachedSizes(); // adjust overflow size


    this._adjustOverflowSize(true); // handle an initial resize


    this._handleResize(true); // eslint-disable-next-line no-unused-vars
    ...
    this._addResizeListenerFunc.call(null, this._elem, this._handleResizeFunc);

    this._addResizeListenerFunc.call(null, this._contentContainer, this._handleResizeFunc); // notify the child that it's being re-attached to the DOM AFTER attaching it
  ...
  ConveyorBeltCommon.prototype.destroy = function () {
    ...
    cbcClass._removeBubbleEventListener(this._overflowContainer, 'touchstart', this._touchStartListener, true);

    cbcClass._removeBubbleEventListener(this._overflowContainer, 'touchmove', this._touchMoveListener, false);
    ...
    this._scrollListener = null; // remove listeners before reparenting original children and clearing member
    // variables

    this._removeResizeListenerFunc.call(null, elem, this._handleResizeFunc);

    this._removeResizeListenerFunc.call(null, this._contentContainer, this._handleResizeFunc);

    this._handleResizeFunc = null; // move the original content children from the _contentContainer back to the
    // original DOM element

    this._reparentChildrenFromContentContainer(this._contentContainer, elem); // the content container is a child of the overflow container
  ...
  ConveyorBeltCommon.prototype._reparentChildrenToContentContainer = function (fromNode, toNode) {
    ...
    while (fromNodeChildren.length > 0) {
      var child = fromNodeChildren[0]; // notify the child that it's being detached from the DOM BEFORE detaching it
  ...
  ConveyorBeltCommon._getElemInnerWidth = function (elem) {
    ...
    var computedStyle = cbcClass._getComputedStyle(elem); // the computedStyle width is the inner width of the elem
  ...
  ConveyorBeltCommon._getElemInnerHeight = function (elem) {
    ...
    var computedStyle = cbcClass._getComputedStyle(elem); // the computedStyle height is the inner height of the elem
  ...
  ConveyorBeltCommon.prototype._handleResize = function (bSetup) {
    ...
    this._clearCachedSizes(); // measure content size


    this._totalSize = this._measureContents(); // if this is not the first call, need to adjust the overflow size (if this
  ...
  ConveyorBeltCommon.prototype._adjustOverflowSize = function (bInit) {
    ...
    var cbcClass = ConveyorBeltCommon;
    var elemInnerSize = bHoriz ? cbcClass._getElemInnerWidth(this._elem) : cbcClass._getElemInnerHeight(this._elem);
    this._minScroll = 0; // take the button size into account for max scroll position

    this._maxScroll = bHoriz ? contentContainer.offsetWidth - elemInnerSize + this._buttonWidth : contentContainer.offsetHeight - elemInnerSize + this._buttonHeight; // constrain max scroll
  ...
  ConveyorBeltCommon.prototype._createInnerContainers = function () {
    ...
    this._addStyleClassNameFunc(contentContainer, this._contentContainerStyleClass); // reparent children from elem to contentContainer before adding
  ...
  ConveyorBeltCommon.prototype._getContentElements = function () {
    var arContentElements = []; // if there is a nested contentParent, then we need to put its children into
    ...
    arContentElements = filterFunc(arContentElements); // if the content elements are direct children of the contentContainer, meaning there's no nested
  ...
  ConveyorBeltCommon.prototype._createPrevButton = function (buttonStyleClass, icon) {
    ...
    prevButton.setAttribute('class', buttonStyleClass); // hide the button from screen readers because it is not keyboard accessible
  ...
  ConveyorBeltCommon.prototype._createNextButton = function (buttonStyleClass, icon) {
    ...
    nextButton.setAttribute('class', buttonStyleClass); // hide the button from screen readers because it is not keyboard accessible
  ...
  ConveyorBeltCommon.prototype._measureContents = function () {
    ...
    if (arContentElements.length > 0) {
      ...
      var contentWidth = 0; // get the width of the contentContainer, not the contentParent, because
      ...
      for (var i = 0; i < children.length; i++) {
        ...
        if (child.nodeType === 1) {
          ...
          var childParent; // calculating the start assumes that the browser has done the appropriate layout;
  ...
  ConveyorBeltCommon.prototype._updateButtonVisibility = function (scroll) {
    ...
    var bNeedsScroll = this._needsScroll(); // if scrolling to the start, hide the prev button and reclaim its space
  ...
  ConveyorBeltCommon.prototype._setCurrScrollHelper = function (scroll, bImmediate) {
    ...
    this._bScrolling = true; // eslint-disable-next-line no-param-reassign

    scroll = this._constrainScroll(scroll); // update button visibility before scrolling

    this._updateButtonVisibility(scroll);

    var scrollFunc = this._scrollFunc; // if making the change immediately, simply call the anim end function
    ...
    if (bImmediate || !scrollFunc || scroll === this._getCurrScroll()) {
      ...
      var cbcClass = ConveyorBeltCommon; // 1.1 px/ms is the desired animation speed, so calculate the duration based on the distance to scroll

      var duration = Math.abs(this._getCurrScroll() - scroll) / cbcClass._SCROLL_SPEED;

      var self = this;

      var onEndFunc = function onEndFunc() {
        self._onScrollAnimEnd(scroll); // FIX : resolve busy state after animating a scroll
      ...
      scrollFunc.call(null, this._overflowContainer, this._convertScrollLogicalToBrowser(scroll), duration, onEndFunc);
    }
  };
  /**
  ...
  ConveyorBeltCommon.prototype._needsScroll = function () {
    ...
    return this._isHorizontal() ? contentContainer.offsetWidth > overflowContainer.offsetWidth : contentContainer.offsetHeight > overflowContainer.offsetHeight;
  };
  /**
  ...
  ConveyorBeltCommon.prototype._handleTouchStart = function (event) {
    ...
    if (this._needsScroll() && !this._bScrolling && eventTouches.length === 1) {
      this._bTouch = true; // save off some initial information at the start of a swipe
  ...
  ConveyorBeltCommon.prototype._handleTouchMove = function (event) {
    ...
    var diff = touchCoord - this._touchStartCoord; // in non-RTL, if swiping left or up, scroll next; otherwise scroll prev
    ...
    if (this._bTouch && canScrollInSwipeDirection) {
      ...
      var cbcClass = ConveyorBeltCommon;
      var container = this._overflowContainer;
      var threshold = cbcClass._SWIPE_THRESHOLD * (bHoriz ? container.offsetWidth : container.offsetHeight); // if swiping under the threshold, just move the conveyor with the swipe

      if (Math.abs(diff) < threshold) {
        this._setCurrScroll(this._touchStartScroll - diff, true); // if we're under the threshold, but we've already scrolled to the end,
        ...
        this._setCurrScroll(bNext ? this._touchStartNextScroll : this._touchStartPrevScroll, false); // don't scroll again for this same swipe
  ...
  ConveyorBeltCommon.prototype._handleTouchEnd = function (event) {
    ...
    this._bTouch = false; // FIX : reset the flag indicating if we've scrolled for this touch
    // event
  ...
  ConveyorBeltCommon.prototype._calcNextScroll = function () {
    ...
    var scroll = 0; // if single item is bigger than viewport, then scroll by viewport size
  ...
  ConveyorBeltCommon.prototype._convertScrollLogicalToBrowser = function (scroll) {
    ...
    if (this._bRtl && this._isHorizontal()) {
      ...
      if (this._bAgentGecko || this._bAgentWebkit && this._agentVersion >= 85 || this._bAgentSafari && this._agentVersion >= 10) {
        newScroll = -scroll;
      } else if (this._bAgentWebkit && this._agentVersion < 85 || this._bAgentSafari && this._agentVersion < 10) {
  ...
  (function () {
     ...
     * @type {string}
     */
    var _WARNING_DISABLED_OPTION = "JET ConveyorBelt: 'disabled' property not supported"; // make sure the collapseEventTimeout param is less than the one used in the unit tests
    ...
    oj.__registerWidget('oj.ojConveyorBelt', $.oj.baseComponent, {
      ...
      _ComponentCreate: function _ComponentCreate() {
        ...
        var options = this.options; // FIX : log warning message when "disabled" attribute set
      ...
      refresh: function refresh() {
        ...
        this._super(); // Check if the reading direction have changed


        var bRTL = this._GetReadingDirection() === 'rtl';
        var bDirectionChanged = this._bRTL !== bRTL; // save and restore scroll position only if the reading direction is not changed

        var scroll; // save the current scroll position
        ...
        this._setup(); // restore the saved scroll position
      ...
      _NotifyShown: function _NotifyShown() {
        this._super(); // FIX : perform a deferred layout
      ...
      _setup: function _setup() {
        ...
        if (!this._cbCommon) {
          ...
          this._cbCommon = new ConveyorBeltCommon(elem[0], orientation, contentParentElem, this._bRTL, buttonInfo, callbackInfo, styleInfo, agentInfo);
        }

        var cbCommon = this._cbCommon;
        cbCommon.setup();
        var children = elem.find('.oj-conveyorbelt-overflow-indicator'); // need to setup listeners for styles on each button individually so that callbacks for that
      ...
      _destroy: function _destroy() {
        ...
        var elem = this.element;
        elem.removeClass('oj-conveyorbelt oj-component oj-conveyorbelt-vertical'); // call superclass last
      ...
      _setOption: function _setOption(key, value, flags) {
        ...
        this._super(key, value, flags); // if recreating, setup the new ConveyorBeltCommon after calling superclass
        // _setOption
       ...
       * @private
       */
      _destroyCBCommon: function _destroyCBCommon() {
        var cbCommon = this._cbCommon;
      ...
      _canCalculateSizes: function _canCalculateSizes() {
        ...
        var style = div.style;
        style.width = '10px';
        style.height = '10px'; // make sure flexbox layout doesn't grow/shrink the item
       ...
       * @private
       */
      _setupButtonMouseStyles: function _setupButtonMouseStyles(element) {
        ...
        this._AddActiveable({
          ...
          afterToggle: function afterToggle(eventtype) {
            if (eventtype === 'mousedown' || eventtype === 'touchstart' || eventtype === 'mouseenter') {
              element.removeClass('oj-default');
            } else if (eventtype === 'mouseup' || eventtype === 'touchend' || eventtype === 'touchcancel' || eventtype === 'mouseleave') {
              element.addClass('oj-default');
            }
          }
        });
       ...
       * @private
       */
      _createIcon: function _createIcon(iconStyleClass, tooltip) {
       ...
       * @private
       */
      _animateScrollLeft: function _animateScrollLeft(elem, value, duration, onEndFunc) {
        var props = {};
        props.scrollLeft = value; // use swing instead of easeInOutCubic because easeInOutCubic isn't found
       ...
       * @private
       */
      _animateScrollTop: function _animateScrollTop(elem, value, duration, onEndFunc) {
        var props = {};
        props.scrollTop = value; // use swing instead of easeInOutCubic because easeInOutCubic isn't found
        // when running in the cookbook
       ...
       * @private
       */
      _addStyleClassName: function _addStyleClassName(elem, styleClass) {
       ...
       * @private
       */
      _removeStyleClassName: function _removeStyleClassName(elem, styleClass) {
       ...
       * @private
       */
      _hasStyleClassName: function _hasStyleClassName(elem, styleClass) {
       ...
       * @private
       */
      _filterContentElements: function _filterContentElements(arContentElements) {
        var ret = [];

        for (var i = 0; i < arContentElements.length; i++) {
          var contentElem = arContentElements[i];

          if (!this._hasStyleClassName(contentElem, 'oj-helper-detect-expansion') && !this._hasStyleClassName(contentElem, 'oj-helper-detect-contraction')) {
      ...
      getSubIdByNode: function getSubIdByNode(node) {
        ...
        var endIndicator = this.getNodeBySubId({
          subId: 'oj-conveyorbelt-end-overflow-indicator'
        });
        var topIndicator = this.getNodeBySubId({
          subId: 'oj-conveyorbelt-top-overflow-indicator'
        });
      ...
      scrollElementIntoView: function scrollElementIntoView(element) {
        ...
        var elementOffLeft = element.offsetLeft; // FIX : in IE, the conveyor items all report offsetLeft=0,
        // so we need to get the offset from the parent wrapping table cell div
        // instead
        ...
        if (this._cbCommon._isHorizontal()) {
          ...
          if (elementOffLeft + element.offsetWidth <= currentScroll + currentViewportSize && elementOffLeft >= currentScroll) {
            return;
          } // if vertical conveyor belt and the element is in the current vertical view port, then we don't need to scroll

        } else if (elementOffTop + element.offsetHeight <= currentScroll + currentViewportSize && elementOffTop >= currentScroll) {
          return;
        }

        var contentContainer = this._cbCommon._contentContainer;
        var cbcClass = ConveyorBeltCommon;
        var elemInnerSize = this._cbCommon._isHorizontal() ? cbcClass._getElemInnerWidth(this.element[0]) : cbcClass._getElemInnerHeight(this.element[0]);
        this._cbCommon._minScroll = 0; // take the button size into account for max scroll position

        this._cbCommon._maxScroll = this._cbCommon._isHorizontal() ? contentContainer.offsetWidth - elemInnerSize + this._cbCommon._buttonWidth : contentContainer.offsetHeight - elemInnerSize + this._cbCommon._buttonHeight; // constrain max scroll
