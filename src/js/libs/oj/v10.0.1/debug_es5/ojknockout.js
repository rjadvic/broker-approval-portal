...
define(['jqueryui-amd/widget', 'ojs/ojkoshared', 'ojs/ojcore', 'ojs/ojlogger', 'knockout', 'ojs/ojdomutils', 'jquery', 'ojs/ojcustomelement-utils', 'ojs/ojkeysetimpl', 'ojs/ojcontext', 'ojs/ojtemplateengine', 'ojs/ojknockouttemplateutils', 'ojs/ojresponsiveknockoututils'], function (widget, BindingProviderImpl, oj$1, Logger, ko, DomUtils, $, ojcustomelementUtils, KeySetImpl, Context, templateEngine, KnockoutTemplateUtils, ResponsiveKnockoutUtils) {
  ...
  var ComponentChangeTracker = function ComponentChangeTracker(component, queue) {
    this.Init(component, queue);
  }; // Subclass from oj.Object


  oj$1.Object.createSubclass(ComponentChangeTracker, oj$1.Object, 'ComponentBinding.ComponentChangeTracker');
  /**
  ...
  var ComponentBinding = function ComponentBinding(name, options) {
  ...
  ComponentBinding.prototype._init = function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
  ...
  ComponentBinding.prototype._update = function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
  ...
  ComponentBinding.prototype._initComponent = function (element, ctx) {
    ...
    var destroyListener = function destroyListener(evt) {
      if (evt.target && evt.target === element) {
        ...
        ComponentBinding._deliverCreateDestroyEventToManagedProps(false, specifiedManagedAttrs, element, comp, ctx.valueAccessor, ctx.allBindingsAccessor, ctx.bindingContext);
    ...
    var managedAttrMap = ComponentBinding._resolveManagedAttributes(this._managedAttrOptions, ctx.specifiedOptions, componentName); // Always use managed attribute behavior from the default instance
    ...
    var mutationOptions = ComponentBinding.__removeDotNotationOptions(resolvedInitialOptions); // Initialization of the component happens here
    ...
    ComponentBinding._deliverCreateDestroyEventToManagedProps(true, specifiedManagedAttrs, element, comp, ctx.valueAccessor, ctx.allBindingsAccessor, ctx.bindingContext);
  ...
  ComponentBinding.prototype._getManagedAttributes = function (specifiedOptions, componentName) {
    return ComponentBinding._resolveManagedAttributes(this._managedAttrOptions, specifiedOptions, componentName);
  };
  /**
   * @private
   */
    ...
    for (var k = 0; k < keys.length; k++) {
      ...
      if (ctx.componentName.match(pattern)) {
        ...
        for (var i = 0; i < eventInfos.length; i++) {
          ...
          jelem.on(info.event + ComponentBinding._HANDLER_NAMESPACE, {
            ...
            for (var nm = 0; nm < names.length; nm++) {
              ...
              try {
                if (ctx.specifiedOptions.indexOf(name) >= 0) {
                  ...
                  ComponentBinding._writeValueToProperty(name, target, nameValues[name], expr, ctx.bindingContext, cachedWriterFunctionEvaluators);
                }
              } finally {
  ...
  ComponentBinding._writeValueToProperty = function (name, target, value, propertyExpression, bindingContext, cachedWriterFunctionEvaluators) {
  ...
  ComponentBinding._deliverCreateDestroyEventToManagedProps = function (isCreate, managedAttrMap, element, comp, valueAccessor, allBindingsAccessor, bindingContext) {
  ...
  function _modifyOjComponentBinding(node, bindingName, wrapped, bindingContext, accessorMap) {
    ...
    accessorMap = oj$1.CollectionUtils.copyInto({}, accessorMap); // Add accessor for the V2 version of the Component binding
    // eslint-disable-next-line no-param-reassign
  ...
  BindingProviderImpl.addPostprocessor({
    nodeHasBindings: function nodeHasBindings(node, wrappedReturn) {
      ...
      return wrappedReturn || node.nodeType === 1 && ojcustomelementUtils.CustomElementUtils.isElementRegistered(node.nodeName);
    },
      ...
      if (node.nodeType === 1 && oj$1.BaseCustomElementBridge) {
        ...
        if (ojcustomelementUtils.CustomElementUtils.isElementRegistered(name)) {
          ...
          wrappedReturn._ojCustomElement = function () {
            return {
              composite: ojcustomelementUtils.CustomElementUtils.isComposite(name)
            };
          };
        }
      }
  ...
  ko.bindingHandlers.ojContextMenu = {
    ...
    update: function update(element, valueAccessor, allBindings, viewModel, bindingContext) {
      ...
      .addClass(DomUtils.isTouchSupported() ? 'oj-menu-context-menu-launcher' : ''); // At least some of the time, the pressHold gesture also fires a click event same as a short tap.  Prevent that here.
      ...
      function getContextMenu() {
        ...
        var constructor = oj$1.Components.__GetWidgetConstructor(getContextMenuNode()[0], 'ojMenu'); // The JET Menu of the first element found.
      ...
      function launch(event, eventType, pressHold) {
        ...
        if (isPressHold) {
          $element.one('touchend' + eventNamespace, function () {
            ...
            setTimeout(function () {
              // @HTMLUpdateOK delaying our own callback
      ...
      function touchstartMousedownKeydownListener(event) {
        ...
        if (event.type === 'touchstart') {
          ...
          launch.bind(undefined, event, 'touch', true), pressHoldThreshold);
        }
  ...
  var ExpressionPropertyUpdater = function ExpressionPropertyUpdater(element, bindingContext, skipThrottling) {
    ...
    this.setupPropertyBinding = function (attrVal, propName, metadata, providedValueSetter, providedPropName) {
      ...
      if (metadata._domListener) {
        var event = ojcustomelementUtils.AttributeUtils.eventListenerPropertyToEventType(propName); // If the attribute is removed, there won't be a new expression so the remove call
      ...
      if (attrVal === undefined) {
        ...
        var observable = providedPropName === undefined ? null : (bindingContext.$provided || {})[providedPropName]; // If the observable is present, we set up the implicit binding

        if (observable) {
          evaluator = function evaluator() {
            return observable();
          }; // evaluator is using an implicit binding to a property provided by a container component

        }
      } else {
        var info = ojcustomelementUtils.AttributeUtils.getExpressionInfo(attrVal);
        expr = info.expr;
    ...
    function _setDomListener(_bindingContext, event, listener) {
      ...
      if (listener == null || listener instanceof Function) {
        ...
        if (ojcustomelementUtils.CustomElementUtils.isElementRegistered(element.tagName)) {
          var elementState = ojcustomelementUtils.CustomElementUtils.getElementState(element);
  ...
  var _KnockoutBindingProvider = function _KnockoutBindingProvider() {
    /**
     * @ignore
     */
  ...
  (function () {
    ko.bindingHandlers._ojCustomElement = {
      ...
      init: function init(element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
        ...
        var metadataProps = ojcustomelementUtils.CustomElementUtils.getElementProperties(element); // Compute data about the provided and consumed properties.
        // Also get the function that would perform necessary clean up when the bindings on the provider element are being cleaned

        var _setupProvideAndConsu = _setupProvideAndConsumeMaps(element, metadataProps),
            provideMap = _setupProvideAndConsu.provide,
            consumeMap = _setupProvideAndConsu.consume,
            disposeProviderListeners = _setupProvideAndConsu.cleanup; // Called both when KO's cleanNode() is invoked and when the observable view model is mutated
        ...
        function setup(isComposite, isInitial) {
          _expressionHandler = new ExpressionPropertyUpdater(element, bindingContext, isComposite); // Dummy metadata for passing to the ExpressionPropertyUpdater for DOM listener attributes i.e. on-*
          ...
          var elementState = ojcustomelementUtils.CustomElementUtils.getElementState(element);
          elementState.isInitializingProperties = true; // setupPropertyBinding will only update properties defined in metadata so it's safe to iterate through all element attributes
          ...
          for (var i = 0; i < attrs.length; i++) {
            ...
            var initialValueSetter = void 0;
            var topPropName = void 0;
            var isDomEvent = ojcustomelementUtils.AttributeUtils.isEventListenerProperty(propName) && !metadataProps[propName];

            if (!isDomEvent) {
              topPropName = propName.split('.')[0];
              var info = provideMap[topPropName]; // Initial value for provided properties needs to be set only when setup() is called for the very first time.
            ...
            _expressionHandler.setupPropertyBinding(attr.value, propName, isDomEvent ? domListenerMetadata : metadataProps[topPropName], // Pass in a setter to store the initial value if this property will be
          ...
          var consumingProps = Object.keys(consumeMap);
          consumingProps.forEach(function (consumingProp) {
            if (!element.hasAttribute(ojcustomelementUtils.AttributeUtils.propertyNameToAttribute(consumingProp))) {
              var provideInfo = provideMap[consumingProp]; // Initial value for provided properties needs to be set only when setup() is called for the very first time.
              // After that, we will be using property change listeners to update the values

              var _initialValueSetter = isInitial && provideInfo ? provideInfo.set : undefined;

              _expressionHandler.setupPropertyBinding(undefined, consumingProp, metadataProps[consumingProp], _initialValueSetter, consumeMap[consumingProp]);
            }
          });
          ...
          attributeListener = function attributeListener(evt) {
            ...
            var _isDomEvent = ojcustomelementUtils.AttributeUtils.isEventListenerProperty(_propName) && !metadataProps[_propName];

            var metadata = _isDomEvent ? domListenerMetadata : metadataProps[_propName.split('.')[0]]; // send metadata for top level property

            _expressionHandler.setupPropertyBinding(detail.value, _propName, metadata, undefined
            /* no need to get initial value on attribute change */
        ...
        var contextWithProvideInfo = _getChildContext(bindingContext, provideMap);
    ...
    var _CHANGE_SUFFIX = 'Changed';

    var _bindingPropagationMetadataCache = Object.create(null);
    ...
    function _setupProvideAndConsumeMaps(element, metadataProps) {
      ...
      // transform map
      var provide = Object.create(null); // null prototype for object being used as a map
      // A map of a property name to a name of a provided property that should be consumed via an implicit binding

      var consume = Object.create(null); // Example 'provide binding' metadata:
      ...
      var provideConsumeMeta = _getProvideAndConsumeMetadataViaCache(element, metadataProps); // will return null if the component has no provide/consume metadata


      if (provideConsumeMeta !== null) {
        var propKeys = Object.keys(provideConsumeMeta);
        propKeys.forEach(function (pName) {
          var _provideConsumeMeta$p = _slicedToArray(provideConsumeMeta[pName], 2),
              provideMeta = _provideConsumeMeta$p[0],
              consumeMeta = _provideConsumeMeta$p[1];
          ...
          if (provideMeta !== undefined) {
            ...
            var attrName = ojcustomelementUtils.AttributeUtils.propertyNameToAttribute(pName);
            var hasAttribute = element.hasAttribute(attrName);
            var observables = [];
            var vars = []; // iterate over provided bindings (there may be more than one!) that a single attribute produces

            provideMeta.forEach(function (info) {
              var name = info.name;
              ...
              var obs = _createObservableWithTransform(info.transform, defaultVal);
            ...
            if (vars.length > 0) {
              ...
              var set = _getSingleSetter(observables);
              ...
              var changeListener = _setupChangeListenerForProvidedProperty(set);

              var evtName = pName + _CHANGE_SUFFIX;
              element.addEventListener(evtName, changeListener); // Store listener in a map for future cleanup

              changeListeners[evtName] = changeListener; // If the attribute is present, and its value is not an expression, we won't be getting the initial value
              ...
              if (hasAttribute) {
                ...
                if (!ojcustomelementUtils.AttributeUtils.getExpressionInfo(attrVal).expr) {
                  set(ojcustomelementUtils.AttributeUtils.coerceValue(element, attrName, attrVal, metadataProps[pName].type));
                }
              }
            }
          }
      ...
      function cleanup() {
        var changeListenerKeys = Object.keys(changeListeners);
        changeListenerKeys.forEach(function (key) {
          return element.removeEventListener(key, changeListeners[key]);
        });
      }
    ...
    function _getProvideAndConsumeMetadataViaCache(element, metadataProps) {
      ...
      var entry = _bindingPropagationMetadataCache[elemName];
      ...
      entry = null;
      var propKeys = Object.keys(metadataProps);
      propKeys.forEach(function (pName) {
        var meta = metadataProps[pName];

        var provideMeta = _getOptionalChainResult(meta, ['binding', 'provide']);

        var consumeMeta = _getOptionalChainResult(meta, ['binding', 'consume']);
    ...
    function _setupChangeListenerForProvidedProperty(setter) {
      return function (evt) {
        return setter(evt.detail.value);
      };
    ...
    function _getOptionalChainResult(obj, path) {
      var res = obj;
      return path.some(function (prop) {
        res = res[prop];
    ...
    function _getChildContext(bindingContext, provideMap) {
      var newContext = bindingContext;
      var props = Object.keys(provideMap);

      if (props.length > 0) {
        var oldProvided = bindingContext.$provided;
        var newProvided = oldProvided === undefined ? {} : Object.assign({}, oldProvided);
        props.forEach(function (prop) {
          var vars = provideMap[prop].vars;
          vars.forEach(function (info) {
            newProvided[info.name] = info.obs;
          });
        });
        newContext = bindingContext.extend({
    ...
    function _createObservableWithTransform(transform, initialVal) {
      var observable = ko.observable(initialVal);

      if (transform) {
        return ko.pureComputed({
          // eslint-disable-next-line no-prototype-builtins
          write: function write(value) {
            return observable(transform.hasOwnProperty(value) ? transform[value] : value);
          },
          read: function read() {
            return observable();
          }
    ...
    function _getSingleSetter(observables) {
      return function (val) {
        return observables.forEach(function (observable) {
  ...
  (function () {
        ...
        if (node.nodeType === 1) {
          ...
          if (boundAttrs) {
            ...
            for (i = 0; i < boundAttrs.length; i++) {
              attr = boundAttrs[i]; // For the class attribute we support a string, an array and object types.
              // For all the types we pass to knockout's css binding that also handles arrays and strings
              // since version 3.5.0.

              if (attr === 'class') {
                _setClassEvaluator(wrappedReturn, node, node.getAttribute(_getBoundAttrName(attr)), bindingContext);
              } else {
    ...
    function _setClassEvaluator(wrappedReturn, elem, value, bindingContext) {
      ...
      if (exp == null) {
        ...
        var classValueEvaluator = BindingProviderImpl.createEvaluator(exp, bindingContext).bind(null, bindingContext); // create computed to handle cases when an expression evaluates into an observable or
  ...
  (function () {
    ...
    function getTemplateNodes(elem) {
      ...
      elem._templateNode = defaultTemplate; // eslint-disable-next-line no-param-reassign
    ...
    function OjForEach(element, value, context) {
      this.element = value.element || element; // convert oj-bind-x components into template

      ko.applyBindingsToDescendants(context, this.element);
      getTemplateNodes(this.element); // Find the correct context for the template - it is either the context
    ...
    OjForEach.prototype._initChildrenBindingsAppliedPromise = function () {
      ...
      this._childrenBindingsPromiseResolver = _KnockoutBindingProvider.getInstance().__RegisterBindingAppliedPromiseForChildren(nearestCustomParent, isImmediate);
    ...
    OjForEach.prototype.fetchData = function () {
      ...
      var iterator = this.currentDataProvider.fetchFirst({
        size: -1
    ...
    OjForEach.prototype.getDataMutationHelper = function (event, promiseContainer) {
      ...
      var updateData = updateDetail && updateDetail.data; // Define helper methods for updates, removals and additions, that will be used for
      ...
      var doUpdates = function doUpdates() {
        if (updateData) {
          ...
          var dataIndexes = _this.getIndexesForEvent(updateDetail.indexes, updateDetail.keys);
          ...
          _this.onArrayChange(changes);
        }
      };

      var doRemovals = function doRemovals() {
        if (removeDetail) {
          // Get indexes to delete using either indexes or given keys
          var dataIndexes = _this.getIndexesForEvent(removeDetail.indexes, removeDetail.keys);

          dataIndexes = dataIndexes.filter(function (value) {
            return value !== undefined;
          });

          _this.onArrayChange(dataIndexes.map(valueToChangeDeleteItem));
        }
      };

      var doAdditions = function doAdditions() {
        if (addData) {
          ...
          var dataIndexes = _this.getIndexesForEvent(addDetail.indexes, addDetail.addBeforeKeys ? addDetail.addBeforeKeys : addDetail.afterKeys);
      ...
      if (updateDetail && !Array.isArray(updateData) || addDetail && !Array.isArray(addData)) {
        var updatePromise = updateDetail && !Array.isArray(updateData) ? this.currentDataProvider.fetchByKeys({
          keys: updateDetail.keys
        }) : Promise.resolve();
        var addPromise = addDetail && !Array.isArray(addData) ? this.currentDataProvider.fetchByKeys({
          keys: addDetail.keys
        }) : Promise.resolve();
        busyStateCallback = this.registerBusyState();
        dataPromise = Promise.all([updatePromise, addPromise]).then(function (keyResults) {
          // successfully got all the data
          // if promise is current, retrieve records and update data set;
          // else throw valid rejection error
          if (promiseContainer.head === _this.headDataPromise) {
            // Perform updates
            var updateKeyResults = keyResults[0];

            if (updateKeyResults && updateKeyResults.results.size > 0) {
              updateData = _toConsumableArray(updateDetail.keys).map(function (key) {
                return updateKeyResults.results.get(key).data;
              });
            }

            doUpdates(); // Perform removals

            doRemovals(); // Perform additions

            var addKeyResults = keyResults[1];

            if (addKeyResults && addKeyResults.results.size > 0) {
              addData = _toConsumableArray(addDetail.keys).map(function (key) {
            ...
            _this.resetChainIfCompleted(promiseContainer.current);
          } else {
        ...
        dataPromise.catch(function (reason) {
          _this.promiseRejectHelper(reason, busyStateCallback, promiseContainer.current);
        });
      } else {
    ...
    OjForEach.prototype._addNoData = function () {
      if (this.firstLastNodesList.length === 0 && this.element._noDataTemplateNode) {
        this._noDataNodes = templateEngine.execute(this.element, this.element._noDataTemplateNode, {}, null);
    ...
    OjForEach.prototype._removeNoData = function () {
      if (this._noDataNodes) {
        this._noDataNodes.forEach(function (node) {
      ...
      init: function init(element, valueAccessor, bindings, vm, context) {
        ...
        ko.computed(function () {
          ...
          if (ffe) {
            ...
            var updatedPrimeData; // retrive the array data that might be defined as an observable
            ...
            if (Array.isArray(currentPrimeData) && Array.isArray(updatedPrimeData)) {
              ...
              var changeSet = ko.utils.compareArrays(currentPrimeData, updatedPrimeData, compareArrayOptions);
  ...
  koStringTemplateEngine.install = function () {
    ...
    localKo.templates = _templateText; // make this new template engine our default engine
