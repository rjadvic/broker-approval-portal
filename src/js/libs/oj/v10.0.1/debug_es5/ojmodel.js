 ...
 * @ignore
 */
define(['exports', 'ojs/ojcore-base', 'ojs/ojlogger', 'jquery', 'ojs/ojconfig'], function (exports, oj, Logger, $, Config) {
  'use strict';
  ...
  /** @lends Events */
  {
     ...
     * @memberof Events
     */
    on: function on(eventType, callback, context) {
     ...
     * @memberof Events
    */
    off: function off(eventType, callback, context) {
    ...
    trigger: function trigger(eventType) {
      ...
      var args = Array.prototype.slice.call(arguments); // Inject a silent setting in there: if this is being called outside we want to fire all relevant
      // events
     ...
     * @memberof Events
     */
    once: function once(eventType, callback, context) {
     ...
     * @memberof Events
     */
    listenTo: function listenTo(otherObj, eventType, callback) {
      var eventArray;
      var e;
      var event;
      var attr;
      ...
      for (prop in eventMap) {
        ...
        if (eventMap.hasOwnProperty(prop)) {
          ...
          for (e = 0; e < eventArray.length; e += 1) {
            ...
            this._listeningTo.push(listenerObj); // fire
     ...
     * @memberof Events
     */
    listenToOnce: function listenToOnce(otherObj, eventType, callback) {
      var eventArray;
      var e;
      var event;
      var attr;
     ...
     * @memberof Events
     */
    stopListening: function stopListening(otherObj, eventType, callback) {
      var eventArray;
      var actualType;
      var eventMap = {};
      var e;
      ...
      if (arguments == null || arguments.length <= 1) {
        len = this._listeningTo ? this._listeningTo.length : 0; // Remove everything

        for (i = 0; i < len; i++) {
          event = this._listeningTo[i]; // If we have an "otherObj" argument, make sure that passes muster
      ...
      actualType = eventType; // Account for missing otherObj
  ...
  Events.TriggerInternal = function (silent, eventType) {
    ...
    for (e = 0; e < eventArray.length; e += 1) {
      event = eventArray[e].event;
      attr = eventArray[e].attribute; // Do specific event...
    ...
    for (e = 0; e < eventsToFire.length; e += 1) {
      ...
      for (i = 0; i < (handlers ? handlers.length : 0); i += 1) {
        if (handlers[i].attribute === eventsToFire[e].attribute && handlers[i].callback) {
          ...
          if (handlers && handlers[i] && handlers[i].once) {
            ...
            this._removeHandler(Events._getHandlers(this.eventHandlers, eventsToFire[e].event, true), handlers[i]); // Now take it out of the other object's "listen to" list, if relevant
          ...
          if (handlers && handlers[i] && this._shouldFire(handlers[i])) {
            callback = handlers[i].callback; // If this isn't a silent firing or this handler always wants to be called, make the call
      ...
      for (i = 0; i < (allHandlers ? allHandlers.length : 0); i += 1) {
        ...
        if (allHandlers && allHandlers[i] && allHandlers[i].callback && this._shouldFire(allHandlers[i])) {
          callback = allHandlers[i].callback; // If this isn't a silent firing or this handler always wants to be called, make the call
        ...
        if (allHandlers && allHandlers[i] && allHandlers[i].once) {
          ...
          this._removeHandler(this._getHandlers(this.eventHandlers, Events.EventType.ALL, true), allHandlers[i]); // Now take it out of the other object's "listen to" list, if relevant


          if (allHandlers[i].otherObj) {
            // Clean up the "other object" if this was a triggered listenOnce
            allHandlers[i].otherObj.stopListening(this, Events.EventType.ALL, allHandlers[i].callback);
          }
        }
      }
    }
    ...
    for (prop in eventMap) {
      ...
      if (eventMap.hasOwnProperty(prop)) {
        ...
        for (i = 0; i < eventArray.length; i += 1) {
          ...
          if (this._checkForHandler(this.eventHandlers[event], eventObj, Events._handlersIdentical) === -1) {
  ...
  Events._getEventMap = function (eventType, callback, context) {
    ...
    if (eventType.constructor === String) {
      ...
      eventMap = eventType; // If eventType is a map of events->callbacks, then the callback argument is now context
  ...
  var RestImpl = function RestImpl(rootURL, model) {
  ...
  RestImpl._HEADER_PROP = 'headers'; // Add the properties in options to starter, if not already there
  ...
  RestImpl.prototype.updateRecord = function (callback, recordID, record, error, options, context, patch) {
    var opt = options || {};
    var isJsonp = opt.dataType === 'jsonp';
  ...
  RestImpl.prototype._getData = function (data, options, urlInfo) {
    if (RestImpl._emulateJSON(options)) {
      ...
      var retObj = {
        _method: urlInfo._method ? urlInfo._method : urlInfo.type
      };
  ...
  RestImpl.prototype._getURL = function (operation, rootURL, customURL, recordID, context, options) {
    ...
    if ($.isFunction(customURL)) {
      var result = customURL.call(this, operation, context, RestImpl.SetCustomURLOptions(recordID, context, options));
    ...
    var retObj = {
      url: RestImpl.GetPropValue(null, rootURL),
      type: httpMethod.method
    };
  ...
  */
  ...
  Model._init = function (model, attributes, opt, properties) {
    ...
    var options = opt || {}; // Deep copy actual data if found
    ...
    model.SetCid(); // Grab collection option, if there
  ...
  Model.extend = function (properties, classProperties) {
    ...
    var obj;
    obj = new Model();
    Model._justExtending = false; // Add regular properties from this "parent"
    // Events.Mixin(obj, this.prototype);

    $.extend(obj, this.prototype); // Grab properties
    ...
    $.extend(model, this);
    model.prototype = obj; // Allow extending resulting obj

    model.extend = Model.extend;
    model.prototype.constructor = model; // Add class properties from this
  ...
  Model.prototype.Merge = function (model, comparator, silent) {
    ...
    this.SetupId(); // Only fire master change if there were any changes
  ...
  Model.prototype.SetCollection = function (coll) {
    ...
    this.collection = coll; // This can depend on the collection
  ...
  Model.prototype.SetupId = function () {
    ...
    var id = null; // Ask for collection's function if available
  ...
  Model.prototype._setPropInternal = function (prop, value, copyRegardless) {
    ...
    if (copyRegardless || !equality) {
      ...
      this.SetupId(); // Return value management here seems bizarre due to backbone tests: do the direct set if copyRegardless,
  ...
  Model._cloneAttributes = function (oldData, nd) {
    var newData = nd || {}; // Handle not overwriting defaults with undefined
    // IE11 issue with Object.keys

    var newDataKeys = _typeof(newData) === 'object' ? Object.keys(newData) : [];
    var canUseJson = true;
    var prop;
    ...
    if (newDataKeys.length > 0) {
      for (prop in newData) {
        // eslint-disable-line no-restricted-syntax
        if (Object.prototype.hasOwnProperty.call(newData, prop) && Object.prototype.hasOwnProperty.call(oldData, prop)) {
    ...
    } else if (_typeof(oldData) === 'object') {
      // IE11 issue with Object.keys
      oj.CollectionUtils.copyInto(newData, oldData, undefined, true, 10000);
    }
  ...
  Model.prototype.clone = function () {
    ...
    c.attributes = Model._cloneAttributes(this.attributes, null); // Remove the cid--this should be unique

    delete c.cid; // Set a new cid
  ...
  Model.prototype.set = function (property, value, options) {
    ...
    if (arguments && arguments.length > 0) {
      ...
      if (!oj.StringUtils.isString(property)) {
        ...
        opts = value || {}; // For set, pass entire thing to setProp
  ...
  Model.prototype._unsetInternal = function (property, opts, clear) {
    ...
    if (this.has(property)) {
      ...
      this._addChange(property, undefined); // if (!silent) {
  ...
  Model.prototype.save = function (attributes, options) {
    ...
    opts.saveAttrs = opts.wait ? this._attrUnion(attrArgs) : this.attributes; // Must temporarily at least set attrs for toJSON()

    oldAttrs = this.attributes; // Swap in what's to be saved and call toJSON()
    ...
    if (!forceNew && !this.isNew()) {
      ...
      opts.success = function (resp) {
        var attrs; // Make sure we pass xhr
        ...
        if (resp && !oj.Object.isEmpty(resp)) {
          ...
          self.attributes = $.extend(true, self.attributes, attrs); // Merge attrs from response/parse into arg attrs if different--server takes priority in case
          // of 'wait'
    ...
    opts.success = function (resp) {
      ...
      if (resp && !oj.Object.isEmpty(resp)) {
        ...
        self.attributes = $.extend(true, self.attributes, attrs); // Merge attrs from response/parse into arg attrs if different--server takes priority in case of 'wait'
    ...
    opts.parse = true; // Bizarre case tested by backboneJS--if this is a new model, but we're patching, make sure we only save the
  ...
  Model.prototype.hasChanged = function (attribute) {
    if (attribute !== undefined) {
      return Model._hasProperties(this.changed) && Object.prototype.hasOwnProperty.call(this.changed, attribute);
    }
  ...
  Model.prototype.destroy = function (options) {
    ...
    callback = Model._getSuccess(opts); // Grab the collection off the model in case we need to update
  ...
  Model._internalSync = function (method, model, opt) {
    var options = opt || {}; // If Model/Collection has OAuth object, then create Authorization header (see RestImpl.addOptions)
  ...
  var sync = function sync(method, model, options) {
    ...
    if (method.valueOf() === 'read') {
      if (model instanceof Model) {
        ...
        restService = new RestImpl(url, model);
        return _fireAndReturn(restService.getRecord(success, error, model.GetId(), tempOpt, Model.GetContext(tempOpt, model)));
      } // Collection fetch
    ...
    if (method.valueOf() === 'update') {
      return _fireAndReturn(restService.updateRecord(success, recordId, tempOpt.parsedData, error, tempOpt, model, false));
    }

    if (method.valueOf() === 'patch') {
      return _fireAndReturn(restService.updateRecord(success, recordId, tempOpt.parsedData, error, tempOpt, model, true));
    }
  ...
  Collection.extend = function (properties, classProperties) {
    ...
    obj = new Collection();
    Collection._justExtending = false; // Add regular properties from this "parent"
    // Events.Mixin(obj, this.prototype);
    ...
    if (properties && properties.constructor && Object.prototype.hasOwnProperty.call(properties, 'constructor')) {
      ...
      coll = function coll(models, options) {
    ...
    $.extend(coll, this);
    coll.prototype = obj; // Allow extending resulting obj
  ...
  Collection._init = function (collection, models, options, properties) {
    ...
    var modelList;
    var prop;
    collection.Init(); // Augment with Event

    Events.Mixin(collection); // First, copy all properties passed in
    ...
    var opt = options || {};
    optionlist = ['comparator', 'customPagingOptions', 'customURL', Collection._FETCH_SIZE_PROP, 'model', 'modelLimit', 'sortDirection', 'url'];

    for (i = 0; i < optionlist.length; i++) {
      if (Object.prototype.hasOwnProperty.call(opt, optionlist[i]) && opt[optionlist[i]] !== undefined) {
  ...
  Collection.prototype._setChangeAt = function (start, count) {
    for (var at = start; at < start + count; at++) {
      if (this.changes.indexOf(at) === -1) {
        this.changes.push(at); // this.changes.sort(function(a,b) { return a-b;});
      }
    }
  };
  /**
  ...
  Collection.prototype._removePrevNext = function (model) {
    ...
    var oldNext = model.GetNext(); // Link the two surrounding previous/next elements to each other, because this one is being replaced and moved
    // to the head
  ...
  Collection.prototype._spliceModelIndices = function (start, end) {
    ...
    var count = localEnd - start + 1; // Reduce the indexes of any models above the endpoint by the number of models removed
  ...
  Collection.prototype._clearModelIndices = function (start, end) {
    var localEnd = end === undefined ? start : end; // Knock out any of the deleted model's indexes from the list
  ...
  Collection.prototype._cloneInternal = function (withProperties) {
    var c = new this.constructor();
    var i; // Only copy locally if virtual
  ...
  Collection.prototype._newModel = function (m, parse, options, ignoreDefaults) {
    var newModel = null;
    var validationValue;
    var opt = options || {}; //    opt.noparse = !parse;
  ...
  Collection.prototype._addInternal = function (m, options, fillIn, deferred) {
    ...
    function addToCollection(collection, newModel) {
      if (at === undefined) {
        ...
        collection._getModel(index).SetCid(); // Increment at so that later models will be added right after their predecessors, if an array is
        // passed in
    ...
    function resortAndFireEvents(collection, existingModel, modelFoundInCollection, newModel, resortDeferred) {
      ...
      var comparator = resortOpt.comparator || collection._hasComparator(); // If we're filling in to a blank, also check that there's no comparator
      ...
      if (fillInSort && needSort && existingModel === undefined && !sort && at === undefined && collection._getLength() > 1) {
        ...
        sortOpt.add = true;
        collection.sort(sortOpt); // Reset index--can't get it back if virtual--set to -1
      ...
      if (added) {
        ...
        if (newModel) {
          ...
          modelFoundInCollection.TriggerInternal(silent, Events.EventType.ADD, modelFoundInCollection, collection, resortOpt);
          addedModels.push(modelFoundInCollection);
        }
      }
    }

    function mergeAttrs(collection, modelToTryAndMerge, modelFoundInCollection, newModel, mergeDeferred) {
      var existingModel;
      var modelAdded = null;
      ...
      if (!force && merge && modelFoundInCollection) {
        ...
        if (!force) {
          ...
          if (existingModel && fillIn && at !== existingModel.index) {
            ...
            Logger.warn('Duplicate ID fetched or added without merging, the id = ' + existingModel.GetId());
          }
        }
    ...
    function doAdd(collection, model, addDeferred) {
      ...
      if (newModel != null) {
        index = -1; // Make sure ID is up to date

        newModel.SetupId(); // Use original model array not cloned model if merging--otherwise we won't find the model in the
        // collection
        ...
        if (addDeferred) {
          if (force) {
            return new Promise(function (resolve) {
              var mergedModel = mergeAttrs(collection, modelToTryAndMerge, undefined, newModel, addDeferred);
              modelReturnList.push(mergedModel);
              resolve(mergedModel);
            });
          }

          return collection._getInternal(modelToTryAndMerge, {
            silent: true
          }, addDeferred, true).then(function (modInfo) {
            modelFoundInCollection = modInfo.m;
            var mod = mergeAttrs(collection, modelToTryAndMerge, modelFoundInCollection, newModel, addDeferred);
        ...
        var modelAdded = mergeAttrs(collection, modelToTryAndMerge, modelFoundInCollection, newModel, addDeferred);
    ...
    if (!fillIn && (this.IsVirtual() || deferred)) {
      ...
      return new Promise(function (allResolve, allReject) {
        var doTask = function doTask(loc) {
  ...
  Collection.prototype.sort = function (options) {
    ...
    var self;
    var eventOpts; // Check for comparator
    ...
    this._realignModelIndices(0); // Indicate this sort is due to an add
  ...
  Collection.prototype.last = function (n, options) {
    ...
    if (deferred || this.IsVirtual()) {
      // Loop using deferred
      var start = len - n; // Handle edge or no totalResults cases
  ...
  Collection.prototype.setModelLimit = function (n) {
    this.modelLimit = n; // Clean out down to the new limit, if necessary
  ...
  Collection.prototype._removeInternal = function (model, index, options) {
    ...
    if (n > -1) {
      var mod = modInfo.m; // only unset the collection setting if it's mine
  ...
  Collection.prototype.refresh = function (options) {
    ...
    return this._addPromise(function () {
      return new Promise(function (resolve, reject) {
        ...
        self._resetLRU(); // Clear totalresults.
        ...
        if (startIndex !== undefined && startIndex !== null) {
          ...
          self._setRangeLocalInternal(startIndex, self._getFetchSize(optCopy)).then(function (actual) {
            resolve(actual);
          }, function (err) {
            reject(err);
          });
  ...
  Collection.prototype.reset = function (data, options) {
    ...
    if (!data) {
      this._setLength(); // Clear totalresults
    ...
    var retObj = null; // Parse collection
  ...
  Collection.prototype._addPromise = function (promiseTask) {
    var self = this; // Undefined, so set it up initially
    ...
    this._promises = this._promises.then(promiseTask.bind(self)).then(function (arg) {
      ...
      if (self._promiseCount === 0) {
        self._promises = undefined; // Fire the ready event
  ...
  Collection.prototype.abort = function () {
    ...
    function createPromise(index, resolve) {
      self._xhrs[index].then(function (data, status) {
        if (status === 'abort') {
          // Remove from list
          self._xhrs.splice(index, 1); // If this is the last one, resolve the promise we returned
  ...
  Collection.prototype._deferredAt = function (index, options) {
    var self = this; // If it's virtual, we need to see if this item has been fetched or not: if not, we need to fetch it + fetchSize
  ...
  Collection.prototype._getInternal = function (id, options, deferred, fillIn) {
    ...
    if (this.IsVirtual()) {
      ...
      var self = this;
      return new Promise(function (resolve, reject) {
        var resp = function resp(response) {
          if (response != null) {
            var ind = self._getOffset(); // Check that the model at index is the right one
  ...
  Collection.prototype._setRangeLocalInternal = function (start, count, options) {
    ...
    return new Promise(function (resolve, reject) {
      self._setRangeLocalFetch(start, count, -1, {
        start: start,
        count: count
      }, resolve, reject, true, options);
    });
  };
  /**
   * @private
  ...
  Collection.prototype._setRangeLocalFetch = function (start, count, placement, original, resolve, reject, fill, options) {
    var self = this;
    ...
    var resp = function resp() {
      ...
      if (fill && self._hasTotalResults() && actual.count < original.count) {
        ...
        var newPlacement = actual.start + actual.count; // Try the next block...don't repeat the request

        var newStart = localStart + (self.lastFetchCount ? self.lastFetchCount : count);

        if (newStart < self.totalResults) {
          self._setRangeLocalFetch(newStart, count, newPlacement, original, resolve, reject, fill, options);
        } else {
    ...
    var limit = localStart + count; // Get the greater of the limit-localStart or fetchSize
    ...
    if (this.IsVirtual()) {
      ...
      if (newStart > localStart) {
        localStart = newStart; // Recheck the limit

        limit = localStart + count; // Get the greater of the limit-localStart or fetchSize
  ...
  Collection.prototype._getLocalRange = function (start, count) {
      ...
      if (this._getModelsLength() > 0) {
        if (start + count > this._getModelsLength()) {
          ...
          return {
            start: start,
            count: c,
            models: this._getModelArray(start, c)
          };
        }
        ...
        return {
          start: start,
          count: count,
          models: this._getModelArray(start, count)
        };
      }
    ...
    var limit = this._getMaxLength(start, count); // Adjust for no totalResults
    ...
    return {
      start: start,
      count: localCount,
      models: this._getModelArray(start, localCount)
    };
  };
  /**
  ...
  Collection.prototype._fetchInternal = function (options, placement, fillIn) {
    ...
    opt.success = function (response) {
      ...
      if (!opt.add && !self.model) {
        ...
        if (!fillIn) {
          if (self.IsVirtual()) {
            ...
            doReset(self, opt, fillIn, resetTotalResults); // Check for passed in offset
        ...
        doReset(self, opt, fillIn, resetTotalResults); // Parse each returned model (if appropriate), and put it into the collection, either from the
      ...
      var silent = !!opt.silent;
      self.TriggerInternal(silent, Events.EventType.SYNC, self, response, opt); // Call the caller's success callback, if specified
  ...
  Collection.prototype._putDataIntoCollection = function (data, placement, manageLRU) {
    ...
    if (data) {
      ...
      var addOpt = {}; // Only manage the LRU if we're not trying to achieve a range
      ...
      for (var i = 0; i < dataList.length; i += 1) {
        ...
        this._addInternal(dataList[i], addOpt, true, false); // If virtual, make sure the item was really added where we thought--in other words, what's there now
  ...
  Collection.prototype._fillInCollectionWithParsedData = function (data, placement, manageLRU, options) {
    var opt = options || {};
    var parse = opt.parse;
    ...
    if (data) {
      dataList = data instanceof Array ? data : [data];
      var addOpt = {};
      var parsedModel; // Only manage the LRU if we're not trying to achieve a range
      ...
      if (opt.useset && !virtual) {
        ...
        for (i = 0; i < dataList.length; i += 1) {
          ...
          this._addInternal(parsedModel, addOpt, true, false); // If virtual, make sure the item was really added where we thought--in other words, what's there
  ...
  Collection.prototype._getReturnProperty = function (customObj, response, property, optionValue, defaultValue) {
  ...
  Collection.prototype._setPagingReturnValues = function (response, options, data, fillIn) {
    var customObj = {}; // See if there's a custom call out
    ...
    var opt = options || {};
    this.lastFetchSize = this._getReturnProperty(customObj, response, 'limit', opt.fetchSize, this.fetchSize); // What offset was actually used to generate this response?

    this.offset = this._getReturnProperty(customObj, response, 'offset', opt.startIndex, 0); // How many records actually came back?

    this.lastFetchCount = this._getReturnProperty(customObj, response, 'count', this.lastFetchCount, this.lastFetchCount); // What is the total number of records possible for this collection?

    this.totalResults = this._cleanTotalResults(this._getReturnProperty(customObj, response, 'totalResults', this.totalResults, this.totalResults)); // Is there more?

    this.hasMore = this._getHasMore(Collection._getProp(customObj, response, 'hasMore'), this.offset, this.lastFetchSize, this.totalResults); // Adjust total results to account for the case where the server tells us there's no more data, and
    ...
    if (!fillIn) {
      ...
      var totalResultsReturned = this._cleanTotalResults(parseInt(Collection._getProp(customObj, response, 'totalResults'), 10));
      ...
      retVal = totalResultsReturned === undefined || isNaN(totalResultsReturned) || totalResultsReturned === null;
  ...
  Collection.prototype._adjustTotalResults = function (totalResultsReturned, hasMore, offset, lastFetchCount, dataLength) {
    ...
    if (deferred || this.IsVirtual()) {
      return new Promise(function (resolve) {
        ...
        doAdd(newModel, addOpts).then(function () {
          ...
          var model = doSave(self, newModel, validate, opt); // make sure that success is called first if successful...promise resolved
          // second
  ...
  Collection.prototype._whereInternal = function (attrs, options) {
    ...
    if (this.IsVirtual()) {
      return new Promise(function (resolve, reject) {
        var success = function success(collection, fetchedModels) {
        ...
        var opts = {
          ...
          error: function error(xhr, status, _error) {
            reject(Collection._createRejectionError(xhr, status, _error, self, opt, true));
          }
        };
  ...
  Collection.prototype.without = function (var_args) {
    ...
    var model; // Test each model in the collection
    ...
    for (var i = 0; i < this._getModels().length; i++) {
      ...
      for (j = 0; j < arguments.length; j += 1) {
        // Get the cid
        cid = arguments[j].GetCid(); // Get the id
    ...
    for (var i = 0; i < this._getModels().length; i++) {
      ...
      for (j = 0; j < arguments.length; j += 1) {
        ...
        for (k = 0; k < arguments[j].length; k++) {
          // Get the cid
          cid = arguments[j][k].GetCid(); // Get the id
  ...
  Collection.prototype._swapModels = function (oldIndex, newIndex, remove, add) {
    ...
    var newModel = this._getModel(newIndex); // this._getModels()[oldIndex] = newModel;


    this._setModel(oldIndex, newModel);

    newModel.SetIndex(oldIndex); // this._getModels()[newIndex] = oldModel;
  ...
  Collection.prototype._deferredSet = function (modelList, modelsCopy, options, remove, add, merge, parse) {
    var foundModels = [];
    var i; // Go through the passed in models and determine what to do
  ...
  Collection.prototype._updateModel = function (model, add, merge, deferred) {
    ...
    if (deferred || this.IsVirtual()) {
      var self = this;
      return new Promise(function (resolve) {
        self._getInternal(model, {
          silent: true
        }, deferred).then(function (found) {
  ...
  Collection.prototype.first = function (n, options) {
    ...
    if (elementCount > this._getModelsLength()) {
      if (this.IsVirtual() && !this._hasTotalResults()) {// Virtual, no total results: don't restrict elementCount
      } else {
  ...
  Collection.prototype.include = Collection.prototype.contains; // Only look on models already fetched
  ...
  Collection.prototype.GetCollectionFetchUrl = function (opt) {
    var url = RestImpl.GetPropValue(this, 'url'); // Adorn it with options, if any

    if (this.IsVirtual()) {
      var options = opt || {};
      var all = options.all; // Put in page size
  ...
  */
  ...
  var OAuth = function OAuth(header, attributes) {
    OAuth._init(this, attributes || {}, header || 'Authorization');
  };

  oj._registerLegacyNamespaceProp('OAuth', OAuth); // Subclass from oj.Object
  ...
  OAuth.prototype.clientCredentialGrant = function () {
    var headers = {};
    var self = this;
    headers[self.accessTokenRequest.auth_header] = 'Basic ' + OAuth._base64_encode(self.accessTokenRequest.client_id + ':' + self.accessTokenRequest.client_secret);
    $.ajax({
      type: 'POST',
      async: false,
