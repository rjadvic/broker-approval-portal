 ...
 * @ignore
 */
define(['touchr', 'ojdnd', 'ojs/ojeditablevalue', 'ojs/ojinputnumber', 'ojs/ojmenu', 'ojs/ojpopup', 'ojs/ojbutton', 'ojs/ojdatasource-common', 'ojs/ojdataprovideradapter', 'ojs/ojlistdataproviderview', 'ojs/ojselector', 'ojs/ojcore-base', 'jquery', 'ojs/ojdomutils', 'ojs/ojlogger', 'ojs/ojcontext', 'ojs/ojconfig', 'ojs/ojtranslation', 'ojs/ojthemeutils', 'ojs/ojcomponentcore', 'ojs/ojdatacollection-common', 'ojs/ojanimation', 'ojs/ojdomscroller', 'ojs/ojcustomelement-utils', 'ojs/ojkeyset', 'ojs/ojvalidator-regexp'], function (touchr, ojdnd, ojeditablevalue, ojinputnumber, ojmenu, ojpopup, ojbutton, ojdatasourceCommon, ojdataprovideradapter, ListDataProviderView, ojselector, oj, $, DomUtils, Logger, Context, ojconfig, ojtranslation, ojthemeutils, ojcomponentcore, DataCollectionUtils, ojanimation, DomScroller, ojcustomelementUtils, ojkeyset, RegExpValidator) {
  'use strict';
  ...
  Table.prototype._isStickyLayoutEnabled = function () {
    if (this._defaultOptions == null) {
      this._defaultOptions = ojthemeutils.parseJSONFromFontFamily('oj-table-option-defaults');
    }
    ...
    while (node != null && node !== table) {
      ...
      if (node.nodeType !== 3) {
        ...
        if (nodeName.match(type)) {
          var tabIndex = node.getAttribute(Table.DOM_ATTR._TABINDEX); // ignore elements with tabIndex == -1
  ...
  Table.prototype._clearAllComponentBusyStates = function () {
    this._clearBusyStateStack(); // clear any pending fetch state to ensure clearing data waiting state below does not queue addition fetches
  ...
  Table.prototype._cleanComponent = function (isDestroy) {
    ...
    if (isDestroy) {
      ...
      this.element.children().remove('.' + Table.CSS_CLASSES._TABLE_BODY_MESSAGE_ROW_CLASS); //  - DomUtils.unwrap() will avoid unwrapping if the node is being destroyed by Knockout

      DomUtils.unwrap(this.element, $(this._getTableContainer()));
      this.element[0].classList.remove(Table.CSS_CLASSES._TABLE_CLASS); // If this._data is a TableDataSourceAdapter, call destroy so that it can remove listeners
      ...
      if (this._hasHeaderTemplate || this._hasCellTemplate || this._hasFooterTemplate || this._hasRowTemplate) {
  ...
  Table.prototype._draw = function () {
    this._setFinalTask(function () {
      ...
      if (this._resetAriaLabel && this._accRowIndex != null && this._accColumnIndex != null) {
        this._updateAccStatusInfo(this._accRowIndex, this._accColumnIndex, this._accHeaderIndex, this._accFooterIndex);
      }
    ...
    this._createInitialTable(this._isTableHeaderless(), this._isTableFooterless()); // style the initial table structure


    this._styleInitialTable();

    this._queueTask(function () {
      this._getLayoutManager().notifyTableUpdate(Table._UPDATE._REFRESH); // populate the table header DOM with header content


      this._refreshTableHeader(); // populate the table footer DOM with footer content
  ...
  Table.prototype._refreshTableHeader = function () {
    ...
    var columnsCount = columns.length; // replace existing col elements
    ...
    for (i = 0; i < columnsCount; i++) {
      ...
      if (headerRenderer) {
        ...
        var headerContext = this._getRendererContextObject(headerColumn, {}); // Copy additional properties to top-level context to work with custom element
    ...
    if (this._isDefaultSelectorEnabled()) {
      ...
      tableHeaderRow.insertBefore(tableHeaderSelectorColumn, // @HTMLUpdateOK
    ...
    if (columns.length > 0) {
      ...
      for (var i = 0; i < columnsCount; i++) {
        ...
        if (footerRenderer) {
          ...
          var footerContext = this._getRendererContextObject(footerCell, {}); // Copy additional properties to top-level context to work with custom element
          ...
          if (footerCellContent != null) {
            // if the renderer returned a value then we set it as the content
            // for the footer cell
            $(footerCell).empty(); // Use jquery append() for this as a convenience because
    ...
    if (rows.length === 0 && tableBodyRows.length === 0) {
      ...
      if (startIndex > 0 && this._isOverflow() && !this._fetchBySyncScroll) {
        return new Promise(function (resolve, reject) {
          this._setIdleRenderBusyState(); // clone the rows array since we'll manipulate it


          this._renderRowsWhenIdle(rows.slice(0), tableBody, startIndex, resolve, reject, resultObject.isMouseWheel);
        }.bind(this));
      }
  ...
  Table.prototype._isDefaultSelectorEnabled = function () {
    ...
    if (this._defaultOptions.enableSelector === true && this._getRowSelectionMode() === Table._OPTION_SELECTION_MODES._MULTIPLE) {
      return true;
    }
  ...
  Table.prototype._refreshTableBodyRow = function (rowIdx, row, tableBodyRow, docFrag, docFragStartIdx) {
    ...
    if (rowRenderer != null || this._isDefaultRowTemplateSlotValid() && this._getSlotTemplate('rowTemplate') !== null) {
      if (rowRenderer != null) {
        ...
        if (templateEngine != null) {
          ...
          var nodes = templateEngine.execute(componentElement, rowSlotTemplate, slotContext, this.options.as);
      ...
      this._styleTableBodyRow(tableBodyRow, false); // set the cell attributes and styling.
      ...
      for (var _i = 0; _i < tableBodyCellsCount; _i++) {
        tableBodyCell = tableBodyCells[_i];

        this._setTableBodyCellAttributes(rowIdx, row[Table._CONST_KEY], rowHashCode, _i, tableBodyCell);

        this._styleTableBodyCell(_i, tableBodyCell, false);
      } // sort the re-ordered columns in place


      if (this._columnsDestMap != null) {
        for (var _i2 = 0; _i2 < this._columnsDestMap.length; _i2++) {
          var moveTableBodyCell = tableBodyCells[this._columnsDestMap[_i2]];
  ...
  Table.prototype._refreshRow = function (rowIdx, resetFocus) {
    ...
    return this._queueTask(function () {
      ...
      return dataprovider.fetchByKeys({
        ...
        var tableBodyRow = this._getTableBodyRow(rowIdx); // eslint-disable-next-line no-param-reassign
  ...
  Table.prototype._showStatusMessage = function () {
    ...
    if (!this._statusMessageShown && !tempSkeletonRow) {
      ...
      if (this._isSkeletonSupport() && !this._isPagingModelDataProvider()) {
  ...
  Table.prototype._showNoDataMessage = function () {
    if (!this._noDataMessageShown) {
      ...
      if (this._isDefaultTemplateSlotValid('noData') && noDataTemplate !== null) {
        ...
        if (templateEngine != null) {
          var nodes = templateEngine.execute(this._getRootElement(), noDataTemplate, {}, null);
        ...
        var messageText = dataprovider != null ? emptyTextMsg : this.getTranslatedString(Table._BUNDLE_KEY._MSG_INITIALIZING);
    ...
    if (this._noDataMessageShown) {
      ...
      if (tableBodyMessageRow != null) {
        ...
        if (table.classList.contains(Table.CSS_CLASSES._TABLE_NO_DATA_CONTAINER_CLASS)) {
          ...
          var noDataRow = table.querySelector('tr.' + Table.CSS_CLASSES._TABLE_NO_DATA_CONTAINER_CLASS);
  ...
  Table.prototype._handleContextMenuResizePopup = function () {
    var spinner = document.getElementById(this._getTableId() + '_resize_popup_spinner');
    ...
    this._queueTask(function () {
      ...
      layoutManager.notifyTableUpdate(Table._UPDATE._COL_RESIZE); // delay the focus to ensure table is fully updated before focus is restored
  ...
  Table.prototype._handleContextMenuSelect = function (event, ui) {
    ...
    var headerColumn = this._getFirstAncestor(this._contextMenuEvent.target, '.' + Table.CSS_CLASSES._COLUMN_HEADER_CELL_CLASS, true);

    headerColumn = headerColumn == null ? this._contextMenuEventHeaderColumn : headerColumn;

    var tableBodyCell = this._getFirstAncestor(this._contextMenuEvent.target, '.' + Table.CSS_CLASSES._TABLE_DATA_CELL_CLASS, true);
    ...
    if (menuItemCommand === 'oj-table-sortAsc') {
      ...
      this._removeTableBodyRowTouchSelectionAffordance(); // update to disable command
  ...
  Table.prototype._registerResizeListener = function () {
    ...
    if (!this._resizeListener) {
      this._resizeListener = function () {
        ...
        var tableContainer = this._getTableContainer(); // Check if the width and height values should result in a sizing refresh


        if (layoutManager.isSizingRefreshRequired(tableContainer.offsetWidth, tableContainer.offsetHeight)) {
          layoutManager.notifyTableUpdate(Table._UPDATE._RESIZE); // refresh dimensions if no tasks are pending since refresh dimensions runs during final task
    ...
    if (!this._isResizeListenerAdded) {
      DomUtils.addResizeListener(element, this._resizeListener, 50);
  ...
  Table.prototype._isTableHeaderless = function () {
    ...
    for (var i = 0; i < columnsCount; i++) {
      ...
      if (columns[i].headerText != null || columns[i].headerStyle != null || columns[i].sortable != null && columns[i].sortable !== Table._OPTION_DISABLED || columns[i].sortProperty != null || columns[i].headerRenderer != null || headerSlotTemplate != null) {
        return false;
      }
  ...
  Table.prototype._handleMouseEnterColumnHeader = function (element) {
    if (element) {
      element.classList.add(Table.MARKER_STYLE_CLASSES._HOVER); // get the column index of the header element

      var columnIdx = this._getElementColumnIdx(element); // add hover class if element is sort icon


      if (element.classList.contains(Table.CSS_CLASSES._COLUMN_HEADER_ASC_ICON_CLASS) || element.classList.contains(Table.CSS_CLASSES._COLUMN_HEADER_DSC_ICON_CLASS) || element.classList.contains(Table.CSS_CLASSES._COLUMN_HEADER_DEFAULT_SORT_ICON_CLASS)) {
  ...
  Table.prototype._handleMouseLeaveColumnHeader = function (element) {
    if (element) {
      element.classList.remove(Table.MARKER_STYLE_CLASSES._HOVER); // get the column index of the header element

      var columnIdx = this._getElementColumnIdx(element); // hide the sort icon for the header
  ...
  Table.prototype._hideTableHeaderColumnSortIcon = function (columnIdx) {
    ...
    if (this._getColumnDefs()[columnIdx].sortable === Table._OPTION_ENABLED) {
      var tableHeaderColumn = this._getTableHeaderColumn(columnIdx); // check if the column is currently sorted
  ...
  Table.prototype._getColumnMetadata = function () {
    ...
    for (i = 0; i < defaultedColumnsCount; i++) {
      ...
      if (sortSupportedData && (columnsSortedArray[i].sortable == null || columnsSortedArray[i].sortable === Table._OPTION_AUTO)) {
    ...
    this._domScrollerSuccessFunc = function (result) {
      ...
      if (result != null) {
        ...
        if (result.maxCountLimit) {
          ...
          var i; // remove any duplicate rows that are already in the DOM
  ...
  Table.prototype._clearIdleCallback = function () {
    if (this._idleCallback != null) {
      if (!window.requestIdleCallback || !window.cancelIdleCallback) {
        ...
        window.cancelIdleCallback(this._idleCallback); // requestAnimationFrame might have been used
  ...
  Table.prototype._renderRowsWhenIdle = function (rows, tableBody, startIndex, resolve, reject, isMouseWheel) {
    if (rows.length === 0) {
      this._idleCallback = null;

      this._afterRowsRendered(tableBody).then(function (value) {
        resolve(value);
      }, function (reason) {
        reject(reason);
      });
  ...
  Table.prototype._getColumnIdxsForElementsWithStyleClass = function (styleClass) {
    ...
    if (elements && elements.length > 0) {
      ...
      for (var i = 0; i < elementsCount; i++) {
        ...
        if (columnIdx === null && (elements[i].classList.contains(Table.CSS_CLASSES._COLUMN_HEADER_SELECTOR_CELL_CLASS) || elements[i].classList.contains(Table.CSS_CLASSES._TABLE_FOOTER_SELECTOR_CELL_CLASS))) {
          columnIdx = -1;
        }
  ...
  Table.prototype._addSingleTableBodyRow = function (rowIdx, row, docFrag, docFragStartIdx) {
    ...
    this._styleTableBodyRow(tableBodyRow, true); // insert the <tr> element in to the table body DOM
  ...
  Table.prototype._initFetch = function (options, isSortUpdate) {
    ...
    var layoutManager = this._getLayoutManager(); // do an initial fetch if a TableDataSource
    // paging control should do the fetches for PagingTableDataSource


    if (dataprovider != null && oj.DataProviderFeatureChecker.isDataProvider(dataprovider) && (!this._isPagingModelDataProvider() || isSortUpdate)) {
      return this._queueTask(function () {
        layoutManager.notifyTableUpdate(isSortUpdate ? Table._UPDATE._DATA_SORT : Table._UPDATE._REFRESH); // reset the scrollTop when we do an initial fetch
      ...
      return this._queueTask(function () {
        layoutManager.notifyTableUpdate(isSortUpdate ? Table._UPDATE._DATA_SORT : Table._UPDATE._REFRESH);
        return Promise.resolve();
      });
    }
  ...
  Table.prototype._initTemplateEngine = function () {
    ...
    this._queueTask(function () {
      ...
      try {
        enginePromise = ojconfig.__getTemplateEngine().then(function (engine) {
  ...
  Table.prototype._initializeFetchFirstOptions = function (options) {
    var updatedOptions = options || {}; // Create a clientId symbol that uniquely identify this consumer so that
  ...
  Table.prototype._invokeDataFetchRows = function (options) {
    ...
    if (dataprovider != null) {
      return new Promise(function (resolve) {
        ...
        this._dataProviderAsyncIterator = dataprovider.fetchFirst(updatedOptions)[Symbol.asyncIterator]();

        var helperFunction = function (result, currentMetadata, scrollToKey) {
          var updatedScrollToKey = scrollToKey; // checks whether the key is fetched, otherwise we'll continue to fetch
          ...
          var fetchMoreData = nextPromise.then(function (value) {
            // eslint-disable-next-line no-param-reassign
            result.done = value.done; // eslint-disable-next-line no-param-reassign

            result.value.data = result.value.data.concat(value.value.data); // eslint-disable-next-line no-param-reassign
        ...
        Promise.all([dataPromise, scrollToKeyPromise]).then(function (values) {
          var result = values[0];
          ...
          var metadataArray = value[Table._CONST_METADATA]; // Need to clear DOM scroller before refreshAll potentially triggers
  ...
  Table.prototype._invokeDataSort = function (sortField, ascending, event) {
    ...
    this._beforeDataRefresh(); // show the Fetching Data... message
      ...
      for (var i = 0; i < columnsMetadataCount; i++) {
        ...
        for (var j = 0; j < props.length; j++) {
          ...
          if (columnsMetadata[i][prop] !== this._columnDefArray[i][prop]) {
            if (prop !== 'id' || columnsMetadata[i][prop] == null || columnsMetadata[i][prop].indexOf(Table._COLUMN_HEADER_ID_PREFIX) !== 0 || this._columnDefArray[i][prop] == null || this._columnDefArray[i][prop].indexOf(Table._COLUMN_HEADER_ID_PREFIX) !== 0) {
      ...
      for (var i = 0; i < indexesCount; i++) {
        rowIdxRowArray.push({
          row: {
            ...
            metadata: resultObject[Table._CONST_METADATA] ? resultObject[Table._CONST_METADATA][i] : null,
  ...
  Table.prototype._isPagingModelDataProvider = function () {
    ...
    return dataprovider.getStartItemIndex != null && dataprovider.getStartItemIndex() !== null && dataprovider.getStartItemIndex() >= 0;
  };
  /**
  ...
  var TableDndContext = function TableDndContext(component) {
    this.component = component;
    this.Init();
  };

  oj._registerLegacyNamespaceProp('TableDndContext', TableDndContext); // Subclass from oj.Object
  ...
  TableDndContext.prototype._removeDragMarkerClass = function () {
    var dragColumns = this.component._getTableElementsByClassName(this.component._getTableHeader(), 'oj-table-drag-source-opaque');
  ...
  TableDndContext.prototype._cloneTableContainer = function (tableContainer) {
    var tableHeader = this.component._getTableElementsByTagName(tableContainer, Table.DOM_ELEMENT._THEAD)[0];

    var tableBody = this.component._getTableElementsByTagName(tableContainer, Table.DOM_ELEMENT._TBODY)[0];

    var scrollLeft = $(tableBody).scrollLeft();
    var tableContainerClone = tableContainer.cloneNode();

    var tableElementClone = this.component._getTableElementsByTagName(tableContainer, Table.DOM_ELEMENT._TABLE)[0].cloneNode();
    ...
    this.component._rowsDragged.forEach(function (row) {
      ...
      if (this.component._isDefaultSelectorEnabled()) {
        var selector = this.component._getTableElementsByClassName(rowClone, Table.CSS_CLASSES._TABLE_DATA_ROW_SELECTOR_CLASS)[0];
      ...
      rowClone.classList.remove('oj-selected');
      rowClone.classList.remove('oj-focus');
      rowClone.classList.remove('oj-focus-highlight');
      clonedCells.forEach(function (cell, index) {
        // eslint-disable-next-line no-param-reassign
        cell.style.width = cells[index].offsetWidth + Table.CSS_VAL._PX;
        cell.classList.remove('oj-selected');
        cell.classList.remove('oj-hover');
      });
      tableBodyClone.appendChild(rowClone);
    }.bind(this));

    tableElementClone.appendChild(tableBodyClone);
    tableContainerClone.appendChild(tableElementClone); // Use absolute positioning with a large negative top to put it off the screen without
    ...
    if (this.component._isTableHeaderless()) {
      ...
      tableContainerClone.style[Table.CSS_PROP._HEIGHT] = tableBody.scrollHeight + tableHeader.offsetHeight + Table.CSS_VAL._PX;
  ...
  TableDndContext.prototype._getOverRowIndex = function (event) {
    ...
    var overRow = this.component._getFirstAncestor(event.target, Table.DOM_ELEMENT._TR, true); // Find the index from the DOM directly instead of calling this.component._getElementRowIdx.  There was a change in
    // this.component._getElementRowIdx to use cached elements so it's not accurate for dnd purpose.


    var cell = this.component._getFirstAncestor(event.target, '.' + Table.CSS_CLASSES._TABLE_DATA_CELL_CLASS + ', .' + Table.CSS_CLASSES._TABLE_SELECTOR_CELL, true);
    ...
    if (cell != null) {
      ...
      newRowIndex = tableBodyRows.length; // When drop target is empty table
  ...
  TableDndContext.prototype.handleColumnDragStart = function (event) {
    if (this._isColumnReorderEnabled()) {
      ...
      this._setReorderColumnsDataTransfer(event, this._dragStartColumnIdxs); // Remove text selection
      ...
      this._dragStartColumnIdxs.forEach(function (columnIdx) {
  ...
  TableDndContext.prototype.handleColumnReorderDragOver = function (event) {
    ...
    if (this._dragStartColumnIdxs != null && this._dragStartColumnIdxs.length > 0 && this._dragStartColumnIdxs.indexOf(columnIdx) === -1) {
      ...
      if (!(this._currentDropColumnBefore && this._dragStartColumnIdxs.indexOf(columnIdx - 1) > -1) && !(!this._currentDropColumnBefore && this._dragStartColumnIdxs.indexOf(columnIdx + 1) > -1)) {
  ...
  TableDndContext.prototype.handleColumnReorderDrop = function (event) {
    ...
    if (this._dragStartColumnIdxs.indexOf(columnIdx) === -1) {
      this.component._columnsDestMap = this.component._moveTableHeaderColumn(this._dragStartColumnIdxs, columnIdx, event);
    }
  ...
  TableDndContext.prototype._invokeDndCallback = function (dndType, itemType, callbackType, event, ui) {
  ...
  TableDndContext.prototype._isDragOverBeforeColumn = function (event) {
  ...
  TableDndContext.prototype._setDragRowsDataTransfer = function (event, dataTypes, selArray) {
    var rowDataArray = [];
    this.component._rowsDragged = []; // Add data for selected rows to an array

    for (var i = 0; i < selArray.length; i++) {
      var rowData = this.component.getDataForVisibleRow(selArray[i]);

      var row = this.component._getTableBodyRow(selArray[i]);
    ...
    if (rowDataArray.length) {
      ...
      this._dragImage = this._setDragRowsImage(event.originalEvent, this.component._getTableContainer());
      return {
        rows: rowDataArray
      };
    }
  ...
  TableDndContext.prototype._setDragRowsImage = function (nativeEvent, tableContainer) {
    ...
    var offsetY = Math.max(0, nativeEvent.offsetY); // Drag source styles
  ...
  TableDndContext.prototype._setReorderColumnsDataTransfer = function (event, columnIdxs) {
    ...
    dataTransfer.setData('Text', Table._DND_REORDER_TABLE_ID_DATA_KEY + ':' + tableIdHashCode + ':' + columnIdxs.join('-'));
  ...
  TableDndContext.prototype._updateDragRowsState = function (event, newRowIndex) {
    if (this._dropRowIndex !== newRowIndex) {
      ...
      this._dropRowIndex = newRowIndex; // indicator style based on whether dnd within component or across component

      var indicatorStyle = event.currentTarget.classList.contains('oj-table-drag-source') || event.currentTarget.classList.contains('oj-table-drop-target-empty') ? 'space' : 'line';

      this.component._displayDragOverIndicatorRow(this._dropRowIndex, overRow, indicatorStyle, event.currentTarget.classList.contains('oj-table-drag-source'));
    }
  };
  /**
   * @license
  ...
  var TableLayoutManager = function TableLayoutManager(table) {
    this._table = table;
  ...
  TableLayoutManager.prototype.isSizingRefreshRequired = function (width, height) {
    if (width != null && height != null) {
      ...
      return width > 0 && height > 0 && (width !== sizingState.outerWidth || height !== sizingState.outerHeight);
  ...
  TableLayoutManager.prototype._finalizeTableDimensions = function (scrollTop, scrollLeft) {
    ...
    sizingState.outerHeight = tableContainer.offsetHeight;
  ...
  TableLayoutManager.prototype._getBoxStyle = function (style) {
    return {
      boxSizing: style[Table.CSS_PROP._BOX_SIZING],
      borderWidth: (parseFloat(style[Table.CSS_PROP._BORDER_RIGHT_WIDTH]) || 0) + (parseFloat(style[Table.CSS_PROP._BORDER_LEFT_WIDTH]) || 0),
      paddingWidth: (parseFloat(style[Table.CSS_PROP._PADDING_RIGHT]) || 0) + (parseFloat(style[Table.CSS_PROP._PADDING_LEFT]) || 0)
    };
  };
  /**
   * @private
  ...
  TableLayoutManager.prototype._applyColumnHeaderHeight = function (headerColumnCell, height) {
    var headerColumnDiv = this._table._getTableElementsByClassName(headerColumnCell, Table.CSS_CLASSES._COLUMN_HEADER_CLASS);
  ...
  TableLayoutManager.prototype.getMinimumForcedOffsetWidth = function (columnIndex) {
    var minWidth = 0; // find minimum header width
  ...
  TableLayoutManager.prototype._isVerticalScrollBarHidden = function () {
    return !this._table._isTouchDevice() && this._table._getTableContainer().classList.contains(Table.MARKER_STYLE_CLASSES._HIDE_VERTICAL_SCROLLBAR);
  };
  /**
  ...
  TableLayoutManager.prototype._removeTableDimensionsStyling = function () {
    ...
    tableContainer.classList.remove(Table.CSS_CLASSES._TABLE_SCROLL_HORIZONTAL_CLASS); // first remove any styling so that the browser sizes the table
    ...
    if (tableHeader != null) {
      ...
      var headerColumnTextDivs = this._table._getTableElementsByClassName(tableHeaderRow, Table.CSS_CLASSES._COLUMN_HEADER_TEXT_CLASS);
  ...
  TableLayoutManager.prototype._getTableContainerScrollableState = function (bottomSlotHeight) {
    ...
    if (tableContainer.clientHeight > 0) {
      var innerHeight = tableElem.offsetHeight + bottomSlotHeight; // don't use tableContainer[0].scrollHeight as it may not be accurate
    ...
    if (tableContainer.clientWidth > 0) {
      if (tableElem.offsetWidth > tableContainer.clientWidth) {
        // overflow
        result[1] = 1;
      } else if (tableContainer.clientWidth - tableElem.offsetWidth > 0) {
        // underflow
        result[1] = -1;
      } else {
        result[1] = 0;
  ...
  TableLayoutManager.prototype._setForcedColumnWidths = function () {
    ...
    var tableBodyRows = this._table._getTableBodyRows(); // find and set the widths of the forced width columns, cells, and footers first
    ...
    for (var i = 0; i < columnsCount; i++) {
      ...
      if (columnWidth != null) {
        ...
        var forcedColumnCellWidth = null; // update column header cell width

        headerCell = this._table._getTableHeaderColumn(i);

        if (headerCell != null) {
          var forcedColumnWidth = this._getForcedColumnWidth(headerCell, window.getComputedStyle(headerCell), columnWidth);
        ...
        if (tableBodyCell != null) {
          if (!this._table._hasRowOrCellRendererOrTemplate(i)) {
            ...
            forcedColumnCellWidth = this._getForcedColumnWidth(tableBodyCell, columnCellCompStyle, columnWidth);
          }
          ...
          for (var j = 0; j < tableBodyRows.length; j++) {
            ...
            if (tableBodyCell != null) {
              if (columnCellCompStyle == null) {
                ...
                var forcedCellWidth = this._getForcedColumnWidth(tableBodyCell, cellCompStyle, columnWidth);
        ...
        if (footerCell != null) {
          this._applyForcedColumnWidth(footerCell, this._getForcedColumnWidth(footerCell, window.getComputedStyle(footerCell), columnWidth));
        }
      } else {
  ...
  TableLayoutManager.prototype._verifyMinAndMaxWidths = function () {
    ...
    var continueSizing = true; // flag to check track whether full sizing run was successfull
  ...
  TableLayoutManager.prototype._applyColMinMax = function (columns, columnIndex, rowsCount, isMin) {
    ...
    var colTestWidth = isMin ? this._getPixelStyleEquivalent(columns[columnIndex].minWidth) : this._getPixelStyleEquivalent(columns[columnIndex].maxWidth);

    if (colTestWidth != null) {
      headerCell = this._table._getTableHeaderColumn(columnIndex); // if first row is in edit mode then fetching cell style from second row
      ...
      if (colWidth > 0 && (isMin ? colWidth < colTestWidth : colWidth > colTestWidth)) {
        valueApplied = true;
        this._forcedWidthColumns[columnIndex] = colTestWidth; // update column header cell width

        if (headerCell != null) {
          var forcedColumnWidth = this._getForcedColumnWidth(headerCell, window.getComputedStyle(headerCell), colTestWidth);
        ...
        if (!this._table._hasRowOrCellRendererOrTemplate(columnIndex)) {
          columnCellCompStyle = window.getComputedStyle(tableBodyCell);
          forcedColumnCellWidth = this._getForcedColumnWidth(tableBodyCell, columnCellCompStyle, colTestWidth);
        }
        ...
        for (var i = 0; i < rowsCount; i++) {
          ...
          if (tableBodyCell != null) {
            if (columnCellCompStyle == null) {
              ...
              var forcedCellWidth = this._getForcedColumnWidth(tableBodyCell, cellCompStyle, colTestWidth);
        ...
        if (footerCell != null) {
          this._applyForcedColumnWidth(footerCell, this._getForcedColumnWidth(footerCell, window.getComputedStyle(footerCell), colTestWidth));
        }
      }
    }
  ...
  var TableLegacyLayoutManager = function TableLegacyLayoutManager(table) {
  ...
  TableLegacyLayoutManager.prototype.getRowBottomOverflowDiff = function (tableBodyRow) {
    ...
    var rowRect = tableBodyRow.getBoundingClientRect();
    var scrollingElementRect = scroller.getBoundingClientRect();
    return rowRect.bottom - scrollingElementRect.bottom + (scroller.offsetHeight - scroller.clientHeight - scroller.clientTop);
  };
  /**
  ...
  TableLegacyLayoutManager.prototype._setColumnWidths = function (scrollBarWidth) {
    ...
    var columnsCount = this._table._getColumnDefs().length; // loop through remaining columns to get the remaining column widths
    ...
    for (i = 0; i < columnsCount; i++) {
      if (this._forcedWidthColumns[i] !== false) {
        ...
        if (headerCell != null) {
          headerWidths[i] = this._getForcedColumnWidth(headerCell, window.getComputedStyle(headerCell)); // also find the column header cell height

          var headerTextDiv = this._table._getTableElementsByClassName(headerCell, Table.CSS_CLASSES._COLUMN_HEADER_TEXT_CLASS);
        ...
        if (tableBodyCell != null) {
          cellWidths[i] = this._getForcedColumnWidth(tableBodyCell, window.getComputedStyle(tableBodyCell));
        ...
        if (footerCell != null) {
          footerWidths[i] = this._getForcedColumnWidth(footerCell, window.getComputedStyle(footerCell));
        }
      }
    ...
    for (i = 0; i < columnsCount; i++) {
      ...
      if (this._forcedWidthColumns[i] !== false) {
        ...
        if (i === columnsCount - 1 && !this.isTableWidthConstrained() && scrollBarWidth > 0) {
          if (headerCell != null) {
            this._applyForcedColumnWidth(headerCell, this._forcedWidthColumns[i] + scrollBarWidth); // update column header cell height
        ...
        if (i === columnsCount - 1 && !this.isTableWidthConstrained() && scrollBarWidth > 0) {
          // extend width of last header and footer cell by scrollbar width only when verical scrollbar is present
          if (headerCell != null) {
            this._applyForcedColumnWidth(headerCell, headerWidths[i] + scrollBarWidth); // also update column header cell height
          ...
          if (headerCell != null) {
            this._applyForcedColumnWidth(headerCell, headerWidths[i]); // also update column header cell height
  ...
  TableLegacyLayoutManager.prototype.refreshTableDimensions = function () {
    ...
    if (!tableUpdates.has(Table._UPDATE._DATA_REFRESH) && !tableUpdates.has(Table._UPDATE._DATA_SORT) && !tableUpdates.has(Table._UPDATE._RESIZE) && !tableUpdates.has(Table._UPDATE._REFRESH) && !tableUpdates.has(Table._UPDATE._COL_RESIZE) && !tableUpdates.has(Table._UPDATE._COL_REORDER) && !tableUpdates.has(Table._UPDATE._ROW_REFRESH) && (!tableUpdates.has(Table._UPDATE._ROWS_ADDED) || sizingState.hasVerticalOverflow) && (!tableUpdates.has(Table._UPDATE._ROWS_REMOVED) || !sizingState.hasVerticalOverflow)) {
    ...
    var tableBottomSlot = this._table._getTableBottomSlot(); // preserve the scrollTop & scrollLeft
    ...
    this._table._styleTableContainer(tableContainer); // set the forced column widths before checking for scroll state, otherwise we
    // may miss cases where the columns[].width attribute values cause overflow


    this._setForcedColumnWidths();

    this._verifyMinAndMaxWidths(); // find Table's bottom slot height to use when determining if overflow if present


    var bottomSlotHeight;

    if (tableBottomSlot != null && tableBottomSlot.clientHeight > 0 && tableBottomSlot.style[Table.CSS_PROP._DISPLAY] !== Table.CSS_VAL._NONE) {
    ...
    if (sizingState.hasVerticalOverflow || sizingState.hasHorizontalOverflow) {
      ...
      if (browserScrollBarSize > 0) {
        if (!sizingState.hasHorizontalOverflow && !this._isVerticalScrollBarHidden()) {
          ...
          tableElem.style[Table.CSS_PROP._WIDTH] = containerClientWidth + browserScrollBarSize + Table.CSS_VAL._PX;
          ...
          tableElem.style[Table.CSS_PROP._HEIGHT] = containerClientHeight + browserScrollBarSize - bottomSlotHeight + Table.CSS_VAL._PX;
          ...
          if (tableContainer.clientHeight < containerClientHeight + browserScrollBarSize) {
            ...
            if (containerClientHeight - browserScrollBarSize < tableElem.clientHeight + bottomSlotHeight) {
              sizingState.hasVerticalOverflow = true;
            }
          } else {
            containerClientHeight = tableContainer.clientHeight;
      ...
      if (sizingState.hasVerticalOverflow) {
        ...
        var noDataRow = tableElem.querySelector('tr.' + Table.CSS_CLASSES._TABLE_NO_DATA_CONTAINER_CLASS);
      ...
      if (caption.length > 0) {
        ...
        if (tableHeader != null) {
          ...
          tableHeader.style[Table.CSS_PROP._BORDER_TOP] = tableContainerStyle[Table.CSS_PROP._BORDER_TOP];
        }
      ...
      var tableBodyHeight = containerClientHeight - tableHeaderHeight - tableFooterHeight - captionHeight - bottomSlotHeight - tableBodyBorderHeight;
  ...
  var TableStickyLayoutManager = function TableStickyLayoutManager(table) {
  ...
  TableStickyLayoutManager.prototype.refreshTableDimensions = function () {
    ...
    if (tableUpdates.has(Table._UPDATE._REFRESH)) {
      this._clearAllCache();
    } else if (tableUpdates.has(Table._UPDATE._DATA_REFRESH) || tableUpdates.has(Table._UPDATE._RESIZE) || tableUpdates.has(Table._UPDATE._COL_RESIZE) || tableUpdates.has(Table._UPDATE._COL_REORDER) || tableUpdates.has(Table._UPDATE._ROWS_ADDED) && !sizingState.hasVerticalOverflow || tableUpdates.has(Table._UPDATE._ROWS_REMOVED) && sizingState.hasVerticalOverflow) {
    ...
    var tableBottomSlot = this._table._getTableBottomSlot(); // find Table's bottom slot height to use when determining if overflow if present
  ...
  TableStickyLayoutManager.prototype._setupTableHeight = function (bottomSlotHeight) {
    ...
    var tableScroller = this._table._getTableScroller(); // clear any previous table scroller styling


    tableScroller.removeAttribute(Table.DOM_ATTR._STYLE); // Size the table scroller to fit in the container height

    var containerClientHeight = tableContainer.clientHeight;
    var tableScrollerHeight = containerClientHeight - bottomSlotHeight;

    if (tableScroller.offsetHeight !== tableScrollerHeight) {
  ...
  TableStickyLayoutManager.prototype._determineColumnWidths = function () {
    ...
    this._savePreferredColWidths(); // set column widths on the actual column elements, and save the final inner table elem width


    var overallWidth = this._setAllColumnWidths(); // remove any forced column width values from the initial fetch as they are no longer needed
  ...
  TableStickyLayoutManager.prototype._savePreferredColWidths = function () {
    ...
    if (this._columnInitWidths == null) {
      ...
      var columnsCount = this._table._getColumnDefs().length; // loop through columns to get the remaining column widths
  ...
  Table.prototype._updateAccStatusInfo = function (rowIndex, columnIndex, headerIndex, footerIndex) {
    ...
    var label = '';
    var stateInfo = '';
    var tableHeaderColumn; // include overall Table aria-label if needed
    ...
    if (headerIndex != null) {
      ...
      this._contextInfo.textContent = this.getTranslatedString('accessibleColumnHeaderContext', {
        index: headerIndex + 1
      });
      ...
      if (headerIndex === -1) {
        ...
        if (sorted != null) {
          if (sorted === Table._COLUMN_SORT_ORDER._ASCENDING) {
            stateInfo += this.getTranslatedString('accessibleSortAscending', {
              id: ''
            }) + ' ';
          } else {
            stateInfo += this.getTranslatedString('accessibleSortDescending', {
              id: ''
            }) + ' ';
          }
        } else if (column.sortable === Table._OPTION_ENABLED) {
          stateInfo += this.getTranslatedString('accessibleSortable', {
            id: ''
          }) + ' ';
        }
      ...
      this._contextInfo.textContent = this.getTranslatedString('accessibleColumnFooterContext', {
        index: footerIndex + 1
      });
      label += this._contextInfo.id + ' ';
      var tableFooterCell;
      ...
      if (rowIndex !== this._accRowIndex) {
        this._contextInfo.textContent += this.getTranslatedString('accessibleRowContext', {
          index: rowIndex + 1
        }) + ' ';
      }

      if (columnIndex !== this._accColumnIndex) {
        this._contextInfo.textContent += this.getTranslatedString('accessibleColumnContext', {
          index: columnIndex + 1
        });
      }

      label += this._contextInfo.id + ' '; // populate cell information

      var tableBodyCellElements = this._getTableElementsByClassName(this._getTableBodyRow(rowIndex), Table.CSS_CLASSES._TABLE_DATA_CELL_CLASS);
  ...
  Table.prototype._setRowLabelledBy = function (rowIdx, tableBodyRow) {
    ...
    for (i = 0; i < tableBodyCellElements.length; i++) {
      label += this._getTableHeaderColumn(i).id + ' ' + tableBodyCellElements[i].id + ' ';
    }
    ...
    this._stateInfo.textContent = stateInfo; // apply new label
  ...
  Table.prototype._setHeaderColumnLabelledBy = function (columnIdx) {
    ...
    var stateInfo = ''; // handle select all column header case
    ...
    label += this._contextInfo.id + ' ' + tableHeaderColumn.id + ' ' + this._stateInfo.id; // apply new label
  ...
  Table.prototype._getData = function () {
    if (!this._data && this.options.data != null) {
      ...
      if (oj.TableDataSource && dataprovider instanceof oj.TableDataSource || this._isPagingModelTableDataSource()) {
        this._data = new oj.TableDataSourceAdapter(dataprovider);
      } else if (oj.DataProviderFeatureChecker.isDataProvider(dataprovider)) {
        if (!(dataprovider instanceof ListDataProviderView) && !oj.DataProviderFeatureChecker.isTreeDataProvider(dataprovider)) {
  ...
  Table.prototype._beforeDataRefresh = function () {
    ...
    this._adjustScrollPositionOnFetch();
  ...
  Table.prototype._executeTableBodyRowsRemove = function (eventDetail, addEventDetail) {
    ...
    if (eventDetailKeys && eventDetailKeys.size > 0) {
      this._queueTask(function () {
        ...
        self._getLayoutManager().notifyTableUpdate(Table._UPDATE._ROWS_REMOVED); // sort array
        ...
        function _afterRemoveRows() {
          // update selection state based on row removals
          self._updateSelectionStateFromEventDetailRemove(eventDetail, addEventDetail); // row values may have changed so refresh the footer
  ...
  Table.prototype._executeTableBodyRowsAdd = function (eventDetail) {
    ...
    if (eventDetailKeys && eventDetailKeys.size > 0) {
      this._queueTask(function () {
        ...
        this._getLayoutManager().notifyTableUpdate(Table._UPDATE._ROWS_ADDED); // see if we should batch add
        ...
        if (rows.length > 1) {
          ...
          if (isContiguous) {
            ...
            for (i = 0; i < rowsCount; i++) {
              addedTableBodyRows.push(this._addSingleTableBodyRow(rows[i].rowIdx, rows[i].row, tableBodyDocFrag, rows[0].rowIdx));
            }
        ...
        this._clearCachedDomRowData(); // row values may have changed so refresh the footer


        this._refreshTableFooter(); // for high watermark scrolling, we need to reset the domscroller in case there are more rows to fetch
  ...
  Table.prototype._getRowsFromEventDetailAdd = function (eventDetail) {
    ...
    var initialKeyLength = initialKeys.length; // don't add rows if the component is empty and loadMoreOnScroll is set
    ...
    if (dataprovider instanceof oj.TableDataSourceAdapter) {
      ...
      } else if (this._isPagingModelTableDataSource() && oj.FlattenedTreeTableDataSource && dataprovider.tableDataSource.dataSource instanceof oj.FlattenedTreeTableDataSource) {
  ...
  Table.prototype._createContextMenuContainer = function () {
    ...
    var enableNonContiguousSelectionMenu = this._isTouchDevice() ? this._getRowSelectionMode() === Table._OPTION_SELECTION_MODES._MULTIPLE : false;

    if (!menuContainer && (this._isTableSortable() || enableNonContiguousSelectionMenu || this._isTableColumnsResizable())) {
      if (this._IsCustomElement()) {
      ...
      if (this._GetDefaultContextMenu()) {
        ...
        if (listItems.length > 0) {
          ...
          for (i = 0; i < listItems.length; i++) {
            if (listItems[i].tagName === 'OJ-OPTION' || listItems[i].getElementsByTagName(Table.DOM_ELEMENT._A).length === 0) {
              command = listItems[i].getAttribute('data-oj-command').split('-');
              newItem = this._createContextMenuItem(command[command.length - 1], this._IsCustomElement());
  ...
  Table.prototype._createContextMenuResizePopup = function (initialSize) {
    ...
    if (this._IsCustomElement()) {
      ...
      tableContainer = this._getTableContainer(); // create the base popup
  ...
  Table.prototype._createTableBodyDefaultSelector = function (rowKey, tableBodyRow) {
    ...
    selector.addEventListener('selectedKeysChanged', function (event) {
      if (event.detail.updatedFrom === 'internal') {
        this._setSelected({
          row: event.detail.value,
          column: new ojkeyset.KeySetImpl()
        }, false, true);
      }
    }.bind(this));
  ...
  Table.prototype._getTouchAffordanceGlassPane = function () {
    ...
    var touchAffordanceGlassPane = this._getTableElementsByClassName(tableContainer, Table.CSS_CLASSES._TABLE_TOUCH_AFFORDANCE_GLASS_PANE_CLASS);
  ...
  Table.prototype._createTouchAffordanceGlassPane = function () {
    ...
    if (!touchAffordanceGlassPane) {
      ...
      touchAffordanceGlassPane.classList.add(Table.CSS_CLASSES._TABLE_TOUCH_AFFORDANCE_GLASS_PANE_CLASS);
  ...
  Table.prototype._createTableBodyRowTouchSelectionAffordance = function (rowIdx) {
    ...
    if (!topAffordance) {
      ...
      topAffordance.classList.add(Table.CSS_CLASSES._TABLE_DATA_ROW_TOUCH_SELECTION_AFFORDANCE_TOP_CLASS);
      topAffordance.classList.add(Table.CSS_CLASSES._TABLE_DATA_ROW_TOUCH_SELECTION_AFFORDANCE_TOUCH_AREA_CLASS);
      var topIcon = document.createElement(Table.DOM_ELEMENT._DIV); // @HTMLUpdateOK

      topIcon.classList.add(Table.CSS_CLASSES._TABLE_DATA_ROW_TOUCH_SELECTION_AFFORDANCE_TOP_ICON_CLASS);
    ...
    if (!bottomAffordance) {
      ...
      bottomAffordance.classList.add(Table.CSS_CLASSES._TABLE_DATA_ROW_TOUCH_SELECTION_AFFORDANCE_BOTTOM_CLASS);
      bottomAffordance.classList.add(Table.CSS_CLASSES._TABLE_DATA_ROW_TOUCH_SELECTION_AFFORDANCE_TOUCH_AREA_CLASS);
      var bottomIcon = document.createElement(Table.DOM_ELEMENT._DIV); // @HTMLUpdateOK

      bottomIcon.classList.add(Table.CSS_CLASSES._TABLE_DATA_ROW_TOUCH_SELECTION_AFFORDANCE_BOTTOM_ICON_CLASS);
  ...
  Table.prototype._createTableContainer = function () {
    ...
    this._cacheDomElement(Table.CSS_CLASSES._TABLE_CONTAINER_CLASS, tableContainer); // add main css class to container
  ...
  Table.prototype._createTableColGroup = function () {
    ...
    this._cacheDomElement(Table.CSS_CLASSES._TABLE_COLGROUP_CLASS, tableColGroup); // check if tfoot is already there. If so add relative to tfoot.
  ...
  Table.prototype._createTableHeaderColumn = function (columnIdx) {
    ...
    this._styleTableHeaderColumn(columnIdx, headerColumnCell, true); // add abbr for acc
  ...
  Table.prototype._createTableHeaderColumnDragImage = function (columnIdxs) {
    ...
    headerColumnDragImage.style[Table.CSS_PROP._ZINDEX] = '-999'; // The drag image element must be either visible or a child of document.body
  ...
  Table.prototype._createTableHeaderColumnResizeIndicator = function () {
    ...
    if (!tableHeaderColumnResizeIndicator) {
      ...
      tableHeaderColumnResizeIndicator.classList.add(Table.CSS_CLASSES._COLUMN_HEADER_RESIZE_INDICATOR_CLASS);

      this._cacheDomElement(Table.CSS_CLASSES._COLUMN_HEADER_RESIZE_INDICATOR_CLASS, tableHeaderColumnResizeIndicator);
    }
  ...
  Table.prototype._createTableStatusMessage = function () {
    ...
    if (!this._isSkeletonSupport()) {
      ...
      statusMessageLoadingIndicator.setAttribute('aria-label', this.getTranslatedString(Table._BUNDLE_KEY._MSG_FETCHING_DATA));
  ...
  Table.prototype._displayDragOverIndicatorRow = function (rowIdx, modelRow, indicatorStyle, isReorder) {
  ...
  Table.prototype._getElementColumnIdx = function (element) {
    var tableBodyCell = this._getFirstAncestor(element, '.' + Table.CSS_CLASSES._TABLE_DATA_CELL_CLASS, true);

    if (tableBodyCell != null) {
      return $(tableBodyCell.parentNode).children('.' + Table.CSS_CLASSES._TABLE_DATA_CELL_CLASS).index(tableBodyCell);
    }

    var tableHeaderColumn = this._getFirstAncestor(element, '.' + Table.CSS_CLASSES._COLUMN_HEADER_CELL_CLASS, true);

    if (tableHeaderColumn != null) {
      return $(tableHeaderColumn.parentNode).children('.' + Table.CSS_CLASSES._COLUMN_HEADER_CELL_CLASS).index(tableHeaderColumn);
    }

    var tableFooterCell = this._getFirstAncestor(element, '.' + Table.CSS_CLASSES._TABLE_FOOTER_CELL_CLASS, true);

    if (tableFooterCell != null) {
      return $(tableFooterCell.parentNode).children('.' + Table.CSS_CLASSES._TABLE_FOOTER_CELL_CLASS).index(tableFooterCell);
    }
  ...
  Table.prototype._getElementRowIdx = function (element) {
    var tableBodyRow = this._getFirstAncestor(element, '.' + Table.CSS_CLASSES._TABLE_DATA_ROW_CLASS, true);
  ...
  Table.prototype._getTableBodyMessageCell = function () {
    ...
    if (tableBody) {
      var messageCell = this._getTableElementsByClassName(tableBody, Table.CSS_CLASSES._TABLE_BODY_MESSAGE_CLASS);
  ...
  Table.prototype._getTableBodyMessageRow = function () {
    ...
    if (tableBody) {
      var messageRow = this._getTableElementsByClassName(tableBody, Table.CSS_CLASSES._TABLE_BODY_MESSAGE_ROW_CLASS);
  ...
  Table.prototype._getTableBodyRows = function () {
    if (!this._cachedDomTableBodyRows) {
      ...
      if (tableBody != null) {
        this._cachedDomTableBodyRows = this._getTableElementsByClassName(tableBody, Table.CSS_CLASSES._TABLE_DATA_ROW_CLASS);
      } else {
  ...
  Table.prototype._getTableTempSkeletonRow = function () {
    ...
    if (tableBody != null) {
      return this._getChildElementByClassName(tableBody, Table.CSS_CLASSES._TABLE_TEMP_SKELETON_ROW_CLASS);
    }
  ...
  Table.prototype._getTableBodyRowTouchSelectionAffordanceTop = function () {
    ...
    var topAffordance = this._getTableElementsByClassName(tableContainer, Table.CSS_CLASSES._TABLE_DATA_ROW_TOUCH_SELECTION_AFFORDANCE_TOP_CLASS);
  ...
  Table.prototype._getTableBodyRowTouchSelectionAffordanceBottom = function () {
    ...
    var bottomAffordance = this._getTableElementsByClassName(tableContainer, Table.CSS_CLASSES._TABLE_DATA_ROW_TOUCH_SELECTION_AFFORDANCE_BOTTOM_CLASS);
  ...
  Table.prototype._getTableFooterCells = function () {
    ...
    var tableFooterCells = tableFooterRow != null ? this._getTableElementsByClassName(tableFooterRow, Table.CSS_CLASSES._TABLE_FOOTER_CELL_CLASS) : null;
  ...
  Table.prototype._getTableCol = function (columnIdx) {
    ...
    if (tableColGroup != null) {
      var tableColElements = this._getTableElementsByClassName(tableColGroup, Table.CSS_CLASSES._TABLE_COL_CLASS);
  ...
  Table.prototype._getTableHeaderColumnResizeIndicator = function () {
    return this._getTableElementByClassName(Table.CSS_CLASSES._COLUMN_HEADER_RESIZE_INDICATOR_CLASS, true);
  };
  /**
  ...
  Table.prototype._getTableHeaderColumns = function () {
    ...
    if (tableHeaderRow != null) {
      var headerColumnElements = this._getTableElementsByClassName(tableHeaderRow, Table.CSS_CLASSES._COLUMN_HEADER_CELL_CLASS);
  ...
  Table.prototype._insertTableBodyCell = function (rowIdx, rowKey, rowHashCode, columnIdx, tableBodyCell, tableBodyRow, isNew) {
    ...
    if (columnIdx === 0) {
      ...
      var tableBodyCells = this._getTableElementsByClassName(tableBodyRow, Table.CSS_CLASSES._TABLE_DATA_CELL_CLASS);
  ...
  Table.prototype._insertTableFooterCell = function (columnIdx, tableFooterCell) {
    ...
    var tableFooterCells = this._getTableElementsByClassName(tableFooterRow, Table.CSS_CLASSES._TABLE_FOOTER_CELL_CLASS); // save the column index on the element
  ...
  Table.prototype._insertTableHeaderColumn = function (columnIdx, tableHeaderColumn) {
    ...
    var tableHeaderColumns = this._getTableHeaderColumns(); // save the column index on the element


    this._setTableHeaderColumnAttributes(columnIdx, tableHeaderColumn); // if there is an existing th at the index then replace it
  ...
  Table.prototype._moveTableBodyRowTouchSelectionAffordanceTop = function (rowIdx) {
    ...
    if (topAffordance != null) {
      ...
      topAffordance.style[Table.CSS_PROP._TOP] = tableBodyRowRect.top - touchAffordanceGlassPaneRect.top - topAffordance.clientHeight / 2 + Table.CSS_VAL._PX;
  ...
  Table.prototype._moveTableBodyRowTouchSelectionAffordanceBottom = function (rowIdx) {
    ...
    if (bottomAffordance != null) {
      ...
      var touchAffordanceGlassPaneRect = touchAffordanceGlassPane.getBoundingClientRect();
      bottomAffordance.style[Table.CSS_PROP._TOP] = tableBodyRowRect.top - touchAffordanceGlassPaneRect.top + tableBodyRowRect.height - bottomAffordance.clientHeight / 2 + Table.CSS_VAL._PX;
  ...
  Table.prototype._moveTableHeaderColumn = function (columnIdxs, destIdx, event) {
    var _this$_columnDefArray, _this$_columnsDestMap;
    ...
    for (var i = 0; i < columnIdxs.length; i++) {
      var columnIdx = columnIdxs[i];
      tableHeaderColumns[i] = this._getTableHeaderColumn(columnIdx);
      tableFooterCells[i] = this._getTableFooterCell(columnIdx);
    }

    for (var _i3 = 0; _i3 < columnIdxs.length; _i3++) {
      if (tableHeaderColumns[_i3] != null) {
        colSpan = tableHeaderColumns[_i3].getAttribute(Table.DOM_ATTR._COLSPAN);

        if (destTableHeaderColumn != null && (colSpan == null || colSpan === 1)) {
          if (afterColumn) {
            destTableHeaderColumn.parentNode.insertBefore(tableHeaderColumns[_i3], // @HTMLUpdateOK
            destTableHeaderColumn.nextSibling);
            destTableHeaderColumn = tableHeaderColumns[_i3];
          } else {
            destTableHeaderColumn.parentNode.insertBefore(tableHeaderColumns[_i3], // @HTMLUpdateOK
      ...
      if (tableFooterCells[_i3] != null) {
        colSpan = tableFooterCells[_i3].getAttribute(Table.DOM_ATTR._COLSPAN);
        destTableFooterCell = this._getTableFooterCell(destIdx);

        if (destTableFooterCell != null && (colSpan == null || colSpan === 1)) {
          if (afterColumn) {
            destTableFooterCell.parentNode.insertBefore(tableFooterCells[_i3], // @HTMLUpdateOK
            destTableFooterCell.nextSibling);
            destTableFooterCell = tableFooterCells[_i3];
          } else {
            destTableFooterCell.parentNode.insertBefore(tableFooterCells[_i3], destTableFooterCell); // @HTMLUpdateOK
          }
        }
      }
    }
    ...
    for (var _i4 = 0; _i4 < tableBodyRows.length; _i4++) {
      for (var j = 0; j < columnIdxs.length; j++) {
        tableBodyCells[j] = this._getTableBodyCell(_i4, columnIdxs[j], null);
      }

      destTableBodyCell = this._getTableBodyCell(_i4, destIdx, null);

      for (var _j = 0; _j < columnIdxs.length; _j++) {
        if (tableBodyCells[_j] != null) {
          colSpan = tableBodyCells[_j].getAttribute(Table.DOM_ATTR._COLSPAN);

          if (destTableBodyCell != null && (colSpan == null || colSpan === 1)) {
            if (afterColumn) {
              destTableBodyCell.parentNode.insertBefore(tableBodyCells[_j], // @HTMLUpdateOK
              destTableBodyCell.nextSibling);
              destTableBodyCell = tableBodyCells[_j];
            } else {
              destTableBodyCell.parentNode.insertBefore(tableBodyCells[_j], destTableBodyCell); // @HTMLUpdateOK
            }
          }
        }
      }
    ...
    for (var _i5 = 0; _i5 < columnsCount; _i5++) {
      clonedColumnsOption[_i5] = $.extend({}, {}, this.options.columns[_i5]);
    }
    ...
    if (!this._columnsDestMap) {
      ...
      for (var _i6 = 0; _i6 < clonedColumnsOption.length; _i6++) {
        this._columnsDestMap[_i6] = _i6;
      }
    }

    reversedColumnIdxs.forEach(function (columnIdx) {
    ...
    clonedColumnsOption.splice.apply(clonedColumnsOption, [destColIdx + 1, 0].concat(columnOptions)); // re-order the column definition metadata

    (_this$_columnDefArray = this._columnDefArray).splice.apply(_this$_columnDefArray, [destColIdx + 1, 0].concat(columnDefs));

    (_this$_columnsDestMap = this._columnsDestMap).splice.apply(_this$_columnsDestMap, [destColIdx + 1, 0].concat(columnsDestMapItems)); // clone the array so we can trigger that it's changed
  ...
  Table.prototype._removeDragOverIndicatorColumn = function () {
    var indicatorElements = this._tableQuerySelectorAll(this._getTable(), '.' + Table.CSS_CLASSES._COLUMN_HEADER_DRAG_INDICATOR_BEFORE_CLASS + ',' + '.' + Table.CSS_CLASSES._COLUMN_HEADER_DRAG_INDICATOR_AFTER_CLASS);

    var indicatorElementsCount = indicatorElements.length;

    for (var i = 0; i < indicatorElementsCount; i++) {
      indicatorElements[i].classList.remove(Table.CSS_CLASSES._COLUMN_HEADER_DRAG_INDICATOR_BEFORE_CLASS);
      indicatorElements[i].classList.remove(Table.CSS_CLASSES._COLUMN_HEADER_DRAG_INDICATOR_AFTER_CLASS);
    }
  };
  /**
  ...
  Table.prototype._removeDragOverIndicatorRow = function () {
    ...
    var indicatorRowBeforeElements = this._getTableElementsByClassName(tableBody, Table.CSS_CLASSES._TABLE_DATA_ROW_DRAG_INDICATOR_CLASS);
  ...
  Table.prototype._removeTableBodyRow = function (rowIdx) {
    ...
    if (tableBodyRow != null) {
      ...
      if (this._hasCellTemplate || this._hasRowTemplate) {
        this._cleanTemplateNodes(tableBodyRow); // No need to set this._hasCellTemplate to false here since only one row is removed
    ...
    if (tableBodyRows.length > 0) {
      ...
      if (tableBody != null) {
        ...
        if (this._hasCellTemplate || this._hasRowTemplate) {
          ...
          this._hasRowTemplate = false; // need to re-register DOM event listeners after cleaning the node
  ...
  Table.prototype._removeTableBodyRowTouchSelectionAffordance = function () {
    ...
    var touchAffordance = this._getTableElementsByClassName(tableContainer, Table.CSS_CLASSES._TABLE_DATA_ROW_TOUCH_SELECTION_AFFORDANCE_TOUCH_AREA_CLASS);
  ...
  Table.prototype._setTableBodyCellAttributes = function (rowIdx, rowKey, rowHashCode, columnIdx, tableBodyCell) {
  ...
  Table.prototype._setTableFooterColumnAttributes = function (columnIdx, tableFooterCell) {
    ...
    if (!tableFooterCell.getAttribute(Table.DOM_ATTR._ID)) {
      tableFooterCell.setAttribute(Table.DOM_ATTR._ID, this._getTableId() + ':' + column.id + ':ftr'); // @HTMLUpdateOK
    }
  };
  /**
  ...
  Table.prototype._styleTableBody = function (tableBody) {
    tableBody.classList.add(Table.CSS_CLASSES._TABLE_BODY_CLASS); // Add a special marker attribute to tell child components that they are container within table
  ...
  Table.prototype._styleTableBodyCell = function (columnIdx, tableBodyCell, isNew) {
    ...
    if (this._isVerticalGridEnabled() && (isNew || !tableBodyCell.classList.contains(Table.CSS_CLASSES._TABLE_VGRID_LINES_CLASS))) {
  ...
  Table.prototype._styleTableFooterCell = function (columnIdx, tableFooterCell) {
    var column = this._getColumnDefs()[columnIdx]; // merge styling with existing styling in case it was specified in a custom renderer
    ...
    if (this._isVerticalGridEnabled() && !tableFooterCell.classList.contains(Table.CSS_CLASSES._TABLE_VGRID_LINES_CLASS)) {
  ...
  Table.prototype._styleTableHeader = function (tableHeader) {
    ...
    tableHeader.classList.add(Table.CSS_CLASSES._TABLE_HEADER_CLASS); // eslint-disable-next-line no-param-reassign
  ...
  Table.prototype._styleTableHeaderColumn = function (columnIdx, tableHeaderColumn, isNew) {
    ...
    if (this._isVerticalGridEnabled()) {
      if (isNew || !tableHeaderColumn.classList.contains(Table.CSS_CLASSES._TABLE_VGRID_LINES_CLASS)) {
  ...
  Table.prototype._isVerticalGridEnabled = function () {
    return this.options.verticalGridVisible === Table._OPTION_ENABLED || this.options.verticalGridVisible === Table._OPTION_AUTO && this.options.display === Table._OPTION_DISPLAY._GRID;
  };
  /**
  ...
  Table.prototype._refreshTouchAffordanceGlassPanePosition = function () {
    ...
    if (touchAffordanceGlassPane) {
      ...
      touchAffordanceGlassPane.style[Table.CSS_PROP._TOP] = scrollerRect.top - tableContainerRect.top + Table.CSS_VAL._PX;
      touchAffordanceGlassPane.style[Table.CSS_PROP._BOTTOM] = scrollerRect.bottom - tableContainerRect.bottom + Table.CSS_VAL._PX;
  ...
  Table.prototype._refreshTableStatusPosition = function (showStatus) {
    ...
    if (!this._isSkeletonSupport()) {
      ...
      tableStatusMessageText.style[Table.CSS_PROP._TOP] = (statusMessageHeight - tableStatusMessageText.offsetHeight) / 2 + Table.CSS_VAL._PX;
    }
  };
  /**
  ...
  Table.prototype._filterTableOwnedElements = function (elements, containerClone) {
    ...
    if (!containerClone) {
      ...
      for (var i = 0; i < tables.length; i++) {
        if (tables[i].parentNode === containerClone || tables[i].parentNode.parentNode === containerClone) {
          table = tables[i];
          break;
        }
      }
  ...
  Table.prototype._getSortIconContainer = function (header) {
    return this._getTableElementsByClassName(header, Table.CSS_CLASSES._TABLE_SORT_ICON_CONTAINER_CLASS)[0];
  };
  /**
  ...
  Table.prototype._getTableBodySelectorCell = function (tableBodyRow) {
    if (tableBodyRow != null) {
      var selectorCell = this._getTableElementsByClassName(tableBodyRow, Table.CSS_CLASSES._TABLE_SELECTOR_CELL);
  ...
  Table.prototype._getTableSelectorColumn = function () {
    ...
    if (tableHeaderRow != null) {
      var selectorHeader = this._getTableElementsByClassName(tableHeaderRow, Table.CSS_CLASSES._COLUMN_HEADER_SELECTOR_CELL_CLASS);
  ...
  Table.prototype._getTableFooterSelectorCell = function () {
    ...
    if (tableFooterRow != null) {
      var selectorCell = this._getTableElementsByClassName(tableFooterRow, Table.CSS_CLASSES._TABLE_FOOTER_SELECTOR_CELL_CLASS);
  ...
  Table.prototype._events = {
    ...
    'focus .oj-table-data-cell': function focusOjTableDataCell(event) {
      ...
      if (rowIdx === this._getEditableRowIdx()) {
        ...
        if (this._focusEditCell && this._focusEditCell !== eventTarget && this._focusEditCell.classList.contains(Table.CSS_CLASSES._TABLE_DATA_CELL_EDIT_CLASS)) {
          this._focusEditCell.classList.remove(Table.CSS_CLASSES._TABLE_DATA_CELL_EDIT_CLASS);
    ...
    keydown: function keydown(event) {
      ...
      this._addKeyboardKey(key); // ignore key event on the footer or target is editable


      var keyboardCode1 = this._getKeyboardKeys()[0];

      if (!DataCollectionUtils.isEscapeKeyEvent(keyboardCode1) && !DataCollectionUtils.isEnterKeyEvent(keyboardCode1) && !DataCollectionUtils.isF2KeyEvent(keyboardCode1) && !DataCollectionUtils.isTabKeyEvent(keyboardCode1) && (this._isNodeEditable(event.target) || this._getTableFooter() != null && $(this._getTableFooter()).has(event.target).length > 0)) {
        return;
      } // process single or two key events


      if (this._getKeyboardKeys().length === 1 || this._getKeyboardKeys().length === 2 && event[Table._KEYBOARD_CODES._MODIFIER_SHIFT]) {
        if (this._isKeyPressMatch(DataCollectionUtils.isArrowUpKeyEvent) || this._isKeyPressMatch(DataCollectionUtils.isArrowDownKeyEvent) || this._isKeyPressMatch(DataCollectionUtils.isArrowLeftKeyEvent) || this._isKeyPressMatch(DataCollectionUtils.isArrowRightKeyEvent) || this._isKeyPressMatch(DataCollectionUtils.isHomeKeyEvent) || this._isKeyPressMatch(DataCollectionUtils.isEndKeyEvent)) {
        ...
        if (this._isKeyPressMatch(DataCollectionUtils.isArrowUpKeyEvent) || this._isKeyPressMatch(DataCollectionUtils.isArrowDownKeyEvent)) {
          this._handleKeydownUpDown(event, isExtend && this._getRowSelectionMode() === Table._OPTION_SELECTION_MODES._MULTIPLE);
        } else if (this._isKeyPressMatch(DataCollectionUtils.isArrowLeftKeyEvent) || this._isKeyPressMatch(DataCollectionUtils.isArrowRightKeyEvent)) {
          this._handleKeydownLeftRight(event, isExtend && this._getColumnSelectionMode() === Table._OPTION_SELECTION_MODES._MULTIPLE);
    ...
    keyup: function keyup(event) {
      ...
      if (this._getKeyboardKeys().length === 1) {
        ...
        if (this._isNodeEditable(event.target) || this._getTableFooter() != null && $(this._getTableFooter()).has(event.target).length > 0) {
    ...
    'mousedown .oj-table-column-header-cell': function mousedownOjTableColumnHeaderCell(event) {
      ...
      if (event.which === 1) {
        if (!this._handleHeaderColumnResizeStart(event)) {
          ...
          var columnIdx = this._getElementColumnIdx(this._getEventTargetElement(event)); // set the column focus if shift key is not pressed
        ...
        if (this._isFF() && DomUtils.isMetaKeyPressed(event)) {
          event.preventDefault();
        }
      }
    },
     ...
     * set the column header focus.
     */
    'mousedown .oj-table-footer-cell': function mousedownOjTableFooterCell(event) {
      if (DataCollectionUtils.isEventClickthroughDisabled(event, this._getTable())) {
        return;
      }
    ...
    'mousedown .oj-table-data-cell': function mousedownOjTableDataCell(event) {
      ...
      var rowIdx = this._getElementRowIdx(eventTarget); // set row focus only if shift key is not pressed
    ...
    'click .oj-table-body-row': function clickOjTableBodyRow(event) {
      ...
      this._fireActionEvent(rowIdx, event); // check if we are selecting
      ...
      if (isShift) {
        ...
        if (isTouch && rowSelected && !this._nonContiguousSelection && this._getRowSelectionMode() === Table._OPTION_SELECTION_MODES._MULTIPLE) {
    ...
    'click .oj-table-column-header-cell': function clickOjTableColumnHeaderCell(event) {
      ...
      var columnIdx = this._getElementColumnIdx(eventTarget); // check if we need to focus
      ...
      if (isShift) {
        ...
        if (focusedHeaderColumnIdx != null && this._getColumnSelectionMode() === Table._OPTION_SELECTION_MODES._MULTIPLE) {
     ...
     * focus and selection. If Ctrl is not pressed then we have single column selection.
     */
    'click .oj-table-footer-cell': function clickOjTableFooterCell(event) {
      if (DataCollectionUtils.isEventClickthroughDisabled(event, this._getTable())) {
        return;
      } // get the column index
      ...
      if (isShift) {
        ...
        if (focusedFooterColumnIdx != null && this._getColumnSelectionMode() === Table._OPTION_SELECTION_MODES._MULTIPLE) {
          // shift selection is always from the focused column to the target column
  ...
  Table.prototype._registerTouchEvents = function () {
    ...
    var passiveTouchEventsAndListeners = {
      ...
      'touchstart .oj-table-body-row-touch-selection-affordance-touch-area': function touchstartOjTableBodyRowTouchSelectionAffordanceTouchArea(event) {
        ...
        if (fingerCount === 1 && this._getRowSelectionMode() === Table._OPTION_SELECTION_MODES._MULTIPLE) {
          if (event.target.classList.contains(Table.CSS_CLASSES._TABLE_DATA_ROW_TOUCH_SELECTION_AFFORDANCE_TOP_CLASS) || event.target.classList.contains(Table.CSS_CLASSES._TABLE_DATA_ROW_TOUCH_SELECTION_AFFORDANCE_TOP_ICON_CLASS)) {
            event.preventDefault();
            this._mouseDownRowIdx = $(this._getTableBodyRowTouchSelectionAffordanceBottom()).data('rowIdx');
          } else if (event.target.classList.contains(Table.CSS_CLASSES._TABLE_DATA_ROW_TOUCH_SELECTION_AFFORDANCE_BOTTOM_CLASS) || event.target.classList.contains(Table.CSS_CLASSES._TABLE_DATA_ROW_TOUCH_SELECTION_AFFORDANCE_BOTTOM_ICON_CLASS)) {
            event.preventDefault();
            this._mouseDownRowIdx = $(this._getTableBodyRowTouchSelectionAffordanceTop()).data('rowIdx');
          }
        }
      },
    ...
    var tableContainer = this._getTableContainer(); // register regular touch events
  ...
  Table.prototype._getFocusedRowIdx = function () {
    ...
    return this._getRowIdxsForElementsWithStyleClass('.' + Table.CSS_CLASSES._TABLE_DATA_ROW_CLASS + '.' + Table.MARKER_STYLE_CLASSES._FOCUS_HIGHLIGHT + ',' + '.' + Table.CSS_CLASSES._TABLE_DATA_ROW_CLASS + '.' + Table.MARKER_STYLE_CLASSES._FOCUS)[0];
  };
  /**
  ...
  Table.prototype._getFocusedHeaderColumnIdx = function () {
    ...
    return this._getColumnIdxsForElementsWithStyleClass('.' + Table.CSS_CLASSES._COLUMN_HEADER_CELL_CLASS + '.' + Table.MARKER_STYLE_CLASSES._FOCUS_HIGHLIGHT + ',' + '.' + Table.CSS_CLASSES._COLUMN_HEADER_CELL_CLASS + '.' + Table.MARKER_STYLE_CLASSES._FOCUS + ',' + '.' + Table.CSS_CLASSES._COLUMN_HEADER_SELECTOR_CELL_CLASS + '.' + Table.MARKER_STYLE_CLASSES._FOCUS_HIGHLIGHT + ',' + '.' + Table.CSS_CLASSES._COLUMN_HEADER_SELECTOR_CELL_CLASS + '.' + Table.MARKER_STYLE_CLASSES._FOCUS)[0];
  };
  /**
   * @private
   */
  ...
  Table.prototype._getFocusedFooterColumnIdx = function () {
    ...
    return this._getColumnIdxsForElementsWithStyleClass('.' + Table.CSS_CLASSES._TABLE_FOOTER_CELL_CLASS + '.' + Table.MARKER_STYLE_CLASSES._FOCUS_HIGHLIGHT + ',' + '.' + Table.CSS_CLASSES._TABLE_FOOTER_CELL_CLASS + '.' + Table.MARKER_STYLE_CLASSES._FOCUS + ',' + '.' + Table.CSS_CLASSES._TABLE_FOOTER_SELECTOR_CELL_CLASS + '.' + Table.MARKER_STYLE_CLASSES._FOCUS_HIGHLIGHT + ',' + '.' + Table.CSS_CLASSES._TABLE_FOOTER_SELECTOR_CELL_CLASS + '.' + Table.MARKER_STYLE_CLASSES._FOCUS)[0];
  };
  /**
  ...
  Table.prototype._setCurrentRow = function (currentRow, event, optionChange) {
    ...
    var isClearCurrentRow = currentRow == null || currentRow.rowKey == null && currentRow.rowIndex == null;
    ...
    if (!isClearCurrentRow) {
      ...
      if (localRowIndex !== -1 && (!dataprovider || localRowIndex < -1 || localRowIndex === null || rowKey === null)) {
    ...
    if (currentRowChanged) {
      ...
      var isExistingCurrentRowClear = existingCurrentRow == null || existingCurrentRow.rowKey == null && existingCurrentRow.rowIndex == null;
      ...
      if (!isExistingCurrentRowClear) {
        ...
        if (updateEditable === false) {
          ...
          }.bind(this)); // do not update the currentRow to the new value if updateEditable returned false
  ...
  Table.prototype._compareCurrentRowValues = function (currentRowValue1, currentRowValue2) {
    ...
    if (currentRowValue1 != null && currentRowValue2 != null) {
      if (currentRowValue1.rowIndex === currentRowValue2.rowIndex && oj.KeyUtils.equals(currentRowValue1.rowKey, currentRowValue2.rowKey)) {
        return true;
      }
  ...
  Table.prototype._setRowFocus = function (rowIdx, focused, updateCurrentRow, element, event) {
    ...
    if (focused) {
      ...
      this._clearFocusedFooterColumn(); // unset table actionable mode
  ...
  Table.prototype._setFooterColumnFocus = function (columnIdx, focused, skipScrollColumn) {
    ...
    this._setFooterColumnState(columnIdx, {
      focused: focused
    });
  };
  /**
  ...
  Table.prototype._applyHeaderColumnState = function (headerColumn, columnIdx, state) {
    ...
    var selected = state.selected; // todo: can we optimize so that we don't look up the header element all the time with index?
  ...
  Table.prototype._applyFooterColumnState = function (footerColumn, columnIdx, state) {
    var focused = state.focused;
    var selected = state.selected; // todo: can we optimize so that we don't look up the footer element all the time with index?
  ...
  Table.prototype._adjustScrollPositionOnFetch = function () {
    ...
    if (selectedRowIdxs != null && selectedRowIdxs.length > 0 && this._isRowSelectionEnabled() && this._isScrollToKey()) {
  ...
  Table.prototype._setScrollX = function (x) {
    var scroller = this._getLayoutManager().getScroller(); // flag it so that handleScroll won't do anything
  ...
  Table.prototype._setScrollY = function (y) {
    ...
    var scroller = layoutManager.getScroller(); // flag it so that handleScroll won't do anything

    this._skipScrollUpdate = true;
    var initialScrollTop = scroller.scrollTop;
    scroller.scrollTop = y === -1 ? layoutManager.getScrollHeight() - layoutManager.getClientHeight() : y; // only set scroll pos busy state if a scroll was triggered
  ...
  Table.prototype._getScrollCoordinates = function (scrollPosition) {
    ...
    if (isNaN(y) && !isNaN(scrollPosition.y)) {
      ...
      if (scrollPosition.y > layoutManager.getScrollHeight()) {
        ...
        y = this._hasMoreToFetch() ? -1 : layoutManager.getScrollHeight() - layoutManager.getClientHeight();
      } else {
    ...
    if (value) {
      ...
      if (focusedElement != null) {
        ...
        if (tabbableElementsInFocusedElement != null && tabbableElementsInFocusedElement.length > 0) {
          ...
          this._focusOutHandler($(focusedElement)); // set focus on the first tabbable element
      ...
      var resetFocus = $.contains(this._getTableBody(), document.activeElement); // disable all focusable elements
  ...
  Table.prototype._setTableEditable = function (editable, cancelled, columnIdx, forwardSearch, event, skipFocusReset) {
    if (!this._isTableEditMode()) {
      return undefined;
    }
  ...
  Table.prototype._handleKeydownLeftRight = function (event, isExtend) {
    ...
    var current;
    var target; // pressing left/right navigates the column headers
    ...
    if (focusedFooterIndex != null) {
      ...
      if (isExtend) {
        this._isNavigate = false; // select all columns between focusedFooterIndex and target
  ...
  Table.prototype._handleKeydownUpDown = function (event, isExtend) {
    ...
    var visibleIndex; // handle ctrl+up and ctrl+down cases
    ...
    if (focusedRowIdx != null) {
      ...
      if (isExtend) {
        ...
        this._isNavigate = true;
        visibleIndex = this._getFirstVisibleColumnIndex(0, true); // if row is focused then up/down navigates the rows
  ...
  Table.prototype._handleKeydownTab = function (event) {
    ...
    if (this._isTableActionableMode() || this._hasEditableRow()) {
      ...
      if (focusedRowIdx != null && this._getEditableRowIdx() === focusedRowIdx) {
        ...
        if (rowElementTabIndex === tabbableElementsInRowCount - 1 && !event[Table._KEYBOARD_CODES._MODIFIER_SHIFT]) {
  ...
  Table.prototype._handleKeydownEnter = function (event) {
    ...
    if (focusedColumnIdx != null) {
      if (focusedColumnIdx !== -1 && this._getColumnDefs()[focusedColumnIdx].sortable === Table._OPTION_ENABLED) {
  ...
  Table.prototype._handleKeydownHome = function (event) {
    ...
    var visibleIndex = this._getFirstVisibleColumnIndex(0, true); // pressing Home focuses on first column header
  ...
  Table.prototype._handleKeydownEnd = function (event) {
    ...
    var visibleIndex = this._getFirstVisibleColumnIndex(lastColumnIndex, false); // pressing End focuses on last column header
  ...
  Table.prototype._tableBodyCellDefaultRenderer = function (rowIdx, columnIdx, row, rowHashCode, context) {
    var tableBodyRow = context.rowContext.parentElement;
    ...
    this._insertTableBodyCell(rowIdx, row.key, rowHashCode, columnIdx, tableBodyCell, tableBodyRow, true);
    ...
    if (!cellRenderer && !cellSlotTemplate) {
      ...
      var cellContext = this._getRendererContextObject(tableBodyCell, {
        row: row
      }); // Copy additional properties to top-level context to work with custom element
      ...
      if (cellRenderer) {
        ...
        if (templateEngine != null) {
          ...
          var cellContent = templateEngine.execute(componentElement, cellSlotTemplate, slotContext, this.options.as);
  ...
  Table.prototype._getRendererContextObject = function (parentElement, options) {
    var context = {};
    context.component = ojcomponentcore.__GetWidgetConstructor(this.element, 'ojTable');
    var dataSource = this.options.data; // unwrap the datasource if we have a PagingTableDataSource
    ...
    if (options.row != null) {
      ...
      if (this._hasEditableRow()) {
        ...
        var editableRowKey = this._getEditableRowKey(); // only set to edit mode for the editable row
  ...
  Table.prototype._setResizeCursor = function (event) {
    ...
    if (column.resizable === Table._OPTION_DISABLED && this._resizeStartColumnIdx !== columnIdx - 1 && this._resizeStartColumnIdx !== columnIdx + 1) {
      return false;
    }
  ...
  Table.prototype._handleHeaderColumnResizeEnd = function (event) {
    ...
    var columnIdx = this._getElementColumnIdx(eventTarget); // only resize if we end the resize on the same column or adjacent columns
    ...
    if (columnIdx !== null && columnIdx === this._resizeStartColumnIdx || this._resizeColumnStart && columnIdx === this._resizeStartColumnIdx - 1 || !this._resizeColumnStart && columnIdx === this._resizeStartColumnIdx + 1) {
      ...
      if (headerColumn != null) {
        ...
        var headerColumnWidth = headerColumnRect.width; // legacy only: when scrollbar is present, reduce last column width by scrollbar width

        var isLegacyImpl = !this._isStickyLayoutEnabled();

        if (isLegacyImpl && this._resizeStartColumnIdx === columnsCount - 1 && layoutManager.isTableHeightConstrained()) {
          headerColumnWidth -= layoutManager.getScrollBarWidth();
        }

        var widthChange;

        if (this._GetReadingDirection() === 'rtl' && this._resizeColumnStart || this._GetReadingDirection() === 'ltr' && !this._resizeColumnStart) {
        ...
        if (Math.abs(widthChange) > 2) {
          var startColWidth = headerColumnWidth + widthChange; // ensure new column width is not less than the minimum required width
          ...
          var headerColumnAdjacentIdx = this._resizeColumnStart ? this._resizeStartColumnIdx - 1 : this._resizeStartColumnIdx + 1; // legacy only: when scrollbar is present, reduce last column width by scrollbar width

          if (isLegacyImpl && headerColumnAdjacentIdx === columnsCount - 1 && layoutManager.isTableHeightConstrained()) {
          ...
          if (headerColumnAdjacent) {
            ...
            clonedColumnsOption[headerColumnAdjacentIdx].width = layoutManager.getWidthPropertyFromOffsetWidth(adjacentColWidth, headerColumnAdjacent);
          }

          clonedColumnsOption[this._resizeStartColumnIdx].width = layoutManager.getWidthPropertyFromOffsetWidth(startColWidth, headerColumn);
          this.option('columns', clonedColumnsOption, {
            _context: {
              writeback: true,
              internalSet: true
          ...
          this._queueTask(function () {
            layoutManager.notifyTableUpdate(Table._UPDATE._COL_RESIZE); // delay the focus to ensure table resize information is not cleared before 'click' handling occurs
  ...
  Table.prototype._setTableHeaderColumnResizeIndicator = function (columnIdx) {
    ...
    if (this._resizeColumnStart) {
      if (this._GetReadingDirection() === 'rtl') {
        tableHeaderColumnResizeIndicator.style.left = headerColumnRect.left + headerColumnRect.width - scrollerRect.left + 'px';
      } else {
        tableHeaderColumnResizeIndicator.style.left = headerColumnRect.left - scrollerRect.left + 'px';
      }
  ...
  Table.prototype._getRowSelectionMode = function () {
    var rowSelectionMode = this.options.selectionMode == null ? null : this.options.selectionMode[Table._CONST_ROW];
  ...
  Table.prototype._isRowSelectionEnabled = function () {
    ...
    return rowSelectionMode === Table._OPTION_SELECTION_MODES._SINGLE || rowSelectionMode === Table._OPTION_SELECTION_MODES._MULTIPLE;
  };
  /**
  ...
  Table.prototype._getColumnSelectionMode = function () {
    var columnSelectionMode = this.options.selectionMode == null ? null : this.options.selectionMode[Table._CONST_COLUMN];
  ...
  Table.prototype._isColumnSelectionEnabled = function () {
    ...
    return colSelectionMode === Table._OPTION_SELECTION_MODES._SINGLE || colSelectionMode === Table._OPTION_SELECTION_MODES._MULTIPLE;
  };
  /**
   * @private
   */
  ...
  Table.prototype._updateSelectionStateFromEventDetailRemove = function (eventDetail, addEventDetail) {
    var selected = this.option('selected');
  ...
  Table.prototype._processRowRangeSelection = function (rangeObj, rowKeySet, columnKeySet) {
    ...
    if (startRowIndex === undefined && endRowIndex === undefined) {
      ...
      for (var i = startRowIdx; i <= endRowIdx; i++) {
        // update keySet
        var rowKey = this._getRowKeyForRowIdx(i); // eslint-disable-next-line no-param-reassign
  ...
  Table.prototype._processColumnRangeSelection = function (rangeObj, rowKeySet, columnKeySet) {
    ...
    if (startColumnIndex === undefined && endColumnIndex === undefined) {
      ...
      for (var i = startColumnIndex; i <= endColumnIndex; i++) {
        // update keySet
        var columnKey = this._getColumnKeyForColumnIdx(i); // eslint-disable-next-line no-param-reassign
  ...
  Table.prototype._getSelectedEquivalent = function (selection) {
    ...
    if (selection != null) {
      ...
      for (var i = 0; i < selectionCount; i++) {
        ...
        if (rangeObj.startKey == null && rangeObj[Table._CONST_STARTINDEX] == null || rangeObj.endKey == null && rangeObj[Table._CONST_ENDINDEX] == null) {
  ...
  Table.prototype._applyColumnSelection = function (columnIdx, selected) {
    ...
    if (reorderOption && reorderOption.columns === Table._OPTION_ENABLED) {
      ...
      var containsDraggbleClass = headerColumn.classList.contains(Table.MARKER_STYLE_CLASSES._DRAGGABLE);
  ...
  Table.prototype._handleSelectionGesture = function (index, isRow, isMultiSelectGesture) {
    ...
    if (isRow) {
      ...
      if (this._isRowSelectionEnabled()) {
        ...
        } else if (!isMultiSelectGesture && this._getSelectedRowIdxs().length > 1 && rowSelectionMode === Table._OPTION_SELECTION_MODES._MULTIPLE) {
          // selection is now the new row key
          rowKeySet = new ojkeyset.KeySetImpl([rowKey]);
        } else {
      ...
      if (index > -1) {
        ...
        if (!columnKeySet.has(columnKey)) {
          // handle multiple selection gesture
          if (isMultiSelectGesture && columnSelectionMode === Table._OPTION_SELECTION_MODES._MULTIPLE) {
        ...
        } else if (!isMultiSelectGesture && this._getSelectedHeaderColumnIdxs().length > 1 && columnSelectionMode === Table._OPTION_SELECTION_MODES._MULTIPLE) {
          // selection is now the new column key
          columnKeySet = new ojkeyset.KeySetImpl([columnKey]);
        } else {
  ...
  Table.prototype._getSelectedRowIdxs = function () {
    ...
    return this._getRowIdxsForElementsWithStyleClass('.' + Table.CSS_CLASSES._TABLE_DATA_ROW_CLASS + '.' + Table.MARKER_STYLE_CLASSES._SELECTED);
  };
  /**
  ...
  Table.prototype._getSelectedHeaderColumnIdxs = function () {
    ...
    return this._getColumnIdxsForElementsWithStyleClass('.' + Table.CSS_CLASSES._COLUMN_HEADER_CELL_CLASS + '.' + Table.MARKER_STYLE_CLASSES._SELECTED);
  };
  /**
  ...
  Table.prototype._getSelectedFooterColumnIdxs = function () {
    // selected column headers have the selected css class
    return this._getColumnIdxsForElementsWithStyleClass('.' + Table.CSS_CLASSES._TABLE_FOOTER_CELL_CLASS + '.' + Table.MARKER_STYLE_CLASSES._SELECTED);
  };
  /**
  ...
  Table.prototype._updateSelector = function (selected) {
    ...
    var headerSelectorElements = table.getElementsByClassName(Table.CSS_CLASSES._TABLE_HEADER_SELECTOR_CLASS);
  ...
  var TableFixedLayoutManager = function TableFixedLayoutManager(table) {
    TableFixedLayoutManager.superclass.constructor.call(this, table);
  }; // Subclass from oj.Object


  oj.Object.createSubclass(TableFixedLayoutManager, TableStickyLayoutManager, 'TableFixedLayoutManager');
  /**
   * @private
   */
  ...
  TableFixedLayoutManager.prototype._initializeColumnLayouts = function () {
    var i;
    var isResetTableElemStyle = false; // determine selector min width if needed
    ...
    var columnsCount = columns.length; // determine column weights if needed

    if (this._columnWeights == null) {
      isResetTableElemStyle = true;
      this._columnWeights = []; // loop through columns to get the remaining column widths

      for (i = 0; i < columnsCount; i++) {
        var columnWeight = parseFloat(columns[i].weight); // TODO: throw a warning here when weight is less than 1
    ...
    if (this._columnMinWidths == null) {
      isResetTableElemStyle = true;
      this._columnMinWidths = []; // loop through columns to get the remaining column widths

      for (i = 0; i < columnsCount; i++) {
        var minWidth = this._getPixelStyleEquivalent(columns[i].minWidth); // minWidth MUST be set and greater than 0 when fixed layout is specified
    ...
    if (this._columnMaxWidths == null) {
      isResetTableElemStyle = true;
      this._columnMaxWidths = []; // loop through columns to get the remaining column widths

      for (i = 0; i < columnsCount; i++) {
        var maxWidth = this._getPixelStyleEquivalent(columns[i].maxWidth); // maxWidth MUST be null or greater than 0
    ...
    if (this._columnInitWidths == null) {
      ...
      this._columnInitWidths = []; // loop through columns to get the remaining column widths
  ...
  TableFixedLayoutManager.prototype._resetTableElementStyling = function () {
    ...
    tableElem.style = ''; // reset styling on each col group
      ...
      while (continueSizing) {
        ...
        for (i = 0; i < columnsCount; i++) {
          if (forcedColumnWidths[i] === false) {
            var currentWidth = columnWidths[i]; // only adjust widths for columns that are visible
            ...
            if (currentWidth > 0) {
              ...
              var newWidth = Math.floor(currentWeight / currentFreeWeightTotal * widthDiff); // if calculated width breaks min or max width requirement, force width as needed, and restart weight distribution logic
   ...
   * rendering performance as well.  If displaying large numbers of items is necessary, consider use a paging control with Table
  ...
  Table.prototype.options = {
     ...
     * @type {string}
     * @ojvalue {string} "contents" Default column sizing is determined by the contents of the data.
     * @ojvalue {string} "fixed" Default column sizing is determined by column weights.
     * @default "contents"
    ...
    if (subId === 'oj-table-cell') {
      ...
      if (tableHeaderColumn != null) {
        if (subId === 'oj-table-header') {
          ...
          var tableHeaderColumnSortAsc = this._getTableElementsByClassName(tableHeaderColumn, Table.CSS_CLASSES._COLUMN_HEADER_ASC_ICON_CLASS);

          if (tableHeaderColumnSortAsc.length > 0) {
            return tableHeaderColumnSortAsc[0];
          }
        } else {
          var tableHeaderColumnSortDsc = this._getTableElementsByClassName(tableHeaderColumn, Table.CSS_CLASSES._COLUMN_HEADER_DSC_ICON_CLASS);
  ...
  Table.prototype.getSubIdByNode = function (node, ignoreSortIcons) {
    ...
    var headerSortAsc = this._getFirstAncestor(node, '.' + Table.CSS_CLASSES._COLUMN_HEADER_ASC_ICON_CLASS, true);
    ...
    var headerSortDsc = this._getFirstAncestor(node, '.' + Table.CSS_CLASSES._COLUMN_HEADER_DSC_ICON_CLASS, true);
    ...
    var header = this._getFirstAncestor(node, '.' + Table.CSS_CLASSES._COLUMN_HEADER_CELL_CLASS, true);
  ...
  Table.prototype._NotifyAttached = function () {
    ...
    this._getLayoutManager().notifyTableUpdate(Table._UPDATE._RESIZE); // refresh dimensions if no tasks are pending since refresh dimensions runs during final task
  ...
  Table.prototype._NotifyContextMenuGesture = function (contextMenu, event, eventType) {
    ...
    if (tableBodyCell != null) {
      ...
      headerColumn = this._getFirstAncestor(this._contextMenuEvent.target, '.' + Table.CSS_CLASSES._COLUMN_HEADER_CELL_CLASS, true);
    }
    ...
    if (this._contextMenuEvent.type === 'keydown') {
      ...
      var footerCell = this._getFirstAncestor(this._contextMenuEvent.target, '.' + Table.CSS_CLASSES._TABLE_FOOTER_CELL_CLASS, true);
    ...
    var contextMenuNode = contextMenu.element ? contextMenu.element[0] : contextMenu; // Populate context menu items
  ...
  Table.prototype._getLayoutManager = function () {
    if (this._layoutManager == null) {
      if (this._isStickyLayoutEnabled()) {
        ...
        this._layoutManager = new TableLegacyLayoutManager(this); // layout attribute is not supported with legacy rendering

        if (this._isFixedLayoutEnabled()) {
          Logger.error('The current theme does not support the layout="fixed" attribute setting of the <oj-table>.');
        }
      }
    }
  ...
  ojcomponentcore.setDefaultOptions({
    ojTable: {
      display: ojcomponentcore.createDynamicPropertyGetter(function () {
        return (ojthemeutils.parseJSONFromFontFamily('oj-table-option-defaults') || {}).display;
      })
    }
  });
});
