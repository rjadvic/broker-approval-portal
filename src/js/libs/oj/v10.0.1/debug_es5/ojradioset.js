 ...
 * @ignore
 */
define(['ojs/ojcore', 'jquery', 'ojs/ojcomponentcore', 'ojs/ojeditablevalue', 'ojs/ojradiocheckbox', 'ojs/ojoption', 'ojs/ojdataprovider', 'ojs/ojcore-base', 'ojs/ojlogger', 'ojs/ojlabelledbyutils'], function (ojcore, $, ojcomponentcore, ojeditablevalue, ojradiocheckbox, ojoption, ojdataprovider, oj, ojlogger, LabeledByUtils) {
  'use strict';
    ...
    oj.__registerWidget('oj.ojRadioset', $.oj.editableValue, {
      ...
      _InitOptions: function _InitOptions(originalDefaults, constructorOptions) {
        ...
        if (!this._IsCustomElement()) {
          ojeditablevalue.EditableValueUtils.initializeOptionsFromDom(props, constructorOptions, this); // component, app, and constructor are merged into this.options.option by the time _InitOptions
      ...
      _ComponentCreate: function _ComponentCreate() {
        ...
        this.uiRadioset = $element.addClass('oj-radioset oj-component').attr('role', 'radiogroup'); // need to grab all elements using contents first and then do filter because jquery children will automatically
      ...
      _processOjOptions: function _processOjOptions() {
        if (this._IsCustomElement()) {
          ...
          var novaluespan = domElem.querySelector('[data-no-value-span]'); // Remove the old no-value span, if there is one and there is a current selection or not in readonly mode
          ...
          var options = this.element.children('oj-option').add(this.element.children('.oj-radioset-wrapper').find('oj-option'));
      ...
      _processReadonlyOptions: function _processReadonlyOptions(ojOption) {
      ...
      _initInputLabelFromOjOption: function _initInputLabelFromOjOption(elem) {
        ...
        if (!alreadyProcessed) {
          ...
          radio.setAttribute('id', radioId); // in readonly mode, if a option is selected, the <oj-option> will be surrounded by <label> tag
          ...
          if (ojoption.parentElement.nodeName === 'LABEL') {
            ...
            label.parentElement.insertBefore(radio, label); // also we need to remove the oj-helper-hidden class from the parentElement
      ...
      _events: {
        ...
        'click .oj-radioset-wrapper': function clickOjRadiosetWrapper(event) {
