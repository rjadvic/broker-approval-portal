 ...
 * @ignore
 */
define(['exports', 'ojs/ojmodel', 'ojs/ojdatasource-common', 'jquery'], function (exports, ojmodel, ojdatasourceCommon, $) {
  'use strict';
  ...
  var CollectionTableDataSource = function CollectionTableDataSource(data, options) {
    // Initialize
    this.data = {}; // This was put in to keep closure happy...

    if (!(data instanceof ojmodel.Collection)) {
  ...
  oj._registerLegacyNamespaceProp('CollectionTableDataSource', CollectionTableDataSource); // Subclass from oj.DataSource


  oj.Object.createSubclass(CollectionTableDataSource, oj.TableDataSource, 'oj.CollectionTableDataSource');
  /**
   * @export
  ...
  CollectionTableDataSource.prototype.at = function (index, options) {
    ...
    var self = this; // the at() call can result in fetches
    ...
    return new Promise(function (resolve, reject) {
      if (model != null) {
        model.then(function (resolvedModel) {
          ...
          row = {
            data: resolvedModel.attributes,
            index: index,
            key: resolvedModel.id
          };
          resolve(row);
        }, function (e) {
          self._isFetchingForAt = false;
  ...
  CollectionTableDataSource.prototype._addCollectionEventListeners = function () {
    ...
    this._collection.on(ojmodel.Events.EventType.SYNC, function (event) {
      if (event instanceof ojmodel.Model) {
        oj.TableDataSource.superclass.handleEvent.call(self, oj.TableDataSource.EventType.CHANGE, {
          data: [event.attributes],
          keys: [event.id],
          indexes: [event.index]
        });
      } else if (event instanceof ojmodel.Collection) {
        if (!self._isFetchingForAt && !self._isFetching) {
          var startIndex = event.offset;
          var pageSize = event.lastFetchCount || event.lastFetchSize; // Do not call _getRowArray if this datasource is paged,
    ...
    this._collection.on(ojmodel.Events.EventType.ALLADDED, function (event, modelArray) {
      var rowArray = [];
      var keyArray = [];
      var indexArray = [];
    ...
    this._collection.on(ojmodel.Events.EventType.ALLREMOVED, function (event, modelArray) {
      var rowArray = [];
      var keyArray = [];
      var indexArray = [];

      for (var i = 0; i < modelArray.length; i++) {
        var model = modelArray[i]; // no need to wrapWritableValue as we are just deleting
    ...
    this._collection.on(ojmodel.Events.EventType.RESET, function (event) {
      oj.TableDataSource.superclass.handleEvent.call(self, oj.TableDataSource.EventType.RESET, event);
    });

    this._collection.on(ojmodel.Events.EventType.SORT, function (event, eventOpts) {
    ...
    this._collection.on(ojmodel.Events.EventType.CHANGE, function (event) {
      oj.TableDataSource.superclass.handleEvent.call(self, oj.TableDataSource.EventType.CHANGE, {
        data: [event.attributes],
        keys: [event.id],
        indexes: [event.index]
  ...
  CollectionTableDataSource.prototype._fetchInternal = function (_options) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      self._collection.setRangeLocal(self._startIndex, pageSize).then(function (actual) {
        var result; // Do not call _getRowArray if this datasource is paged,
        // or if the underlying Collection is virtual since _getRowArray
        // assumes collection.at returns Model objects, which is not the case
        // for virtual collection.
      ...
      (function (localProp) {
        ...
        Object.defineProperty(returnObj, localProp, {
          ...
          set: function set(newValue) {
            localModel.set(localProp, newValue, {
              silent: true
            });
          },
          enumerable: true
  ...
  exports.CollectionTableDataSource = CollectionTableDataSource;
