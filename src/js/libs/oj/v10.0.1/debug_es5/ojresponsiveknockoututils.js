 ...
 * @ignore
 */
define(['exports', 'ojs/ojcore-base', 'knockout', 'jquery', 'ojs/ojresponsiveutils'], function (exports, oj, ko, $, ResponsiveUtils) {
  'use strict';
  ...
  ResponsiveKnockoutUtils.createMediaQueryObservable = function (queryString) {
    ...
    var observable = ko.observable(query.matches); // add a listener for future changes

    query.addListener(function (_query) {
      observable(_query.matches); // console.log("query listener called! query.matches = " + query.matches + ", size = " + (document.outerWidth || document.body.clientWidth));
  ...
  ResponsiveKnockoutUtils.createScreenRangeObservable = function () {
    ...
    var smQuery = ResponsiveUtils.getFrameworkQuery(ResponsiveUtils.FRAMEWORK_QUERY_KEY.SM_UP); // observables

    var xxlObservable = xxlQuery == null ? null : ResponsiveKnockoutUtils.createMediaQueryObservable(xxlQuery);
    var xlObservable = xlQuery == null ? null : ResponsiveKnockoutUtils.createMediaQueryObservable(xlQuery);
    var lgObservable = lgQuery == null ? null : ResponsiveKnockoutUtils.createMediaQueryObservable(lgQuery);
    var mdObservable = mdQuery == null ? null : ResponsiveKnockoutUtils.createMediaQueryObservable(mdQuery);
    var smObservable = smQuery == null ? null : ResponsiveKnockoutUtils.createMediaQueryObservable(smQuery);
  ...
  var createScreenRangeObservable = ResponsiveKnockoutUtils.createScreenRangeObservable;
  var createMediaQueryObservable = ResponsiveKnockoutUtils.createMediaQueryObservable;
