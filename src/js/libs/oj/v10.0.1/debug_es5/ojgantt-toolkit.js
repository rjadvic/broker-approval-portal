 ...
 * @ignore
 */
define(['exports', 'ojs/ojdvt-toolkit', 'ojs/ojdvt-timecomponent', 'ojs/ojtimeaxis-toolkit'], function (exports, dvt, ojdvtTimecomponent, ojtimeaxisToolkit) {
  'use strict';
  /**
   * @license
   * Copyright (c) 2014, 2021, Oracle and/or its affiliates.
  ...
  DvtGanttStyleUtils.getStandaloneTaskHeight = function (options) {
    ...
    if (resources && resources['taskDefaults'] && resources['taskDefaults']['height'] != null) return DvtGanttStyleUtils.getSizeInPixels(resources['taskDefaults']['height']);
  ...
  DvtGanttStyleUtils.getActualTaskHeight = function (options) {
    return DvtGanttStyleUtils.getStandaloneTaskHeight(options) - DvtGanttStyleUtils.getBaselineMarginTop(options) - DvtGanttStyleUtils.getBaselineTaskHeight(options);
  };
  /**
  ...
  DvtGanttStyleUtils.getBaselineTaskHeight = function (options) {
    ...
    if (resources && resources['taskDefaults'] && resources['taskDefaults']['baseline'] && resources['taskDefaults']['baseline']['height'] != null) {
  ...
  DvtGanttStyleUtils.getTimeAxisHeight = function (options, axisProp) {
    ...
    if (resources && resources[axisProp] && resources[axisProp]['height'] != null) return DvtGanttStyleUtils.getSizeInPixels(resources[axisProp]['height']);
  ...
  */
  ...
  DvtGanttDependencyNode.prototype.nodeType = 'dependency'; // supported dependency types
  ...
  DvtGanttDependencyNode._calcDepLineHelper = function (predecessorNode, successorNode, isTypeBeginFinish, isTypeEndFinish) {
    ...
    var y_intermediate = y2 >= y1 ? DvtGanttDependencyNode._getTaskBottom(predecessorNode) + dependencyLineGap : DvtGanttDependencyNode._getTaskTop(predecessorNode) - dependencyLineGap; // Lock values at .5 px grid to ensure consistent SVG rendering of crispedges across browsers
    ...
    var x2_flank = isTypeEndFinish ? x2 + taskFlankLength : x2 - taskFlankLength; // Start at (x1, y1)
  ...
  DvtGanttRowLabelContent.prototype._getCustomContent = function (rowObj, availableWidth) {
    ...
    var rowLabelPadding = DvtGanttStyleUtils.getRowLabelPadding(options); // maxWidth in the renderer context should be -1 if width is unbounded AND max-width is unbounded. Otherwise, width is bounded in some way, so give them that value in px
  ...
  DvtGanttEventManager.prototype.ProcessMarqueeEvent = function (event) {
    ...
    if (this._gantt.isMarqueeSelectEnabled()) {
      ...
      if (event.subtype === 'start') {
        ...
        var targets = layoutObjs.taskObjs.map(function (taskObj) {
          return taskObj['node'];
        });
  ...
  DvtGanttEventManager.prototype._dragStartSetup = function () {
    ...
    this._dragInitialViewportEnd = this._gantt.getViewportEndTime(); // Disable pointer-events on reference objects (which lay on top of the row drop targets and interferes)
  ...
  DvtGanttEventManager.prototype._dragCancelCleanup = function () {
    ...
    this._dragInitialViewportEnd = null; // Re-enable pointer-events on referece objects
  ...
  DvtGanttEventManager.prototype.OnMouseDown = function (event) {
    ...
    if (objAndDisp.logicalObject && objAndDisp.logicalObject.nodeType === 'task') {
      ...
      if (this.IsDragSupported(sourceType)) {
        ...
        this._gantt.SetMarqueeEnabled(false); // Normally this can be handled on dragstart event, but
  ...
  DvtGanttTask.prototype.renderBaseline = function (progressHeight) {
    ...
    if (baselineProps) {
      ...
      if (baselineDim) {
        ...
        yOffset = isMilestone ? Math.max(this._container.getValue('height'), height) + DvtGanttStyleUtils.getMilestoneBaselineYOffset(options) - height : this._container.getValue('height') + DvtGanttStyleUtils.getBaselineMarginTop(options);
  ...
  DvtGanttTaskNode.prototype.scrollIntoView = function (xPriority, yPriority) {
    ...
    var options = this._gantt.getOptions(); // Ensure task is in the DOM
  ...
  DvtGanttTaskNode.prototype._renderTaskMoveDragFeedback = function (referenceFinalLocalX, referenceFinalLocalY, container) {
    ...
    if (this._mainDragFeedbacks) {
      for (var i = 0; i < this._mainDragFeedbacks.length; i++) {
        ...
        this._mainDragFeedbackStartTimes[i] = ojtimeaxisToolkit.TimeAxisUtils.getPositionDate(ganttStartTime, ganttEndTime, isRTL ? ganttContentLength - newFeedbackX : newFeedbackX, ganttContentLength);
      }
    } else {
  ...
  DvtGanttRowNode.prototype._finalizeRowLabelRender = function (gantt) {
    ...
    if (labelContent.getDisplayableType() === 'g') {
      ...
      dvt.TextUtils.fitText(labelDisplayable, rowAxis.getWidth() - 2 * rowLabelPadding, height, rowAxis, 1); // Always start align labels
  ...
  DvtGanttRowNode.prototype._renderHorizontalGridline = function (gantt, container) {
    ...
    if (gantt.isHorizontalGridlinesVisible()) {
      ...
      this._gantt.getAnimationManager().preAnimateHorizontalGridline(this._horizontalLine, finalStates, renderState); // horizontal grid line in row axis
      ...
      if (bRowAxisEnabled) {
        ...
        if (!this._rowAxisHorizontalLine) {
          this._rowAxisHorizontalLine = new dvt.Line(this._gantt.getCtx(), this._horizontalLine.getX1(), this._horizontalLine.getY1(), rowAxisHorizontalLineFinalStates['x2'], this._horizontalLine.getY2());
        ...
        rowAxis.addChild(this._rowAxisHorizontalLine);
      ...
      if (this._rowAxisHorizontalLine) {
        rowAxis.removeChild(this._rowAxisHorizontalLine);
      }
  ...
  DvtGanttRowNode.prototype._removeRowLabel = function () {
    ...
    if (rowLabelContent && rowAxis) {
      ...
      if (rowLabelDisplayable) {
        ...
        onEnd = function onEnd() {
          rowAxis.removeChild(rowLabelDisplayable);
          rowAxis.removeChild(self._rowAxisHorizontalLine);
        };
  ...
  DvtGanttDataLayoutManager.prototype._calcTaskHeight = function (taskObj) {
    ...
    if (baselineProps && !(taskObj['baselineStartTime'] == null && taskObj['baselineEndTime'] == null)) {
      ...
      if (baselineProps['height'] == null && taskDefaults['baseline']['height'] === DvtGanttStyleUtils.getBaselineTaskHeight(options)) {
        baselineHeight = isBaselineMilestone ? DvtGanttStyleUtils.getActualTaskHeight(options) : taskDefaults['baseline']['height'];
      ...
      if (taskProps['height'] == null && taskDefaults['height'] === DvtGanttStyleUtils.getStandaloneTaskHeight(options)) {
        // No need to account for baseline margin-top in milestone case, as that is emulated with stroke.
        taskHeight = DvtGanttStyleUtils.getActualTaskHeight(options) + isBaselineMilestone * baselineMarginTop;
      }
    }
    ...
    var heightTaskToptoBaselineBottom = isBaselineMilestone ? DvtGanttStyleUtils.getMilestoneBaselineYOffset(options) + Math.max(baselineHeight, taskHeight) : taskHeight + baselineBarMarginTop + baselineHeight;
  ...
  DvtGanttDataLayoutManager.prototype.getLayoutObjectsInBBox = function (bbox) {
    ...
    var ganttContentLength = this._gantt.getContentLength(); // In RTL, the bbox right is the start; in LTR the bbox left is the start


    var bboxStartTime = ojtimeaxisToolkit.TimeAxisUtils.getPositionDate(ganttStartTime, ganttEndTime, isRTL ? ganttContentLength - (bbox.x + bbox.w) : bbox.x, ganttContentLength);
    var bboxEndTime = ojtimeaxisToolkit.TimeAxisUtils.getPositionDate(ganttStartTime, ganttEndTime, isRTL ? ganttContentLength - bbox.x : bbox.x + bbox.w, ganttContentLength);
    var relevantRowObjs = [];
    var relevantTaskObjs = [];

    for (var i = minRowInd; i <= maxRowInd; i++) {
      var rowObj = this._rowObjs[i];
      relevantRowObjs.push(rowObj); // TODO: Replace this with binary search, similar to _findRowIndRange
      ...
      for (var j = 0; j < taskObjs.length; j++) {
        ...
        if (this._isIntervalOverlap(taskObj['overallStartTime'], taskObj['overallEndTime'], bboxStartTime, bboxEndTime) && this._isIntervalOverlap(taskObj['y'], taskObj['y'] + taskObj['height'], bbox.y, bbox.y + bbox.h)) {
    ...
    return {
      'rowObjs': relevantRowObjs,
      'taskObjs': relevantTaskObjs
    };
  };
  /**
  ...
  var DvtGanttReferenceObjects = function DvtGanttReferenceObjects(gantt, type, refObjs) {
  ...
  DvtGanttReferenceObjects.prototype._applyStyles = function () {
    ...
    var defaultStyleClass = this._type === 'line' ? this._gantt.GetStyleClass('referenceObjectLine') : this._gantt.GetStyleClass('referenceObjectArea');
    var styleClass = svgClassName != null ? defaultStyleClass + ' ' + svgClassName : defaultStyleClass;

    this._ref.setClassName(styleClass, true);
  };
  /**
   * Computes a single path command that would draw all the reference objects.
   * @param {number=} offset px offset in x direction. Default 0.
   * @returns {string} the path command
   * @private
   */


  DvtGanttReferenceObjects.prototype._calcCmds = function (offset) {
    var isRTL = dvt.Agent.isRightToLeft(this._gantt.getCtx());
    ...
    if (this._type === 'line') {
      this._refObjs.forEach(function (refObj) {
        ...
        cmds += dvt.PathUtils.moveTo(pos, this._gantt.getDatabodyStart()) + dvt.PathUtils.verticalLineTo(this._gantt.getDatabodyStart() + this._gantt._canvasSize - this._gantt.getAxesHeight());
      }, this);
    } else {
      this._refObjs.forEach(function (refObj) {
        ...
        cmds += dvt.PathUtils.moveTo(leftPos, this._gantt.getDatabodyStart()) + dvt.PathUtils.horizontalLineTo(rightPos) + dvt.PathUtils.verticalLineTo(this._gantt.getDatabodyStart() + this._gantt._canvasSize - this._gantt.getAxesHeight()) + dvt.PathUtils.horizontalLineTo(leftPos) + dvt.PathUtils.verticalLineTo(this._gantt.getDatabodyStart());
      }, this);
    }
  ...
  DvtGanttReferenceObjects.prototype.render = function (container) {
    if (this._refObjs.length === 0) {
      return;
    }

    this._ref = new dvt.Path(this._gantt.getCtx(), this._calcCmds());

    if (this._type === 'line') {
      // For reference lines, there's a light "border" counterpart for contrast
      this._partnerRef = new dvt.Path(this._gantt.getCtx(), this._calcCmds(1));

      this._partnerRef.setClassName(this._gantt.GetStyleClass('referenceObjectInnerLine'), true);
  ...
  DvtGanttRenderer.renderGantt = function (gantt) {
    ...
    if (gantt.hasValidOptions()) {
      ...
      DvtGanttRenderer._renderZoomControls(gantt); // Render Marquee artifacts container
  ...
  DvtGanttRenderer.renderRowAxis = function (gantt, container) {
    ...
    if (gantt.isRowAxisEnabled()) {
      ...
      gantt._canvasLength = gantt._canvasLength - rowAxisSpace; // Render row axis background. Position is adjusted in rowAxis.adjustPosition()

      var rowAxisBackgroundContainer = rowAxis.getBackgroundContainer();
      rowAxisBackgroundContainer.removeChildren(); // row axis backgrond rect. extra 1px to hide border touching the gantt.
      ...
      rowAxisBackgroundContainer.addChild(rowAxisBackground); // row axis background horizontal divider that lines up with the time axis.
  ...
  DvtGanttRenderer._renderMarqueeArtifactsContainer = function (gantt, container) {
    var databody = gantt.getDatabody(); // don't bother in the no data case
    ...
    if (artifactsContainer == null) {
      artifactsContainer = new dvt.Container(gantt.getCtx());
      gantt.setMarqueeArtifactsContainer(artifactsContainer); // Initial state: sync with databody's initial state
    ...
    var cp = new dvt.ClipPath();
    cp.addRect(0, gantt.getDatabodyStart(), gantt.getContentLength(), gantt.getDatabodyHeight());
    artifactsContainer.setClipPath(cp); // Set event handlers

    var eventManager = gantt.getEventManager(); // Marquee Select

    if (gantt.getOptions()['selectionMode'] === 'multiple') {
      var bounds = new dvt.Rectangle(1, 1, gantt.getContentLength() - 1, gantt.getContentHeight() - 1); // Placeholders, dvt.MarqueeHandler will set the appropriate css class
  ...
  DvtGanttRenderer._renderEmptyText = function (gantt, noData) {
    ...
    var text = gantt.renderEmptyText(gantt._canvas, emptyTextStr, new dvt.Rectangle(0, 0, gantt._backgroundWidth, gantt._backgroundHeight), gantt.EventManager, null);
  ...
  DvtGanttRenderer._renderReferenceObjects = function (gantt, container, type) {
    var referenceObjectsContainer = type === 'line' ? gantt.getReferenceLinesContainer() : gantt.getReferenceAreasContainer();
    ...
    while (j < referenceObjects.length) {
      ...
      if (refObj.svgStyle || refObj.svgClassName || refObj.shortDesc) {
        new DvtGanttReferenceObjects(gantt, type, referenceObjects.slice(i, j)).render(referenceObjectsContainer);
        new DvtGanttReferenceObjects(gantt, type, [referenceObjects[j]]).render(referenceObjectsContainer);
        i = j + 1;
      }
  ...
  DvtGanttRenderer._createTriangleMarker = function (gantt) {
    ...
    return this._createMarker(gantt, id, width, height, pathCmd);
  };
  /**
  ...
  DvtGanttRenderer._createMarker = function (gantt, id, width, height, pathCmd) {
    var statusClassName = gantt.GetStyleClass('dependencyLineConnector');
    var elem = dvt.SvgShapeUtils.createElement('marker', id);
    dvt.ToolkitUtils.setAttrNullNS(elem, 'class', statusClassName);
  ...
  var Gantt = function Gantt(context, callback, callbackObj) {
  ...
  Gantt.prototype.Init = function (context, callback, callbackObj) {
    ...
    Gantt.superclass.Init.call(this, context, callback, callbackObj); // Create the defaults object
  ...
  Gantt.prototype.SetOptions = function (options) {
    ...
    Gantt.superclass.SetOptions.call(this, options); //initial setup
  ...
  Gantt.prototype.render = function (options, width, height) {
    ...
    Gantt.superclass.render.call(this, options, width, height); // Render an aria live region for accessibility during DnD
  ...
  Gantt.prototype.renderAriaLiveRegion = function () {
  ...
  Gantt.prototype.getAriaLiveRegion = function () {
  ...
  Gantt.prototype.updateLiveRegionText = function (text) {
    if (this._ariaLiveRegion) {
  ...
  Gantt.prototype.removeAriaLiveRegion = function () {
    if (this._ariaLiveRegion) {
  ...
  Gantt.prototype.setDataRegionTranslateY = function (translateY, bFromScrolling) {
    ...
    if (this.isMarqueeSelectEnabled() && this._marqueeArtifactsContainer) this._marqueeArtifactsContainer.setTranslateY(translateY);
  ...
  Gantt.prototype.panBy = function (deltaX, deltaY, diagonal) {
    diagonal = typeof diagonal !== 'undefined' ? diagonal : this.getCtx().getThemeBehavior() !== 'alta'; // scroll horizontally and make sure it's scrolling in one direction only
