...
define(['exports', 'ojs/ojcore-base'], function (exports, oj) {
  ...
  DataCollectionUtils.disableElement = function (element) {
    var tabIndex = parseInt(element.getAttribute(DataCollectionUtils._TAB_INDEX), 10); // store the tabindex as an attribute
  ...
  DataCollectionUtils.getFocusableElementsIncludingDisabled = function (element) {
    var inputElems = [];
    var nodes = element.querySelectorAll(DataCollectionUtils._FOCUSABLE_ELEMENTS_QUERY + ',[' + DataCollectionUtils._DATA_OJ_TABMOD + ']');
  ...
  DataCollectionUtils.isMobileTouchDevice = function () {
    var agentInfo = oj.AgentUtils.getAgentInfo();
    return agentInfo.os === oj.AgentUtils.OS.IOS || agentInfo.os === oj.AgentUtils.OS.ANDROID || agentInfo.os === oj.AgentUtils.OS.WINDOWSPHONE;
  };

  DataCollectionUtils.getNoJQFocusHandlers = function (focusIn, focusOut) {
    var noJQFocusInHandler = function noJQFocusInHandler(element) {
      return focusIn($(element));
    };

    var noJQFocusOutHandler = function noJQFocusOutHandler(element) {
      return focusOut($(element));
    };

    return {
      focusIn: noJQFocusInHandler,
      focusOut: noJQFocusOutHandler
    };
  };
  ...
  DataCollectionUtils.getAddEventKeysResult = function (initialKeys, addEventDetail, isLoadAll) {
    ...
    var eventKey;
    var eventIndex;
    var beforeIndex; // returns an object that stores the required insertion information for a given key
    ...
    var returnKeys = _toConsumableArray(initialKeys);

    var eventKeys = [];
    addEventDetail.keys.forEach(function (key) {
      eventKeys.push(key);
    });
    var eventBeforeKeys = []; // afterKeys is deprecated, but continue to support it until we can remove it.
    // forEach can be called on both array and set.

    var beforeKeyIter = addEventDetail.addBeforeKeys ? addEventDetail.addBeforeKeys : addEventDetail.afterKeys;
    ...
    if (eventBeforeKeys.length === eventKeys.length) {
      ...
      while (eventKeys.length !== leftOverLength) {
        leftOverLength = eventKeys.length; // loop through in reverse order as most DP impls have an increasing global index order in their events.
  ...
  var disableDefaultBrowserStyling = DataCollectionUtils.disableDefaultBrowserStyling;
  var enableAllFocusableElements = DataCollectionUtils.enableAllFocusableElements;
  var getAddEventKeysResult = DataCollectionUtils.getAddEventKeysResult;
  ...
  var isArrowDownKeyEvent = DataCollectionUtils.isArrowDownKeyEvent;
  var isArrowLeftKeyEvent = DataCollectionUtils.isArrowLeftKeyEvent;
  var isArrowRightKeyEvent = DataCollectionUtils.isArrowRightKeyEvent;
  var isArrowUpKeyEvent = DataCollectionUtils.isArrowUpKeyEvent;
  var isClickthroughDisabled = DataCollectionUtils.isClickthroughDisabled;
  var isEndKeyEvent = DataCollectionUtils.isEndKeyEvent;
  var isEnterKeyEvent = DataCollectionUtils.isEnterKeyEvent;
  var isEscapeKeyEvent = DataCollectionUtils.isEscapeKeyEvent;
  var isEventClickthroughDisabled = DataCollectionUtils.isEventClickthroughDisabled;
  var isF2KeyEvent = DataCollectionUtils.isF2KeyEvent;
  var isHomeKeyEvent = DataCollectionUtils.isHomeKeyEvent;
  var isMobileTouchDevice = DataCollectionUtils.isMobileTouchDevice;
  var isSpaceBarKeyEvent = DataCollectionUtils.isSpaceBarKeyEvent;
  var isTabKeyEvent = DataCollectionUtils.isTabKeyEvent;
  var KEYBOARD_KEYS = DataCollectionUtils.KEYBOARD_KEYS;
  var CHECKVIEWPORT_THRESHOLD = DataCollectionUtils.CHECKVIEWPORT_THRESHOLD;
