...
define(['ojs/ojeditablevalue', 'ojs/ojoptgroup', 'ojs/ojoption', 'ojs/ojhighlighttext', 'ojs/ojcore-base', 'jquery', 'ojs/ojdomutils', 'ojs/ojset', 'ojs/ojtimerutils', 'ojs/ojcontext', 'ojs/ojlistdataproviderview', 'ojs/ojtreedataproviderview', 'ojs/ojtranslation', 'ojs/ojlogger', 'ojs/ojcustomelement-utils', 'ojs/ojcomponentcore', 'ojs/ojthemeutils'], function (ojeditablevalue, ojoptgroup, ojoption, ojhighlighttext, oj$1, $, DomUtils, ojSet, TimerUtils, Context, ListDataProviderView, TreeDataProviderView, Translation, Logger, ojcustomelementUtils, Components, ThemeUtils) {
  ...
  var _ComboUtils = {
    ...
    KEY: {
      ...
      isPasteAction: function isPasteAction(event) {
        var agentInfo = oj$1.AgentUtils.getAgentInfo();
        var os = agentInfo.os;
        var code = event.code;
        return code === _ComboUtils.KEY.CODE.KEY_V && (os === oj$1.AgentUtils.OS.MAC && event.metaKey || os !== oj$1.AgentUtils.OS.MAC && event.ctrlKey);
      }
    },
    ...
    _focus: function _focus(widget, $el) {
      ...
      if (widget._focusTimer != null) {
        widget._focusTimer.clear(); // eslint-disable-next-line no-param-reassign
      ...
      var resolveBusyState = _ComboUtils._addBusyState(widget.container, 'setting focus'); // Set a 40 timeout. In voiceover mode, previous partial value was read. See 
    ...
    isDataProvider: function isDataProvider(data) {
      return data && oj$1.DataProviderFeatureChecker ? oj$1.DataProviderFeatureChecker.isDataProvider(data) : false;
    },
    isTreeDataProvider: function isTreeDataProvider(data) {
      return data && oj$1.DataProviderFeatureChecker ? oj$1.DataProviderFeatureChecker.isTreeDataProvider(data) : false;
    },
     ...
     * @ignore
     */
    getDataProviderKeySet: function getDataProviderKeySet(widget, initialKeys) {
    ...
    syncValueWithValueOption: function syncValueWithValueOption(ojContext, valueOption, value, nativeRender) {
      ...
      var isCustomElement = ojContext._IsCustomElement(); //  - resetting value when value-option and placeholder are set throws exception
    ...
    isValueOptionsForPlaceholder: function isValueOptionsForPlaceholder(multiple, valOpts) {
      return valOpts == null || !multiple && valOpts.value == null || oj$1.Object.compareValues(valOpts, _ComboUtils.getFixupValueOptionsForPlaceholder(multiple));
    },
    ...
    isValueForPlaceholder: function isValueForPlaceholder(multiple, value) {
      return value == null || oj$1.Object.compareValues(value, _ComboUtils.getValueForPlaceholder(multiple));
    },
    //  - need to be able to specify the initial value of select components bound to dprv
    ...
    wrapDataProviderIfNeeded: function wrapDataProviderIfNeeded(widget, opts) {
      ...
      if (_ComboUtils.isDataProvider(dataProvider)) {
        ...
        if (isTree && !(dataProvider instanceof TreeDataProviderView) || !isTree && !(dataProvider instanceof ListDataProviderView)) {
    ...
    _handleDataProviderEvents: function _handleDataProviderEvents(widget, event) {
      var resolveBusyState = _ComboUtils._addBusyState(widget.widget(), 'updating value on dataprovider mutation event'); // JET-39059 - oj-select-one does not set initial value on first binding
     ...
     * @memberof _ComboUtils
     */
    _handleDataProviderMutationEvent: function _handleDataProviderMutationEvent(widget, event) {
    ...
    addDropdownMessage: function addDropdownMessage(container, context, messageText) {
      ...
      container._saveDropdownMessage = msgParent; // jet-39078 - accessibility: jaws does not read "no matches found" for oj-select-many
      // add the message text to the live region for it be read

      var widget = _ComboUtils.getWidget(context);
    ...
    fetchByKeyFromDataProvider: function fetchByKeyFromDataProvider(widget, options, query) {
      ...
      if (container._fetchByKeys && container._fetchByKeys.promise && oj$1.Object.compareValues(query.value, container._fetchByKeys.key)) {
        fetchPromise = container._fetchByKeys.promise;
      } else {
        fetchPromise = dataProvider.fetchByKeys({
          keys: new Set(query.value)
     ...
     * @ignore
     */
    getNormalizedValueArray: function getNormalizedValueArray(value) {
     ...
     * @ignore
     */
    getWidget: function getWidget(ojContext) {
      // for oj-select
      if (ojContext.select != null) {
     ...
     * @ignore
     */
    createProxyDropdownElement: function createProxyDropdownElement(containerId) {
  ...
  var _ojChoiceDefaults = {
    ...
    // eslint-disable-next-line no-unused-vars
    sanitizeData: function sanitizeData(data, useUnparsedValue) {
      return data;
    },
  ...
  var _AbstractOjChoice = _ComboUtils.clazz(Object, {
    ...
    _init: function _init(_opts) {
      ...
      this._defaultResultsMaxHeight = Number.parseInt(this.results.css('max-height'), 10); // if html ul element is provided, use it instead
      ...
      this._idsFromDropdown = _ComboUtils.getDataProviderKeySet(opts.ojContext); // initialize the container
        ...
        if (this._classNm === 'oj-combobox') {
          if (this.ojContext.multiple) {
            ...
            if (this.ojContext.options.labelledBy) {
              ...
              var ariaLabelledBy = ojeditablevalue.EditableValueUtils._getOjLabelAriaLabelledBy(this.ojContext.options.labelledBy, defaultLabelId);
    ...
    _prepareOpts: function _prepareOpts(_opts) {
      ...
      opts.parseData = function (data) {
        ...
        var parsedData = {};
        var parsedValue;
        var formattedLabel; // If the provided data is a placeholder, return the same
    ...
    _usingHandler: function _usingHandler(pos, props) {
      ...
      var $dropdown = this.dropdown;
      var $results = this.results; // Resize dropdown if it overflows

      var availableSize = oj$1.PositionUtils.calcAvailablePopupSize(pos, props); // We need to use the dropdown height for calculating the avialable height.
      ...
      var dropdownHeight = $dropdown.outerHeight(true); // Extra elements height includes height of the search field (if exists),
      // dropdown message (if exists) and dropdown margin

      var extraElementsHeight = dropdownHeight - resultsHeight;
      var netAvailableHeight = availableSize.height - extraElementsHeight;
      var newHeight = Math.min(this._defaultResultsMaxHeight, netAvailableHeight); // Make sure the max-width and max-height attributes are cleared properly from
      ...
      $results.css('max-height', newHeight); // Call position without using since we are calling it from the
      // using handler. The dropdown should be placed in the same position
      // but the overflow should have been handled.

      var dropdownPosition = this._getDropdownPosition(true);
    ...
    // _AbstractOjChoice
    _getDropdownPosition: function _getDropdownPosition(excludeUsingHandler) {
      var position = {
        my: 'start top',
        at: 'start bottom',
    ...
    /**
       ...
       * @ignore
       */
    open: function open(e, dontUpdateResults, dontShowSearchbox) {
    ...
    _showDropDown: function _showDropDown() {
      ...
      if (this._classNm === 'oj-select' && this.opts.multiple !== true && popupRoot && popupRoot.hasClass('oj-listbox-drop-layer')) {
        oj$1.PopupService.getInstance().triggerOnDescendents(popupRoot, oj$1.PopupService.EVENT.POPUP_REFRESH);
      } else {
        ...
        psEvents[oj$1.PopupService.EVENT.POPUP_REMOVE] = $.proxy(this._surrogateRemoveHandler, this);
        psEvents[oj$1.PopupService.EVENT.POPUP_AUTODISMISS] = $.proxy(this._clickAwayHandler, this);
        ...
        oj$1.PopupService.getInstance().open(psOptions); // move the global id to the correct dropdown
    ...
    close: function close(event, shouldReopenOnNewData) {
      ...
      this._getActiveContainer().attr('aria-expanded', false); // force clear the aria attributes related to dropdown
    ...
    // _AbstractOjChoice
    _updateMatchesCount: function _updateMatchesCount(translatedString, forcedUpdate) {
    ...
    setVal: function setVal(val, event, context) {
      ...
      if (!Array.isArray(val) && !this.ojContext._IsCustomElement()) {
        ...
        if (!oj$1.Object.compareValues(previousVal, [val]) || _ComboUtils.hasInvalidComponentMessages(this.ojContext)) {
     ...
     * @ignore
     */
    _UpdateValueOptionsWithConverter: function _UpdateValueOptionsWithConverter() {},
     ...
     */
    // eslint-disable-next-line no-unused-vars
    _SyncRawValue: function _SyncRawValue(event, setDirty) {},
     ...
     * @ignore
     */
    _selectItemByValue: function _selectItemByValue(value) {
      var options = this.ojContext.options;
      var selectionOptions = {
        trigger: _ComboUtils.ValueChangeTriggerTypes.ENTER_PRESSED
      }; // Normalize the value to an Array

      var _value = _ComboUtils.getNormalizedValueArray(value);
     ...
     * @ignore
     */
    _selectItemFromDataProvider: function _selectItemFromDataProvider(values, options) {
      var dataProvider = _ComboUtils.getDataProvider(this.opts);

      var keys = new Set(values);
      var fetchByKeysPromise = dataProvider.fetchByKeys({
        keys: keys
      });
      return fetchByKeysPromise.then(this._bind(function (fetchByKeysResult) {
        var results = fetchByKeysResult.results;
        var items = [];
        values.forEach(this._bind(function (value) {
          var itemResult;

          if (results.has(value)) {
            var item = results.get(value);
            var itemData = item.data;
            var itemMetadata = item.metadata;
            itemResult = _ComboUtils.createItemResult(itemData, itemMetadata); // Add the id to the map
     ...
     * @ignore
     */
    _selectItemFromArray: function _selectItemFromArray(values, options) {
      var optionsArray = this.ojContext.options.options;
      var foundItems = new Map();
      var items = [];
      ...
      for (var i = 0; i < optionsArray.length; i++) {
        var _item = optionsArray[i];

        if (values.includes(this.id(_item))) {
          foundItems.set(this.id(_item), _item); // prematurely break the loop if all values are found
      ...
      values.forEach(this._bind(function (value) {
        ...
        if (foundItems.has(value)) {
          _item = foundItems.get(value); // Add the id to the map
     ...
     * @ignore
     */
    _selectItemFromOjOptions: function _selectItemFromOjOptions(values, options) {
      var $ojOptions = this.datalist.find('oj-option');
      var foundItems = new Map();
      var items = [];

      _ComboUtils.each2($ojOptions, this._bind(function (index, ojOption) {
        var itemData = this._optionToData(ojOption);
     ...
     * @ignore
     */
    _getItemForCustomInput: function _getItemForCustomInput(value) {
     ...
     */
    // eslint-disable-next-line no-unused-vars
    _InvokeSelection: function _InvokeSelection(items, options) {}
  });
  /**
   * @license
   * Copyright (c) 2014, 2021, Oracle and/or its affiliates.
  ...
  var _AbstractMultiChoice = _ComboUtils.clazz(_AbstractOjChoice, {
     ...
     */
    applyReadonlyState: function applyReadonlyState() {
      _AbstractMultiChoice.superclass.applyReadonlyState.apply(this, arguments); // We need to rerender the selected options to render the choices in readonly mode
    ...
    _prepareOpts: function _prepareOpts() {
      ...
      if (tagName === 'input' && opts.element.attr('list') || tagName === 'select' && opts.element.children().length > 0 || opts.ojContext._IsCustomElement() && !opts.options || opts.list) {
        ...
        opts.initSelection = function (element, callback) {
          var selected;
          var data = []; // clear the stored ids from dropdown
          ...
          if (self.getVal()) {
            ...
            for (var i = 0; i < ids.length; i++) {
              ...
              if (selected && selected.length) {
                var currentData = self._optionToData(selected);

                data.push(currentData); // the data is from options, so add it to the set
            ...
            _ComboUtils.each2(selected, function (index, elm) {
              var currentData = self._optionToData(elm);
        ...
        if (_ComboUtils.isDataProvider(opts.options) || $.isFunction(opts.options)) {
          ...
          opts.initSelection = function (element, callback) {
            ...
            var matches = []; // clear the stored ids from dropdown

            self._idsFromDropdown.clear(); // This data will be saved after querying the options.
              ...
              for (var i = 0; i < ids.length; i++) {
                ...
                for (var j = 0; j < matches.length; j++) {
                  ...
                  if (oj$1.Object.compareValues(id, opts.id(match))) {
                    ordered.push(match); // the data is from options, so add it to the set
                  ...
                  if (qryResult && qryResult.results) {
                    ...
                    if (results && results.length) {
                      ...
                      if (_ComboUtils.isDataProvider(opts.options)) {
                        ...
                        if (oj$1.Object.compareValues(ids, vals) && !_ComboUtils.isValueChanged(self.ojContext)) {
                          concat = results;
                        } else {
                          var v;
                          var found;
    ...
    _initContainer: function _initContainer() {
      ...
      this.search.on('blur keyup', this._bind(function (e) {
        ...
        if (this.opts.manageNewEntry && this.search.val() && this.results.find('.oj-hover').length <= 0) {
          ...
          // in oj.Combobox and this will called synchronously for both sync and async validators
          // as the validation happens after the parsing.
          this.shouldApplyConverter = true; // When creating new entry include the unparsed value as well as we will be sending
    ...
    _mouseDownHandler: function _mouseDownHandler(event) {
      ...
      if (this._opened()) {
        this.close(event); // combobox loses focus when clicked on the container and not in the input
    ...
    _updateSelection: function _updateSelection(data) {
      ...
      var filtered = [];
      var self = this;
      var lastChoiceIndex; // filter out duplicates
    ...
    _onSelect: function _onSelect(data, options, event) {
      ...
      if (!this.ojContext.isValid()) {
        ...
        valOpts = this.currentItem.slice(0); // In this case, we might have some unparsed invalid values, so we need to force check and
        // apply the converter
      ...
      parsedId = this.id(parsedData); // Create a keyset of the selected values (use dp's implementation if one is provided)

      valKeySet = _ComboUtils.getDataProviderKeySet(this.ojContext, val); // Only add the value if it not already present in the key set and the id is not an empty string
      ...
      if (!valKeySet.has(parsedId) && id !== '') {
        ...
        if (isSelectCombobox) {
          ...
          valOpts.push(valueOptionFromResult); // if converter is not set to be applied or if the user inputted value is matched with
     ...
     * @memberof! _AbstractMultiChoice
     */
    _createChoice: function _createChoice(isEnabled, isReadOnly, isLastChoice) {
      var $choice;

      if (!isEnabled) {
        $choice = $("<li class='" + this._classNm + '-selected-choice ' + this._classNm + "-locked'>" + '<div></div>' + '</li>');
      } else if (isReadOnly) {
        $choice = $("<li class='" + this._classNm + "-selected-choice'>" + '    <div></div>' + '</li>'); // if it is not the last choice, add the separator span

        if (!isLastChoice) {
          var separator = Translation.getTranslatedString('oj-converter.plural-separator');
          var separatorSpan = $("<span class='" + this._classNm + "-selected-choice-separator'>" + '</span>'); // Since the translation comes from an external source, use jQuery.text to add the
        ...
        $choice = $("<li class='" + this._classNm + "-selected-choice'>" + '    <div></div>' + "    <a href='#' onclick='return false;' role='button' aria-label='remove' class='" + this._classNm + '-clear-entry ' + '      oj-component-icon oj-clickable-icon-nocontext ' + this._classNm + "-clear-entry-icon' tabindex='-1'>" + '    </a>' + '</li>');
      }

      return $choice;
    },
    _addSelectedChoice: function _addSelectedChoice(data, isLastChoice) {
      ...
      if (formatted !== undefined) {
        ...
        choice.attr('valueText', formatted); // add aria-labelledby to the choice node to point to the div
    ...
    setVal: function setVal(val, event, context) {
      ...
      if (!oj$1.Object.compareValues(previousVal, unique) || _ComboUtils.hasInvalidComponentMessages(this.ojContext)) {
    ...
    _InvokeSelection: function _InvokeSelection(items, options) {
      ...
      context = _ComboUtils.getContextWithExtraData(context, opts, valueOptionsData, valueOptionsMetadata); // set valueOptions

      this.setValOpts(items);
      this._skipSetValueOptions = true;
      var values = items.map(this.id);
      var setValueReturn = this.setVal(values, null, context);
  ...
  var _OjMultiCombobox = _ComboUtils.clazz(_AbstractMultiChoice, {
    ...
    _setPlaceholder: function _setPlaceholder() {
      ...
      // no value.
      if (_ComboUtils.isValueForPlaceholder(true, this.getVal()) && _ComboUtils.isValueOptionsForPlaceholder(true, this.getValOpts())) {
    ...
    _UpdateValueOptionsWithConverter: function _UpdateValueOptionsWithConverter() {
      ...
      var updatedValOpts = []; // if no valOpts is present, return doing nothing
     ...
     */
    _SyncRawValue: function _SyncRawValue(event, setDirty) {
      var valueOptions = (this.getValOpts() || []).slice(0);
      var searchText = this.search.val();
      var rawValue = valueOptions.map(function (vo) {
        return vo.label;
      });
  ...
  var _AbstractSingleChoice = _ComboUtils.clazz(_AbstractOjChoice, {
    ...
    _initContainer: function _initContainer() {
      ...
      this.search.on('blur keyup', this._bind(function (e) {
        ...
        if (this.search.val() !== undefined && this.results.find('.oj-hover').length <= 0 && (e.type !== 'keyup' || !this.enterKeyEventHandled)) {
          ...
          if (this.opts.manageNewEntry) {
            var value = this.search.val(); // The converter should only be applied for the user input.
            // This flag will be used in the overriden _parseValue method
            // in oj.Combobox and this will called synchronously for both sync and async validators
            // as the validation happens after the parsing.
    ...
    _prepareOpts: function _prepareOpts() {
      ...
      if (tagName === 'input' && opts.element.attr('list') || tagName === 'select' && opts.element.children().length > 0 || opts.ojContext._IsCustomElement() && !opts.options || opts.list) {
        ...
        opts.initSelection = function (element, callback) {
          var selected;
          var value = self.getVal(); // clear the ids from dropdown set as it can hold only one value for single choice
          ...
          if (value === undefined || value === null) {
            ...
            if (self._classNm === 'oj-select' && selected === undefined && opts.ojContext._IsCustomElement()) {
              ...
              Logger.info('Select identified by ' + self.container.attr('id') + ' defaults to first option because the value is not set.');
            }
          }
        ...
        if (_ComboUtils.isDataProvider(opts.options) || $.isFunction(opts.options)) {
          ...
          opts.initSelection = function (element, callback) {
            ...
            var match = null; // clear the ids from dropdown set as it can hold only one value for single choice
            ...
            if (id == null) {
              ...
              if (tagName === 'select' && !self.ojContext._HasPlaceholderSet() && (!optionsKeys || !optionsKeys.value && !optionsKeys.label)) {
                _ComboUtils.fetchFirstBlockFromDataProvider(self.container, opts, 1).then(function (data) {
                  ...
                  var isValueUpdated = self._getValueItem() != null; // At this point if we still don't have a selected value then default to the first item
            ...
            if (!match) {
              ...
              if (currentItem && currentItem.length && oj$1.Object.compareValues(id, opts.id(currentItem[0]))) {
                match = currentItem[0];
                Logger.info('Select identified by ' + self.container.attr('id') + ' defaults to first option due to invalid value.');
              }
            } // valueChangeTrigger will have one of the values from
            // _ComboUtils._ValueChangeTriggerTypes, which represents the
            // what triggered the value change. But if value is programmatically
            ...
            if (!match && !self.valueChangeTrigger) {
              opts.query({
                ...
                callback: !$.isFunction(callback) ? $.noop : function (qryResult) {
                  //  - While fetching the label for the initial value,
                  // user can still interact the component and pick a new value.
                  if (!_ComboUtils.isDataProvider(opts.options) || oj$1.Object.compareValues(id, self.getVal()) && !_ComboUtils.isValueChanged(self.ojContext)) {
                    if (qryResult && qryResult.results) {
            ...
            opts.query({
              ...
              callback: !$.isFunction(callback) ? $.noop : function () {
                ...
                if (!match && tagName === 'select' && !self.ojContext._isOptionDataPending()) {
                  match = first;
                  Logger.info('Select identified by ' + self.container.attr('id') + ' defaults to ' + (usePlaceholder ? 'placeholder' : 'first option') + ' due to invalid value.');
                }
    ...
    _postprocessResults: function _postprocessResults(data, initial, noHighlightUpdate) {
      ...
      _ComboUtils.each2(highlightableChoices, function (i, elm) {
        ...
        if (valueItem != null && oj$1.Object.compareValues(valueItem, self.id(elm.data(self._elemNm)))) {
          selected = i;
          return false;
        }
    ...
    _onSelect: function _onSelect(data, options, event) {
      ...
      var valueOptionFromResult; // if the data is from dropdown add it to the set
      ...
      if (id.length === 0) {
        ...
        val = id;
        valueOptionFromResult = _ComboUtils.findOptionFromResult(this, parsedId, valopt); // if the converter is not set to be applied or if the user inputted value is matched with
        // one of provided options, add the id to the set.
    ...
    _InvokeSelection: function _InvokeSelection(items, options) {
      ...
      return new Promise(this._bind(function (resolve, reject) {
        var onSelectReturn = this._onSelect(item, options, null);
  ...
  var _OjSingleCombobox = _ComboUtils.clazz(_AbstractSingleChoice, {
     ...
     */
    _CreateContentElements: function _CreateContentElements() {
      var contentStructure = ["<div class='oj-text-field-container' role='presentation'>", "  <div class='oj-combobox-choice oj-combobox-accessible-container' tabindex='-1' role='presentation'>", "   <div class='oj-text-field-middle'>", "     <input type='text' autocomplete='off' autocorrect='off' autocapitalize='off'", "       spellcheck='false' class='oj-combobox-input' role='combobox' aria-expanded='false' aria-autocomplete='list' />", '   </div>', "   <abbr class='oj-combobox-clear-entry' role='presentation'></abbr>", "   <span class='oj-combobox-divider' role='presentation'></span>", "   <span class='oj-text-field-end'>", "     <a class='oj-combobox-arrow oj-combobox-icon oj-component-icon oj-clickable-icon-nocontext oj-combobox-open-icon'", "       role='presentation' aria-label='expand'></a>", '   </span>', '  </div>', '</div>', "<div class='oj-listbox-drop' role='presentation'>", "   <ul class='oj-listbox-results' role='listbox'>", '   </ul>', '</div>', "<div class='oj-helper-hidden-accessible oj-listbox-liveregion' aria-live='polite'></div>"];
    ...
    _updateSelectedOption: function _updateSelectedOption(selected) {
      if (selected !== undefined && selected !== null) {
        ...
        this._updateSelection(parsedData); // if value is a new entry
    ...
    _UpdateValueOptionsWithConverter: function _UpdateValueOptionsWithConverter() {
      ...
      var valOpt = this.getValOpts(); // if no valOpt is present, return doing nothing
  ...
  * @memberof! oj.ojComboboxMany
  ...
  oj$1.__registerWidget('oj.ojCombobox', $.oj.editableValue, {
    ...
    * @protected
    */
    _GetNormalizedAsyncValidatorsFromOption: ojeditablevalue.EditableValueUtils._GetNormalizedAsyncValidatorsFromOption,
    ...
    /**
       ...
       * @instance
       */
    _parseValue: function _parseValue(_submittedValue, event, setValid) {
      var parsedVal;
      var submittedValue = _submittedValue; // Converter should only be applied for the user inputs and not
      ...
      if (!this.combobox.shouldApplyConverter && !this.combobox.forceApplyConverter && !this._handleConverterChangeWithInvalidValue) {
      ...
      if (Array.isArray(submittedValue)) {
        if (this.combobox.forceApplyConverter || this._handleConverterChangeWithInvalidValue) {
          ...
          // is a success
          delete this._handleConverterChangeWithInvalidValue; // Check and apply converter for all the values

          parsedVal = [];

          for (var i = 0; i < submittedValue.length; i++) {
            var value = submittedValue[i]; // parse the value only if it not an dropdown item

            if (this.combobox._idsFromDropdown.has(value)) {
              parsedVal.push(value);
            } else {
              var parsed = this._super(value, event, setValid);

              parsedVal.push(parsed);
            }
          }
        } else {
          parsedVal = submittedValue.slice(); // since only one value can be set at a time, we need to parse
          // only the last item in the array

          if (parsedVal.length > 0) {
            var lastValue = parsedVal.slice(-1)[0];

            var _parsed = this._super(lastValue, event, setValid); // replace the last value with the parsed value.


            parsedVal.splice(-1, 1, _parsed);
          }
        }
      } else {
        // Delete the converter change flag before parsing, as for all we care we
    ...
    _SetValue: function _SetValue(newValue, event, options) {
      ...
      if (!resolved || resolved instanceof Promise) {
        ...
        if (this._IsCustomElement()) {
          ...
          isPlaceholderVal = newValue == null || newValue === '' || oj$1.Object.compareValues(newValue, []);
     ...
     * @memberof! oj.ojCombobox
     */
    _getValueUpdatePromise: function _getValueUpdatePromise() {
  ...
  var _OjInputSearchContainer = _ComboUtils.clazz(_OjSingleCombobox, {
  ...
  var _OjMultiSelect = _ComboUtils.clazz(_AbstractMultiChoice, {
    ...
    _containerKeydownHandler: function _containerKeydownHandler(e) {
      if (_ComboUtils.KEY.isPasteAction(e)) {
        this._userTyping = true; // open dropdown if it is not already open
    ...
    _opening: function _opening(event, dontUpdateResults, dontShowSearchbox) {
      // if beforeExpand is not cancelled
      // beforeExpand event will be triggered in base class _shouldOpen method
      ...
      var searchText = null; // Show and update the search box only if the dontShowSearchbox flag
      // is not set

      if (!dontShowSearchbox) {
        searchText = _ComboUtils.getSearchText(event); // select: focus still stay on the selectBox if open dropdown by mouse click
    ...
    close: function close(event) {
      ...
      this.search.attr('aria-expanded', false).removeAttr('aria-controls'); // When the dropdown is open and if the close method is invoked
    ...
    _clearSearch: function _clearSearch() {
      ...
      if (placeholder != null && _ComboUtils.isValueForPlaceholder(true, this.getVal()) && _ComboUtils.isValueOptionsForPlaceholder(true, this.getValOpts())) {
        ...
        node.text(placeholder); // oj-select-many has to append the items to the ul element inside selection
  ...
  var _OjSingleSelect = _ComboUtils.clazz(_AbstractSingleChoice, {
     ...
     */
    _CreateContentElements: function _CreateContentElements() {
      var contentStructure = ["<div class='oj-text-field-container' role='presentation'>", "  <div class='oj-select-choice oj-select-accessible-container' tabindex='0' role='combobox' ", "     aria-autocomplete='none' aria-expanded='false'>", "   <div class='oj-text-field-middle'>", "      <span class='oj-select-chosen'></span>", '   </div>', "   <abbr class='oj-select-search-choice-close' role='presentation'></abbr>", "   <span class='oj-text-field-end'>", "     <a class='oj-select-arrow oj-component-icon oj-clickable-icon-nocontext oj-select-open-icon' role='presentation'>", '     </a>', '   </span>', '  </div>', '</div>', "<div class='oj-listbox-drop' role='dialog'>", "  <div class='oj-listbox-search-wrapper'>", "  <div class='oj-listbox-search'>", "    <input type='text' autocomplete='off' autocorrect='off' autocapitalize='off'", "           spellcheck='false' class='oj-listbox-input' title='Search field' ", "           role='combobox' aria-expanded='false' aria-autocomplete='list' />", "    <span class='oj-listbox-spyglass-box'>", "      <span class='oj-component-icon oj-listbox-search-icon' role='presentation'>", "       <b role='presentation'></b></span>", '    </span>', '  </div>', '  </div>', "   <ul class='oj-listbox-results' role='listbox'>", '   </ul>', '</div>', "<div class='oj-helper-hidden-accessible oj-listbox-liveregion' aria-live='polite'></div>"];
      return $(contentStructure.join(''));
    },
    // _OjSingleSelect
    // eslint-disable-next-line no-unused-vars
    _enable: function _enable(enabled) {
      ...
      if (this._enabled) {
        ...
        // Don't allow focus on a disabled "select"
        this.selection.removeAttr('tabindex'); //  - disabled select icon hover still shows changes
  ...
  * @memberof! oj.ojSelectMany
  ...
  oj$1.__registerWidget('oj.ojSelect', $.oj.editableValue, {
    ...
    _setOptions: function _setOptions(options, flags) {
      ...
      var setValuePromise; // _setOptions is invoked to set multiple options and for some options, we need
      ...
      this._super(options, flags); // Get the top most object from the stack, which represents the current call


      processSetOptions = this._processSetOptions.pop(); // JET-24012 - select should validate value against options asynchronously
      ...
      if (processSetOptions.validateValue) {
        var validateValue = function () {
          var originalResolveValueOptionFlag = this._resolveValueOptionsLater; // if the current valueOption(s) is set by us internally,
        ...
        }.bind(this); // If the value is being set, the value may still not be processed, since
    ...
    _setOption: function _setOption(key, _value, flags) {
      ...
      if (key === 'value') {
        var processSetOptionValue = function () {
          ...
          this._valueUpdatePromise = null; // Wrapped super method to be passed as an argument for helper methods

          var setValueFn = function (valueToSet) {
            selfSuper.call(this, key, valueToSet, flags);
          }.bind(this); // Check if the new value is a placeholder and if it is, set the value and return
          ...
          value = this._treatValueForSetValue(value); // Validate against the dataprovider if one is used, and set the value
          // if it is valid.
          ...
          if (_ComboUtils.getDataProvider(this.options) && value) {
            ...
            var setValueIfValid = function (results) {
              ...
              if (results) {
                ...
                if (Array.isArray(values) && values.length) {
                  ...
                  setValueFn(cval); //  - select component binding broken in mobile
            ...
            this._validateFromDataProviderAndSetValue(value, setValueIfValid); // Store the promise


            this._valueUpdatePromise = returnPromise; //  - ojselect allows to set invalid value when using dataprovider
          ...
          this._validateFromOptionsAndSetValue(value, setValueFn); // No need to update valueUpdatePromise as the value update happens synchronously here


          return Promise.resolve();
        }.bind(this); // If this is called from setOptions, we need to wait till all the other options
          ...
          if (_ComboUtils.getDataProvider(this.options) && hasSelectedValue) {
            var validateValueAgainstNewOptions = function () {
              ...
              if (_ComboUtils.applyValueOptions(this.select, this.options)) {
                ...
                _ComboUtils.validateFromDataProvider(element, options, currentValue).then(function (results) {
                  //  - need to be able to specify the initial value of select components bound to dprv
                  ...
                  if (values) {
                    ...
                    if (!multi && !hasPlaceholderSet) {
                      ...
                      _ComboUtils.fetchFirstBlockFromDataProvider(self.select.container, self.options, 1).then(function (data) {
                        ...
                        // alter the new value.
                        var isValueUpdated = currentValue !== self.select.getVal(); // At this point if we still don't have a selected value then default to the first item
     ...
     * @memberof! oj.ojSelect
     */
    _validateFromDataProviderAndSetValue: function _validateFromDataProviderAndSetValue(value, setValueFn) {
      var element = this._isNative() ? this.element : this.select.container;
      var options = this.options; // update instance variables for jet render mode
     ...
     * @memberof! oj.ojSelect
     */
    _validateFromOptionsAndSetValue: function _validateFromOptionsAndSetValue(value, setValueFn) {
      var multi = this.multiple;
      ...
      if (!customSelectOne) {
        ...
        if (newArr.length > 0 || multi) {
          if (this._isNative()) {
            ...
            setValueFn(newArr); //  - need to be able to specify the initial value of select components bound to dprv
      ...
      if (this._isValidValue(value)) {
        setValueFn(value); //  - need to be able to specify the initial value of select components bound to dprv
    ...
    _isValidValue: function _isValidValue(value) {
      ...
      return !(this.select && this.select.opts.validate) || this.select.opts.validate(element, value) || this._isOptionDataPending();
    },
     ...
     * @memberof! oj.ojSelect
     */
    _treatValueForSetValue: function _treatValueForSetValue(value) {
      var multi = this.multiple;
      var customSelectOne = this._IsCustomElement() && !multi;
      var _value = value; // turn value to an array for widget select-one
     ...
     * @memberof! oj.ojSelect
     */
    _setPlaceholderForValue: function _setPlaceholderForValue(value, setValueFn) {
      var multi = this.multiple; // An empty value is considered a valid only when the folling conditions are met:
