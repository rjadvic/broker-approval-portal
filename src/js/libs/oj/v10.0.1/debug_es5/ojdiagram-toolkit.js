 ...
 * @ignore
 */
define(['exports', 'ojs/ojdvt-toolkit', 'ojs/ojdvt-panzoomcanvas', 'ojs/ojdvt-overview'], function (exports, dvt, ojdvtPanzoomcanvas, ojdvtOverview) {
  'use strict';
  /**
   * @license
   * Copyright (c) 2014, 2021, Oracle and/or its affiliates.
  ...
  DvtDiagramLayoutContextNode.prototype.SetContainerPaddingObj = function (obj) {
    ...
    if (this.isDisclosed()) {
      this._containerPadding = obj; //this function is called by the Diagram when initializing the
      //layout context, so the original bounds already include the container
      //padding, and we need to subtract it out here

      if (obj) {
        if (this._origBounds) {
          this._origBounds = new DvtDiagramRectangle(this._origBounds['x'], this._origBounds['y'], this._origBounds['w'] - (obj['left'] + obj['right']), this._origBounds['h'] - (obj['top'] + obj['bottom']));
        }

        if (this._origContentBounds) {
          this._origContentBounds = new DvtDiagramRectangle(this._origContentBounds['x'], this._origContentBounds['y'], this._origContentBounds['w'] - (obj['left'] + obj['right']), this._origContentBounds['h'] - (obj['top'] + obj['bottom']));
        }
      }
    }
  };
  ...
  DvtDiagramLayoutContextNode.prototype.setContainerPadding = function (top, right, bottom, left) {
    ...
    if (this.isDisclosed()) {
      ...
      if (this._origBounds) {
        this._bounds = new DvtDiagramRectangle(this._origBounds['x'], this._origBounds['y'], this._origBounds['w'] + left + right, this._origBounds['h'] + top + bottom);
      }

      if (this._origContentBounds) {
        this._contentBounds = new DvtDiagramRectangle(this._origContentBounds['x'], this._origContentBounds['y'], this._origContentBounds['w'] + left + right, this._origContentBounds['h'] + top + bottom);
      }
  ...
  DvtDiagramLayoutUtils.convertRectToDiagramRect = function (rect) {
    if (rect === undefined || rect == null) return null;else return new DvtDiagramRectangle(rect.x, rect.y, rect.w, rect.h);
  };
  /**
  ...
  DvtDiagramLayoutUtils.convertPointToDiagramPoint = function (point) {
    if (point === undefined || point == null) return null;else return new DvtDiagramPoint(point.x, point.y);
  };
  /**
  ...
  var DvtBaseDiagram = function DvtBaseDiagram(context, callback, callbackObj) {
    this.Init(context, callback, callbackObj);
  };

  dvt.Obj.createSubclass(DvtBaseDiagram, ojdvtPanzoomcanvas.PanZoomComponent, 'DvtBaseDiagram');
  /**
  ...
  DvtBaseDiagram.prototype.Init = function (context, callback, callbackObj) {
    DvtBaseDiagram.superclass.Init.call(this, context, callback, callbackObj); //: force text to scale linearly
  ...
  DvtBaseDiagram.prototype.ApplyLayoutContext = function (layoutContext, bSaveOffset) {
    ...
    for (var li = 0; li < layoutContext.getLinkCount(); li++) {
      var lc = layoutContext.getLinkByIndex(li); // get link bounds including label
    ...
    for (var ni = 0; ni < layoutContext.getNodeCount(); ni++) {
      ...
      this.ApplyLabelPosition(nc, node, DvtDiagramLayoutUtils.convertDiagramPointToPoint(pos)); //apply new container padding
  ...
  var DvtBaseDiagramNode = function DvtBaseDiagramNode(context, nodeId, diagram) {
  ...
  DvtBaseDiagramNode.prototype.setLabelPosition = function (pos) {
    if (pos) {
      this._labelPos = pos;
      DvtBaseDiagramNode.PositionLabel(this._labelObj, pos, this.getLabelBounds(true), this.getLabelRotationAngle(), this.getLabelRotationPoint());
    }
  };
  /**
  ...
  DvtBaseDiagramNode.prototype.setLabelAlignments = function (halign, valign) {//do nothing, only for DvtDiagramNode
  };
  /**
  ...
  var DvtDiagramDataAnimationPhase = {
    DELETE: 0,
    UPDATE: 1,
    INSERT: 2
  };
  ...
  DvtDiagramNode._addHoverSelectionDefaultStrokes = function (selectionShape, nodeData) {
    ...
    if (isRedwood) {
      if (nodeData.icon && nodeData.icon.color) var dataColor = nodeData.icon.color;else var dataColor = hoverOuterColor;
      var his = new dvt.Stroke(hoverInnerColor, 1, 1, true);
      var hos = new dvt.Stroke(dataColor, 1, 3, true);
      var sis = new dvt.Stroke(hoverInnerColor, 1, 1, true);
      var sos = new dvt.Stroke(selectionColor, 1, 3, true);
  ...
  DvtDiagramNode._addHoverSelectionOuterStrokes = function (selectionShape, nodeData) {
    ...
    if (isRedwood) {
      var his = new dvt.Stroke(hoverInnerColor, 1, 1, true);
      var hos = new dvt.Stroke(hoverOuterColor, 1, 3, true);
      var sis = new dvt.Stroke(hoverInnerColor, 1, 1, true);
      var sos = new dvt.Stroke(selectionColor, 1, 3, true);
      selectionShape.setHoverStroke(his, hos).setSelectedStroke(sis, sos);
    } else {
  ...
  DvtDiagramNode.prototype._animateCustomUpdate = function (animationHandler, oldNode) {
    ...
    animationHandler.add(playable, DvtDiagramDataAnimationPhase.UPDATE); // animate scale and fade in new node
  ...
  DvtDiagramNode.prototype.animateInsert = function (animationHandler) {
    this.setAlpha(0);
    animationHandler.add(new dvt.AnimFadeIn(this.getCtx(), this, animationHandler.getAnimationDuration()), DvtDiagramDataAnimationPhase.INSERT);
  };
  /**
  ...
  DvtDiagramLinkUnderlay.prototype.CreateConnectorUnderlay = function (points, connectorType, connectorTemplate, parentLink, connectorPos) {
    ...
    DvtDiagramLinkConnectorUtils.TransformConnector(connectorUnderlay, connectorType, connectorTemplate, points, connectorPos);
  ...
  var DvtBaseDiagramLink = function DvtBaseDiagramLink(context, linkId, diagram) {
  ...
  DvtBaseDiagramLink.prototype.CreateFeedbackUnderlay = function (strokeColor, strokeAlpha, strokeWidthOffset, styleObj, styleClass) {
    ...
    if (this._startConnector && this.getStartConnectorType()) feedbackUnderlay.addUnderlayStart(this._points, this.getStartConnectorType(), this.getConnectorTemplate(DvtDiagramLinkConnectorUtils.START_CONNECTOR), this);
    if (this._endConnector && this.getEndConnectorType()) feedbackUnderlay.addUnderlayEnd(this._points, this.getEndConnectorType(), this.getConnectorTemplate(DvtDiagramLinkConnectorUtils.END_CONNECTOR), this);
  ...
  DvtBaseDiagramLink.prototype.setLabelAlignments = function (halign, valign) {//do nothing, only for DvtDiagramNode
  };
  /**
  ...
  DvtDiagramLink.prototype.render = function () {
    ...
    if (groupId) {
      ...
      if (DvtDiagramDataUtils.compareValues(context, groupId, startGroupId) && DvtDiagramDataUtils.compareValues(context, groupId, endGroupId)) linkParent.addChildAt(this, 0);else linkParent.addChild(this); //cross container link inside container
  ...
  DvtDiagramLink._fixLinkPath = function (container) {
    ...
    if (pathElement) {
      ...
      var pathCommands = DvtDiagramLinkUtils.IsPath(points) ? points : DvtDiagramLinkUtils.ConvertToPath(points);
  ...
  DvtDiagramEventManager.prototype.HandleTouchClickInternal = function (event) {
    return this.GetEventInfo(event, ojdvtPanzoomcanvas.PanZoomCanvasEventManager.EVENT_INFO_PANNED_KEY);
  };
  /**
   * @override
   */
  ...
  DvtDiagramEventManager.prototype.StoreInfoByEventType = function (key) {
    if (key == ojdvtPanzoomcanvas.PanZoomCanvasEventManager.EVENT_INFO_PANNED_KEY) {
      return false;
    }
  ...
  var DvtBaseDiagramKeyboardHandler = function DvtBaseDiagramKeyboardHandler(component, manager) {
    this.Init(component, manager);
  };

  dvt.Obj.createSubclass(DvtBaseDiagramKeyboardHandler, ojdvtPanzoomcanvas.PanZoomCanvasKeyboardHandler, 'DvtBaseDiagramKeyboardHandler');
  /**
  ...
  DvtBaseDiagramKeyboardHandler.prototype._getLinkDirection = function (node, link) {
    if (node == this.GetVisibleNode(link.getEndId ? link.getEndId() : link.getData().getEndId())) return DvtBaseDiagramKeyboardHandler._INGOING;else return DvtBaseDiagramKeyboardHandler._OUTGOING;
  };
  /**
  ...
  DvtDiagramOverviewUtils._createOverviewNodeShape = function (diagram, node) {
    ...
    if (nodeData['overview']) ovIconData = dvt.JsonUtils.merge(nodeData['overview']['icon'], ovIconData); // determine node shape using the following rules:
    ...
    ovNode.setSolidFill(nodeData['icon']['color']); //apply styles - svg style and class names
  ...
  dvt.Obj.createSubclass(DvtDiagramOverview, ojdvtOverview.Overview);
  /**
  ...
  DvtDiagramOverview.prototype.animateUpdate = function (animationHandler, oldDiagramOverview) {
    ...
    animationHandler.add(new dvt.AnimFadeIn(this.getCtx(), this.Content, animationHandler.getAnimationDuration()), DvtDiagramDataAnimationPhase.UPDATE);
    ...
    animationHandler.add(fadeOutAnim, DvtDiagramDataAnimationPhase.UPDATE); //animate viewport
  ...
  */
  ...
  DvtDiagramDataAnimationState.prototype._addAncestorStates = function (parentId) {
    ...
    while (parentNode) {
      var keepOriginal = DvtDiagramDataUtils.compareValues(this._context, parentNode.getId(), parentId) || parentNode.isDisclosed() && this._diagram.getOptions()['renderer'];
  ...
  var Diagram = function Diagram(context, callback, callbackObj) {
    this.Init(context, callback, callbackObj);
  };

  dvt.Obj.createSubclass(Diagram, DvtBaseDiagram);
  /**
   * Initialization method called by the constructor
   *
   * @param {dvt.Context} context The rendering context.
  ...
  Diagram.prototype.Init = function (context, callback, callbackObj) {
    Diagram.superclass.Init.call(this, context, callback, callbackObj); // Create the defaults object
  ...
  Diagram.prototype.SetOptions = function (options) {
    Diagram.superclass.SetOptions.call(this, options); //initial setup
  ...
  Diagram.prototype.handleTouchStart = function () {// noop: Called from HandleImmediateTouchStartInternal
  };
  /**
