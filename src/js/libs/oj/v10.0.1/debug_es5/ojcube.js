 ...
 * @ignore
 */
define(['exports', 'ojs/ojcore-base', 'jquery', 'ojs/ojdatasource-common'], function (exports, oj, $, ojdatasourceCommon) {
  'use strict';
  ...
  var CubeAxisValue = function CubeAxisValue(value, label, level, parent) {
    this.Init();
    this._children = [];
    this._parent = parent;
  ...
  oj._registerLegacyNamespaceProp('CubeAxisValue', CubeAxisValue); // Subclass from oj.Object
  ...
  CubeAxisValue.prototype.getParents = function () {
    var parents = [];
    var parent = this._parent; // Add parents, don't add root
  ...
  CubeAxisValue.prototype.GetChild = function (key) {
    ...
    var levelAttr = childLevel.attribute; // Do we have a key value for this?
  ...
  var CubeAxis = function CubeAxis(levels, axis, cube) {
    this.Init();
    this.axis = axis;
    this._levels = [];
    ...
    this._cube = cube; // Set up a root for the tree of axis values

    this._values = new CubeAxisValue(null, null, null, null);
  };

  oj._registerLegacyNamespaceProp('CubeAxis', CubeAxis); // Subclass from oj.Object
  ...
  var CubeDataValue = function CubeDataValue(value, indices, aggType, rows, square) {
    this.Init();
    this._data = {};
    this._data.value = value;
  ...
  oj._registerLegacyNamespaceProp('CubeDataValue', CubeDataValue); // Subclass from oj.Object
  ...
  var CubeKeys = function CubeKeys() {
    // Nodes referencing the data values
    this._key = []; // List of dataValue name/data value pairs
  ...
  CubeKeys.prototype.GetHashCodes = function () {
    ...
    if (this._data.length === 0) {
      codes.push({
        key: JSON.stringify(keyHash)
      });
    } else {
      // One pairing for each data value
      for (var d = 0; d < this._data.length; d++) {
        var copy = $.extend(true, {}, keyHash); // This is done for easier lookups when finding child
  ...
  var Cube = function Cube(rowset, layout) {
    this.Init(); // Wrap the incoming data in the appropriate interface

    this._rows = rowset;
    this._layout =
    /** @type {Array.<{axis,levels}>} */
    layout;
    this.BuildCube();
  };

  oj._registerLegacyNamespaceProp('Cube', Cube); // Subclass from oj.Object
  ...
  Cube.prototype.getAxes = function () {
    ...
    var axes = [];
    Array.prototype.push.apply(axes, cube ? cube.getAxes() : this._axes); // Add on the page axes
  ...
  Cube.prototype.getValues = function (indices) {
    // Get pinned cube
    var cube = this._getPinnedCube(); // Normalize the argument to be objects with start/count properties
    ...
    var val = origRet; // Unpack single values tucked in arrays
  ...
  Cube.prototype.pivot = function (axisFrom, levelFrom, axisTo, levelTo, type) {
    ...
    var axisFromObj = this._findAxisInLayout(axisFrom); // No from axis found: can't pivot
    ...
    var levelFromObj = levelFrom < levelsFrom.length ? levelsFrom[levelFrom] : null; // No from level here, can't pivot
    ...
    if (levelTo >= levelsTo.length) {
      ...
      levelsTo.push(levelFromObj); // Remove it from the old location
  ...
  Cube.prototype._walkIndex = function (indices, depth, location, returnValue) {
    if (indices.length === 0) {
      ...
      var loc = location.slice(0); // loc.reverse();
    ...
    var count = indices[0].count; // eslint-disable-next-line no-param-reassign
  ...
  Cube.prototype._normalizeIndices = function (indices) {
    ...
    for (var a = 0; a < numAxes; a++) {
      ...
      if (index instanceof Object && (hasOwnProperty.call(index, 'start') || hasOwnProperty.call(index, 'count'))) {
  ...
  Cube.prototype.GenerateAxes = function () {
    ...
    var i; // If we have pages--only generate those for this cube, and set up the default 0th pin
  ...
  Cube.prototype.BuildCube = function () {
    this._axes = []; // Associative array for data

    this._data = []; // Associative array for cubes

    this._cubes = [];
    this.GenerateAxes(); // Can start a cube with no data--add on from "parent"
    ...
    for (var row = 0; row < this._rows.length; row++) {
      ...
      var pageHashObj = pageKey.GetHashCodes(); // See if we have a cubes for these pageKeys
      ...
      for (var ph = 0; ph < pageHashObj.length; ph++) {
        ...
        var hash = keys.GetHashCodes(); // Must account for data value coming from page, possibly...pass in the hash that has the data

        var dataHash = hash;

        if (pageHashObj[ph].dataValue !== undefined) {
          dataHash = []; // Must be in an array
  ...
  Cube.prototype._aggregate = function (hash, currValue, row) {
    ...
    switch (aggType) {
      ...
      case CubeAggType.AVERAGE:
        if (validCurr && validHash) {
          if (isNumCurr && isNumHash) {
            return this._createAggValue((currValue.sum + hash.value) / (currValue.rows.length + 1), aggType, currValue.rows, row, {
  ...
  */
  ...
  var CubeCellSet = function CubeCellSet(cube, cellRange) {
  ...
  var CubeHeaderSet = function CubeHeaderSet(axis, cube, start, count) {
    // The Cube
    this._cube = cube; // The CubeAxis
  ...
  var CubeDataGridDataSource = function CubeDataGridDataSource(cube) {
    CubeDataGridDataSource.superclass.constructor.call(this, cube);
  };

  oj._registerLegacyNamespaceProp('CubeDataGridDataSource', CubeDataGridDataSource); // Subclass CubeDataGridDataSource to DataGridDataSource
  ...
  CubeDataGridDataSource.prototype.fetchHeaders = function (headerRange, callbacks, callbackObjects) {
    var cubeheaders = new CubeHeaderSet(this._getAxis(headerRange.axis), this.data, headerRange.start, headerRange.count);
  ...
  CubeDataGridDataSource.prototype.fetchCells = function (cellRange, callbacks, callbackObjects) {
    ...
    for (var i = 0; i < cellRange.length; i++) {
      ...
      if (cellRange[i].axis === 'row') {
        count = cellRange[i].count === undefined ? this.data.getAxes()[1].getExtent() : cellRange[i].count;
        obj.row = {
          start: start,
          count: count
        };
      ...
      if (cellRange[i].axis === 'column') {
        count = cellRange[i].count === undefined ? this.data.getAxes()[0].getExtent() : cellRange[i].count;
        obj.column = {
          start: start,
          count: count
        };
  ...
  CubeDataGridDataSource.prototype._getKey = function (indexes, axis, retObj) {
    ...
    var keys = new CubeKeys();
    keys = axisObj ? axisObj.GetCubeKeys(item, keys) : ''; // eslint-disable-next-line no-param-reassign
  ...
  CubeDataGridDataSource.prototype.move = function ( // eslint-disable-next-line no-unused-vars
  ...
  var CubeLevel = function CubeLevel(attribute, axis, dataValue) {
    this.Init();
  ...
  oj._registerLegacyNamespaceProp('CubeLevel', CubeLevel); // Subclass from oj.Object
  ...
  var DataColumnCube = function DataColumnCube(rowset, layout, dataValues) {
    this.Init();
    ...
    this._defaultAggregation = defAgg ? DataColumnCube._getDefaultAgg(defAgg) : {
  ...
  DataColumnCube.prototype.ProcessLevel = function (axis, levelNum, currNode, row, keys, addKeys) {
    ...
    var level = axis.getLevels()[levelNum]; // Not the data value: process this and call for the next
  ...
  var DataValueAttributeCube = function DataValueAttributeCube(rowset, layout, dataValues) {
    this.Init();
    this._dataValues = dataValues;
  ...
  DataValueAttributeCube.prototype.ProcessLevel = function (axis, levelNum, currNode, row, keys, addKeys) {
    if (levelNum >= axis.getLevels().length) {
      return keys;
    }
    ...
    var value = row[level.attribute];
    var node = currNode.AddNode(value, null, level); // Gives us control to avoid adding duplicate key sets below each measure coming from one detail row
  ...
  DataValueAttributeCube.prototype._processDataValue = function (axis, node, row, levelNum, keys) {
    ...
    for (var d = 0; d < this._dataValues.length; d++) {
      ...
      var dvLabel = this._dataValues[d].label; // Only add a node if this row has an entry for the dataValue

      if (Object.prototype.hasOwnProperty.call(row, dvAttr)) {
        var currNode = node.AddNode(dvAttr, dvLabel, axis.getLevels()[levelNum]);
        keys.AddDataValue(dvAttr, row[dvAttr]); // Process the rest of the level.  Only have new nodes added to data storage keys for the first data value we process--don't want to duplicate them

        this.ProcessLevel(axis, levelNum + 1, currNode, row, keys, dsKeysAdded); // Shut off the key adding for future passes
