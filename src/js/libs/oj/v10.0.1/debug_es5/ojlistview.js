...
define(['require', 'ojs/ojcore-base', 'jquery', 'ojs/ojcustomelement', 'ojs/ojdataproviderscroller', 'ojs/ojcontext', 'ojs/ojconfig', 'ojs/ojthemeutils', 'ojs/ojcomponentcore', 'ojs/ojdatacollection-common', 'ojs/ojanimation', 'ojs/ojlogger', 'ojs/ojkeyset', 'ojs/ojmap', 'ojs/ojdomutils', 'ojs/ojdataprovideradapter', 'ojs/ojcustomelement-utils', 'ojs/ojindexer'], function (require, oj, $, ojcustomelement, DataProviderScroller, Context, Config, ThemeUtils, Components, DataCollectionUtils, AnimationUtils, Logger, ojkeyset, KeyMap, DomUtils, ojdataprovideradapter, ojcustomelementUtils, ojindexer) {
  ...
  var StaticContentHandler = function StaticContentHandler(widget, root) {
  ...
  StaticContentHandler.prototype.notifyShown = function () {// do nothing since all items are present
  };

  StaticContentHandler.prototype.notifyAttached = function () {// do nothing since all items are present
  };
  ...
  StaticContentHandler.prototype.Collapse = function (item) {// nothing to do
  };
  ...
  var TreeDataProviderContentHandler = function TreeDataProviderContentHandler(widget, root, data) {
  ...
  oj.Object.createSubclass(TreeDataProviderContentHandler, DataProviderScroller.DataProviderContentHandler, 'oj.TreeDataProviderContentHandler'); // number of skeleton items to render for expand node
  ...
  TreeDataProviderContentHandler.prototype._fetchChildren = function (parent, parentElem, successCallback, errorCallback) {
    var self = this;
  ...
  TreeDataProviderContentHandler.prototype._handleFetchSuccess = function (dataObj, parent, parentElem, successCallback, templateEngine) {
  ...
  TreeDataProviderContentHandler.prototype.getMetadata = function (index, key, data, parentElem) {
    var context = TreeDataProviderContentHandler.superclass.getMetadata.call(this, index, key, data, parentElem);
  ...
  TreeDataProviderContentHandler.prototype.GetBindingContext = function (context) {
    var bindingContext = TreeDataProviderContentHandler.superclass.GetBindingContext.call(this, context);
  ...
  TreeDataProviderContentHandler.prototype.afterRenderItem = function (item, context, isCustomizeItem) {
    this.signalTaskStart('after rendering an item'); // signal method task start

    TreeDataProviderContentHandler.superclass.afterRenderItem.call(this, item, context, isCustomizeItem);
    var groupStyleClass = this.m_widget.getGroupStyleClass();
    var itemStyleClass = this.m_widget.getItemStyleClass();
    var groupItemStyleClass = this.m_widget.getGroupItemStyleClass(true);
    var groupCollapseStyleClass = this.m_widget.getGroupCollapseStyleClass();
  ...
  TreeDataProviderContentHandler.prototype._handleFetchError = function (status) {
  ...
  TreeDataProviderContentHandler.prototype.addItemsForModelInsert = function (data, indexes, keys, parentKeys, isBeforeKeys, refKeys, metadata) {
    // template engine should have already been loaded
    var templateEngine = this.getTemplateEngine();
  ...
  TreeDataProviderContentHandler.prototype.afterRenderItemForInsertEvent = function (item, context, isCustomizeItem) {
    if (context.leaf === false) {
      // if it's a group node, we just need to call afterRenderItem
      this.afterRenderItem(item, context, isCustomizeItem);
    } else {
      TreeDataProviderContentHandler.superclass.afterRenderItemForInsertEvent.call(this, item, context, isCustomizeItem);
    }
  };
  /**
   * @protected
  ...
  TreeDataProviderContentHandler.prototype.handleModelRefreshEvent = function (event) {
    ...
    this.m_widget.ClearCache(true); // clear cached child DataProviders
  ...
  TreeDataProviderContentHandler.prototype.renderInitialSkeletons = function () {
    ...
    if (skeletonDimension.width > 0 && skeletonDimension.height > 0) {
      var skeletonSetHeight = skeletonDimension.height * (1 + TreeDataProviderContentHandler.NUM_CHILD_SKELETONS); // one parent + 3 children
  ...
  TreeDataProviderContentHandler.prototype._destroyExpandSkeletons = function (elem) {
    $(elem).empty();
  };
  ...
  var _ojListViewExpandedKeySet = function _ojListViewExpandedKeySet() {
  ...
  var _ojListView = _ListViewUtils.clazz(Object,
  /** @lends oj._ojListView.prototype */
  {
    ...
    _applyExitAnimation: function _applyExitAnimation(cards) {
      ...
      cards.forEach(function (card) {
        AnimationUtils.slideOut(card, {
          offsetY: '300px',
          duration: '300ms',
          persist: 'all'
        });
        promise = AnimationUtils.fadeOut(card, {
          duration: '300ms',
          persist: 'all'
        });
     ...
     */
    _isScrollToKey: function _isScrollToKey() {
     ...
     */
    adjustScrollPositionValueOnFetch: function adjustScrollPositionValueOnFetch() {
    ...
    CreateDataContentHandler: function CreateDataContentHandler(data) {
      ...
      if (typeof oj.TableDataSource !== 'undefined' && data instanceof oj.TableDataSource) {
        ...
        contentHandler = new DataProviderScroller.IteratingDataProviderContentHandler(this, this.element[0], new oj.TableDataSourceAdapter(data));
        ...
        return adapterPromise.then(function (TreeDataSourceAdapter) {
          return new TreeDataProviderContentHandler(_this, _this.element[0], // eslint-disable-next-line new-cap
        ...
        contentHandler = new DataProviderScroller.IteratingDataProviderContentHandler(this, this.element[0], data);
      } else {
    ...
    _showLoadingIcon: function _showLoadingIcon() {
      ...
      if (containerHeight < statusHeight && minHeight < statusHeight) {
        container.css('minHeight', Math.max(containerHeight, statusHeight + this.getListContainerBorderWidth())); // save it to restore later
    ...
    _addNoData: function _addNoData() {
      if (this.ojContext._IsCustomElement()) {
        ...
        if (slot && slot.length > 0 && slot[0].tagName.toLowerCase() === 'template') {
          ...
          noDataContentRoot.setAttribute('role', 'presentation');
    ...
    renderComplete: function renderComplete() {
      ...
      this.m_items = null;
      this.m_groupItems = null; // clear cached scrollHeight

      this.m_scrollHeight = null; // if grid role and card layout and non-heirarchical and presentation div is empty, remove presentation div to clear out the element
        ...
        if (selection.length > 0) {
          this._validateSelection(selection).then(function (validRowKeyData) {
            ...
            if (validRowKeyData.validKeys.length === 0) {
              ...
              self._setSelectionOption(ojkeyset.KeySetUtils.toKeySet(validRowKeyData.validKeys), null, null, validRowKeyData.validData[0].data);
      ...
      var _animationPromise = null; // apply card entrance animation

      if (this._isAnimateCards()) {
        var isInitial = this.element[0].querySelector('.oj-listview-card-animated') == null;
        var delay = 0;

        var increment = this._getCardEntranceAnimationDelay(); // so that scrollbar would not be shown when slide in
        ...
        this.element[0].querySelectorAll('.oj-listview-card:not(.oj-listview-card-animated)').forEach(function (card) {
          ...
          if (isInitial) {
            AnimationUtils.slideIn(card, {
              offsetY: '300px',
              delay: delay + 'ms',
              duration: '300ms'
            });
            _animationPromise = AnimationUtils.fadeIn(card, {
              delay: delay + 'ms',
              duration: '300ms'
            });
     ...
     */
    getScrollToKey: function getScrollToKey() {
     ...
     * @private
     */
    _validateKeyForScroll: function _validateKeyForScroll(key, skipLocal) {
      var self = this; // scrollToKey set to never or DataProvider cannot have the right capability
     ...
     */
    restoreFocusAfterDrag: function restoreFocusAfterDrag() {
      if (this.m_active == null) {
    ...
    */
     ...
     * @private
     */
    _isShowHover: function _isShowHover() {
    ...
    HandleMouseClick: function HandleMouseClick(event) {
      ...
      if (event.button === 0) {
        ...
        if (target.hasClass(expandIconClass)) {
          ...
          // 1) item cannot be focus
          // 2) target is an oj-component
          // 3) target or one of its ancestors has the oj-clickthrough-disabled marker class
     ...
     * @private
     */
    _handleHeaderClick: function _handleHeaderClick(event, target, item) {
      var groupItemClass = this.getGroupItemStyleClass(); // clicking on header will expand/collapse item
    ...
    _isClickthroughDisabled: function _isClickthroughDisabled(elem) {
      return elem.length > 0 && DataCollectionUtils.isClickthroughDisabled(elem.get(0)) || elem.hasClass('oj-clickthrough-disabled');
    },
    ...
    _isGridlinesVisible: function _isGridlinesVisible() {
      if (this.m_gridlinesVisible == null) {
        var gridlines = this.GetOption('gridlines'); // would be null for NavList
    ...
    _getItemsCache: function _getItemsCache() {
      if (this.m_items == null) {
        ...
        this.m_items = this.element.find(selector).filter(function () {
          ...
          if (this.tagName !== 'LI') {
            var parentElement = this.parentElement; // skip items that are about to be remove
    ...
    _setSelectionOption: function _setSelectionOption(newValue, event, selectedElems, firstSelectedItemData) {
      ...
      var selection = ojkeyset.KeySetUtils.toArray(newValue); // check if the value has actually changed, based on key
    ...
    _isExpandAll: function _isExpandAll(key) {
      ...
      if (expanded.isAddAll) {
        // if drillMode is none and no expanded state is specified, expand all
        return !this.isExpandable() && expanded instanceof _ojListViewExpandedKeySet ? true : expanded.isAddAll() && expanded.has(key);
      }
      ...
      if (!isNaN(itemHeight) && itemHeight > 0) {
        ...
        if (result != null) {
          ...
          if (isHierData) {
            ...
            scrollPosition.index = $(parent).children().index(elem); // handle sticky header here too
    ...
    _registerScrollHandler: function _registerScrollHandler() {
      ...
      scrollElem[0].addEventListener('scroll', this._scrollListener); // only do this for high-water mark scrolling, other cases we have (and should not care) no knowledge about the scroller
     ...
     * @private
     */
    mergeScrollListener: function mergeScrollListener() {
      if (this._scrollListener) {
        var scrollElem = this._getScrollEventElement();
     ...
     * @private
     */
    unmergeScrollListener: function unmergeScrollListener() {
      if (this._scrollListener) {
        var scrollElem = this._getScrollEventElement();
   ...
   *    events or overriding action specific style classes on the animated item.  See the documentation of <a href="AnimationUtils.html">oj.AnimationUtils</a>
   *    class for details.</p>
   *
   ...
   *           will not select or set the item as current when user clicks on the button.</p>
   *       </td>
   *       <td>
   *         <pre class="prettyprint"><code>&lt;oj-list-view id="listView">
