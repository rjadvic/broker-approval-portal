...
define(['exports', 'ojs/ojdvt-toolkit'], function (exports, dvt) {
  ...
  var DvtPictoChartImageMarker = function DvtPictoChartImageMarker(picto, cx, cy, width, height, source, sourceSelected, sourceHover, sourceHoverSelected, id) {
  ...
  DvtPictoChartImageMarker.prototype.animateUpdate = function (handler, oldMarker) {
    ...
    var animator = animation.getAnimator(); // Position and size animation

    var endParams = this._getAnimationParams();

    this._setAnimationParams(oldMarker._getAnimationParams());

    animator.addProp(dvt.Animator.TYPE_NUMBER_ARRAY, this, this._getAnimationParams, this._setAnimationParams, endParams); // Hide old item
  ...
  var DvtPictoChartShapeMarker = function DvtPictoChartShapeMarker(picto, shape, cx, cy, width, height, id) {
  ...
  DvtPictoChartShapeMarker.prototype.animateUpdate = function (handler, oldMarker) {
    var animation = new dvt.CustomAnimation(this.getCtx(), this, this._picto.getAnimationDuration() * 0.75);
    var animator = animation.getAnimator(); // Color animation
  ...
  */
  ...
  DvtPictoChartShapeMarker.prototype.setDataColor = function (dataColor, bSkipStroke) {
    ...
    if (isRedwood) {
      ...
      if (!bSkipStroke) {
        ...
        this.setHoverStroke(new dvt.Stroke(innerColor, 1, 1.5, true), new dvt.Stroke(hoverColor, 1, 4.5, true));
        this.setSelectedStroke(new dvt.Stroke(innerColor, 1, 1.5, true), new dvt.Stroke(outerColor, 1, 4.5, true));
        this.setSelectedHoverStroke(new dvt.Stroke(innerColor, 1, 1.5, true), new dvt.Stroke(outerColor, 1, 4.5, true));
      }
    } else {
      DvtPictoChartShapeMarker.superclass.setDataColor.call(this, dataColor, bSkipStroke);
    }
  };
  /**
   * @license
  ...
  var DvtPictoChartUtils = {
    /**
     * Returns whether the pictoChart is vertical.
     * @param {PictoChart} picto
     * @return {boolean}
     */
    isVertical: function isVertical(picto) {
     ...
     * @return {boolean}
     */
    isOriginBottom: function isOriginBottom(picto) {
     ...
     * @return {boolean}
     */
    isOriginRight: function isOriginRight(picto) {
      var origin = picto.getOptions()['layoutOrigin'];
  ...
  var DvtPictoChartKeyboardHandler = function DvtPictoChartKeyboardHandler(eventManager) {
  ...
  DvtPictoChartKeyboardHandler.getNextNavigable = function (picto, currentNavigable, event, originalNavigable) {
    ...
    if (!originalNavigable) originalNavigable = currentNavigable; // Handle edge cases where shape==none is the first or last item
    ...
    var isForward = event.keyCode == dvt.KeyboardEvent.LEFT_ARROW && isOriginRight || event.keyCode == dvt.KeyboardEvent.RIGHT_ARROW && !isOriginRight || event.keyCode == dvt.KeyboardEvent.UP_ARROW && isOriginBottom || event.keyCode == dvt.KeyboardEvent.DOWN_ARROW && !isOriginBottom;
    var isDirectional = event.keyCode == dvt.KeyboardEvent.LEFT_ARROW && isVertical || event.keyCode == dvt.KeyboardEvent.RIGHT_ARROW && isVertical || event.keyCode == dvt.KeyboardEvent.UP_ARROW && !isVertical || event.keyCode == dvt.KeyboardEvent.DOWN_ARROW && !isVertical;
    var nextIdx = navigableItems.indexOf(currentNavigable) + (isForward ? 1 : -1);
    if (isDirectional) nextItem = dvt.KeyboardHandler.getNextNavigable(currentNavigable, event, navigableItems);else if (nextIdx < navigableItems.length && nextIdx >= 0) nextItem = navigableItems[nextIdx];

    if (nextItem.getShape() == 'none') {
      // Recurse till next item in that direction that is not shape==none is found
      if (nextItem != currentNavigable) nextItem = DvtPictoChartKeyboardHandler.getNextNavigable(picto, nextItem, event, originalNavigable);else // Handle edge case where shape==none is on the border
  ...
  DvtPictoChartKeyboardHandler.prototype.processKeyDown = function (event) {
    ...
    } else return DvtPictoChartKeyboardHandler.superclass.processKeyDown.call(this, event);
  };
  /**
   * @license
  ...
  var DvtPictoChartItem = function DvtPictoChartItem(picto, item) {
  ...
  DvtPictoChartItem.prototype.Init = function (picto, item) {
    ...
    this._keyboardTooltipLocation = new dvt.Point(0, 0); // Apply the selecting cursor if selectable or drillable

    if (this.isSelectable() || this.isDrillable()) this.setCursor(dvt.SelectionEffectUtils.getSelectingCursor()); // Associate using the event manager
  ...
  DvtPictoChartItem.prototype.isDrillable = function () {
    ...
    if (drilling && drilling != 'inherit') return drilling == 'on';else return this._picto.getOptions()['drilling'] == 'on';
  };
  /**
  ...
  DvtPictoChartItem.prototype.getDatatip = function (target) {
    if (this._isNoneShape) return ''; // Custom Tooltip via Function
    ...
    if (this.getShortDesc() != null) return this.getShortDesc(); // Default Tooltip Support

    var tds = [];
    var name = this.getName();
    if (name) tds.push(dvt.HtmlTooltipManager.createElement('td', options['_tooltipLabelStyle'], name, ['oj-dvt-datatip-label']));
  ...
  DvtPictoChartItem.prototype.setSelected = function (bSelected) {
    ...
    for (var i = 0; i < this.getNumChildren(); i++) {
      ...
      if (child instanceof DvtPictoChartShapeMarker || child instanceof DvtPictoChartImageMarker) child.setSelected(bSelected);
    }
  };
  /**
   * @override
  ...
  DvtPictoChartItem.prototype.showHoverEffect = function () {
    for (var i = 0; i < this.getNumChildren(); i++) {
      var child = this.getChildAt(i);
      if (child instanceof DvtPictoChartShapeMarker || child instanceof DvtPictoChartImageMarker) child.showHoverEffect();
    }
  };
  /**
   * @override
  ...
  DvtPictoChartItem.prototype.hideHoverEffect = function () {
    for (var i = 0; i < this.getNumChildren(); i++) {
      var child = this.getChildAt(i);
      if (child instanceof DvtPictoChartShapeMarker || child instanceof DvtPictoChartImageMarker) child.hideHoverEffect();
    }
  }; //---------------------------------------------------------------------//
  ...
  DvtPictoChartItem.prototype.getAriaLabel = function () {
    ...
    var shortDesc;
    var name = this.getName();
    if (this.getShortDesc() != null) shortDesc = this.getShortDesc();else if (name == null) shortDesc = this._getCountString();else shortDesc = dvt.ResourceUtils.format(translations.labelAndValue, [name, this._getCountString()]);
  ...
  DvtPictoChartItem.prototype.getCategories = function (category) {
    if (this._item['_itemData']) return this._item['categories'];else return this._item['categories'] || [this.getId()];
  }; //---------------------------------------------------------------------//
  ...
  DvtPictoChartItem.prototype.getNextNavigable = function (event) {
    ...
    if (event.type == dvt.MouseEvent.CLICK || keyboardHandler.isMultiSelectEvent(event)) return this;else if (keyboardHandler.isNavigationEvent(event)) return DvtPictoChartKeyboardHandler.getNextNavigable(this._picto, this, event);else return null;
  };
  /**
   * @override
   */
  ...
  var DvtPictoChartAutomation = function DvtPictoChartAutomation(picto) {
    this._picto = picto;
  };
  ...
  DvtPictoChartAutomation.prototype.getDomElementForSubId = function (subId) {
    if (subId == dvt.Automation.TOOLTIP_SUBID) return this.GetTooltipElement(this._picto);
    ...
    if (parenIdx > 0 && subId.substring(0, parenIdx) === 'item') {
      ...
      if (item) return item.getElem();else return null;
    }
  ...
  var DvtPictoChartDefaults = function DvtPictoChartDefaults(context) {
    this.Init({
      'alta': DvtPictoChartDefaults.VERSION_1
    }, context);
  };
  ...
  var DvtPictoChartEventManager = function DvtPictoChartEventManager(picto) {
  ...
  DvtPictoChartEventManager.prototype.ProcessRolloverEvent = function (event, obj, bOver) {
    ...
    if (options['hoverBehavior'] == 'none') return; // Compute the new highlighted categories and update the options
  ...
  DvtPictoChartEventManager.prototype.OnClickInternal = function (event) {
    ...
    if (!obj) return; // Only drill if not selectable. If selectable, drill with double click.
  ...
  DvtPictoChartEventManager.prototype.HandleTouchHoverEndInternal = function (event) {
    var obj = this.GetLogicalObject(event.target);
    if (!obj) return; // Only drill if not selectable. If selectable, drill using double click.
  ...
  DvtPictoChartEventManager.prototype.processDrillEvent = function (obj) {
    if (obj instanceof DvtPictoChartItem && obj.isDrillable()) this.FireEvent(dvt.EventFactory.newDrillEvent(obj.getId()));
  };
  /**
   * @license
   * Copyright (c) 2014, 2021, Oracle and/or its affiliates.
  ...
  DvtPictoChartRenderer.render = function (picto, container, availSpace, info) {
    ...
    var isOriginBottom = DvtPictoChartUtils.isOriginBottom(picto);
    var isOriginRight = DvtPictoChartUtils.isOriginRight(picto); // Use a 2D cell map to keep track of which cells are occupied
    ...
    for (var i = 0; i < info.items.length; i++) {
      ...
      while (count > 0) {
        ...
        if (colSpan != prevColSpan || rowSpan != prevRowSpan) remainder = 0; // If there's no remainder, use a new cell. Otherwise, render on the previous cell

        if (remainder == 0) cell = DvtPictoChartRenderer._findNextAvailableCell(cellMap, colSpan, rowSpan, info.colCount, info.rowCount, isVert);
        if (cell == null) break; // Marker coords
        ...
        var y = availSpace.y + (isOriginBottom ? availSpace.h - yOffset : yOffset); // Draw fractional marker if remaining count is less than one, or if there's a remainder from the previous item

        var fraction = Math.min(1 - remainder, count); // Coords for hit area and clip path
        ...
        if (item.getSource()) {
          // Custom image marker
          marker = new DvtPictoChartImageMarker(picto, x, y, w, h, item.getSource(), item.getSourceSelected(), item.getSourceHover(), item.getSourceHoverSelected(), markerId + '_I');
        } else {
          ...
          item.addChild(hitArea); // Create shape marker

          marker = new DvtPictoChartShapeMarker(picto, item.getShape(), x, y, w - info.colWidth * gapRatio, h - info.rowHeight * gapRatio, markerId + '_S');
  ...
  DvtPictoChartRenderer.getInfo = function (picto, width, height) {
    ...
    if (!itemObjs) return {}; // Create a boolean map of hidden categories for better performance

    var categoryMap = dvt.ArrayUtils.createBooleanMap(options['hiddenCategories']); // Create items and compute the number of cells that are needed to fit all items
    ...
    for (var i = 0; i < itemObjs.length; i++) {
      ...
      var colSpan = item.getColSpan();
      var rowSpan = item.getRowSpan();
      if (colSpan <= 0 || rowSpan <= 0) continue; // Compute the maximum colSpan and rowSpan

      if (colSpan > maxColSpan) maxColSpan = colSpan;
      if (rowSpan > maxRowSpan) maxRowSpan = rowSpan; // Compute minimum span for gap computation
    ...
    if (numCells == 0) return {}; // Default colWidth and rowHeight for flowing layout
    ...
    if (!colCount && !rowCount) {
      ...
      if (width && height) {
        if (isVert) rowCount = DvtPictoChartRenderer._ceil(Math.sqrt(numCells * height / width), maxRowSpan);else colCount = DvtPictoChartRenderer._ceil(Math.sqrt(numCells * width / height), maxColSpan);
      } else if (width) colCount = Math.max(Math.floor(width / colWidth), 1);else if (height) rowCount = Math.max(Math.floor(height / rowHeight), 1);else {
        if (isVert) rowCount = DvtPictoChartRenderer._ceil(Math.sqrt(numCells), maxRowSpan);else colCount = DvtPictoChartRenderer._ceil(Math.sqrt(numCells), maxColSpan);
      }
    } // Now only either colCount or rowCount is undefined


    if (!colCount) colCount = DvtPictoChartRenderer._ceil(numCells / rowCount, maxColSpan);else if (!rowCount) rowCount = DvtPictoChartRenderer._ceil(numCells / colCount, maxRowSpan); // Default colWidth and rowHeight for fixed layout

    if (width && height) {
      if (!colWidth) colWidth = rowHeight ? rowHeight : Math.min(width / colCount, height / rowCount);
      if (!rowHeight) rowHeight = colWidth;
    }
  ...
  DvtPictoChartRenderer.renderEmptyText = function (picto, container, availSpace) {
    ...
    var emptyText = picto.renderEmptyText(container, emptyTextStr, availSpace.clone(), picto.getEventManager(), options['_statusMessageStyle']);
  ...
  var PictoChart = function PictoChart(context, callback, callbackObj) {
    this.Init(context, callback, callbackObj);
  };

  dvt.Obj.createSubclass(PictoChart, dvt.BaseComponent);
  /**
   * Returns a new instance of PictoChart.
   * @param {dvt.Context} context The rendering context.
   * @param {string} callback The function that should be called to dispatch component events.
   * @param {object} callbackObj The optional object instance on which the callback function is defined.
   * @return {PictoChart}
  ...
  PictoChart.newInstance = function (context, callback, callbackObj) {
  ...
  PictoChart.prototype.Init = function (context, callback, callbackObj) {
    PictoChart.superclass.Init.call(this, context, callback, callbackObj); // Create the event handler and add event listeners
    ...
    if (!dvt.Agent.isTouchDevice()) this.EventManager.setKeyboardHandler(new DvtPictoChartKeyboardHandler(this.EventManager)); // Create the defaults object

    this.Defaults = new DvtPictoChartDefaults(context); // Create array of logical objects for pictoChart items

    this._items = []; // PictoChart sets the width and height of its svg so no need for fix for 
  ...
  PictoChart.prototype._getPreferredSize = function (width, height) {
  ...
  PictoChart.prototype.render = function (options, width, height) {
    ...
    var oldHeight = this.Height ? this.Height : 0; // Clean up
    ...
    this.StopAnimation(); // Update if a new options object has been provided or initialize with defaults if needed
    ...
    if (!width && !height) {
      ...
      context.getSvgDocument().style.display = 'block'; // Determine whether the layout is fixed or flowing.
    ...
    DvtPictoChartRenderer.render(this, this._container, new dvt.Rectangle(0, 0, this.Width, this.Height), this._info); // Construct the new animation playable
    ...
    if (this.Animation) {
      ...
      dvt.ToolkitUtils.setSvgSize(context, Math.max(oldWidth, this.Width), Math.max(oldHeight, this.Height)); // Remove event listeners and empty text temporarily
  ...
  PictoChart.prototype.SetOptions = function (options) {
    ...
    if (selectionMode == 'single') this._selectionHandler = new dvt.SelectionHandler(this.getCtx(), dvt.SelectionHandler.TYPE_SINGLE);else if (selectionMode == 'multiple') this._selectionHandler = new dvt.SelectionHandler(this.getCtx(), dvt.SelectionHandler.TYPE_MULTIPLE);else this._selectionHandler = null; // Pass to event manager
  ...
  PictoChart.prototype._onRenderEnd = function () {
    ...
    dvt.ToolkitUtils.setSvgSize(this.getCtx(), this.Width, this.Height); // Set the initial keyboard focus
    ...
    this.EventManager.setFocusObj(initialFocus); // Set initial selection

    if (this._selectionHandler) this._selectionHandler.processInitialSelections(this.Options['selection'], this._items); // Set initial highlighting
  ...
  PictoChart.prototype.registerItems = function (items) {
    this._items = items;
  };
  /**
  ...
  PictoChart.prototype.getItems = function () {
    return this._items;
  };
  /**
  ...
  PictoChart.prototype.registerMarker = function (marker) {
  ...
  PictoChart.prototype.registerEmptyText = function (text) {
  ...
  PictoChart.prototype.getTotalCount = function () {
    var count = 0;
  ...
  PictoChart.prototype.getAnimationDuration = function () {
  ...
  PictoChart.prototype._getAnimationOnDisplay = function () {
    ...
    if (dvt.BlackBoxAnimationHandler.isSupported(animOnDisplay)) return dvt.BlackBoxAnimationHandler.getInAnimation(context, animOnDisplay, this._container, bounds, duration);
    var playables = [];
    ...
    if (playables.length > 0) return new dvt.ParallelPlayable(context, playables);else return null;
  };
  /**
   * @override
   */
  ...
  PictoChart.prototype.highlight = function (categories) {
    // Update the options
    var options = this.getOptions();
    options['highlightedCategories'] = dvt.JsonUtils.clone(categories); // Perform the highlighting and propagate to children
  ...
  PictoChart.prototype.select = function (selection) {
    // Update the options
    var options = this.getOptions();
    options['selection'] = dvt.JsonUtils.clone(selection); // Perform the selection

    if (this._selectionHandler) this._selectionHandler.processInitialSelections(selection, this.getItems());
  };
  /**
  ...
  PictoChart.prototype.getAutomation = function () {
