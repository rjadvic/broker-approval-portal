 ...
 * @ignore
 */
define(['ojs/ojcore', 'ojs/ojeditablevalue', 'jqueryui-amd/widgets/draggable', 'ojs/ojtouchproxy', 'ojs/ojcore-base', 'jquery', 'ojs/ojtranslation', 'ojs/ojdomutils'], function (ojcore, ojeditablevalue, draggable, ojtouchproxy, oj, $, Translations, DomUtils) {
  'use strict';
  ...
  (function () {
    ...
    var __oj_slider_metadata = {
      "properties": {
        ...
        "type": {
          "type": "string",
          "enumValues": ["fromMax", "fromMin", "single"],
          "value": "fromMin"
        },
        "userAssistanceDensity": {
          "type": "string",
    ...
    * @memberof! oj.ojSlider
    ...
    oj.__registerWidget('oj.ojSlider', $.oj.editableValue, {
      ...
      _ComponentCreate: function _ComponentCreate() {
        this._super(); // ojSlider could support both <div> and <input> tags


        this._inputtag = false;

        if (this.element.is('INPUT')) {
          this._inputtag = true; // Save the input tag style, apply to sliderWrapper later in the code.
      ...
      _componentCreateStyling: function _componentCreateStyling() {
      ...
      // styling.
      _setOrientationStyles: function _setOrientationStyles(init) {
        var elementWrapped = this._elementWrapped[0];
      ...
      _copyLabelledbyToThumb: function _copyLabelledbyToThumb(labelId) {
        ...
        if (this._isCustomRangeSlider()) {
          ...
          var higherValueThumbString = Translations.getTranslatedString('oj-ojSlider.higherValueThumb');
      ...
      _setAriaInfo: function _setAriaInfo() {
        var ariaLabelString;
        var label;
        var ariaLabelledBy;
        var thumb; // for oj-slider, if labelled-by attribute is set, use that to
        ...
        if (this.OuterWrapper) {
          if (this.options.labelledBy) {
            ...
            ariaLabelledBy = ojeditablevalue.EditableValueUtils._getOjLabelAriaLabelledBy(this.options.labelledBy, defaultLabelId);
          ...
          label = this._GetLabelElementLocal(); // Copy any labelled-by on the <input labelled-by="id"> to the slider thumb.
          //
          ...
          if (label) {
            ...
            if (ariaLabelString) {
              thumb = this._elementWrapped.find('.oj-slider-thumb'); // Set the aria-labelledby attribute of the thumb to the returned id
       ...
       * @private
       */
      _GetLabelElementLocal: function _GetLabelElementLocal() {
       ...
       * @private
       */
      _getAriaLabelForElementLocal: function _getAriaLabelForElementLocal() {
        var id;
       ...
       * @private
       */
      _getAriaLabelledByElementLocal: function _getAriaLabelledByElementLocal() {
        ...
        // aria-labelledby.
        var ariaId = this.element.attr('aria-labelledby'); // Handle direct labelling case for custom elements
      ...
      _getElementId: function _getElementId() {
        if (this.OuterWrapper) {
      ...
      //
      _getThumbId: function _getThumbId(index) {
      ...
      //
      _getBarValueId: function _getBarValueId() {
        var elementId = this._getElementId();
      ...
      //
      _getBarBackgroundId: function _getBarBackgroundId() {
        var elementId = this._getElementId();
      ...
      // Return the id of the slider bar.
      //
      _getSliderWrapperId: function _getSliderWrapperId() {
        var elementId = this._getElementId();

        var sliderWrapperId = elementId + '-sliderWrapper';
        return sliderWrapperId;
      },
      _createThumbs: function _createThumbs() {
        var i;
        var thumbCount;
        ...
        var thumbClasses = "class='oj-slider-thumb ui-state-default' tabindex='0' role='slider'" + ariaMin + ariaMax + '></span>';
        var thumbs = [];
      ...
      //
      _refreshThumbOptions: function _refreshThumbOptions() {
        var that = this;
        var i = 0;
        ...
        this._thumbs.each(function () {
          ...
          if (that.options.readOnly) {
            $(this).attr('title', 'read only'); // $(this).removeAttr("tabindex")
          } else {
      ...
      //
      _createSliderContainer: function _createSliderContainer() {
      ...
      _createBarBackground: function _createBarBackground() {
        ...
        this._barback.addClass(classes); // Place the background bar element immediately after the hidden input tab.
        ...
        if (this._isCustomRangeSlider()) {
          ...
          if (this.options.value == null) {
            ...
            if (this.options.value.length !== 2) {
              ...
              //
              var firstValue;
              if (this.options.value.length > 0) firstValue = this.options.value[0];else firstValue = this._valueMin();
      ...
      _createRange: function _createRange() {
        ...
        if (options.type) {
          ...
          this._range.on('mousedown' + that.eventNamespace, function (event) {
            ...
            var thumb = that._getActiveThumb(); // if we add oj-active here it "sticks"
          ...
          var newClass = '';
          if (options.type === 'fromMin') newClass = ' oj-slider-range-min';else if (options.type === 'fromMax') newClass = ' oj-slider-range-max';
      ...
      _setupTouch: function _setupTouch(e) {
        this._touchProxy = oj._TouchProxy.addTouchListeners(e);
      },
      _tearDownTouch: function _tearDownTouch(e) {
      ...
      _setupEvents: function _setupEvents() {
        ...
        this._thumbs.toArray().forEach(function (current) {
          var thumb = $(current); // setup keyboard events on each thumb.

          this._UnregisterChildNode(thumb);

          this._on(thumb, this._thumbEvents); // setup touch events on each thumb


          this._setupTouch(thumb); // Each thumb can be focusable.
      ...
      //
      _destroySliderDom: function _destroySliderDom() {
      ...
      //
      _repositionThumb: function _repositionThumb(event) {
        var position;
        var normValue;
        var distance;
        var o = this.options;
        ...
        if (this._multipleThumbs) {
          this._thumbs.each(function (i) {
            ...
            if (distance > thisDistance || distance === thisDistance && (i === that._lastChangedValueIndex || that._getMultiValues(i) === o.min)) {
        ...
        if (!this._closestThumb) return true; // This call is needed to support 'click-to-reposition' the thumb
        ...
        thumb.addClass('oj-active').focus(); // For mobile theming, we need to change the color of the value bar when active.
      ...
      //
      _initDragging: function _initDragging(event, thumb) {
        var o = this.options;
        if (o.disabled) return false;
        if (o.readOnly) return false; // tabbing could have added oj-focus-highlight to the thumb,
      ...
      _mouseDragInternal: function _mouseDragInternal(event, thumb) {
      ...
      _mouseStop: function _mouseStop(event, thumb) {
        ...
        this._slide(event, this._thumbIndex, normValue); // _change is needed for click positioning
      ...
      //
      _getOrientationAdjustedFrac: function _getOrientationAdjustedFrac(frac) {
        var fracReturn = frac;
      ...
      //
      _getNormValueFromMouse: function _getNormValueFromMouse(position) {
        var valueTotal;
        var valueMouse;
      ...
      //
      _getFracFromMouse: function _getFracFromMouse(position) {
        var pixelTotal;
        var pixelMouse;
        var fracMouse;
      ...
      //
      _getFracFromThumb: function _getFracFromThumb(thumbParam) {
        var pixelTotal;
        var pixelMouse;
        var fracThumb;
      ...
      _getNormValueFromThumb: function _getNormValueFromThumb(thumb) {
        var fracThumb;
        var valueTotal;
        var valueMouse;
      ...
      _getOtherThumbValue: function _getOtherThumbValue(index) {
      ...
      _setSingleValue: function _setSingleValue(event, newValue, rawOnly) {
      ...
      _change: function _change(event, index, rawOnly) {
        if (this._multipleThumbs) {
      ...
      //
      _getSingleValue: function _getSingleValue() {
        return this._getValueAligned();
      },
      //
      // Return the value for the specified thumb.
      //
      _getMultiValues: function _getMultiValues(index) {
        //
      ...
      //
      _setMultiValue: function _setMultiValue(event, index, newValue, rawOnly) {
      ...
      */
      ...
      _setOption: function _setOption(key, value, flags) {
        ...
        if (key === 'value') {
          if (this._isCustomRangeSlider()) {
            ...
            if (!isNaN(value[0])) {
              ...
              //
              this._multipleThumbs = false;
              coercedValue = this._parse(key, value[0]); // verify that the new value is within range.

              this._checkValueBounds(coercedValue, this._valueMin(), this._valueMax());
            }
          } else {
            this._multipleThumbs = false; // Only coerce values for widget syntax,
          ...
          if (key === 'min') {
            this._checkMinMax(coercedValue, this._valueMax()); // Coerce any out of bounds "value" attributes to be within the min bounds
            ...
            this._checkMinMax(this._valueMin(), coercedValue); // Coerce any out of bounds "value" attributes to be within the max bounds
        ...
        this._super(key, coercedValue, flags); // when a dom element supports readonly, use that, and not aria-readonly.
        ...
        switch (key) {
          case 'disabled':
            this._refreshThumbOptions(); // disable the dragging if disabled is true.
      ...
      _getValueAligned: function _getValueAligned() {
      ...
      _getThumbsValueFrac: function _getThumbsValueFrac(index) {
        return (this._getMultiValues(index) - this._valueMin()) / (this._valueMax() - this._valueMin()); // Note - (max - min) is checked in options to make sure that
      ...
      _setRange: function _setRange(val) {
      ...
      //
      _setRangeMultiThumb: function _setRangeMultiThumb(val, index) {
      ...
      _thumbEvents: {
        keydown: function keydown(event) {
          ...
          switch (event.keyCode) {
            ...
            case $.ui.keyCode.PAGE_UP:
              newVal = this._trimAlignValue(curVal + (this._valueMax() - this._valueMin()) / this._numPages);
              break;
      ...
      _InitOptions: function _InitOptions(originalDefaults, constructorOptions) {
        ...
        if (!this._IsCustomElement()) {
          ojeditablevalue.EditableValueUtils.initializeOptionsFromDom(props, constructorOptions, this, // post-process callback
          function (_initializedOptions) {
            var initializedOptions = _initializedOptions; // coerce regardless of where the option value came from - dom/constructor
      ...
      _checkValueBounds: function _checkValueBounds(value, min, max) {
        if (min != null) {
          if (value < min) {
      ...
      _checkMinMax: function _checkMinMax(min, max) {
      ...
      _parse: function _parse(option, val) {
        ...
        if (isNaN(returnValue)) {
          throw new Error(this.getTranslatedString('optionNum', {
            option: option
          }));
        }
      ...
      _getEndInterval: function _getEndInterval() {
        return this._barback.offset().left + this._barback.width();
      },
      // return the startpoint of the bar
      _getStartInterval: function _getStartInterval() {
      ...
      //
      _callDraggable: function _callDraggable(thumbParam) {
        var g = this._getGrid();

        var cachedStyle = thumbParam[0].style;
        var axisValue;
        if (!this._isVertical()) axisValue = 'x';else axisValue = 'y';
        var that = this;
        thumbParam.draggable({
          ...
          drag: function drag(event, ui) {
            ...
            if (that._multipleThumbs) {
              var otherThumb; // var thumb = that._getActiveThumb();
              ...
              if (!that._isVertical()) {
                ...
                var parentLeft = that._barback.offsetParent().offset().left; // pos1 = thumbParam.offset().left + halfThumbWidth - parentLeft;
                ...
                var parentTop = that._barback.offsetParent().offset().top; // pos1 = thumbParam.offset().top + halfThumbHeight - parentTop;
      ...
      //
      _disableDraggable: function _disableDraggable() {
        if (this._multipleThumbs) {
          this._thumbs.toArray().forEach(function (current) {
            var thumb = $(current);
