 ...
 * @ignore
 */
define(['knockout', 'ojs/ojcontext', 'ojs/ojmodule', 'ojs/ojcomposite'], function (ko, Context, ojmodule, Composite) {
  'use strict';
  ...
  function moduleViewModel(context) {
    ...
    function updateConfig() {
      if (!self.busyCallback) {
        self.busyCallback = Context.getContext(element).getBusyContext().addBusyState({
          description: 'oj-module is waiting on config Promise resolution'
        });
      }
  ...
  var moduleValue = '{"view":config().view, "viewModel":config().viewModel,' + '"cleanupMode":config().cleanupMode,"animation":animation}';
