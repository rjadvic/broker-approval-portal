 ...
 * @ignore
 */
define(['ojs/ojcore-base', 'jquery', 'ojs/ojlogger', 'ojs/ojtranslation', 'ojs/ojdatasource-common'], function (oj, $, Logger, Translations, ojdatasourceCommon) {
  'use strict';
  ...
  var ArrayTableDataSource = function ArrayTableDataSource(data, options) {
    // Initialize
  ...
  oj._registerLegacyNamespaceProp('ArrayTableDataSource', ArrayTableDataSource); // Subclass from oj.DataSource
      ...
      if (row != null) {
        ...
        if (this._sorted === true && this._rows.data.length > 0) {
          ...
          for (var j = 0; j < this._rows.data.length; j++) {
            if (ArrayTableDataSource._sortFunc(row, this._rows.data[j], self._getComparator(), self) < 0) {
    ...
    if ($.isArray(idAttribute)) {
      ...
      for (i = 0; i < idAttribute.length; i++) {
        if (idAttribute[i] in row) {
          ...
          errDetail = Translations.applyParameters(ArrayTableDataSource._LOGGER_MSG._ERR_ARRAY_TABLE_DATASOURCE_IDATTR_NOT_IN_ROW, [idAttribute[i]]);
      ...
      errDetail = Translations.applyParameters(ArrayTableDataSource._LOGGER_MSG._ERR_ARRAY_TABLE_DATASOURCE_IDATTR_NOT_IN_ROW, [idAttribute]);
      throw new Error(errDetail);
    }
