...
define(['exports', 'ojs/ojdvt-toolkit'], function (exports, dvt) {
  ...
  var DvtBreadcrumbs = function DvtBreadcrumbs(context, callback, callbackObj, options) {
    this.Init(context, callback, callbackObj, options);
  };

  dvt.Obj.createSubclass(DvtBreadcrumbs, dvt.Container);
  /**
  ...
  DvtSunburstNode.LARGE_DATASET_THRESHOLD = 1000; // Relative radius of the center node compared to the other layers
  ...
  DvtSunburstNode.prototype._createShapeNode = function () {
    ...
    var borderWidth = options['borderWidth'] == null ? nodeDefaults['borderWidth'] : options['borderWidth']; // Apply the border color, border width, selected, and hover strokes
  ...
  DvtSunburstNode.prototype._createTextNode = function (container) {
    ...
    if (this._labelDisplay == 'auto') {
      ...
      // in those browsers.
      if (dvt.Agent.browser !== 'ie' && dvt.Agent.browser !== 'edge' && dvt.Agent.os == 'windows' && !dvt.Agent.isEnvironmentTest()) bRotated = false;else bRotated = true;
  ...
  DvtTreeAutomation.prototype.GetSubIdForDomElement = function (displayable) {
    ...
    if (!logicalObj) {
      ...
      var parent = displayable.getParent();
      if (parent instanceof DvtBreadcrumbs) return DvtTreeAutomation.BREADCRUMBS_PREFIX + '[' + parent.getCrumbIndex(displayable) + ']';
      return null;
    }
  ...
  DvtTreeLegendRenderer.render = function (treeView, availSpace) {
    ...
    if ((sizeValueStr == null || sizeValueStr == "") && (colorValueStr == null || colorValueStr == "")) return;
    var context = treeView.getCtx();
  ...
  var Sunburst = function Sunburst(context, callback, callbackObj) {
    this.Init(context, callback, callbackObj);
  }; // Make Sunburst a subclass of DvtTreeView


  dvt.Obj.createSubclass(Sunburst, DvtTreeView);
  /** @const @private **/
  ...
  Sunburst._BACKGROUND_INLINE_DEFAULT = ''; // Layout Constants
  ...
  Sunburst.prototype.Init = function (context, callback, callbackObj) {
    Sunburst.superclass.Init.call(this, context, callback, callbackObj); // Create the defaults object
  ...
  Sunburst.prototype.Render = function (container, bounds) {
    // Background
    this.RenderBackground(container, Sunburst._BACKGROUND_INLINE_DEFAULT); // Breadcrumbs
    ...
    if (this.__isRotationEnabled() && this.HasValidData()) {
      var buffer = dvt.Agent.isTouchDevice() ? Sunburst._ROTATION_SHAPE_RADIUS_TOUCH : Sunburst._ROTATION_SHAPE_RADIUS;
  ...
  Sunburst.prototype.GetDisplayAnimation = function (container, bounds) {
    if (this.getOptions()['animationOnDisplay'] === Sunburst._ANIMATION_TYPE_FAN && this.HasValidData()) {
      ...
      return anim;
    } else return Sunburst.superclass.GetDisplayAnimation.call(this, container, bounds);
  };
  /**
   * @override
   */
  ...
  var Treemap = function Treemap(context, callback, callbackObj) {
    this.Init(context, callback, callbackObj);
  }; // Make Treemap a subclass of DvtTreeView


  dvt.Obj.createSubclass(Treemap, DvtTreeView);
  /** @const @private **/
  ...
  Treemap._BACKGROUND_INLINE_DEFAULT = 'background-color:' + Treemap._BACKGROUND_FILL_COLOR + ';' + 'border-color:' + Treemap._BACKGROUND_BORDER_COLOR + ';' + 'border-width:2px';
  /**
  ...
  Treemap.prototype.Init = function (context, callback, callbackObj) {
    Treemap.superclass.Init.call(this, context, callback, callbackObj); // Create the defaults object
  ...
  Treemap.prototype.Render = function (container, bounds) {
    // Background
    this.RenderBackground(container, Treemap._BACKGROUND_INLINE_DEFAULT); // Breadcrumbs
