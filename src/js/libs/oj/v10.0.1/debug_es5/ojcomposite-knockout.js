 ...
 * @ignore
 */
define(['ojs/ojcore-base', 'knockout', 'ojs/ojcustomelement', 'ojs/ojcustomelement-utils', 'ojs/ojkoshared', 'ojs/ojlogger', 'ojs/ojtemplateengine'], function (oj, ko, ojcustomelement, ojcustomelementUtils, BindingProviderImpl, Logger, TemplateEngine) {
  'use strict';
  ...
  CompositeTemplateRenderer.renderTemplate = function (params, element, view) {
    ...
    ko.virtualElements.setDomNodeChildren(element, view); // Attached is deprecated in 4.2.0 for connected which is called when the view is first attached to the DOM
    ...
    var bindingContext = CompositeTemplateRenderer._getKoBindingContext(); // Null out the parent references since we don't want the composite View to be able to access the outside context


    var childBindingContext = bindingContext.createChildContext(params.viewModel, undefined, function (ctx) {
  ...
  CompositeTemplateRenderer.getEnclosingComposite = function (node) {
    var enclosing = null; // for upstream dependency we will still rely components being registered on the oj namespace.
  ...
  CompositeTemplateRenderer.invokeViewModelMethod = function (elem, model, name, args) {
    ...
    if (typeof handler === 'function') {
      try {
        ...
        throw new Error('Error while invoking ' + name + ' callback for ' + elem.tagName.toLowerCase() + " with id '" + elem.id + "'.");
      }
    }
  ...
  SlotUtils.cleanup = function (element, bindingContext) {
    var nodeStorage = bindingContext.__oj_nodestorage; // Move all non default template children to nodeStorage to keep bindings alive for
    ...
    init: function init(element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
      ...
      if (assignedNodes) {
        ...
        for (i = 0; i < assignedNodes.length; i++) {
          ...
          var node = assignedNodes[i]; // Get the slot value of this oj-bind-slot element so we can assign it to its
          // assigned nodes for downstream slotting

          node.__oj_slots = slotAttr;
        }

        ko.virtualElements.setDomNodeChildren(element, assignedNodes); // Notifies JET components in node that they have been shown
        // For upstream or indirect dependency we will still rely components being registered on the oj namespace.
  ...
  ko.bindingHandlers._ojBindTemplateSlot_ = {
    init: function init(element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
      ...
      var unwrap = ko.utils.unwrapObservable;
      var slotName = unwrap(values.name) || '';
      var slotChildren = slots[slotName]; // Take the last item matching the slot name

      var template = slotChildren && slotChildren[slotChildren.length - 1]; // If no application provided template, check for default template
      ...
      if (template) {
        ...
        if (template.tagName !== 'TEMPLATE') {
          Logger.error("Slot content for slot '" + slotName + "' under " + composite.tagName.toLowerCase() + " with id '" + composite.id + "' should be wrapped inside a <template> node.");
        } // Get the slot value of this oj-bind-template element so we can assign it to its
        // assigned nodes for downstream slotting


        template.__oj_slots = unwrap(values.slot) || ''; // Re-execute the template if the oj-bind-template-slot bound attributes change

        ko.computed(function () {
          // Use the template engine to execute the template
          var data = unwrap(values.data);
          var as = unwrap(values.as); // Extend the composite's bindingContext for the default template
