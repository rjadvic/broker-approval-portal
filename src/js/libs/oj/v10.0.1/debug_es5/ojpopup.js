 ...
 * @ignore
 */
define(['ojs/ojpopupcore', 'ojs/ojcore-base', 'jquery', 'ojs/ojcontext', 'ojs/ojdomutils', 'ojs/ojthemeutils', 'ojs/ojcomponentcore', 'ojs/ojanimation', 'ojs/ojfocusutils', 'ojs/ojcustomelement-utils'], function (ojpopupcore, oj, $, Context, DomUtils, ThemeUtils, Components, AnimationUtils, FocusUtils, ojcustomelementUtils) {
  'use strict';
    ...
    */
    ...
    oj.__registerWidget('oj.ojPopup', $.oj.baseComponent, {
      ...
      _usingHandler: function _usingHandler(origUsing, pos, props) {
        ...
        if (!tail) {
          ...
          if (props.horizontal === 'left') {
            ...
            tailHOffset -= tailHOffset + DomUtils.getCSSLengthAsInt(tail.css('left')); // eslint-disable-next-line no-param-reassign

            pos.left += tailHOffset - borderFactor;
          } else if (props.horizontal === 'right') {
            tailHOffset = tail.outerWidth();
            tailHOffset -= tailHOffset + DomUtils.getCSSLengthAsInt(tail.css('right')); // eslint-disable-next-line no-param-reassign
          ...
          if (props.vertical === 'top') {
            ...
            tailVOffset -= tailVOffset + DomUtils.getCSSLengthAsInt(tail.css(props.vertical)); // eslint-disable-next-line no-param-reassign
      ...
      _dismissalHandler: function _dismissalHandler(event) {
        ...
        if (!DomUtils.isAncestorOrSelf(launcher[0], target) && !DomUtils.isAncestorOrSelf(layer[0], target)) {
      ...
      _setWhenReady: function _setWhenReady(operation) {
        ...
        this._whenReadyMediator = new ojpopupcore.PopupWhenReadyMediator(this.element, operation, 'ojPopup', this._IsCustomElement());
      },
