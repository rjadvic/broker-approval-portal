 ...
 * @ignore
 */
define(['exports', 'ojs/ojlogger', 'ojs/ojconverterutils-i18n', 'ojs/ojlocaledata', 'ojs/ojconverter', 'ojs/ojtranslation', 'ojs/ojconfig', 'ojs/ojcore-base', 'ojs/ojvalidation-error'], function (exports, Logger, __ConverterUtilsI18n, LocaleData, Converter, Translations, Config, oj$1, ojvalidationError) {
  'use strict';
  ...
  var OraNumberConverter = function () {
    ...
    var _LENIENT_REGEX_PARSE_FLOAT = /([^+-.0-9]*)([+-]?\d*\.?\d*(E[+-]?\d+)?).*$/; // eslint-disable-next-line no-useless-escape
    ...
    var _DIGITAL_TERA = 1024 * 1024 * 1024 * 1024; // prepend or append count zeros to a string.
    ...
    function _throwNumberOutOfRange(value, minimum, maximum, property) {
      var msg = value + ' is out of range.  Enter a value between ' + minimum + ' and ' + maximum + ' for ' + property;
    ...
    function _getNumberSettings(localeElements, _numberSettings, options, locale) {
      ...
      var localeElementsMainNode = __ConverterUtilsI18n.OraI18nUtils.getLocaleElementsMainNode(localeElements);
      ...
      numberSettings.style = options.style; // pattern passed in options
      ...
      var decFormatLength = options.decimalFormat; // if not, check for currencyFormat
      ...
      numberSettings.minusSign = localeElementsMainNode.numbers[numberSettings.numberingSystem].minusSign;
      numberSettings.decimalSeparator = decimalSeparator;
      numberSettings.exponential = localeElementsMainNode.numbers[numberSettings.numberingSystem].exponential;
      ...
      if (options.pattern === undefined) {
        numberSettings.minimumIntegerDigits = _getNumberOption(options, 'minimumIntegerDigits', 1, 21, numberSettings.minimumIntegerDigits);

        if (options.maximumFractionDigits !== undefined) {
          numberSettings.maximumFractionDigits = _getNumberOption(options, 'maximumFractionDigits', 0, 20, numberSettings.maximumFractionDigits);
        ...
        if (options.minimumFractionDigits !== undefined) {
          numberSettings.minimumFractionDigits = _getNumberOption(options, 'minimumFractionDigits', 0, 20, numberSettings.minimumFractionDigits);
        }
        ...
        if (numberSettings.style === 'currency' && options.minimumFractionDigits === undefined && (decFormatLength === undefined || decFormatLength === 'standard')) {
    ...
    function _throwMissingCurrency(prop) {
      var typeError = new TypeError('The property "currency" is required when' + ' the property "' + prop + '" is "currency". An accepted value is a ' + 'three-letter ISO 4217 currency code.');
      var errorInfo = {
    ...
    function _throwMissingUnit(prop) {
      var typeError = new TypeError('The property "unit" is required when' + ' the property "' + prop + '" is "unit". An accepted value is "byte" or "bit".');
      var errorInfo = {
        errorCode: 'optionTypesMismatch',
        parameterMap: {
          propertyName: prop,
    ...
    function _validateNumberOptions(options, caller) {
      ...
      var s = getOption('style', 'string', ['currency', 'decimal', 'percent', 'unit', 'perMill'], 'decimal');
      ...
      s = getOption('roundingMode', 'string', ['UP', 'DOWN', 'FLOOR', 'CEILING', 'HALF_UP', 'HALF_DOWN', 'HALF_EVEN']);
    ...
    function _toDigitalByte(number, options, numberSettings, localeElements) {
      ...
      var lang = numberSettings.lang; // get plural rule: one, many, etc..

      var plural = numberSettings.plurals[lang](count); // plural -> 'unitPattern-count-one' or 'unitPattern-count-many'

      plural = 'unitPattern-count-' + plural; // format the number

      if (number < 0) {
        count = -count;
      }

      var fmt = _toRawFixed(count, options, numberSettings); // format the number based on plural rule: "{0} Gb", etc..
    ...
    function _toCompactNumber(number, options, numberSettings) {
      ...
      if (typeVal[1] !== null) {
        ...
        var plural = numberSettings.plurals[lang](Math.floor(absVal / _decimalTypeValuesMap[typeVal[0]]));
        ...
        if (zeros < decimalFormatType.length) {
          ...
          i = typeVal[1] / i * 10; // eslint-disable-next-line no-param-reassign
    ...
    function _toExponentialPrecision(number, numberSettings) {
      ...
      if (str2.length > 0) {
        str1 += numberSettings.decimalSeparator + numStr1.slice(numberSettings.minimumIntegerDigits) + numberSettings.exponential + posExp;
      } else {
    ...
    function _toRawFixed(number, options, numberSettings) {
      ...
      var decimalSeparator = numberSettings.decimalSeparator; // First round the number based on maximumFractionDigits
      ...
      var precision = Math.min(numberSettings.maximumFractionDigits, right.length - exponent); // round the number only if it has decimal points

      if (split.length > 1 && right.length > exponent) {
        var mode = options.roundingMode || 'DEFAULT'; // eslint-disable-next-line no-param-reassign
      ...
      split = numberString.split('.');
      numberString = split[0];
      right = split.length > 1 ? split[1] : ''; // pad zeros based on the exponent value and minimumFractionDigits
      ...
      if (precision > 0 && right.length > 0) {
        right = right.length > precision ? right.slice(0, precision) : _zeroPad(right, precision, false); // if right is only zeros, truncate it to minimumFractionDigits
      ...
      right = __ConverterUtilsI18n.OraI18nUtils.trimRightZeros(right); // insert grouping separator in the integer part based on groupingSize
    ...
    function _decimalAdjust(value, scale, mode) {
      ...
      var v0 = strValue[0];
      var v1 = strValue[1]; // shift the decimal point based on the scale so that we can apply ceil or floor
      ...
      strValue = _value.toString().split('e'); // need to extract v0 and v1 again because value has changed after applying Math[mode].

      v0 = strValue[0];
      v1 = strValue[1]; // shift the decimal point back to its original position
    ...
    function _formatNumberImpl(value, options, localeElements, numberSettings, locale) {
      ...
      if ((optStyle === 'decimal' || optStyle === 'currency') && formatType !== undefined && formatType !== 'standard') {
      ...
      var ret = ''; // add negative prefix and suffix if number is negative
      ...
      if (numberingSystemKey !== 'latn') {
        ...
        for (idx = 0; idx < ret.length; idx++) {
          if (ret[idx] >= '0' && ret[idx] <= '9') {
            nativeRet.push(__ConverterUtilsI18n.OraI18nUtils.numeringSystems[numberingSystemKey][ret[idx]]);
          } else {
    ...
    function _parseNegativePattern(value, options, numberSettings, localeElements) {
      ...
      var nbSettingPosPrefix = __ConverterUtilsI18n.OraI18nUtils.trimNumber(numberSettings.positivePrefix);

      var nbSettingPosSuffix = __ConverterUtilsI18n.OraI18nUtils.trimNumber(numberSettings.positiveSuffix);

      var nbSettingNegPrefix = __ConverterUtilsI18n.OraI18nUtils.trimNumber(numberSettings.negativePrefix);

      var nbSettingNegSuffix = __ConverterUtilsI18n.OraI18nUtils.trimNumber(numberSettings.negativeSuffix); // try exact match of negative prefix and suffix


      var posPrefRegexp = new RegExp('^' + (nbSettingPosPrefix || '').replace(_ESCAPE_REGEXP, '\\$1'));
      var posSuffRegexp = new RegExp((nbSettingPosSuffix || '').replace(_ESCAPE_REGEXP, '\\$1') + '$');
      var negPrefRegexp = new RegExp('^' + (nbSettingNegPrefix || '').replace(_ESCAPE_REGEXP, '\\$1'));
      var negSuffRegexp = new RegExp((nbSettingNegSuffix || '').replace(_ESCAPE_REGEXP, '\\$1') + '$');
      ...
      if (negPrefRegexp.test(num) === true && negSuffRegexp.test(num) === true) {
        ...
        if (numberSettings.currencyDisplay === undefined || numberSettings.currencyDisplay === 'symbol') {
          repStr = symbol;
        ...
        if (repStr !== undefined) {
          ...
          negSuffRegexp = new RegExp(negSuffix.replace(_ESCAPE_REGEXP, '\\$1') + '$'); // try  match of positive prefix and suffix
    ...
    function _lenientParseNumber(_num, numberSettings) {
      ...
      num = num.split(exponential).join('E'); // remove grouping separator from string
    ...
    function _parseNegativeExponent(_value, numberSettings) {
      ...
      } else if (__ConverterUtilsI18n.OraI18nUtils.startsWith(value, __ConverterUtilsI18n.OraI18nUtils.trimNumber(pos))) {
    ...
    function _getLatnDigits(str, locale) {
      ...
      for (idx = 0; idx < str.length; idx++) {
        var pos = __ConverterUtilsI18n.OraI18nUtils.numeringSystems[numberingSystemKey].indexOf(str[idx]);
    ...
    function _getNumberParts(_num, numberSettings) {
      ...
      var num = _num.replace(/ /g, ''); // determine exponent and number
    ...
    function _parseNumberImpl(str, localeElements, options, locale) {
      ...
      var ret = NaN;
      var value1 = numStr.replace(/ /g, ''); // allow infinity or hexidecimal
      ...
      var exponent = parts.exponent; // build a natively parsable number string
    ...
    function _throwSyntaxError(pattern) {
      var samplePattern = '#,##0.###';
      var msg = 'Unexpected character(s) encountered in the pattern "' + pattern + ' An example of a valid pattern is "' + samplePattern + '".';
    ...
    function _expandAffixes(localeElements, _numberSettings) {
      ...
      if (posPrefixPattern !== null) {
        numberSettings.positivePrefix = _expandAffix(posPrefixPattern, localeElements, numberSettings, curDisplay);
      }

      if (posSuffixPattern !== null) {
        numberSettings.positiveSuffix = _expandAffix(posSuffixPattern, localeElements, numberSettings, curDisplay);
      }

      if (negPrefixPattern !== null) {
        numberSettings.negativePrefix = _expandAffix(negPrefixPattern, localeElements, numberSettings, curDisplay);
      }

      if (negSuffixPattern !== null) {
        numberSettings.negativeSuffix = _expandAffix(negSuffixPattern, localeElements, numberSettings, curDisplay);
      }

      if (curDisplay.name !== undefined) {
        numberSettings.positiveSuffix = "\xA0" + curDisplay.name;
        numberSettings.positivePrefix = '';

        if (numberSettings.lang === 'ar') {
          numberSettings.negativeSuffix = localeElements.numbers[numberSettings.numberingSystem].minusSign + "\xA0" + curDisplay.name;
          numberSettings.negativePrefix = '';
        } else {
          numberSettings.negativeSuffix = "\xA0" + curDisplay.name;
          ...
          switch (c) {
            case _CURRENCY:
              ...
              if (numberSettings.currencyDisplay === undefined || numberSettings.currencyDisplay === 'symbol') {
                ...
                c = name; // eslint-disable-next-line no-param-reassign
    ...
    function _applyPatternImpl(options, pattern, localeElements, _numberSettings) {
      ...
      for (var j = 1; j >= 0 && start < pattern.length; --j) {
        ...
        for (var pos = start; pos < pattern.length; ++pos) {
          ...
          switch (phase) {
            ...
            case 2:
              ...
              if (inQuote) {
              ...
              } else if (ch === _DIGIT || ch === _ZERO_DIGIT || ch === _GROUPING_SEPARATOR || ch === _DECIMAL_SEPARATOR) {
                ...
                if (isPrefix) {
                  prefix = prefix.concat(doubled ? "'\xA4\xA4" : "'\xA4");
                } else {
                  suffix = suffix.concat(doubled ? "'\xA4\xA4" : "'\xA4");
                }
        ...
        if (j === 1) {
          ...
          var digitTotalCount = digitLeftCount + zeroDigitCount + digitRightCount; // The effectiveDecimalPos is the position the decimal is at or
          ...
          numberSettings.maximumIntegerDigits = useExponentialNotation ? digitLeftCount + numberSettings.minimumIntegerDigits : _MAXIMUM_INTEGER_DIGITS;
          numberSettings.maximumFractionDigits = decimalPos >= 0 ? digitTotalCount - decimalPos : 0;
          numberSettings.minimumFractionDigits = decimalPos >= 0 ? digitLeftCount + zeroDigitCount - decimalPos : 0;
      ...
      numberSettings.minExponentDigits = minExponentDigits; // If there was no negative pattern, or if the negative pattern is
      // identical to the positive pattern, then prepend the minus sign to
      // the positive pattern to form the negative pattern.

      if (!gotNegative || negPrefixPattern.localeCompare(posPrefixPattern) === 0 && negSuffixPattern.localeCompare(posSuffixPattern) === 0) {
    ...
    function _resolveNumberSettings(localeElements, options, locale) {
      ...
      if (__ConverterUtilsI18n.OraI18nUtils.numeringSystems[numberSettings.numberingSystemKey] === undefined) {
    ...
    function _resolveOptions(numberSettings, options, locale) {
      ...
      if (numberSettings.style === 'currency') {
        ...
        resOptions.currencyDisplay = options.currencyDisplay === undefined ? 'symbol' : options.currencyDisplay;
      }
    ...
    */
    ...
    function _init() {
      return {
         ...
         * the pattern.
         */
        format: function format(value, localeElements, options, locale) {
         ...
         * pattern.
         */
        parse: function parse(str, localeElements, options, locale) {
         ...
         * missing.
         */
        resolvedOptions: function resolvedOptions(localeElements, options, locale) {
  ...
  var NumberConverter = function NumberConverter() {
    this.Init();
  ...
  var IntlNumberConverter = function IntlNumberConverter(options) {
    this.Init(options);
  };
  /**
  ...
  IntlNumberConverter.prototype.format = function (value) {
    ...
    if (value == null || typeof value === 'string' && oj$1.StringUtils.trim('' + value).length === 0 || typeof value === 'number' && isNaN(value)) {
      return '';
    }
    ...
    if (formatValue === 'NaN') {
      var summary = Translations.getTranslatedString('oj-converter.number.invalidNumberFormat.summary', {
        value: value
      });
  ...
  IntlNumberConverter.prototype.parse = function (value) {
    ...
    var locale;
    var localeElements;
    var resolvedOptions; // null and empty string values are ignored and not parsed. It
    // undefined.
    ...
    try {
      ...
      return this._getWrapped().parse(oj$1.StringUtils.trim(value), localeElements, resolvedOptions, locale);
    } catch (e) {
      converterError = this._processConverterError(e, value);
      throw converterError;
    }
  ...
  IntlNumberConverter.prototype.resolvedOptions = function () {
    var converterError;
    var locale = Config.getLocale();
    var localeElements; // options are resolved and cached for the current locale. when locale changes resolvedOptions
    ...
    if (locale !== this._locale || !this._resolvedOptions) {
      ...
      try {
        ...
        this._resolvedOptions = this._getWrapped().resolvedOptions(localeElements, this.getOptions(), locale);
        this._locale = locale;
      } catch (e) {
    ...
    if (errorInfo) {
      ...
      switch (errorCode) {
        ...
        case 'optionTypeInvalid':
          converterError = __ConverterUtilsI18n.IntlConverterUtils.__getConverterOptionError(errorCode, parameterMap);
          break;
        ...
        case 'unsupportedParseFormat':
          ...
          //
          summary = Translations.getTranslatedString('oj-converter.number.shortLongUnsupportedParse.summary');
          detail = Translations.getTranslatedString('oj-converter.number.shortLongUnsupportedParse.detail');
