 ...
 * @ignore
 */
define(['exports', 'ojs/ojcore-base', 'jquery', 'ojs/ojcontext', 'ojs/ojdomutils', 'ojs/ojtranslation', 'hammerjs', 'ojs/ojthemeutils', 'ojs/ojjquery-hammer', 'ojs/ojcomponentcore', 'touchr', 'ojs/ojprogress-circle'], function (exports, oj, $, Context, DomUtils, Translations, Hammer, ThemeUtils, ojjqueryHammer, ojcomponentcore, touchr, ojprogressCircle) {
  'use strict';
  ...
  var PullToRefreshUtils = {}; // mapping variable definition, used in a no-require environment. Maps the PullToRefreshUtils object to the name used in the require callback.
  ...
  PullToRefreshUtils.setupPullToRefresh = function (element, refreshFunc, options) {
    ...
    var isTouch;
    var type;
    var loadingIcon = (ThemeUtils.parseJSONFromFontFamily('oj-refresher-option-defaults') || {}).loadingIcon; // bind the refresherElement to the scrollerElement for busyState if it exists

    if (options && options.refresherElement) {
      $.data($(element)[0], 'data-pulltorefresh-refresher', options.refresherElement);
    }

    isTouch = DomUtils.isTouchSupported(); // make sure we are clean

    PullToRefreshUtils.tearDownPullToRefresh(element);
    outer = $(document.createElement('div')).addClass('oj-pulltorefresh-outer');

    PullToRefreshUtils._renderAccessibleLink(element, outer, refreshFunc, options); // create the element containing the content
    ...
    panel.on(type + 'start.pulltorefresh', function (event) {
      ...
      if (panel[0].scrollTop === 0) {
        PullToRefreshUtils._handlePull(event, content, options); // extract these info up front instead of in move
      ...
      if (icon != null && icon.length > 0) {
        ...
        icon.attr('title', title); // check whether to show/hide primary/secondary text
  ...
  PullToRefreshUtils._handlePull = function (event, content, options) {
    ...
    content.prev().text(PullToRefreshUtils._getTranslatedString('oj-pullToRefresh.ariaRefreshingLink')); // set it back to auto so we can get the actual height
  ...
  PullToRefreshUtils._handleRelease = function (event, element, content, refreshFunc) {
    ...
    var loadingIcon = (ThemeUtils.parseJSONFromFontFamily('oj-refresher-option-defaults') || {}).loadingIcon;
    height = $.data(content[0], 'data-panelheight');
    content.addClass('oj-pulltorefresh-transition').css('height', height);

    PullToRefreshUtils._fireEvent(event, 'release', content, height); // mark that we are in loading stage
    ...
    var busyElement = $.data($(element)[0], 'data-pulltorefresh-refresher') ? $.data($(element)[0], 'data-pulltorefresh-refresher') : element;
    ...
    refreshFunc().then(function () {
      _listener2 = function listener() {
        PullToRefreshUtils._fireEvent(event, 'complete', content, height); // cleanup after everything is complete
      ...
      content.prev().text(PullToRefreshUtils._getTranslatedString('oj-pullToRefresh.ariaRefreshCompleteLink')); // hide the link
  ...
  PullToRefreshUtils._resolveBusyState = function (element) {
    var elem;
    var busyStateResolve; // check if the refresher element is set on the scroller element

    var busyElement = $.data(element, 'data-pulltorefresh-refresher') ? $.data(element, 'data-pulltorefresh-refresher') : element;
    elem = $(busyElement)[0];
    busyStateResolve =
    /** @type {Function} */
  ...
  var setupPullToRefresh = PullToRefreshUtils.setupPullToRefresh;
  var tearDownPullToRefresh = PullToRefreshUtils.tearDownPullToRefresh;
  exports.setupPullToRefresh = setupPullToRefresh;
  exports.tearDownPullToRefresh = tearDownPullToRefresh;
