 ...
 * @ignore
 */
define(['ojs/ojcore-base', 'ojs/ojdatasource-common', 'ojs/ojlogger'], function (oj, ojdatasourceCommon, Logger) {
  'use strict';
  ...
  var JsonNodeSet = function JsonNodeSet(startNode, endNode, data, currKey, depth) {
  ...
  JsonNodeSet.prototype.getData = function (index) {
    ...
    oj.Assert.assert(index <= this.m_endNode && index >= this.m_startNode); // adjust to relative index
  ...
  JsonNodeSet.prototype.getMetadata = function (index) {
    ...
    var relIndex = index - this.m_startNode;
    metadata.key = this.m_nodes[relIndex].id ? this.m_nodes[relIndex].id : this.m_nodes[relIndex].attr.id;
  ...
  JsonNodeSet.prototype._updateDepth = function (currChild, offset) {
    ...
    offset += 1; // eslint-disable-next-line no-param-reassign
  ...
  var _JsonTreeNodeDataSource = function _JsonTreeNodeDataSource() {
    this.id = null;
    this.depth = 0;
    this.parent = null;
    /** @type {Array} */
  ...
  var JsonTreeDataSource = function JsonTreeDataSource(data) {
    ...
    this.data = this._createTreeDataSource({
      count: 0
    }, tree, data, 0);
    JsonTreeDataSource.superclass.constructor.call(this, tree);
  };

  oj._registerLegacyNamespaceProp('JsonTreeDataSource', JsonTreeDataSource); // Subclass from oj.TreeDataSource
  ...
  JsonTreeDataSource.prototype._createTreeDataSource = function (c, target, source, depth) {
    ...
    for (var prop in source) {
      if (prop === 'children' || depth === 0 && source instanceof Array) {
        ...
        target.children = []; // eslint-disable-next-line no-param-reassign
        ...
        for (var j = 0; j < children.length; j++) {
          ...
          var prp; // eslint-disable-next-line no-restricted-syntax,guard-for-in
          ...
          target.children.push(node); // eslint-disable-next-line no-restricted-syntax,guard-for-in
  ...
  JsonTreeDataSource.prototype.fetchChildren = function (parentKey, range, callbacks, options) {
    ...
    var parent = this._searchTreeById(this.data, parentKey); // should never be null but this prevents blowing up if someone tries to call fetchChildren on a leaf
    ...
    if (!range) {
      // eslint-disable-next-line no-param-reassign
      range = []; // eslint-disable-next-line no-param-reassign

      range.start = 0; // eslint-disable-next-line no-param-reassign
    ...
    childEnd = Math.min(childCount, childStart + range.count); // now populate results from data array
    ...
    var nodeSet = new oj.JsonNodeSet(childStart, childEnd, results, parentKey, parent.depth); // invoke original success callback
  ...
  JsonTreeDataSource.prototype.fetchDescendants = function (parentKey, callbacks, options) {
    ...
    var parent = this._searchTreeById(this.data, parentKey); // should never be null but this prevents blowing up if someone tries to call fetchDescendants on a leaf
    ...
    var nodeSet = new oj.JsonNodeSet(0, results.length, results, parentKey, parent.depth); // ie original success callback
  ...
  JsonTreeDataSource.prototype.move = function (nodeToMove, referenceNode, position, callbacks) {
    ...
    var moveNode = this._searchTreeById(null, moveNodeKey); // if the moveNode doesn't contain the reference node as its sub-tree the action is allowed
    ...
    if (!this._searchTreeById(moveNode, refNodeKey)) {
      ...
      var index; // remove moveNode from the original position;
  ...
  JsonTreeDataSource.prototype.sort = function (criteria, callbacks) {
    ...
    parent._sortRecursive(criteria); // invoke original success callback
