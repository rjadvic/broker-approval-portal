 ...
 * @ignore
 */
define(['ojs/ojcore', 'jquery', 'knockout', 'ojs/ojcomposite', 'ojs/ojknockouttemplateutils', 'ojs/ojcomponentcore', 'ojs/ojlistview', 'ojs/ojprogress'], function (oj, $, ko, Composite, KnockoutTemplateUtils) {
  "use strict";
  ...
  var progressItemView = "    <div class='oj-flex oj-sm-justify-content-space-between'>" + "      <div class='oj-flex-item oj-flex oj-sm-flex-direction-column'>" + "        <span data-bind='text: $properties.data.name' class='oj-progresslist-name'></span>" + "        <div data-bind='text: message' class='oj-progresslist-error-message'></div>" + '      </div>' + "      <div class='oj-flex oj-sm-align-items-center'>" + "        <oj-bind-slot name='itemInfo'>" + "          <div class='oj-flex-item oj-flex oj-progresslist-info'>" + "            <span data-bind='text: $data.getSizeInBKMGT($properties.data.size)'></span>" + '          </div>' + '        </oj-bind-slot>' + "        <div class='oj-flex-item oj-flex'>" + "          <oj-progress-status status='{{status}}'" + "                              progress='{{progress}}'>" + '          </oj-progress-status>' + '        </div>' + '      </div>' + '    </div>';

  function progressItemViewModel(context) {
    var self = this;
    var handleUploadProgress;
    var handleUploadFail;
    var handleUploadDone; // Triggered for upload progress events. The event payload contains:
    ...
    handleUploadFail = function handleUploadFail(event) {
      var item = event.target;
    ...
    handleUploadDone = function handleUploadDone(event) {
      var item = event.target;
    ...
    self.message = ko.observable(''); // return size in B, KB, MB or GB
  ...
  var progressListView = "<oj-list-view data-bind=\"attr: {id: $unique + '_list'}\"" + "              aria-label='list using array'" + "              data='{{$properties.data}}'" + "              item.renderer='{{renderer()}}'" + "              selection-mode='single'>" + '</oj-list-view>' + '' + "<script type='text/html' data-bind=\"attr: {id: tempId}\">" + "  <li class='oj-progresslist-item' tabindex='0'>" + "    <oj-progress-item data='{{$data}}'>" + '    </oj-progress-item>' + '  </li>' + '</script>';
