 ...
 * @ignore
 */
define(['exports', 'ojs/ojcore-base', 'jquery', 'ojs/ojcontext', 'ojs/ojcomponentcore', 'ojs/ojlogger', 'ojs/ojdomutils', 'ojs/ojfocusutils', 'touchr'], function (exports, oj, $, Context, Components, Logger, DomUtils, FocusUtils, touchr) {
  'use strict';
  ...
  var __oj_film_strip_metadata = {
    "properties": {
      ...
      "arrowVisibility": {
        "type": "string",
        "enumValues": ["auto", "hidden", "hover", "visible"],
        "value": "auto"
      },
      "currentItem": {
        "type": "object",
  ...
  var FilmStripPagingModel = function FilmStripPagingModel() {
    this.Init();
  };

  oj._registerLegacyNamespaceProp('FilmStripPagingModel', FilmStripPagingModel); // Subclass from oj.EventSource
    ...
    try {
      ...
      return new Promise(function (resolve) {
        if (prevPageCount !== pageCount) {
          self.handleEvent('pageCount', {
            pageCount: pageCount,
            previousPageCount: prevPageCount
          });
        }
  ...
  (function () {
    ...
    var _ADJACENT = 'adjacent'; // aria-hidden DOM attribute

    var _ARIA_HIDDEN = 'aria-hidden';
    var _AUTO = 'auto'; // display CSS attribute

    var _DISPLAY = 'display'; // threshold to initialize a drag scroll (pixels)

    var _DRAG_SCROLL_INIT_THRESHOLD = 3; // drag scroll threshold (percentage of filmStrip size)

    var _DRAG_SCROLL_THRESHOLD = 0.33; // drag scroll threshold (max distance, pixels)

    var _DRAG_SCROLL_MAX_THRESHOLD = 100;
    var _EMPTY_STRING = ''; // throw an error when invalid "currentItem" property set

    var _ERROR_CURRENT_ITEM_NOT_FOUND = "JET FilmStrip: Value of 'currentItem' property not found: "; // throw an error when "orientation" property set to invalid value

    var _ERROR_INVALID_ORIENTATION = "JET FilmStrip: Unsupported value set as 'orientation' property: "; // throw an error when "arrowPlacement" property set to invalid value

    var _ERROR_INVALID_NAV_ARROW_PLACEMENT = "Unsupported value set as 'arrowPlacement' property: "; // throw an error when "arrowVisibility" property set to invalid value

    var _ERROR_INVALID_NAV_ARROW_VISIBILITY = "Unsupported value set as 'arrowVisibility' property: "; // throw an error when "looping" property set to invalid value

    var _ERROR_INVALID_LOOPING = "Unsupported value set as 'looping' property: ";
    var _FLEX_BASIS = 'flex-basis';
    var _HIDDEN = 'hidden'; // jQuery hidden selector
    ...
    var _PERIOD = '.';
    var _PX = 'px'; // make sure the collapseEventTimeout param is less than the one used in the unit tests
    ...
    var _VISIBLE = 'visible'; // jQuery visible selector

    var _VISIBLE_SELECTOR = ':visible';
    var _WEBKIT_FLEX_BASIS = '-webkit-flex-basis';
    var _WEBKIT_TRANSFORM = '-webkit-transform'; // log warning message when "disabled" property set
    ...
    function _applyTransform(jqObj, transform) {
      jqObj.css(_WEBKIT_TRANSFORM, transform).css(_MS_TRANSFORM, transform).css(_TRANSFORM, transform);
    }
    /**
    ...
    function _removeTransform(jqObj) {
      jqObj.css(_WEBKIT_TRANSFORM, _EMPTY_STRING).css(_MS_TRANSFORM, _EMPTY_STRING).css(_TRANSFORM, _EMPTY_STRING);
    }
    /**
    ...
    oj.__registerWidget('oj.ojFilmStrip', $.oj.baseComponent, {
      ...
      _ComponentCreate: function _ComponentCreate() {
        ...
        elem.addClass('oj-filmstrip oj-component').attr('tabindex', 0).attr('role', 'region'); // ensure a unique id for use with aria-labelledby on navigation arrows
        ...
        if (options.arrowVisibility !== _VISIBLE && options.arrowVisibility !== _HIDDEN && options.arrowVisibility !== _HOVER && options.arrowVisibility !== _AUTO) {
        ...
        this.navArrowHoverableEventNamespace = this.eventNamespace + 'NavArrowHoverable'; // Make sure currentItem is an object of (id, index)

        options.currentItem = this._convertItemToObj(options.currentItem);

        this._setup(true); // update the currentItem object in options.
       ...
       */
      getPagingModel: function getPagingModel() {
      ...
      _setup: function _setup(isInit) {
        ...
        var self = this; // FIX : always create paging model, even if filmstrip is
        ...
        if (isInit) {
          ...
          this._itemSize = -1; // eslint-disable-next-line no-unused-vars
        ...
        pagingModel.setTotalSize(originalItems.length); // wrap the original child DOM BEFORE adding the resize listener

        this._wrapChildren(); // FIX : only need to calculate layout if the filmstrip is
        // not empty


        if (originalItems.length > 0) {
          this._adjustSizes(); // notify the original children of the filmStrip that they were attached
          ...
          DomUtils.addResizeListener(elem[0], this._handleResizeFunc, _RESIZE_LISTENER_COLLAPSE_EVENT_TIMEOUT);
        }
      },
      ...
      _destroy: function _destroy() {
        ...
        var elem = this.element;
        elem.removeClass('oj-filmstrip oj-component ' + _OJ_FILMSTRIP_HOVER).removeAttr('tabindex role').removeAttr('aria-labelledby'); // remove a generated unique id

        elem.removeUniqueId(); // remove passive listeners if added
       ...
       * @private
       */
      _destroyInternal: function _destroyInternal() {
        // FIX : clear the flag to handle a deferred resize
        this._deferredHandleResize = false; // FIX : resolve busy state when destroying

        this._resolveBusyState();

        var elem = this.element;
        DomUtils.removeResizeListener(elem[0], this._handleResizeFunc); // reset item size so it will be recalculated at the next layout
        ...
        var i; // notify the original children of the filmStrip that they were detached from
        // the DOM BEFORE actually detaching so that components can save state
        ...
        var itemContainers = this._getItemContainers(); // remove logical page containers here instead of in _unwrapChildren because
        // they're added in _adjustSizes, not in _wrapChildren


        itemContainers.unwrap();

        this._unwrapChildren(); // notify the original children of the filmStrip that they were attached
      ...
      _setOption: function _setOption(key, value, flags) {
        ...
        switch (key) {
          ...
          case _CURRENT_ITEM:
            ...
            if (currentItem && value && (currentItem.id !== value.id || currentItem.index !== value.index)) {
              newPageIndex = this._findPage(value); // throw error if item not found
       ...
       * @private
       */
      _isLoopingPage: function _isLoopingPage() {
        var options = this.options;
       ...
       * @private
       */
      _getCssPositionAttr: function _getCssPositionAttr() {
       ...
       * @private
       */
      _getCssSizeAttr: function _getCssSizeAttr() {
        var bHorizontal = this._isHorizontal();
      ...
      _canCalculateSizes: function _canCalculateSizes() {
        ...
        var style = div.style; // need to set absolute position in order to get correct offsetwidth when
       ...
       * @private
       */
      _wrapChildren: function _wrapChildren() {
        var elem = this.element;
        ...
        this._pageInfoElem = pageInfoElem; // FIX : only need to create nav buttons if the filmstrip
        // is not empty
       ...
       * @private
       */
      _unwrapChildren: function _unwrapChildren() {
        var elem = this.element;

        var originalItems = this._getItems(); // unregister elem as hoverable


        this._tearDownNavArrowsHoverable(); // remove navigation arrow containers
        ...
        originalItems.removeClass(_OJ_FILMSTRIP_ITEM) // original children
       ...
       * @private
       */
      _createPageInfoElem: function _createPageInfoElem() {
       ...
       * @private
       */
      _updatePageInfoElem: function _updatePageInfoElem() {
        var pagingModel = this._pagingModel;
       ...
       * @private
       */
      _setupNavArrowsHoverable: function _setupNavArrowsHoverable() {
        ...
        var elem = this.element;
        elem.on('mouseenter' + this.navArrowHoverableEventNamespace, function (event) {
       ...
       * @private
       */
      _tearDownNavArrowsHoverable: function _tearDownNavArrowsHoverable() {
        var elem = this.element;
       ...
       * @private
       */
      _navArrowsShownOnHover: function _navArrowsShownOnHover() {
        var options = this.options;
        var arrowVisibility = options.arrowVisibility;
        return arrowVisibility === _HOVER || arrowVisibility === _AUTO && options.arrowPlacement === _OVERLAY;
      },
       ...
       * @private
       */
      _hasPrevPage: function _hasPrevPage() {
        var pagingModel = this._pagingModel;
       ...
       * @private
       */
      _hasNextPage: function _hasNextPage() {
        var pagingModel = this._pagingModel;
      ...
      _prevPage: function _prevPage() {
        ...
        if (this._hasPrevPage()) {
          ...
          var pageCount = pagingModel.getPageCount(); // navigate from first page to last page

          if (this._isLoopingPage() && pageCount > 1) {
            pagingModel.setPage(pageCount - 1, {
              loopDirection: _LOOPING_DIRECTION_PREV
            });
          }
        }
      },
      ...
      _nextPage: function _nextPage() {
        ...
        if (this._hasNextPage()) {
          ...
          var pageCount = pagingModel.getPageCount(); // navigate from last page to first page

          if (this._isLoopingPage() && pageCount > 1) {
            pagingModel.setPage(0, {
              loopDirection: _LOOPING_DIRECTION_NEXT
            });
          }
        }
      },
       ...
       * @private
       */
      _displayNavigationArrow: function _displayNavigationArrow(bShow, jqNavArrow) {
        var options = this.options;
       ...
       * @private
       */
      _updateNavigationArrowsDisplay: function _updateNavigationArrowsDisplay() {
        var options = this.options;
       ...
       * @private
       */
      _createPrevNavArrow: function _createPrevNavArrow() {
        var elem = this.element;
        ...
        var options = this.options;
        var navArrowPlacement = options.arrowPlacement; // need to append prev button when overlay so that it is in front of the
       ...
       * @private
       */
      _createNextNavArrow: function _createNextNavArrow() {
        var elem = this.element;
       ...
       * @private
       */
      _createNavigationArrowContainer: function _createNavigationArrowContainer(locationMarker) {
       ...
       * @private
       */
      _createNavigationArrow: function _createNavigationArrow(parentElem, locationMarker, label, tooltip) {
        ...
        var pageInfoElem = this._pageInfoElem;
        var pageInfoId = pageInfoElem.attr('id');
        arrowElem.attr('aria-labelledby', pageInfoId + ' ' + arrowId); //  - filmstrip: next/previous oj-hover colors doesn't go away in touch device
       ...
       * @private
       */
      _getItemContainers: function _getItemContainers() {
        var pagesWrapper = this._pagesWrapper;
        var items = pagesWrapper.find(_PERIOD + _OJ_FILMSTRIP_ITEM_CONTAINER).filter(this._filterNestedFilmStripsFunc);
        return items;
      },
      ...
      _getItems: function _getItems() {
        var pagesWrapper = this._pagesWrapper;
        var originalItems = pagesWrapper.find(_PERIOD + _OJ_FILMSTRIP_ITEM).filter(this._filterNestedFilmStripsFunc);
       ...
       * @private
       */
      _clearCalculatedSizes: function _clearCalculatedSizes() {
        var pagesWrapper = this._pagesWrapper;
       ...
       * @private
       */
      _adjustSizes: function _adjustSizes(bNotifyAttach) {
        ...
        var items = this._getItemContainers(); // if we haven't saved the item size yet, do it now
        ...
        if (this._itemSize < 0) {
          ...
          var optionItemContainer = $(items[optionItemIndex]); // unhide the item, in case the app has initially hidden it

          var optionItem = optionItemContainer.children(_PERIOD + _OJ_FILMSTRIP_ITEM);
          optionItem.css(_DISPLAY, _EMPTY_STRING); // notify the item that it's being shown
        ...
        this._componentSize = componentSize; // if there are a fixed number of items per page, but the number specified
        ...
        items.css(_FLEX_BASIS, fitItemSize + _PX).css(_WEBKIT_FLEX_BASIS, fitItemSize + _PX);
        var newPageCount = Math.ceil(items.length / fitCount); // wrap items in logical page containers

        var pages = this._getPages();

        var bCreatePages = false; // need to create logical pages if page layout changed, or if we haven't
        // yet created logical pages

        var pagingModel = this._pagingModel;

        if (pagingModel.getPageCount() !== newPageCount || this._itemsPerPage !== fitCount || !pages || pages.length < 1) {
          bCreatePages = true;
          var i;
        ...
        pages.css(_FLEX_BASIS, componentSize + _PX).css(_WEBKIT_FLEX_BASIS, componentSize + _PX); // always need to update the pages container size

        var pagesWrapper = this._pagesWrapper;
        var contentWrapper = this._contentWrapper; // only show a single page at a time
        ...
        if (pagingModel.getPageCount() !== newPageCount || this._itemsPerPage !== fitCount || pagingModel.getPage() !== newPageIndex) {
          pagingModel.setPage(newPageIndex, {
            pageSize: fitCount
          });
       ...
       * @private
       */
      _handlePage: function _handlePage(event) {
        ...
        var pageCount = pagingModel.getPageCount();
        var bImmediate = prevPageIndex < 0 || prevPageIndex === pageIndex || this._itemsPerPage !== pageSize;

        var bLooping = this._isLoopingPage(); // update _itemsPerPage AFTER using it to initialize bImmediate above


        this._itemsPerPage = pageSize; // get the old page, if there is one
        ...
        var cssAttr = this._getCssPositionAttr(); // unhide the new page
        ...
        if (prevPageIndex > -1 && !bImmediate) {
          bNext = pageIndex > prevPageIndex; // if looping is enabled, continue in the direction of the navigation
          ...
          bDeferScroll = true; // set size of pages wrapper to show two pages at a time because both of
          // them will temporarily be visible

          pagesWrapper.css(this._getCssSizeAttr(), 2 * this._componentSize); // if going to the previous page, initially scroll to show the current page
        ...
        if (bDeferScroll) {
          var self = this;
          var bDragInit = this._bDragInit; // if bouncing back because a drag scroll didn't cross the threshold,
       ...
       * @private
       */
      _finishHandlePage: function _finishHandlePage(pageIndex, prevPageIndex, bNext, bImmediate, bDragInit) {
        var pagesWrapper = this._pagesWrapper;

        if (!bImmediate) {
          this._bPageChangeTransition = true;
          pagesWrapper.on('transitionend' + this.eventNamespace + ' webkitTransitionEnd' + this.eventNamespace, this._handleTransitionEndFunc);
        }

        if (bImmediate) {
          this._handleTransitionEnd();
        } else {
          var pages = this._getPages(); // if we're currently drag scrolling, remove the transforms that we used
      ...
      _handleTransitionEnd: function _handleTransitionEnd() {
        ...
        pagesWrapper.off(this.eventNamespace).css(this._getCssSizeAttr(), this._componentSize).css(cssAttr, '0px'); // if the filmStrip contains the focus element, we may need to focus a new
        // element if the old one is on a page that's being hidden

        var focusElem = null;

        if (FocusUtils.containsFocus(pagesWrapper[0]) || this._nextButton && FocusUtils.containsFocus(this._nextButton[0]) || this._prevButton && FocusUtils.containsFocus(this._prevButton[0])) {
          focusElem = document.activeElement;
        }

        var pagingModel = this._pagingModel;
        var pageIndex = pagingModel.getPage(); // hide all pages except for current one
        ...
        pages.removeClass(_OJ_FILMSTRIP_TRANSITION + ' ' + _OJ_FILMSTRIP_TRANSITION_NEXT_OLDPAGE_TO + ' ' + _OJ_FILMSTRIP_TRANSITION_NEXT_NEWPAGE_TO + ' ' + _OJ_FILMSTRIP_TRANSITION_PREV_OLDPAGE_TO + ' ' + _OJ_FILMSTRIP_TRANSITION_PREV_NEWPAGE_TO + ' ' + _OJ_FILMSTRIP_TRANSITION_DISPLAY_AS_FIRSTPAGE + ' ' + _OJ_FILMSTRIP_TRANSITION_DISPLAY_AS_LASTPAGE); // remove transforms left over from drag scrolling

        _removeTransform(pages); // update display of the navigation arrows


        this._updateNavigationArrowsDisplay(); // if the old focus element is being hidden, transfer focus to something visible


        if (focusElem && $(focusElem).is(_HIDDEN_SELECTOR)) {
          var elem = this.element; // focus an element in the new logical page if possible, otherwise focus
        ...
        if (currItemPage !== pageIndex) {
          var newFirstItem = this._getFirstItemOnPage(pageIndex); // FIX : only update currentItem property if the filmstrip is not empty


          if (newFirstItem) {
            this.option(_CURRENT_ITEM, newFirstItem, {
              _context: {
                writeback: true
              }
            });
        ...
        this._updatePageInfoElem(); // FIX : resolve busy state after animating a scroll
      ...
      _getItemIndex: function _getItemIndex(item) {
        ...
        if (item) {
          var items = this._getItems(); // If the item contains both id and index, item id takes precedence.
       ...
       * @private
       */
      _convertItemToObj: function _convertItemToObj(item) {
        var itemObj = null;
       ...
       * @private
       */
      _populateItemObj: function _populateItemObj(item) {
        if (item && this._pagingModel.getPage() >= 0) {
          var index = this._getItemIndex(item); // eslint-disable-next-line no-param-reassign


          item.index = index;

          if (item.id == null && index !== -1) {
            var items = this._getItems(); // eslint-disable-next-line no-param-reassign
       ...
       * @private
       */
      _findPage: function _findPage(item, itemsPerPage) {
       ...
       * @private
       */
      _getFirstItemOnPage: function _getFirstItemOnPage(pageIndex, pageCount, itemsPerPage) {
        var pagingModel = this._pagingModel;
       ...
       * @private
       */
      _hidePage: function _hidePage(page) {
       ...
       * @private
       */
      _unhidePage: function _unhidePage(page) {
        page.css(_DISPLAY, _EMPTY_STRING).removeAttr(_ARIA_HIDDEN); // unhide the items explicitly because the app may have initially hidden them

        var items = page.find(_PERIOD + _OJ_FILMSTRIP_ITEM).filter(this._filterNestedFilmStripsFunc);
        items.css(_DISPLAY, _EMPTY_STRING); // notify the page that it was shown
       ...
       * @private
       */
      _addMouseListeners: function _addMouseListeners() {
        var elem = this.element;
        elem.on('mousedown' + this.mouseEventNamespace, this._handleMouseDownFunc).on('mousemove' + this.mouseEventNamespace, this._handleMouseMoveFunc).on('mouseup' + this.mouseEventNamespace, this._handleMouseUpFunc);
      },
       ...
       * @private
       */
      _removeMouseListeners: function _removeMouseListeners() {
        var elem = this.element;
       ...
       * @private
       */
      _addTouchListeners: function _addTouchListeners() {
        var elem = this.element;

        if (this._IsCustomElement()) {
          var createDelegatedListener = function createDelegatedListener(listener) {
          ...
          elem.on('touchend' + this.touchEventNamespace, this._handleTouchEndFunc).on('touchcancel' + this.touchEventNamespace, this._handleTouchEndFunc);
        } else {
          elem.on('touchstart' + this.touchEventNamespace, this._handleTouchStartFunc).on('touchmove' + this.touchEventNamespace, this._handleTouchMoveFunc).on('touchend' + this.touchEventNamespace, this._handleTouchEndFunc).on('touchcancel' + this.touchEventNamespace, this._handleTouchEndFunc);
        }
      },
       ...
       * @private
       */
      _removeTouchListeners: function _removeTouchListeners() {
        var elem = this.element;
      ...
      _handleKeyDown: function _handleKeyDown(event) {
        ...
        if (newPageIndex > -1 && newPageIndex < pageCount) {
          ...
          var optionsObj = {}; // navigate from last page to first page
      ...
      _handleTouchMove: function _handleTouchMove(event) {
        ...
        this._dragScrollMove(event, firstTouch); // FIX : if the drag scroll was started, prevent the page
        // from scrolling also
       ...
       * @private
       */
      _dragScrollStart: function _dragScrollStart(coordsObj) {
        var pagingModel = this._pagingModel;
       ...
       * @private
       */
      _initDragScroll: function _initDragScroll(coordsObj, bFirstToLast, bLastToFirst) {
        ...
        var pagingModel = this._pagingModel;
        var pageIndex = pagingModel.getPage();
        var pageCount = pagingModel.getPageCount(); // unhide adjacent pages (unhide both because we don't know which way user
        // will scroll)
        ...
        if (bFirstToLast || bLastToFirst) {
          if (bFirstToLast) {
            // unhide last page
            this._unhidePage($(pages[pageCount - 1])); // when unhiding previous page, need to adjust current scroll position
            // to continue showing current page


            pagesWrapper.css(cssAttr, -this._componentSize + _PX); // increment number of pages we need to show

            pageCountToShow += 1; // display last page as first page
          ...
          if (bLastToFirst) {
            // unhide first page
            this._unhidePage($(pages[0])); // increment number of pages we need to show


            pageCountToShow += 1; // display first page as last page

            $(pages[0]).addClass(_OJ_FILMSTRIP_TRANSITION_DISPLAY_AS_LASTPAGE);
          }
        } else {
          if (pageIndex > 0) {
            this._unhidePage($(pages[pageIndex - 1])); // when unhiding previous page, need to adjust current scroll position
            // to continue showing current page
          ...
          if (pageIndex < pageCount - 1) {
            this._unhidePage($(pages[pageIndex + 1])); // increment number of pages we need to show
       ...
       * @private
       */
      _dragScrollMove: function _dragScrollMove(event, coordsObj) {
        ...
        var diff = touchCoord - this._touchStartCoord; //  - cannot scroll vertically in filmstrip component

        var touchCoord2 = bHorizontal ? coordsObj.pageY : coordsObj.pageX;
        var diff2 = touchCoord2 - this._touchStartCoord2; // in non-RTL, if swiping left or up, scroll next; otherwise scroll prev
        // in RTL, if swiping right or up, scroll next; otherwise scroll prev

        var bNext = bHorizontal && this._bRTL ? diff > 0 : diff < 0; // determine whether the filmStrip can be scrolled in the direction of the swipe
        ...
        if (!this._bDragInit) {
          ...
          if (Math.abs(diff2) > Math.abs(diff)) {
            this._bTouch = false; // set a flag indicating we don't scroll for this touch event
          ...
          this._bLastToFirst = bLastToFirst; // return here even if we just initialized the drag so that we'll start
          ...
          this._dragScrollResetPages(); // re-initialize the drag scroll based on the current drag coordinates
        ...
        var canScrollInSwipeDirection = bNext && pageIndex < pagingModel.getPageCount() - 1 || !bNext && pageIndex > 0; // only need to do something if we can scroll in the swipe direction
        ...
        if (canScrollInSwipeDirection || bLooping) {
          ...
          var pages = this._getPages(); // if swiping beyond the threshold, scroll to the next/prev page
          ...
          if (Math.abs(diff) >= threshold) {
            ...
            if (bFirstToLast || bLastToFirst) {
              if (bFirstToLast) {
                newPageIndex = pageCount - 1; // Hide only if more than 2 pages are available

                pageToHide = pageCount > 2 ? 1 : -1;
              } else if (bLastToFirst) {
                newPageIndex = 0; // Hide only if more than 2 pages are available
            ...
            pagesWrapper.css(this._getCssSizeAttr(), 2 * this._componentSize); // don't scroll again for this same swipe
            ...
            var scrollVal = diff;
            var transform = bHorizontal ? 'translate3d(' + scrollVal + 'px, 0, 0)' : 'translate3d(0, ' + scrollVal + 'px, 0)';
       ...
       * @private
       */
      _dragScrollEnd: function _dragScrollEnd() {
        ...
        this._bDragInit = false;
        this._bFirstToLast = false;
        this._bLastToFirst = false; // reset the flag indicating if we've scrolled for this touch event
       ...
       * @private
       */
      _dragScrollResetPages: function _dragScrollResetPages() {
        var pagesWrapper = this._pagesWrapper;
        ...
        var pagingModel = this._pagingModel;
        var pageIndex = pagingModel.getPage();
        var pageCount = pagingModel.getPageCount(); // hide all pages except for current one
      ...
      getNodeBySubId: function getNodeBySubId(locator) {
        ...
        if (subId === 'oj-filmstrip-start-arrow') {
          return this.widget().find(_PERIOD + _OJ_FILMSTRIP_ARROW + _PERIOD + _OJ_START).filter(this._filterNestedFilmStripsFunc)[0];
        }

        if (subId === 'oj-filmstrip-end-arrow') {
          return this.widget().find(_PERIOD + _OJ_FILMSTRIP_ARROW + _PERIOD + _OJ_END).filter(this._filterNestedFilmStripsFunc)[0];
        }

        if (subId === 'oj-filmstrip-top-arrow') {
          return this.widget().find(_PERIOD + _OJ_FILMSTRIP_ARROW + _PERIOD + _OJ_TOP).filter(this._filterNestedFilmStripsFunc)[0];
        }

        if (subId === 'oj-filmstrip-bottom-arrow') {
          return this.widget().find(_PERIOD + _OJ_FILMSTRIP_ARROW + _PERIOD + _OJ_BOTTOM).filter(this._filterNestedFilmStripsFunc)[0];
      ...
      getSubIdByNode: function getSubIdByNode(node) {
        var startArrow = this.getNodeBySubId({
          subId: 'oj-filmstrip-start-arrow'
        });
        var endArrow = this.getNodeBySubId({
          subId: 'oj-filmstrip-end-arrow'
        });
        var topArrow = this.getNodeBySubId({
          subId: 'oj-filmstrip-top-arrow'
        });
        var bottomArrow = this.getNodeBySubId({
          subId: 'oj-filmstrip-bottom-arrow'
        });
