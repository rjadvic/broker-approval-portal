 ...
 * @ignore
 */
define(['ojs/ojcore-base', 'jquery', 'ojs/ojcomponentcore', 'ojs/ojcontext', 'ojs/ojdomutils', 'ojs/ojcustomelement', 'ojs/ojanimation', 'ojs/ojcustomelement-utils'], function (oj, $, Components, Context, DomUtils, ojcustomelement, ojanimation, ojcustomelementUtils) {
  'use strict';
  ...
  var __oj_collapsible_metadata = {
    "properties": {
      ...
      "expandArea": {
        "type": "string",
        "enumValues": ["disclosureIcon", "header"],
        "value": "header"
      },
      "expanded": {
        "type": "boolean",
  ...
  (function () {
    ...
    oj.CustomElementBridge.register('oj-collapsible', {
      metadata: __oj_collapsible_metadata
    });
  })();
  /**
   * @license
  ...
  (function () {
    ...
    oj.__registerWidget('oj.ojCollapsible', $.oj.baseComponent, {
      ...
      _ComponentCreate: function _ComponentCreate() {
        ...
        this._refresh(); //  - collapsible shouldn't implement _init()


        this._initialRender = true; // don't fire event on initial render

        var elem = this.element[0];

        this._expandCollapseHandler(this._createEventObject(elem, this.options.expanded ? 'ojexpand' : 'ojcollapse'));
       ...
       * @private
       */
      _createEventObject: function _createEventObject(element, type) {
        return {
          type: type,
          target: element,
      ...
      _NotifyContextMenuGesture: function _NotifyContextMenuGesture(menu, event, eventType) {
        ...
        this._OpenContextMenu(event, eventType, {
          launcher: this._getCollapsibleIcon().first()
        });
      },
      ...
      _destroy: function _destroy() {
        ...
        this._cleanup(); // clean up main element


        this.element.removeClass('oj-collapsible oj-component oj-expanded oj-collapsed oj-disabled'); // clean up headers
        ...
        focusable.removeAttr('role').removeAttr('aria-controls').removeAttr('aria-expanded').removeAttr('aria-disabled');
      ...
      _cleanup: function _cleanup() {
        // remove listeners
        this._tearDownEvents(); // remove wrapper
       ...
       * @private
       */
      _getExpandAreaSelector: function _getExpandAreaSelector() {
       ...
       * @private
       */
      _getCollapsibleIcon: function _getCollapsibleIcon() {
       ...
       * @private
       */
      _processHeaderSlots: function _processHeaderSlots() {
        var elem = this.element[0];
        var newHeader = false; // add the header node if not specified
       ...
       * @private
       */
      _processDefaultSlots: function _processDefaultSlots() {
        var elem = this.element[0];
      ...
      _processPanels: function _processPanels() {
        ...
        this.header.addClass('oj-collapsible-header'); // process content
        ...
        if (this._isDisabled()) {
          this._findFocusables(this.header).attr('tabIndex', -1);
        }
      },
      ...
      _refresh: function _refresh() {
        ...
        focusable.attr('role', 'button').attr('aria-controls', contentId).attr('aria-expanded', options.expanded);
      ...
      _setupEvents: function _setupEvents() {
        ...
        var expandArea = this.element.find(this._getExpandAreaSelector()); // add listeners on expandArea (event expandArea)
      ...
      _tearDownEvents: function _tearDownEvents() {
        ...
        this._off(expandArea); // remove wrapper listeners
       ...
       * @private
       */
      _toggleHandler: function _toggleHandler(event) {
        ...
        event.stopPropagation(); // set focus on the disclosure icon
       ...
       * @private
       */
      _calcEffectTime: function _calcEffectTime(jelem) {
       ...
       * @private
       */
      _resolveTransition: function _resolveTransition(wrapper) {
        var self = this;
       ...
       * @private
       */
      _expandCollapseHandler: function _expandCollapseHandler(event) {
        ...
        if (this._initialRender || !event.isDefaultPrevented || !event.isDefaultPrevented()) {
          ...
          var self = this;
          event.preventDefault(); // fire option change event
          ...
          this._getCollapsibleIcon().toggleClass(OPEN_ICON, isExpanded) // logic or cause same icon for expanded/collapsed state would remove the oj-icon-class
          .toggleClass(CLOSE_ICON, !isExpanded || OPEN_ICON === CLOSE_ICON).end(); //  - expansion animation on initial render.


          if (this._initialRender || document.hidden || this.element.hasClass('oj-collapsible-skip-animation')) {
            if (!isExpanded) {
            ...
            // do animation
            wrapper.contentHeight = wrapper.outerHeight(); // Add a busy state for the animation.  The busy state resolver will be invoked
            ...
            this._transitionEnded = false; // expanding
       ...
       * @private
       */
      _findFirstFocusableInHeader: function _findFirstFocusableInHeader() {
       ...
       * @private
       */
      _findFocusables: function _findFocusables(start) {
       ...
       */
      expand: function expand(vetoable, event) {
        if (this._isDisabled()) {
          return;
        }
       ...
       * @param {boolean} vetoable if event is vetoable
       */
      collapse: function collapse(vetoable, event) {
        if (this._isDisabled()) {
          return;
        }
       ...
       * @private
       */
      _transitionEndHandler: function _transitionEndHandler(event) {
       ...
       * @private
       */
      _afterExpandCollapse: function _afterExpandCollapse(isExpanded, event) {
        var element = this.element;
        var wrapper = this.wrapper;

        if (isExpanded) {
          element.removeClass('oj-collapsed');
          element.addClass('oj-expanded'); //  - ojcollapsible needs to call oj.components.subtreeshown()/subtreehidden()

          Components.subtreeShown(wrapper[0]);
        } else {
          element.removeClass('oj-expanded');
          element.addClass('oj-collapsed'); //  - ojcollapsible needs to call oj.components.subtreeshown()/subtreehidden()
       ...
       * @private
       */
      _changeExpandedOption: function _changeExpandedOption(value) {
        this.option('expanded', value, {
          _context: {
            writeback: true,
            internalSet: true
          }
      ...
      getSubIdByNode: function getSubIdByNode(node) {
        var headerIcon = this.getNodeBySubId({
