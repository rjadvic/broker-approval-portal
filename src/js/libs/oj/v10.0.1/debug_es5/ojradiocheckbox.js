 ...
 * @ignore
 */
define(['ojs/ojcore-base', 'jquery', 'ojs/ojlogger', 'ojs/ojcomponentcore'], function (oj, $, Logger, Components) {
  'use strict';
  ...
  */
  ...
  oj.__registerWidget('oj._ojRadioCheckbox', $.oj.baseComponent, {
     ...
     */
    refreshDisabled: function refreshDisabled() {
     ...
     */
    setSelectedClass: function setSelectedClass(checked) {
    ...
    _InitOptions: function _InitOptions(originalDefaults, constructorOptions) {
      ...
      this._super(originalDefaults, constructorOptions); // CHECKED:
      ...
      if (!('checked' in constructorOptions)) {
        ...
        checkedFromDom = !!this.element.prop('checked'); // writeback not needed since "not in constructorOptions" means "not bound"

        this.option('checked', checkedFromDom, {
          _context: {
            internalSet: true
          }
        });
      ...
      if (!('disabled' in constructorOptions)) {
        ...
        disabledFromDom = !!this.element.prop('disabled'); // writeback not needed since "not in constructorOptions" means "not bound"

        this.option('disabled', disabledFromDom, {
          _context: {
            internalSet: true
          }
        });
      ...
      if (!('type' in constructorOptions)) {
        this.option('type', this.element.prop('type'), {
          _context: {
            internalSet: true
          }
        });
    ...
    _ComponentCreate: function _ComponentCreate() {
      ...
      this._AddActiveable(this.$choiceItem); // the input gets focus on keyboard tabbing. It bubbles up, so in case the
      ...
      this._AddActiveable(this.$label); // loop through each label to add dom and styles
    ...
    _setCheckedOnDom: function _setCheckedOnDom(checked) {
    ...
    _renderDisabled: function _renderDisabled() {
      ...
      if (effectivelyDisabled) {
        ...
        this.element.prop('disabled', true).removeAttr('aria-disabled').removeClass('oj-enabled').addClass('oj-disabled');
        ...
        // when a dom element supports disabled, use that, and not aria-disabled.
        // having both is an error.
        this.element.prop('disabled', false).removeAttr('aria-disabled').removeClass('oj-disabled').addClass('oj-enabled');
     ...
     * @private
     */
    _getLabelsForElement: function _getLabelsForElement() {
      ...
      if ($labelClosestParent.length !== 0) {
        Logger.error("Found a label that is an input's ancestor." + ' This is not supported in the ojCheckboxset or ojRadioset component and the component will\n' + 'not work correctly. ' + "Use a label as a sibling to the input and use the label 'for' attribute to tie the two together.");
      ...
      if ($labelForElems.length === 0) {
        ...
        // the element
        $labelSibling = this.element.siblings(labelForQuery); // .add() method creates a new set and leaves the original set unchanged
     ...
     * @returns {Object}
     */
    _getChoiceItem: function _getChoiceItem() {
      var choiceItem = null;
      var elementParent;
      var labelSelector;
      var ojChoiceItemSpanString;
      var siblingLabel;
      elementParent = this.element.parent(); // oj-choice-row and oj-choice-row-inline have been deprecated on December 7, 2016 in
      // version 3.0.0. Use oj-choice-item instead.

      if (elementParent && (elementParent.hasClass('oj-choice-item') || elementParent.hasClass('oj-choice-row') || elementParent.hasClass('oj-choice-row-inline'))) {
        choiceItem = elementParent;
      } else {
        Logger.warn("The radioset/checkboxset's input and label dom should be wrapped in a dom " + "node with class 'oj-choice-item'. JET is adding this missing dom to make the component work correctly."); // Since we can't find the oj-choice-item, create one.
        // It needs to wrap the input and its label (if any)

        ojChoiceItemSpanString = "<span class='oj-choice-item oj-choice-item-added'></span>"; // the most common case is an <input id='foo'><label for='foo'> pair, so look for that first
    ...
    _destroy: function _destroy() {
      ...
      var type = this.options.type; // this.$label is the label for the checkbox/radio, NOT the label for the radioset/checkboxset.
      ...
      var self = this; // loop through each label to remove things we added
      ...
      this.element.unwrap(); // remove the oj-choice-item span only if I added it.
  ...
  RadioCheckboxUtils.renderOptions = function () {
    ...
    if (optionsDataArray) {
      ...
      var wrapperClass = choiceset.tagName === 'OJ-RADIOSET' ? 'oj-radioset-wrapper' : 'oj-checkboxset-wrapper';
      var wrapper = choiceset.querySelector('.' + wrapperClass); // Remove all the existing option items
      ...
      for (var _i = 0; _i < optionItems.length; _i++) {
        var _item = optionItems[_i];
      ...
      if (typeof optionRenderer !== 'function') {
        ...
        optionRenderer = function optionRenderer(optionContext) {
      ...
      for (var _i2 = 0; _i2 < optionsDataArray.length; _i2++) {
        var optionContext = {
          component: choiceset,
          index: _i2,
          data: optionsDataArray[_i2]
        };
  ...
  RadioCheckboxUtils.generateOptionsFromData = function () {
    var dataProvider = this.options.options; // Remove any existing DataProvider listeners

    RadioCheckboxUtils.removeDataListener.call(this); // Nothing else to do here if no DataProvider is used
    ...
    fetchPromise.then(function () {
      RadioCheckboxUtils.renderOptions.call(self); // Add back DataProvider listeners after the options are rendered

      RadioCheckboxUtils.addDataListener.call(self); // Resolve busy state
  ...
  RadioCheckboxUtils.addDataListener = function () {
    ...
    if (this.options.options && oj.DataProviderFeatureChecker.isDataProvider(this.options.options)) {
      ...
      // eslint-disable-next-line no-param-reassign
      this._optionsDataProvider = this.options.options; // eslint-disable-next-line no-param-reassign
  ...
  RadioCheckboxUtils.removeDataListener = function () {
    if (this._optionsDataListener) {
      ...
      this._optionsDataProvider.removeEventListener('mutate', this._optionsDataListener); // eslint-disable-next-line no-param-reassign


      this._optionsDataProvider = null; // eslint-disable-next-line no-param-reassign
