 ...
 * @ignore
 */
define(['ojs/ojcomponentcore', 'ojs/ojcustomelement', 'ojs/ojcustomelement-utils'], function (Components, ojcustomelement, ojcustomelementUtils) {
  'use strict';
  /**
   * @license
  ...
  var ojSwitcher = function ojSwitcher(context) {
    ...
    var _slotMap = null; // Our version of GCC has a bug where the second param of MutationObserver.observe must be of
          ...
          if (mutation.addedNodes) {
            ...
            Array.prototype.forEach.call(mutation.addedNodes, function (item) {
              if (item.nodeType === 1) {
                ...
                _applyValueToItem(item, itemSlotAttributeValue); // force to re calculate slot map nex time
    ...
    function _show(item) {
      ...
      if (!isCurrentlyVisible) {
        // eslint-disable-next-line no-param-reassign
        item.style.display = '';
        Components.subtreeShown(item, isInitialRender ? {
          initialRender: true
        } : undefined);
      } else if (isInitialRender) {
        Components.subtreeShown(item, {
          initialRender: true
        });
      }
    }
    ...
    this.createDOM = function () {
      self._caseElementMutationObserver.observe(element, {
        childList: true
      });
    };
    ...
    function _applyValueToItem(item, itemSlotAttributeValue) {
      ...
      if (item[SWITCHER_ORIG_DISPLAY_STYLE] === undefined) {
        Object.defineProperty(item, SWITCHER_ORIG_DISPLAY_STYLE, {
          value: item.style.display
        });
      }
  ...
  (function () {
    ...
    oj.CustomElementBridge.register('oj-switcher', {
      metadata: __oj_switcher_metadata
    });
  })();
});
