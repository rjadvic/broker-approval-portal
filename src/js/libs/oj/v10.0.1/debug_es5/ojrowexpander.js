 ...
 * @ignore
 */
define(['ojs/ojdatasource-common', 'ojs/ojcore-base', 'jquery', 'ojs/ojdomutils', 'ojs/ojcomponentcore', 'ojs/ojlogger'], function (ojdatasourceCommon, oj, $, DomUtils, ojcomponentcore, ojlogger) {
  'use strict';
  ...
  var EmptyNodeSet = function EmptyNodeSet(parent, start) {
  ...
  var FlattenedNodeSet = function FlattenedNodeSet(nodeSet, actualStart) {
  ...
  var NodeSetWrapper = function NodeSetWrapper(nodeSet, metadataCallback, range, collapsedKeys) {
    this.m_nodeSet = nodeSet;
  ...
  var MergedNodeSet = function MergedNodeSet(nodeSet1, nodeSet2, mergeAt) {
  ...
  var FlattenedTreeDataSource = function FlattenedTreeDataSource(treeDataSource, options) {
  ...
  FlattenedTreeDataSource.prototype.Init = function () {
    // super
    FlattenedTreeDataSource.superclass.Init.call(this); // we have to react if the underlying TreeDataSource has changed
      ...
      if (this._getLastIndex() < 0) {
        ...
        if (count === -1 || index < count - 1) {
          ...
          if (!processed) {
            ...
            var nodeSet = new EmptyNodeSet(null, range.start); // invoke original success callback
  ...
  FlattenedTreeDataSource.prototype._handleFetchSuccess = function (nodeSet, parent, depth, originalRange, requestedRange, count, callbacks, options) {
    ...
    nodeSet = new NodeSetWrapper(nodeSet, this.insertMetadata.bind(this), flattenedRange); // expand the fetched nodes to get the node set as full as possible from the original fetch
  ...
  FlattenedTreeDataSource.prototype._verifyFetchResults = function (nodeSet, parent, depth, originalRange, requestedRange, count, callbacks, options) {
    var mergedNodeSet;
    var processed;
  ...
  FlattenedTreeDataSource.prototype._fetchFromAncestors = function (parent, depth, range, callbacks, maxFetchSize, options) {
  ...
  FlattenedTreeDataSource.prototype._handleFetchDescendantsSuccess = function (nodeSet, range, callbacks) {
    var options;
    var actualStart;
    ...
    if (range.start > this._getLastIndex()) {
      ...
      _nodeSet = new NodeSetWrapper(_nodeSet, this.insertMetadata.bind(this), null, this.m_collapsedKeys);
  ...
  FlattenedTreeDataSource.prototype._processDescendantsNodeSet = function (nodeSet, parent, depth, lastEntry, maxCount, options) {
    var nodeStart = nodeSet.getStart();
    var nodeCount = nodeSet.getCount(); // walk the node set and populate the internal cache
  ...
  FlattenedTreeDataSource.prototype.handleExpandSuccess = function (rowKey, nodeSet, childCount, options) {
    var queue;
  ...
  FlattenedTreeDataSource.prototype._insertRow = function (index, metadata, parentKey, childIndex, depth) {
    var key = metadata.key;
  ...
  oj.__registerWidget('oj.ojRowExpander', $.oj.baseComponent, {
     ...
     * @private
     */
    _isDataProvider: function _isDataProvider() {
     ...
     * @private
     */
    _getDepth: function _getDepth(context) {
     ...
     * @private
     */
    _getRowKey: function _getRowKey(context) {
      var metadata = context;
     ...
     * @private
     */
    _getState: function _getState(context) {
      var metadata = context;
     ...
     * @private
     */
    _getParentKey: function _getParentKey(context) {
      var metadata = context;
     ...
     * @private
     */
    _getIndex: function _getIndex(context) {
      var metadata = context;
     ...
     * @private
     */
    _getLeaf: function _getLeaf(context) {
      var metadata = context;
    ...
    _handleKeyDownEvent: function _handleKeyDownEvent(event) {
      var targetContext = ojcomponentcore.__GetWidgetConstructor(this.component.element.get(0))('getContextByNode', event.target);
    ...
    _updateContextState: function _updateContextState(newState) {
      ...
      this._setState(context, newState); // need to reuse the same object so mark it as changed ourselves
     ...
     * @private
     */
    _setState: function _setState(context, newSate) {
      var metadata = context;
