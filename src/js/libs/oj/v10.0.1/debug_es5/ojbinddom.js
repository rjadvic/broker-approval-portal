 ...
 * @ignore
 */
define(['ojs/ojkoshared', 'ojs/ojcustomelement-utils', 'knockout', 'ojs/ojcore-base', 'ojs/ojlogger', 'ojs/ojcontext'], function (BindingProviderImpl, ojcustomelementUtils, ko, oj, Logger, Context) {
  'use strict';
  ...
  ko.bindingHandlers._ojBindDom_ = {
    init: function init(element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
      ...
      function configChanged(configPromise) {
        ...
        configPromise.then(function (config) {
          if (configPromise === _currentPromise) {
            try {
              ko.virtualElements.setDomNodeChildren(element, config.view || []); // Null out the parent references since we don't want the view to be able to access the outside context

              var childBindingContext = bindingContext.createChildContext(config.data, undefined, function (ctx) {
                ctx.$parent = null;
      ...
      function findNearestCustomParent(parentTrackingContext) {
        ...
        while (nearestCustomParent && !ojcustomelementUtils.ElementUtils.isValidCustomElementName(nearestCustomParent.localName)) {
          nearestCustomParent = nearestCustomParent.parentNode;
        }

        if (!nearestCustomParent) {
          nearestCustomParent = parentTrackingContext ? parentTrackingContext._nearestCustomParent : null;
        }
      ...
      function findImmediateState(nearestCustomParent, parentTrackingContext) {
        ...
        var nestedElement = parentTrackingContext && Object.prototype.hasOwnProperty.call(parentTrackingContext, '_immediate');
      ...
      function initChildrenBindingsAppliedPromise() {
        if (!_childrenBindingsPromiseResolver) {
          ...
          _childrenBindingsPromiseResolver = oj._KnockoutBindingProvider.getInstance().__RegisterBindingAppliedPromiseForChildren(nearestCustomParent, isImmediate);
        }
      }
      ...
      function registerBusyState() {
        if (!_resolveBusyStateCallback) {
          ...
          _resolveBusyStateCallback = Context.getContext(element.parentNode).getBusyContext().addBusyState({
            description: 'oj-bind-dom is waiting on config Promise resolution'
          });
        }
      }
