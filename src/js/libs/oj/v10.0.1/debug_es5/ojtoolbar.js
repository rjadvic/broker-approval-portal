 ...
 * @ignore
 */
define(['ojs/ojcore-base', 'jquery', 'ojs/ojthemeutils', 'ojs/ojcomponentcore', 'ojs/ojlogger'], function (oj, $, ThemeUtils, Components, Logger) {
  'use strict';
  ...
  (function () {
    ...
    */
    ...
    oj.__registerWidget('oj.ojToolbar', $.oj.baseComponent, {
      ...
      _isButtonDisabled: function _isButtonDisabled(button) {
        var disabled;
      ...
      _handleInitialFocus: function _handleInitialFocus() {
        ...
        var self = this;
        var elem = this.element[0];
        var disabledChangedEventType = 'disabledChanged'; // remove this event handling as we only want to run this setup logic on the first focusin event

        elem.removeEventListener('focusin', this._focusinListener, true);
        this._hasInitialFocusHandler = false;
        this.topLevelChildren = elem.querySelectorAll('oj-button, oj-menu-button, oj-buttonset-one, oj-buttonset-many'); // Add a MutationObserver to handle add and remove of descendants
        ...
        if (!this._mutationObserver) {
          ...
          this._mutationObserver = new MutationObserver(function (mutationList) {
            ...
            mutationList.forEach(function (mutation) {
              if (mutation.type === 'childList') {
                var i; // addedNodes and removedNodes are NodeList

                if (mutation.addedNodes) {
                  for (i = 0; i < mutation.addedNodes.length; i++) {
                    if (ojElementNames.indexOf(mutation.addedNodes[i].nodeName) >= 0) {
                      mutation.addedNodes[i].addEventListener(disabledChangedEventType, self._disabledChangedListener);
                    }
                  }
                }
                ...
                if (mutation.removedNodes) {
                  for (i = 0; i < mutation.removedNodes.length; i++) {
                    if (ojElementNames.indexOf(mutation.removedNodes[i].nodeName) >= 0) {
                      mutation.removedNodes[i].removeEventListener(disabledChangedEventType, self._disabledChangedListener);
                    }
                  }
                }
              }
          ...
          this._mutationObserver.observe(elem, {
            childList: true,
            subtree: true
          });
        ...
        var ojElements = elem.querySelectorAll('oj-button, oj-menu-button, oj-buttonset-one, oj-buttonset-many, oj-option');
      ...
      _refreshTabindex: function _refreshTabindex() {
        var self = this; // We need to re-select buttons because with refresh buttons in toolbar may be added/deleted.
